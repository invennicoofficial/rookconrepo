...
$(document).ready(function() {
    ...
    $('#add_row_eq').on( 'click', function () {
		...
		clone.find('#eqprojectunit_0').attr('id', 'eqprojectunit_'+add_new_s);
...
function selectEquipmentCategory(sel) {
	...
	$.ajax({
		type: "GET",
		url: "project_manage_ajax_all.php?fill=eq_cat_config&value="+stage+"&ratecardid="+ratecardid,
...
function selectEquipmentUnSn(sel) {
	...
	$.ajax({
		type: "GET",
		url: "project_manage_ajax_all.php?fill=eq_un_sn_config&value="+stage+"&ratecardid="+ratecardid,
		dataType: "html",   //expect html to be returned
		success: function(response){
       ...
       <?php
        ...
        if(!empty($_GET['projectmanageid'])) {
            ...
            foreach($each_data as $id_all) {
                if($id_all != '') {
                    ...
                    $get_equipment_field .= '**'.$data_all[0].'#'.$data_all[2].'#'.$data_all[1].'#'.$data_all[3];
                }
            }
        }
        ...
        <?php if(!empty($get_equipment_field)) {
            ...
            <div class="form-group clearfix" id="<?php echo 'equipment_'.$id_loop; ?>" >
                ...
                <div class="col-sm-1" ><label for="company_name" class="col-sm-4 show-on-mob control-label">Unit Of Measure:</label>
                    <input name="eqprojectunit[]" id="<?php echo 'eqprojectunit_'.$id_loop; ?>" value="<?php echo $unit; ?>" type="text" class="form-control" />
                </div>
        ...
        <div class="additional_eq clearfix">
            ...
            <div class="form-group clearfix" id="equipment_0">
                ...
                <div class="col-sm-1" ><label for="company_name" class="col-sm-4 show-on-mob control-label">Unit of Measure:</label>
                    <input name="eqprojectunit[]" id='eqprojectunit_0' type="text" class="form-control" />
                </div>
                <div class="col-sm-1" ><label for="company_name" class="col-sm-4 show-on-mob control-label">Total:</label>
