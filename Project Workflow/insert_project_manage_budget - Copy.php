...
foreach ($_POST['contactid'] as $contactid_all) {
    if($contactid_all != '') {
        ...
        $review_profit_loss .= '<tr><td>Staff</td><td>'.decryptIt($query['first_name']).' '.decryptIt($query['last_name']).'</td> <td>$'.$query['cost'].'</td><td>$'.$_POST['stestimateprice'][$j].'</td><td '.$color_off.'>$'.$plus_minus.'</td></tr>';
