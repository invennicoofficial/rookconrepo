...
$(document).ready(function() {
    ...
    $('#add_row_p').on( 'click', function () {
		...
		clone.find('#pprojectunit_0').attr('id', 'pprojectunit_'+add_new_s);
...
function selectProductProduct(sel) {
	...
	$.ajax({
		type: "GET",
		url: "project_manage_ajax_all.php?fill=p_product_config&value="+stage,
...
function selectProductCat(sel) {
	...
	$.ajax({
		type: "GET",
		url: "project_manage_ajax_all.php?fill=p_cat_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function selectProductHeading(sel) {
	...
	$.ajax({
		type: "GET",
		url: "project_manage_ajax_all.php?fill=p_head_config&value="+stage+"&ratecardid="+ratecardid,
		dataType: "html",   //expect html to be returned
		success: function(response){
       ...
       <?php
        ...
        if(!empty($_GET['projectmanageid'])) {
            ...
            foreach($each_productsid as $id_all) {
                if($id_all != '') {
                    ...
                    $get_products .= '**'.$productsid_all[0].'#'.$productsid_all[2].'#'.$productsid_all[1].'#'.$data_all[3];
                }
            }
        }
            ...
            <div class="form-group clearfix" id="<?php echo 'products_'.$id_loop; ?>" >
               ...
               <?php if (strpos($field_config_product, ','."Minimum Billable".',') !== FALSE) { ?>
                ...
                <div class="col-sm-1" ><label for="company_name" class="col-sm-4 show-on-mob control-label">Unit Of Measure:</label>
                    <input name="pprojectunit[]" id="<?php echo 'pprojectunit_'.$id_loop; ?>" value="<?php echo $unit; ?>" type="text" class="form-control" />
                </div>
        ...
        <div class="additional_p clearfix">
            ...
            <div class="form-group clearfix" id="products_0">
                ...
                <div class="col-sm-1" ><label for="company_name" class="col-sm-4 show-on-mob control-label">Unit of Measure:</label>
                    <input name="pprojectunit[]" id='pprojectunit_0' type="text" class="form-control" />
                </div>
				<div class="col-sm-1" ><label for="company_name" class="col-sm-4 show-on-mob control-label">Total:</label>
