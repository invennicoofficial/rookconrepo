...
if (isset($_POST['submit'])) {
    ...
    if($vpl_name == 'ADD_NEW_VPL') {
        $vpl_name = filter_var($_POST['vpl_name_new'],FILTER_SANITIZE_STRING);
    }
    ...
    echo '<script type="text/javascript"> window.location.replace("?vpl=1&vendorid='.$vendorid.'&vpl_name='.$vpl_name.'&category='.$category.'"); </script>';
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#vpl_name').change(function() {
            changeVPLName();
        });
    });

    function filterVPLNames() {
        var vendorid = $('#vendor').val();
        $('[name="vpl_name"] option:not([value="ADD_NEW_VPL"])').hide();
        $('[name="vpl_name"] option[data-vendor="'+vendorid+'"]').show();
        $('[name="vpl_name"]').trigger('change.select2');
    }
    function changeVPLName() {
        var vpl_name = $('[name="vpl_name"]').val();
        if(vpl_name == 'ADD_NEW_VPL') {
            $('[name="vpl_name_new"]').show();
        } else {
            $('[name="vpl_name_new"]').hide();
        }
    }
...
<div class="standard-body-content pad-10">
	<div class="gap-top double-gap-bottom"><a href="?vpl=1&vendorid=<?= $_GET['vendorid'] ?>&vpl_name=<?= $_GET['vpl_name'] ?>&category=<?php echo $category; ?>" class="btn config-btn">Back to Dashboard</a></div>
        ...
        <div id="collapse_<?php echo $j;?>" class="panel-collapse collapse">
            <div class="panel-body">
                ...
                <?php } ?>
                  ...
                  <?php if (strpos($value_config, ','."VPL Name".',') !== FALSE) { ?>
                  <div class="form-group">
                    <label for="fax_number" class="col-sm-4 control-label">Vendor Price List:</label>
                    <div class="col-sm-8">
                        <select data-placeholder="Choose a Vendor Price List..." id="vpl_name" name="vpl_name" class="chosen-select-deselect form-control" width="380">
                          <option value=""></option>
                          <?php $vpl_list = mysqli_fetch_all(mysqli_query($dbc, "SELECT `vpl_name`, `vendorid` FROM `vendor_price_list` WHERE `deleted` = 0 GROUP BY CONCAT(`vendorid`,`vpl_name`) ORDER BY `vpl_name`"),MYSQLI_ASSOC);
                          foreach($vpl_list as $vpl_list_row) { ?>
                              <option data-vendor="<?= $vpl_list_row['vendorid'] ?>" value="<?= $vpl_list_row['vpl_name'] ?>" <?= $vpl_name == $vpl_list_row['vpl_name'] ? 'selected' : '' ?>><?= $vpl_list_row['vpl_name'] ?></option>
                          <?php } ?>
                          <option value="ADD_NEW_VPL">New Vendor Price List</option>
                        </select>
                        <input type="text" name="vpl_name_new" class="form-control" value="" style="display:none;">
                    </div>
                  </div>
                  <?php } ?>
	...
	<div class="form-group">
	  ...
	  <div class="form-group">
		<div class="col-sm-6">
			...
			<a href="?vpl=1&vendorid=<?= $_GET['vendorid'] ?>&vpl_name=<?= $_GET['vpl_name'] ?>&category=<?php echo $category; ?>" class="btn brand-btn btn-lg">Back</a>
