...
$(document).ready(function() {
	...
	$('input.purchase_order_includer').on('change', function() {
		...
		type: "GET",
		url: "../ajax_all.php?fill=include_in_orders&type=po&name=vpl&value="+value+"&status="+id,
	...
	$('input.sales_order_includer').on('change', function() {
		...
		$.ajax({    //create an ajax request to load_page.php
		type: "GET",
		url: "../ajax_all.php?fill=include_in_orders&type=so&name=vpl&value="+value+"&status="+id,
		dataType: "html",   //expect html to be returned
		success: function(response){
		}
		});
	...
	$('input.point_of_sale_includer').on('change', function() {
		...
		$.ajax({    //create an ajax request to load_page.php
		type: "GET",
		url: "../ajax_all.php?fill=include_in_orders&type=pos&name=vpl&value="+value+"&status="+id,
		dataType: "html",   //expect html to be returned
		success: function(response){
		}
		});
...
<div class="container">
	<div class="row">
        ...
        <div class="col-sm-10">
			<h1><?php if(get_tile_title_vpl($dbc) == '' || get_tile_title_vpl($dbc) == NULL ) { $poser = "Vendor Price List"; } else { $poser = get_tile_title_vpl($dbc); } ; echo $poser; ?> Dashboard</h1>
		</div>
		<div class="col-sm-2 double-gap-top">
        <?php
			if(config_visible_function($dbc, 'vpl') == 1) {
				echo '<a href="field_config_inventory.php?type=tab" class="mobile-block pull-right "><img style="width: 50px;" title="Tile Settings" src="../img/icons/settings-4.png" class="settings-classic wiggle-me"></a>'; ?>
				<span class="popover-examples gap-top pull-right pad-5"><a data-toggle="tooltip" data-placement="top" title="Click here for the settings within this tile. Any changes made will appear on your dashboard."><img src="<?= WEBSITE_URL; ?>/img/info.png" width="20"></a></span><?php
			}
			// See if user wants VPL items to be sorted by tabs or a drop down menu VVV
			$dropdownornot ='';
				...
				// See if user wants VPL items to be sorted by tabs or a drop down menu ^^^
			?>
		</div>
		...
		<form name="form_sites" method="post" action="" class="form-inline" role="form">
            ...
            <?php
				if(isset($_GET['order_list'])) {
					 ...
					 echo "<a href='inventory.php?currentlist&category=Top".$order_list."'><button type='button' class='btn brand-btn mobile-block ".$active."' >Order List Items</button></a>&nbsp;&nbsp;";
						}
				}
                ...
                echo '<span class="popover-examples" style="margin:2px 2px 0 0;"><a data-toggle="tooltip" data-placement="top" title="Filter the Vendor Price List by the Last 25 items added."><img src="'. WEBSITE_URL .'/img/info.png" width="20"></a></span><a href="inventory.php?category=Top'.$order_list.'"><button type="button" class="btn brand-btn mobile-block mobile-100 '.$active_all.'">Last 25 Added</button></a>&nbsp;&nbsp;';
                ...
                foreach ($each_tab as $cat_tab) {
                    ...
                    echo "<a href='inventory.php?category=".$cat_tab.$order_list."'><button type='button' class='btn brand-btn mobile-100 mobile-block ".$active_daily."' >".$cat_tab."</button></a>&nbsp;&nbsp;";
                }
				} else {	?>
					...
					<select name="search_category" class="chosen-select-deselect form-control category_actual" onchange="location = this.value;">
					  ...
					  <?php
						...
						while($row = mysqli_fetch_assoc($sql)){
							...
							echo "<option value='inventory.php?category=".$cat_tab.$order_list."' ".$active_daily." >".$cat_tab."</option>";
						}
					  ?>
					</select>
				</div>
            ...
            if(vuaed_visible_function($dbc, 'inventory') == 1) {
                if($_GET['category'] != 'Top') {
				    echo '<a href="add_inventory.php?category='.$category.$order_list.'" class="double-gap-top btn brand-btn mobile-block gap-bottom pull-right">Add Product</a>';
                }
                ?>
		...
		<div id="no-more-tables">
			...
			if($inventory != '') {
				$query_check_credentials = "SELECT * FROM vendor_price_list WHERE deleted=0 AND (name LIKE '%" . $inventory . "%' OR code LIKE '%" . $inventory . "%' OR part_no LIKE '%" . $inventory . "%' OR category = '$inventory' OR sub_category LIKE '%" . $inventory . "%' OR description LIKE '%" . $inventory . "%' OR purchase_cost LIKE '%" . $inventory . "%' OR min_bin LIKE '%" . $inventory . "%' OR date_of_purchase LIKE '%" . $inventory . "%') LIMIT $offset, $rowsPerPage LIMIT $offset, $rowsPerPage";
                $query = "SELECT count(*) as numrows FROM vendor_price_list WHERE deleted=0 AND (name LIKE '%" . $inventory . "%' OR code LIKE '%" . $inventory . "%' OR part_no LIKE '%" . $inventory . "%' OR category = '$inventory' OR sub_category LIKE '%" . $inventory . "%' OR description LIKE '%" . $inventory . "%' OR purchase_cost LIKE '%" . $inventory . "%' OR min_bin LIKE '%" . $inventory . "%' OR date_of_purchase LIKE '%" . $inventory . "%')";
			} else {
                /*
                ...
                */
				...
				if(isset($_GET['order_list'])) {
                    ...
                    $query_check_credentials = "SELECT * FROM vendor_price_list WHERE deleted = 0 ORDER BY inventoryid DESC LIMIT $offset, $rowsPerPage";
			...
			if($num_rows > 0) {
                ...
                if(empty($_GET['category']) || $_GET['category'] == 'Top') {
                    $get_field_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT inventory_dashboard FROM field_config_vpl WHERE inventory_dashboard IS NOT NULL"));
            ...
            if(vuaed_visible_function($dbc, 'vpl') == 1) {
                if($_GET['category'] != 'Top') {
				    echo '<a href="add_inventory.php?category='.$category.'" class="btn brand-btn mobile-block gap-bottom pull-right">Add Product</a>';
                }
                ?>
