...
$(document).ready(function() {
    ...
    $('#add_row_in').on( 'click', function () {
        ...
        clone.find('#custominventory_0').attr('id', 'custominventory_'+add_new_in);
        $('#indelete_0').hide();
...
$(document).on('change', 'select[name="assign_inventory[]"]', function() { selectInventoryCodePartName(this); });
$(document).on('change', 'select.inv_code_onchange', function() { selectInventoryCodePartNo(this); });
//Inventory
//Inventory
function selectInventoryCategory(sel) {
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=in_cat_config&value="+stage,
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=in_cat_config_partno&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function selectInventoryCodePartName(sel) {
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=in_code_part_name_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=in_code_part_name_config_number&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
            $("#ininventorypart_"+arr[1]).html(response);
			$("#ininventorypart_"+arr[1]).trigger("change.select2");
...
function selectInventoryCodePartNo(sel) {
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=in_code_part_no_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
            var result = response.split('*FFM*');
            $("#inrp_"+arr[1]).val(result[0]);
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=in_code_part_no_config_name&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
            $("#ininventoryname_"+arr[1]).html(response);
			$("#ininventoryname_"+arr[1]).trigger("change.select2");
    ...
    <div class="col-sm-12">
        ...
        <?php if(!empty($_GET['customid'])) {
            ...
            for($inventory_loop=0; $inventory_loop<$total_count; $inventory_loop++) {
                ...
                if($inventoryid != '') {
            ?>
                <div class="form-group clearfix" id="<?php echo 'custominventory_'.$id_loop; ?>">
                    <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Category:</label>
                        <select data-placeholder="Select a Category..." id="<?php echo 'ininventorycat_'.$id_loop; ?>" class="chosen-select-deselect form-control inventoryid inv_cat_onchange" width="380">
                            <option value=''></option>
                            <?php
                    ...
                    <select data-placeholder="Select a Unit Number..." id="<?php echo 'ininventoryname_'.$id_loop; ?>" name="assign_inventory[]" class="chosen-select-deselect form-control inventoryid" width="380">
                        <option value=''></option>
                        <?php
                    ...
                    <select data-placeholder="Select a Part Number..." id="<?php echo 'ininventorypart_'.$id_loop; ?>" class="chosen-select-deselect form-control inventoryid inv_code_onchange" width="380">
                        <option value=''></option>
                        <?php
        ...
        <div class="additional_in clearfix">
            ...
            <div class="form-group clearfix" id="custominventory_0">

                    <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Category:</label>
                        <select data-placeholder="Select a Category..." id="ininventorycat_0" class="chosen-select-deselect form-control inventoryid inv_cat_onchange" width="380">
                            <option value=''></option>
                            <?php
                    ...
                    <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Product Name:</label>
                    <select data-placeholder="Select a Unit Number..." id="ininventoryname_0" name="assign_inventory[]" class="chosen-select-deselect form-control inventoryid" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT inventoryid, name FROM inventory");
                        while($row = mysqli_fetch_array($query)) {
					...
					<?php if (strpos($value_config, ','."Inventory Part Number".',') !== FALSE) { ?>
					<div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Part Number:</label>
                    <select data-placeholder="Select a Unit Number..." id="ininventorypart_0" class="chosen-select-deselect form-control inventoryid inv_code_onchange" width="380">
                        <option value=''></option>
                        <?php
