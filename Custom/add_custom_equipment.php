...
$(document).ready(function() {
    ...
    $('#add_row_eq').on( 'click', function () {
        ...
        clone.find('#customequipment_0').attr('id', 'customequipment_'+add_new_eq);
        $('#eqdelete_0').hide();
...
function selectEquipmentCategory(sel) {
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=eq_cat_config&value="+stage,
...
function selectEquipmentUnSn(sel) {
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=eq_un_sn_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
    ...
    <div class="col-sm-12">
        ...
        <?php if(!empty($_GET['customid'])) {
            ...
            for($equ_loop=0; $equ_loop<$total_count; $equ_loop++) {
                ...
                if($equipmentid != '') {
            ?>
                <div class="form-group clearfix" id="<?php echo 'customequipment_'.$id_loop; ?>">
                    <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Category:</label>
                        <select data-placeholder="Select a Category..." id="<?php echo 'eqequipmentcat_'.$id_loop; ?>" class="chosen-select-deselect form-control equipmentid equip_cat_onchange" width="380">
                            <option value=''></option>
                            <?php
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Unit/Serial Number:</label>
                    <select data-placeholder="Select a Number..." id="<?php echo 'eqequipmentun_'.$id_loop; ?>" name="assign_equipment[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
        ...
        <div class="additional_eq clearfix">
            ...
            <div class="form-group clearfix" id="customequipment_0">

                    <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Category:</label>
                        <select data-placeholder="Select a Category..." id="eqequipmentcat_0" class="chosen-select-deselect form-control equipmentid equip_cat_onchange" width="380">
                            <option value=''></option>
                            <?php
                            $query = mysqli_query($dbc,"SELECT distinct(category) FROM equipment");
                            while($row = mysqli_fetch_array($query)) {
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Unit/Serial Number:</label>
                    <select data-placeholder="Select a Number..." id="eqequipmentun_0" name="assign_equipment[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT equipmentid, unit_number, serial_number  FROM equipment");
                        while($row = mysqli_fetch_array($query)) {
