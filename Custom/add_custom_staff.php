...
$(document).ready(function() {
    ...
    $('#add_row_st').on( 'click', function () {
        ...
        clone.find('#customstaff_0').attr('id', 'customstaff_'+add_new_st);
        $('#stdelete_0').hide();
...
function selectStaff(sel) {
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=st_config&value="+stage,
    ...
    <div class="col-sm-12">
        ...
        <?php if(!empty($_GET['customid'])) {
            ...
            for($staff_loop=0; $staff_loop<$total_count; $staff_loop++) {
                ...
                if($staffid != '') {
            ?>
                <div class="form-group clearfix" id="<?php echo 'customstaff_'.$id_loop; ?>">
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Contact Person:</label>
                    <select data-placeholder="Select a Staff Member..." id="<?php echo 'stcontactid_'.$id_loop; ?>" name="assign_staff[]" class="chosen-select-deselect form-control" width="380">
                        <option value=''></option>
        ...
        <div class="additional_st clearfix">
            ...
            <div class="form-group clearfix" id="customstaff_0">
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Contact Person:</label>
                    <select data-placeholder="Select a Staff Member..." id="stcontactid_0" name="assign_staff[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
					  <?php $query = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc,"SELECT contactid, first_name, last_name FROM contacts WHERE category IN (".STAFF_CATS.") AND ".STAFF_CATS_HIDE_QUERY." AND deleted=0"),MYSQLI_ASSOC));
						foreach($query as $id) {
							echo "<option value='". $id."'>".get_contact($dbc, $id).'</option>';
