...
$(document).ready(function() {
    ...
    $('#add_row_cnt').on( 'click', function () {
        ...
        clone.find('#customcontractor_0').attr('id', 'customcontractor_'+add_new_cnt);
        $('#cntdelete_0').hide();
...
function selectContractor(sel) {
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=cnt_config&value="+stage,
...
<div class="form-group">
    <div class="col-sm-12">
        ...
        <?php if(!empty($_GET['customid'])) {
            ...
            ?>
                <div class="form-group clearfix" id="<?php echo 'customcontractor_'.$id_loop; ?>">
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Contractor:</label>
                    <select data-placeholder="Select a Contractor..." id="<?php echo 'cocontractorid_'.$id_loop; ?>" name="assign_contractor[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
					  <?php $query = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc,"SELECT contactid, first_name, last_name FROM contacts WHERE category='Contractor' AND deleted=0"),MYSQLI_ASSOC));
						foreach($query as $id) {
        ...
        <div class="additional_cnt clearfix">
            ...
            <div class="form-group clearfix" id="customcontractor_0">
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Contractor:</label>
                    <select data-placeholder="Select a Contractor..." id="cocontractorid_0" name="assign_contractor[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
					  <?php $query = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc,"SELECT contactid, first_name, last_name FROM contacts WHERE category='Contractor' AND deleted=0"),MYSQLI_ASSOC));
						foreach($query as $id) {
