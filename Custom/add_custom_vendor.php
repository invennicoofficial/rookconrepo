...
$(document).ready(function() {
    ...
    $('#add_row_v').on( 'click', function () {
        ...
        clone.find('#customvendor_0').attr('id', 'customvendor_'+add_new_v);
        $('#vdelete_0').hide();
...
$(document).on('change', 'select.pricelist_onchange', function() { selectPricelist(this); });
...
function selectVendor(sel) {
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=vendor_config&value="+stage,
...
function selectPricelist(sel) {
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=vpricelist_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function selectCategory(sel) {
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=vcat_config&value="+stage+"&pricelist="+pricelist,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function selectProduct(sel) {
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=vproduct_config&value="+stage+"&pricelist="+pricelist+"&ratecardid="+ratecardid,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
<div class="form-group">
    <div class="col-sm-12 ">
       ...
       <?php if(!empty($_GET['customid'])) {
            ...
            ?>
                <div class="form-group clearfix" id="<?php echo 'customvendor_'.$id_loop; ?>">
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Vendor:</label>
                    <select data-placeholder="Select a Vendor..." id="<?php echo 'prvendorid_'.$id_loop; ?>" class="chosen-select-deselect form-control equipmentid vendor_onchange" width="380">
                        <option value=''></option>
					  <?php $query = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc,"SELECT contactid, name FROM contacts WHERE category='Vendor' AND deleted=0"),MYSQLI_ASSOC));
						foreach($query as $id) {
							echo "<option ".(get_vendor_pricelist($dbc, $pricelistid, 'vendorid') == $id ? 'selected' : '')." value='". $id."'>".get_client($dbc, $id).'</option>';
						} ?>
                    </select>
                </div>

                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Price List:</label>
                    <select data-placeholder="Select a Price List Item..." id="<?php echo 'prpricelistid_'.$id_loop; ?>" class="chosen-select-deselect form-control equipmentid pricelist_onchange" width="380">
                        <option value=''></option>
                        <?php
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Category:</label>
                    <select data-placeholder="Select a Category..." id="<?php echo 'prcategory_'.$id_loop; ?>" class="chosen-select-deselect form-control equipmentid vendor_cat_onchange" width="380">
                        <option value=''></option>
                        <?php
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Product:</label>
                    <select data-placeholder="Select a Product..." id="<?php echo 'prproduct_'.$id_loop; ?>" name="assign_vendor[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
        ...
        <div class="additional_v clearfix">
            ...
            <div class="form-group clearfix" id="customvendor_0">
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Vendor:</label>
                    <select data-placeholder="Select a Vendor..." id="prvendorid_0" class="chosen-select-deselect form-control equipmentid vendor_onchange" width="380">
                        <option value=''></option>
					  <?php $query = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc,"SELECT contactid, name FROM contacts WHERE category='Vendor' AND deleted=0"),MYSQLI_ASSOC));
						foreach($query as $id) {
							echo "<option ".(get_vendor_pricelist($dbc, $pricelistid, 'vendorid') == $id ? 'selected' : '')." value='". $id."'>".get_client($dbc, $id).'</option>';
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Price List:</label>
                    <select data-placeholder="Select a Price List Item..." id="prpricelistid_0" class="chosen-select-deselect form-control equipmentid pricelist_onchange" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT pricelistid, pricelist_name FROM vendor_pricelist WHERE deleted=0");
                        while($row = mysqli_fetch_array($query)) {
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Category:</label>
                    <select data-placeholder="Select a Category..." id="prcategory_0" class="chosen-select-deselect form-control equipmentid vendor_cat_onchange" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT pricelistid, category FROM vendor_pricelist WHERE deleted=0");
                        while($row = mysqli_fetch_array($query)) {
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Product:</label>
                    <select data-placeholder="Select a Product..." id="prproduct_0" name="assign_vendor[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT pricelistid, name FROM vendor_pricelist WHERE deleted=0");
                        while($row = mysqli_fetch_array($query)) {
