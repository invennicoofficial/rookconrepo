...
$(document).ready(function() {
    ...
    $('#add_row_cl').on( 'click', function () {
        ...
        clone.find('#customclient_0').attr('id', 'customclient_'+add_new_cl);
        $('#cldelete_0').hide();
...
function selectClientName(sel) {
	...
	$.ajax({
		type: "GET",
		url: "custom_ajax_all.php?fill=cl_config&value="+stage,
...
<div class="form-group">
    <div class="col-sm-12">
        ...
        <?php if(!empty($_GET['customid'])) {
            ...
            ?>
                <div class="form-group clearfix" id="<?php echo 'customclient_'.$id_loop; ?>">
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Client:</label>
                    <select data-placeholder="Select a Client..." id="<?php echo 'clclientid_'.$id_loop; ?>" name="assign_clients[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
					  <?php $query = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc,"SELECT contactid, name FROM contacts WHERE category='Client' AND deleted=0"),MYSQLI_ASSOC));
						foreach($query as $id) {
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Contact Person:</label>
                    <select data-placeholder="Select a Contact..." id="<?php echo 'clclientperson_'.$id_loop; ?>" name="clientperson[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
					  <?php $query = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc,"SELECT contactid, first_name, last_name FROM contacts WHERE category='Client' AND deleted=0"),MYSQLI_ASSOC));
						foreach($query as $id) {
							echo "<option ".($contactid == $row['contactid'] ? 'selected' : '')." value='". $id."'>".get_contact($dbc, $id).'</option>';
						} ?>
                    </select>
                </div>
                <?php if (strpos($field_config_clients, ','."Final Retail Price".',') !== FALSE) { ?>
        ...
        <div class="additional_cl clearfix">
            ...
            <div class="form-group clearfix" id="customclient_0">
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Client:</label>
                    <select data-placeholder="Select a Client..." id="clclientid_0" name="assign_clients[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
					  <?php $query = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc,"SELECT contactid, name FROM contacts WHERE category='Client' AND deleted=0"),MYSQLI_ASSOC));
						foreach($query as $id) {
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Contact Person:</label>
                    <select data-placeholder="Select a Contact..." id="clclientperson_0" name="clientperson[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
					  <?php $query = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc,"SELECT contactid, first_name, last_name FROM contacts WHERE category='Client' AND deleted=0"),MYSQLI_ASSOC));
						foreach($query as $id) {
