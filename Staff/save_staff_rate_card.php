...
if($staff_id > 0 && isset($_POST['ratecardid'])) {
	$rate_cards_keep = [];
	foreach($_POST['ratecardid'] as $i => $ratecardid) {
		...
		$work_desc = filter_var($_POST['work_desc'][$i],FILTER_SANITIZE_STRING);
		$history = 'Staff rate card '.($ratecardid == '' ? 'Added' : 'Edited').' by '.get_contact($dbc, $_SESSION['contactid']).' on '.date('Y-m-d h:i:s');

		if(!empty($start_date.$end_date.$alert_date.$alert_staff.$annual.$monthly.$semi_month.$weekly.$daily.$hourly.$hourly_work.$hourly_travel.$field_day_actual.$field_day_bill.$cost.$price_admin.$price_wholesale.$price_commercial.$price_client.$minimum.$unit_price.$unit_cost.$rent_price.$rent_days.$rent_weeks.$rent_months.$rent_years.$num_days.$num_hours.$num_kms.$num_miles.$fee.$hours_estimated.$hours_actual.$service_code.$description.$work_desc)) {
			$sql = '';
			if($ratecardid == '') {
				$sql = "INSERT INTO `staff_rate_table` (`staff_id`,`start_date`,`end_date`,`annual`,`monthly`,`semi_month`,`weekly`,`daily`,`hourly`,`hourly_work`,`hourly_travel`,`field_day_actual`,`field_day_bill`,`cost`,`price_admin`,`price_wholesale`,`price_commercial`,`price_client`,`minimum`,`unit_price`,`unit_cost`,`rent_price`,`rent_days`,`rent_weeks`,`rent_months`,`rent_years`,`num_days`,`num_hours`,`num_kms`,`num_miles`,`fee`,`hours_estimated`,`hours_actual`,`service_code`,`description`,`work_desc`,`history`,`created_by`,`alert_date`,`alert_staff`) VALUES
					($staff_id,'$start_date','$end_date','$annual','$monthly','$semi_month','$weekly','$daily','$hourly','$hourly_work','$hourly_travel','$field_day_actual','$field_day_bill','$cost','$price_admin','$price_wholesale','$price_commercial','$price_client','$minimum','$unit_price','$unit_cost','$rent_price','$rent_days','$rent_weeks','$rent_months','$rent_years','$num_days','$num_hours','$num_kms','$num_miles','$fee','$hours_estimated','$hours_actual','$service_code','$description','$work_desc','$history','".$_SESSION['contactid']."','$alert_date','$alert_staff')";
				$result = mysqli_query($dbc, $sql);
				$ratecardid = mysqli_insert_id($dbc);
			}
			else {
				$sql = "UPDATE `staff_rate_table` SET `staff_id`=$staff_id,`start_date`='$start_date',`end_date`='$end_date',`annual`='$annual',`monthly`='$monthly',`semi_month`='$semi_month',`weekly`='$weekly',`daily`='$daily',`hourly`='$hourly',`hourly_work`='$hourly_work',`hourly_travel`='$hourly_travel',`field_day_actual`='$field_day_actual',`field_day_bill`='$field_day_bill',`cost`='$cost',`price_admin`='$price_admin',`price_wholesale`='$price_wholesale',`price_commercial`='$price_commercial',`price_client`='$price_client',`minimum`='$minimum',`unit_price`='$unit_price',`unit_cost`='$unit_cost',`rent_price`='$rent_price',`rent_days`='$rent_days',`rent_weeks`='$rent_weeks',`rent_months`='$rent_months',`rent_years`='$rent_years',`num_days`='$num_days',`num_hours`='$num_hours',`num_kms`='$num_kms',`num_miles`='$num_miles',`fee`='$fee',`hours_estimated`='$hours_estimated',`hours_actual`='$hours_actual',`service_code`='$service_code',`description`='$description',`work_desc`='$work_desc',`history`=IFNULL(CONCAT(`history`,'<br />\n','$history'),'$history'),`alert_date`='$alert_date',`alert_staff`='$alert_staff' WHERE `rate_id`='$ratecardid'";
				$result = mysqli_query($dbc, $sql);
			}
