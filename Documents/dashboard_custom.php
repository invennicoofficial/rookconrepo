...
if(!empty($_GET['search_type'])) {
	$query_search .= " AND `custom_documents_type` = '".$_GET['search_type']."'";
}
if(!empty($_GET['search_category'])) {
	$query_search .= " AND `category` = '".$_GET['search_category']."'";
}
if(!empty($_GET['search_query'])) {
	$query_search .= " AND (custom_documents_code LIKE '%".$_GET['search_query']."%' OR custom_documents_type LIKE '%".$_GET['search_query']."%' OR category LIKE '%".$_GET['search_query']."%' OR heading LIKE '%".$_GET['search_query']."%' OR name LIKE '%".$_GET['search_query']."%' OR title LIKE '%".$_GET['search_query']."%' OR fee LIKE '%".$_GET['search_query']."%')";
}
$query_check_credentials = "SELECT * FROM custom_documents WHERE deleted = 0 AND `tab_name` = '".$_GET['tab']."' $query_search LIMIT $offset, $rowsPerPage";
$query = "SELECT count(*) as numrows FROM custom_documents WHERE deleted = 0 AND `tab_name` = '".$_GET['tab']."' $query_search";
...
if($num_rows > 0) {
	...
	echo "<tr class='hidden-sm hidden-xs'>";
		if (strpos($value_config, ','."Custom Documents Code".',') !== FALSE) {
			echo '<th>Documents Code</th>';
		}
		if (strpos($value_config, ','."Custom Documents Type".',') !== FALSE) {
		...
		if (strpos($value_config, ','."Custom Price".',') !== FALSE) {
			echo '<th>Price</th>';
		}
...
{
	echo "<tr>";
	$custom_documentsid = $row['custom_documentsid'];
	if (strpos($value_config, ','."Custom Documents Code".',') !== FALSE) {
		echo '<td data-title="Doc. Code">' . $row['custom_documents_code'] . '</td>';
	}
	if (strpos($value_config, ','."Custom Documents Type".',') !== FALSE) {
		echo '<td data-title="Doc. Type">' . $row['custom_documents_type'] . '</td>';
	}
	if (strpos($value_config, ','."Category".',') !== FALSE) {
	...
	if (strpos($value_config, ','."Uploader".',') !== FALSE) {
		...
		$custom_documents_uploads1 = "SELECT * FROM custom_documents_uploads WHERE custom_documentsid='$custom_documentsid' AND type = 'Document' ORDER BY certuploadid DESC";
		$result1 = mysqli_query($dbc, $custom_documents_uploads1);
	...
	if (strpos($value_config, ','."Link".',') !== FALSE) {
		...
		$custom_documents_uploads2 = "SELECT * FROM custom_documents_uploads WHERE custom_documentsid='$custom_documentsid' AND type = 'Link' ORDER BY certuploadid DESC";
		$result2 = mysqli_query($dbc, $custom_documents_uploads2);
	...
	if (strpos($value_config, ','."Custom Price".',') !== FALSE) {
		echo '<td data-title="Price">' . $row['custom_price'] . '</td>';
	}
	if (strpos($value_config, ','."Minimum Billable".',') !== FALSE) {
	...
	if($edit_access == 1) {
	echo '<a href=\'?tile_name='.$tile_name.'&tab='.$_GET['tab'].'&edit='.$custom_documentsid.'\'>Edit</a> | ';
	echo '<a href=\''.WEBSITE_URL.'/delete_restore.php?action=delete&custom_documentsid='.$custom_documentsid.'&from_tile=documents_all&tile_name='.$tile_name.'&tab_name='.$_GET['tab'].'\' onclick="return confirm(\'Are you sure?\')">Archive</a>';
	}
	echo '</td>';
