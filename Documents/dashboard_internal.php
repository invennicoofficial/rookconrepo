...
if(!empty($_GET['search_type'])) {
	$query_search .= " AND `internal_documents_type` = '".$_GET['search_type']."'";
}
...
while($row = mysqli_fetch_array( $result ))
{
	...
	if (strpos($value_config, ','."Uploader".',') !== FALSE) {
		...
		if($num_rows1 > 0) {
			while($row1 = mysqli_fetch_array($result1)) {
				echo '<ul>';
				if(file_get_contents('../Internal Documents/download/'.$row1['document_link'])) {
					$download_link = '../Internal Documents/download/'.$row1['document_link'];
				} else {
	...
	if($edit_access == 1) {
	echo '<a href=\'?tile_name='.$tile_name.'&tab='.$_GET['tab'].'&edit='.$internal_documentsid.'\'>Edit</a> | ';
	echo '<a href=\''.WEBSITE_URL.'/delete_restore.php?action=delete&internal_documentsid='.$internal_documentsid.'&from_tile=documents_all&tile_name='.$tile_name.'\' onclick="return confirm(\'Are you sure?\')">Archive</a>';
	}
	echo '</td>';
