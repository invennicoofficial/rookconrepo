...
if(!empty($_GET['certuploadid'])) {
    ...
    $internal_documentsid = $_GET['edit'];

    echo '<script type="text/javascript"> window.location.replace("?tile_name='.$tile_name.'&tab='.$_GET['tab'].'&edit='.$internal_documentsid.'"); </script>';
}
        ...
        if(!empty($_GET['edit'])) {
            ...
            if($num_rows > 0) {
                while($row = mysqli_fetch_array($result)) {
                    ...
                    echo '<ul>';
                    if(file_get_contents('../Internal Documents/download/'.$row['document_link'])) {
                        $download_link = '../Internal Documents/download/'.$row['document_link'];
                    } else {
                        $download_link = 'download/'.$row['document_link'];
                    }
                    echo '<li><a href="'.$download_link.'" target="_blank">'.$row['document_link'].'</a> - <a href="?tile_name='.$tile_name.'&tab='.$_GET['tab'].'&certuploadid='.$certuploadid.'&edit='.$internal_documentsid.'"> Delete</a></li>';
                    echo '</ul>';
                }
            }
        }
        ...
        if(!empty($_GET['edit'])) {
            ...
            if($num_rows > 0) {
                ...
                while($row = mysqli_fetch_array($result)) {
                    $certuploadid = $row['certuploadid'];
                    echo '<ul>';
                    echo '<li><a target="_blank" href=\''.$row['document_link'].'\'">Link '.$link_no.'</a> - <a href="?tile_name='.$tile_name.'&tab='.$_GET['tab'].'&certuploadid='.$certuploadid.'&edit='.$internal_documentsid.'"> Delete</a></li>';
                    echo '</ul>';
                    $link_no++;
                }
            }
