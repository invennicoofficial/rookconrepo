...
if(!empty($_GET['certuploadid'])) {
    ...
    $query = mysqli_query($dbc,"DELETE FROM custom_documents_uploads WHERE certuploadid='$certuploadid'");
    $custom_documentsid = $_GET['edit'];

    echo '<script type="text/javascript"> window.location.replace("?tile_name='.$tile_name.'&tab='.$_GET['tab'].'&edit='.$custom_documentsid.'"); </script>';
}

if (isset($_POST['add_custom_documents'])) {
    $tab_name = $_POST['tab_name'];
    if($_POST['new_custom_documents'] != '') {
        $custom_documents_type = filter_var($_POST['new_custom_documents'],FILTER_SANITIZE_STRING);
    } else {
        $custom_documents_type = filter_var($_POST['custom_documents_type'],FILTER_SANITIZE_STRING);
    }
    ...
    $custom_documents_code = filter_var($_POST['custom_documents_code'],FILTER_SANITIZE_STRING);
    //$category = filter_var($_POST['category'],FILTER_SANITIZE_STRING);
    ...
    $custom_price = filter_var($_POST['custom_price'],FILTER_SANITIZE_STRING);
    ...
    if(empty($_POST['custom_documentsid'])) {
        $query_insert_vendor = "INSERT INTO `custom_documents` (`tab_name`, `custom_documents_type`, `category`, `custom_documents_code`, `heading`, `cost`, `description`, `quote_description`, `invoice_description`, `ticket_description`, `final_retail_price`, `admin_price`, `wholesale_price`, `commercial_price`, `custom_price`, `minimum_billable`, `estimated_hours`, `actual_hours`, `msrp`, `name`, `title`,  `fee`, `unit_price`, `unit_cost`, `rent_price`, `rental_days`, `rental_weeks`, `rental_months`, `rental_years`, `reminder_alert`, `daily`,`weekly`, `monthly`, `annually`,  `total_days`, `total_hours`, `total_km`, `total_miles`, `upload_date`) VALUES ('$tab_name', '$custom_documents_type', '$category', '$custom_documents_code', '$heading', '$cost', '$description', '$quote_description', '$invoice_description', '$ticket_description', '$final_retail_price', '$admin_price', '$wholesale_price', '$commercial_price', '$custom_price', '$minimum_billable', '$estimated_hours', '$actual_hours', '$msrp', '$name', '$title', '$fee', '$unit_price', '$unit_cost', '$rent_price', '$rental_days', '$rental_weeks', '$rental_months', '$rental_years', '$reminder_alert', '$daily', '$weekly', '$monthly', '$annually', '$total_days', '$total_hours', '$total_km', '$total_miles', '$upload_date')";
        $result_insert_vendor = mysqli_query($dbc, $query_insert_vendor);
        $custom_documentsid = mysqli_insert_id($dbc);
        $url = 'Added';
    } else {
        $custom_documentsid = $_POST['custom_documentsid'];
        $query_update_vendor = "UPDATE `custom_documents` SET `custom_documents_type` = '$custom_documents_type', `category` = '$category',`custom_documents_code` = '$custom_documents_code', `heading` = '$heading', `cost` = '$cost', `description` = '$description', `quote_description` = '$quote_description', `invoice_description` = '$invoice_description', `ticket_description` = '$ticket_description', `final_retail_price` = '$final_retail_price', `admin_price` = '$admin_price', `wholesale_price` = '$wholesale_price', `commercial_price` = '$commercial_price', `custom_price` = '$custom_price', `minimum_billable` = '$minimum_billable', `estimated_hours` = '$estimated_hours', `actual_hours` = '$actual_hours', `msrp` = '$msrp', `name` = '$name', `title` = '$title', `fee` = '$fee', `unit_price` = '$unit_price', `unit_cost` = '$unit_cost', `rent_price` = '$rent_price', `rental_days` = '$rental_days', `rental_weeks` = '$rental_weeks', `rental_months` = '$rental_months', `rental_years` = '$rental_years', `reminder_alert` = '$reminder_alert', `daily` = '$daily', `weekly` = '$weekly', `monthly` = '$monthly', `annually` = '$annually', `total_days` = '$total_days', `total_hours` = '$total_hours', `total_km` = '$total_km', `total_miles` = '$total_miles', `upload_date` = '$upload_date' WHERE `custom_documentsid` = '$custom_documentsid'";
    ...
    for($i = 0; $i < count($_FILES['upload_document']['name']); $i++) {
        ...
        if($document != '') {
            ...
            $query_insert_custom_doc = "INSERT INTO `custom_documents_uploads` (`custom_documentsid`, `type`, `document_link`) VALUES ('$custom_documentsid', 'Document', '$document')";
            $result_insert_custom_doc = mysqli_query($dbc, $query_insert_custom_doc);
        }
    }
    ...
    for($i = 0; $i < count($_POST['support_link']); $i++) {
        ...
        if($support_link != '') {
            $query_insert_custom_doc = "INSERT INTO `custom_documents_uploads` (`custom_documentsid`, `type`, `document_link`) VALUES ('$custom_documentsid', 'Link', '$support_link')";
            $result_insert_custom_doc = mysqli_query($dbc, $query_insert_custom_doc);
        }
    }
...
$(document).ready(function() {
    $("#form1").submit(function( event ) {
        var custom_documents_type = $("#custom_documents_type").val();
        var category = $("input[name=category]").val();
        var title = $("input[name=title]").val();
        if (custom_documents_type == '' || category == '' || title == '' ) {
            alert("Please make sure you have filled in all of the required fields.");
            return false;
        }
    });

    $("#custom_documents_type").change(function() {
        if($("#custom_documents_type option:selected").text() == 'New Documents') {
                $( "#new_custom_documents" ).show();
        } else {
            $( "#new_custom_documents" ).hide();
        }
    });
...
<?php
    ...
    if(!empty($_GET['edit'])) {
        ...
        $custom_documentsid = $_GET['edit'];
        $get_contact = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM custom_documents WHERE custom_documentsid='$custom_documentsid'"));
        $custom_documents_type = $get_contact['custom_documents_type'];
        $category = $get_contact['category'];
        $custom_documents_code = $get_contact['custom_documents_code'];
        ...
        $custom_price = $get_contact['custom_price'];
    ...
    ?>
    <input type="hidden" id="custom_documentsid" name="custom_documentsid" value="<?php echo $custom_documentsid ?>" />
    <?php   }      ?>

    <?php if (strpos($value_config, ','."Custom Documents Type".',') !== FALSE) { ?>
   <div class="form-group">
    <label for="company_name" class="col-sm-4 control-label">Document Type<span class="hp-red">*</span>:</label>
    <div class="col-sm-8">
        <select id="custom_documents_type" name="custom_documents_type" class="chosen-select-deselect form-control" width="380">
            <option value=''></option>
            <?php
            $query = mysqli_query($dbc,"SELECT distinct(custom_documents_type) FROM custom_documents order by custom_documents_type");
            while($row = mysqli_fetch_array($query)) {
                if ($custom_documents_type == $row['custom_documents_type']) {
                    $selected = 'selected="selected"';
                } else {
                    $selected = '';
                }
                echo "<option ".$selected." value='". $row['custom_documents_type']."'>".$row['custom_documents_type'].'</option>';

            }
            echo "<option value = 'Other'>New Documents</option>";
            ?>
        </select>
    </div>
  </div>

   <div class="form-group" id="new_custom_documents" style="display: none;">
    <label for="travel_task" class="col-sm-4 control-label">New Document Type:
    </label>
    <div class="col-sm-8">
        <input name="new_custom_documents" type="text" class="form-control" />
    </div>
  </div>
  <?php } ?>
   ...
   <div class="form-group">
    ...
    <div class="col-sm-8">
        <select id="category" name="category" class="chosen-select-deselect form-control" width="380">
            <option value=''></option>
            <?php
            $query = mysqli_query($dbc,"SELECT distinct(category) FROM custom_documents order by category");
            while($row = mysqli_fetch_array($query)) {
...
<div class="form-group">
    ...
    <div class="col-sm-8">
        <?php
        if(!empty($_GET['edit'])) {
            $query_check_credentials = "SELECT * FROM custom_documents_uploads WHERE custom_documentsid='$custom_documentsid' AND type = 'Document' ORDER BY certuploadid DESC";
            ...
            if($num_rows > 0) {
                while($row = mysqli_fetch_array($result)) {
                    ...
                    echo '<li><a href="'.$download_link .'" target="_blank">'.$row['document_link'].'</a> - <a href="?tile_name='.$tile_name.'&tab='.$_GET['tab'].'&certuploadid='.$certuploadid.'&edit='.$custom_documentsid.'"> Delete</a></li>';
                    echo '</ul>';
                }
            }
        }
...
<div class="form-group">
    ...
    <div class="col-sm-8">
        <?php
        if(!empty($_GET['edit'])) {
            $query_check_credentials = "SELECT * FROM custom_documents_uploads WHERE custom_documentsid='$custom_documentsid' AND type = 'Link' ORDER BY certuploadid DESC";
            $result = mysqli_query($dbc, $query_check_credentials);
            $num_rows = mysqli_num_rows($result);
            if($num_rows > 0) {
                $link_no = 1;
                while($row = mysqli_fetch_array($result)) {
                    $certuploadid = $row['certuploadid'];
                    echo '<ul>';
                    echo '<li><a target="_blank" href=\''.$row['document_link'].'\'">Link '.$link_no.'</a> - <a href="?tile_name='.$tile_name.'&tab='.$_GET['tab'].'&certuploadid='.$certuploadid.'&edit='.$custom_documentsid.'"> Delete</a></li>';
                    echo '</ul>';
                    $link_no++;
                }
            }
...
<?php } ?>
  ...
  <?php if (strpos($value_config, ','."Custom Documents Code".',') !== FALSE) { ?>
  <div class="form-group">
    <label for="company_name" class="col-sm-4 control-label">Documents Code:</label>
    <div class="col-sm-8">
      <input name="custom_documents_code" value="<?php echo $custom_documents_code; ?>" type="text" id="name" class="form-control">
    </div>
  </div>
  <?php } ?>
  ...
  <?php if (strpos($value_config, ','."Custom Price".',') !== FALSE) { ?>
  <div class="form-group">
    <label for="company_name" class="col-sm-4 control-label">Price:</label>
    <div class="col-sm-8">
      <input name="custom_price" value="<?php echo $custom_price; ?>" type="text" class="form-control">
    </div>
  </div>
  <?php } ?>
...
<?php } ?>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-6">
            <button type="submit" name="add_custom_documents" value="Submit" class="btn brand-btn btn-lg pull-right">Submit</button>
