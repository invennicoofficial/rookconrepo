...
<form name="form_jobs" enctype="multipart/form-data" method="post" action="field_jobs.php" class="form-inline" role="form">
    <div id="no-more-tables">
        ...
        if(config_visible_function($dbc, 'purchase_order') == 1) {
            echo '<a href="field_config_purchase_order.php" class="mobile-block pull-right "><img style="width: 50px;" title="Tile Settings" src="../img/icons/settings-4.png" class="settings-classic wiggle-me"></a><br><br>';
        }
        ...
        $query_check_credentials = "SELECT *  FROM purchase_order WHERE deleted=0";
        ...
        while($row = mysqli_fetch_array( $result ))
        {
            ...
            if (strpos($value_config, ','."Ticket".',') !== FALSE) {
            echo '<td data-title="Ticket">' . get_tickets($dbc, $row['ticketid'], 'service_type').' : '.get_tickets($dbc, $row['ticketid'], 'heading') . '</td>';
            }
            if (strpos($value_config, ','."Work Order".',') !== FALSE) {
