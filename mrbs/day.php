...
echo '<span class="popover-examples list-inline"><a data-toggle="tooltip" data-placement="right" title="This color legend shows which type of appointment the patient or time slot is for."><img src="../img/info.png" width="20"></a></span><br />';
show_colour_key();
// Draw the three month calendars
if ($display_calendar_bottom)
{
...
$(document).ready(function() {
    $( ".dwm_main" ).sortable({
		connectWith: ".dwm_main",
		delay: 500,
		handle: ".drag_handle",
		items: "td:not(.row_labels)",
		update: function( event, appt ) {
			var calid = appt.item.find('.celldiv').data('id');
			var time = appt.item.closest('tr').find('td').first().data('seconds');
			var room = 0;
			$('.dwm_main').find('th').each(function() {
				if($(this).position().left == appt.item.position().left) {
					room = $(this).data('room');
				}
			});
			$.ajax({
				method: 'POST',
				url: 'move_entry_ajax.php?action=day_move',
				data: { id: calid, roomid: room, time: time },
