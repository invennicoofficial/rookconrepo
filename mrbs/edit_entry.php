...
$entry_fields = array('rooms', 'type', 'name', 'description', 'start_date', 'end_date', 'areas',
                       'confirmation_status', 'privacy_status');
...
function create_field_entry_custom_field($field, $key, $disabled=FALSE)
{
  ...
  $on_change = '';
  if(get_loc_field_name($tbl_entry, $key) == 'Patient*') {
    $on_change = 'onChange=selectPatient(this)';

    if(!empty($_GET['patient'])) {
        $custom_fields[$key] = $_GET['patient'];
    } else if(!empty($_GET['contactid'])) {
		$custom_fields[$key] = $_GET['contactid'];
	}
  } else if(get_loc_field_name($tbl_entry, $key) == 'Status*') {
    $on_change = 'onChange="if(this.value == \'Cancelled\' || this.value == \'Late Cancellation / No-Show\') { $(\'#cancelReasonDiv\').show(); } else { $(\'#cancelReasonDiv\').hide(); }"';
  }
  echo "<div>\n";
...
$(document).ready(function() {
	$('.iframe_open').click(function(){
		if($(this).hasClass("reactivate_patient")) {
		    $('#iframe_instead_of_window').attr('src', '<?php echo WEBSITE_URL; ?>/Contacts/contacts.php?category=Patient');
		    $('.iframe_title').text('');
		} else if($(this).hasClass("add_injury")) {
			var id = $(this).attr('id');
			var arr = id.split('_');
		    $('#iframe_instead_of_window').attr('src', '<?php echo WEBSITE_URL; ?>/Contacts/add_injury.php?type=contact&contactid='+arr[0]);
		    $('.iframe_title').text('');
		} else if($(this).hasClass("check_in")) {
			var id = $(this).attr('id');
			var arr = id.split('_');
		    $('#iframe_instead_of_window').attr('src', '<?php echo WEBSITE_URL; ?>/Check In/add_checkin_patient.php?bookingid='+arr[0]);
		    $('.iframe_title').text('');
...
<?php
  if(get_loc_field_name($tbl_entry, $key) == 'Patient*') {
    //echo '<br><br>&nbsp;OR&nbsp;<a style="font-weight:bold;" class="iframe_open reactivate_patient" id="2312212">Reactivate From Here</a>';
	echo '&nbsp;OR&nbsp;<a href="#"  onclick=" window.open(\''.WEBSITE_URL.'/Contacts/contacts.php?category=Patient\', \'newwindow\', \'width=900, height=900\'); return false;">Reactivate From Here</a>&nbsp;&nbsp; | &nbsp;&nbsp;<a href="#"  onclick=" window.open(\''.WEBSITE_URL.'/Contacts/add_contacts.php?category=Patient\', \'newwindow\', \'width=900, height=900\'); return false;">Add New Patient</a>';
  }

 if(get_loc_field_name($tbl_entry, $key) == 'Injury*') {
    //$patient_name = explode(' ', $_GET['patient']);
    //$first_name = encryptIt($patient_name[0]);
    //$last_name = encryptIt($patient_name[1]);

    //$sid = sql_query("SELECT contactid FROM contacts WHERE first_name = '$first_name' AND last_name = '$last_name'");
    //$rid = sql_row_keyed($sid, 0);
    //$patientid = $rid['contactid'];
	$patientid = $_GET['patient'];
	 // echo '<br><br>&nbsp;OR&nbsp;<a style="font-weight:bold;" class="iframe_open add_injury" id="'.$patientid.'">Add Injury</a>';
		echo '&nbsp;OR&nbsp;<a href="#"  onclick=" window.open(\''.WEBSITE_URL.'/Contacts/add_injury.php?type=contact&contactid='.$patientid.'\', \'newwindow\', \'width=900, height=900\'); return false;">Add Injury</a>';
  }

  echo "</div>\n";
  
  if(get_loc_field_name($tbl_entry, $key) == 'Status*') {
	  $cancelReason = sql_row_keyed(sql_query("SELECT `cancellation` FROM `booking` WHERE `calid`='".$_GET['id']."'"), 0)['cancellation'];
	echo "<div id='cancelReasonDiv' style='".($custom_fields[$key] != 'Cancelled' && $custom_fields[$key] != 'Late Cancellation / No-Show' ? "display:none;" : '')."'>\n";
	generate_input([ 'label'      => "Cancellation Reason:",
                  'field'       => "entry.cancellation",
                  'name'       => "cancellation",
				  'options'    => [ 'Out of Funds'=>'Out of Funds',
					'Not Improving'=>'Not Improving',
...
$result_waitlist = sql_query("SELECT COUNT(waitlistid) AS total_waitlist_result FROM waitlist WHERE desired_date <= CURDATE()");
$row_waitlist = sql_row_keyed($result_waitlist, 0);
//$room_id = $row['id'];

echo '<input type="hidden" id="total_waitlist" name="total_waitlist" value="'.$row_waitlist['total_waitlist_result'].'" />';
...
if (($edit_type == "series") && $repeats_allowed)
{
  ...
  if (!$disabled || ($rep_type != REP_NONE))
  {
   ...
   echo '<input type="hidden" name="booking_id_cal" value="'.$_GET['id'].'">';

    //

    $cal_id = $_GET['id'];
    $sname = sql_query("SELECT bookingid, follow_up_call_status FROM booking WHERE calid = '$cal_id'");
    $rows = sql_row_keyed($sname, 0);
    $bookingid = $rows['bookingid'];
    $follow_up_call_status = $rows['follow_up_call_status'];

    if($follow_up_call_status == 'Booked Confirmed') {
        //echo '<a style="font-weight:bold;" class="iframe_open check_in" id="'.$bookingid.'"><h3>Check In</h3></a>';
		echo '<a href="#"  onclick="window.open(\''.WEBSITE_URL.'/Check In/add_checkin_patient.php?bookingid='.$bookingid.'\', \'newwindow\', \'width=900, height=900\'); return false;"><h3>Check In</h3></a>';
    }
    if($follow_up_call_status == 'Arrived') {
        $sname = sql_query("SELECT invoiceid FROM invoice WHERE bookingid = '$bookingid'");
        $rows = sql_row_keyed($sname, 0);
    ...
    <script>
    jQuery('#f_patient').addClass('chosen-select-deselect');
    </script>

    <?php $cal_url = WEBSITE_URL; ?>
    <link rel="stylesheet" href="<?php echo $cal_url;?>/css/chosen.css" type="text/css">
    <style>
