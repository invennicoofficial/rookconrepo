...
$(document).ready(function() {
	$('.iframe_open').click(function(){
		if($(this).hasClass("view_history")) {
			var id = $(this).attr('id');
			var arr = id.split('_');
		    $('#iframe_instead_of_window').attr('src', '<?php echo WEBSITE_URL; ?>/Reports/report_patient_appoint_history.php?patientid='+arr[0]);
		    $('.iframe_title').text('');
		} else if($(this).hasClass("view_block_book")) {
			var id = $(this).attr('id');
			var arr = id.split('_');
		    $('#iframe_instead_of_window').attr('src', '<?php echo WEBSITE_URL; ?>/Reports/report_patient_block_booking.php?patientid='+arr[0]);
		    $('.iframe_title').text('');
		} else if($(this).hasClass("check_in")) {
			var id = $(this).attr('id');
			var arr = id.split('_');
		    $('#iframe_instead_of_window').attr('src', '<?php echo WEBSITE_URL; ?>/Check In/add_checkin_patient.php?bookingid='+arr[0]+'&from=calendar');
		    $('.iframe_title').text('');
...
<?php
    $sname = sql_query("SELECT bookingid, patientid FROM booking WHERE calid = '$id'");
    $rows = sql_row_keyed($sname, 0);
    $bookingid = $rows['bookingid'];
    $patientid = $rows['patientid'];

    if($row['patientstatus'] == 'Booked Confirmed') {
        //echo '<a style="font-weight:bold;" class="iframe_open check_in" id="'.$bookingid.'">Check In</a>';
		echo '<a href="#"  onclick=" window.open(\'' . WEBSITE_URL.'/Check In/add_checkin_patient.php?bookingid='.$bookingid.'&from=calendar\', \'newwindow\', \'width=900, height=900\'); return false;">Check In</a>';
    }
    if($row['patientstatus'] == 'Arrived') {
        $sname2 = sql_query("SELECT invoiceid FROM invoice WHERE bookingid = '$bookingid'");
...
<div id="view_entry_nav">
  ...
  if (!$room_disabled)
  {
    ...
    if (!$series)
    {
      if($row['patientstatus'] != 'Arrived' && $row['patientstatus'] != 'Invoiced' && $row['patientstatus'] != 'Paid') {
        echo "<a href=\"edit_entry.php?id=$id&amp;contactid=$patientid&amp;patient=$patientid&amp;returl=$link_returl\">". get_vocab("editentry") ."</a>";
      }
    }
	...
	//echo '<a style="font-weight:bold;" id="'.$patientid.'" class="iframe_open view_history">View Patient History</a>';
    //echo '<a style="font-weight:bold;" id="'.$patientid.'" class="iframe_open view_block_book">View Patient Block Booking</a>';
    //echo '<a href="#"  onclick=" window.open(\'http://' . $_SERVER['SERVER_NAME'].'/Contacts/add_contacts.php?category=Patient&target=All Patient Invoices&contactid='.$patientid.'\', \'newwindow\', \'width=900, height=900\'); return false;">View Patient History</a>';
    //echo '<br><a href="#"  onclick=" window.open(\'http://' . $_SERVER['SERVER_NAME'].'/Contacts/add_contacts.php?category=Patient&target=Patient Block Booking&contactid='.$patientid.'\', \'newwindow\', \'width=900, height=900\'); return false;">View Patient Block Booking</a>';
    echo '<a href="#"  onclick=" window.open(\'' . WEBSITE_URL.'/Reports/report_patient_appoint_history.php?patientid='.$patientid.'&type=sales\', \'newwindow\', \'width=900, height=900\'); return false;">View Patient History</a>';
    echo '<br><a href="#"  onclick=" window.open(\'' . WEBSITE_URL.'/Reports/report_patient_block_booking.php?patientid='.$patientid.'&type=operations\', \'newwindow\', \'width=900, height=900\'); return false;">View Patient Block Booking</a>';

    // Delete and Delete Series
    echo "<div>\n";
    if (!$series)
    {
      //if($row['patientstatus'] != 'Booked Unconfirmed') {
