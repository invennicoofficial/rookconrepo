...
<div class="container">
	...
	<?php if(check_subtab_persmission($dbc,'safety',ROLE,'reporting')) { ?>
	...
	if($tab == '' && strpos($value_config, ',Driving Log,') !== FALSE) {
		echo "<script> window.location.replace('driving_log.php?tab=Driving Log'); </script>";
		$tab = 'Driving Log';
	}
	if($tab == '' && strpos($value_config, ',FLHA,') !== FALSE) {
		$tab = 'FLHA';
	}
	if($tab == '' && strpos($value_config, ',Toolbox,') !== FALSE) {
		$tab = 'Toolbox';
	}
	if($tab == '' && strpos($value_config, ',Tailgate,') !== FALSE) {
		$tab = 'Tailgate';
	}
	if($tab == '' && strpos($value_config, ',Form,') !== FALSE) {
		$tab = 'Form';
	}
	if($tab == '' && strpos($value_config, ',Manual,') !== FALSE) {
		$tab = 'Manual';
	}
	if($tab == '' && strpos($value_config, ',Incident Reports,') !== FALSE) {
		echo "<script> window.location.replace('incident_report.php?&tab=Incident Reports'); </script>";
	...
	if($accordion == 'on') {
        ...
        <?php if($accordion !== 'on') {
			...
			while($row_tab = mysqli_fetch_array($tabs)) {
				...
				if($category !== '') {
					$tab_list .= '<a href="safety.php?site='.$site_get.'&tab='.$tab.'&category='.$category.'"><button type="button" class="mobile-100 btn brand-btn mobile-block  '.$class.'" style="margin-right:9px;" >'.$category.'</button></a>';
				}
			} ?><br>
        ...
        <br>
		<div class='mobile-100-container'><?= $tab_list ?></div>
		<?php } ?>
        <br><br>
        ...
        <?php if(vuaed_visible_function($dbc, 'safety')) {
        ...
        <div class="form-group triple-pad-top triple-pad-bottom clearfix location">
            ...
            <div class="col-sm-12">
                <?php
                if(!empty($_GET['category'])) {
                    echo manual_checklist($dbc, '35', '20', '20', $tab	, $_GET['category']);
                }
                ?>
            </div>
        </div>
