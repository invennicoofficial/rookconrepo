 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        ...
        imagepng($img_big, 'spill_incident_report/download/diagram_'.$safetyid.'.png');

        $query_insert_site = "INSERT INTO `safety_spill_incident_report` (`safetyid`, `today_date`, `contactid`, `fields`, `desc`, `desc1`, `desc2`, `desc3`, `desc4`, `desc5`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$fields', '$desc', '$desc1', '$desc2', '$desc3', '$desc4', '$desc5', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('spill_incident_report_pdf.php');
            echo spill_incident_report_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_spill_incident_report` SET `contactid` = '$contactid', `fields` = '$fields', `desc` = '$desc', `desc1` = '$desc1', `desc2` = '$desc2', `desc3` = '$desc3', `desc4` = '$desc4', `desc5` = '$desc5' WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'spill_incident_report/download/safety_'.$assign_staff_id.'.png');
