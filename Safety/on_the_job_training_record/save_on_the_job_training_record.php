 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        ...
        imagepng($img_big, 'on_the_job_training_record/download/diagram_'.$safetyid.'.png');

        $query_insert_site = "INSERT INTO `safety_on_the_job_training_record` (`safetyid`, `today_date`, `contactid`, `fields`, `desc`, `desc1`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$fields', '$desc', '$desc1', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('on_the_job_training_record_pdf.php');
            echo on_the_job_training_record_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_on_the_job_training_record` SET `contactid` = '$contactid', `fields` = '$fields', `desc` = '$desc', `desc1` = '$desc1' WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'on_the_job_training_record/download/safety_'.$assign_staff_id.'.png');
