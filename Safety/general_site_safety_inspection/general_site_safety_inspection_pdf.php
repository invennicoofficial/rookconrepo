<?php
	function general_site_safety_inspection_pdf($dbc,$safetyid, $fieldlevelriskid) {
    $form_by = $_SESSION['contactid'];
	$get_field_level = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM safety_general_site_safety_inspection WHERE fieldlevelriskid='$fieldlevelriskid'"));
	...
	$result_update_employee = mysqli_query($dbc, "UPDATE `safety_general_site_safety_inspection` SET `status` = 'Done' WHERE fieldlevelriskid='$fieldlevelriskid'");
	...
	$html_weekly = '<h2>General Site Safety Inspection</h2>'; // Form nu heading
    ...
    $html_weekly .= '<tr nobr="true" style="background-color:lightgrey; color:black;">
            <th width="50%">Date of Assessment</th><th width="50%">Company Name</th></tr>';
    ...
    while($row_sa = mysqli_fetch_array( $sa )) {
        ...
        $html_weekly .= '<td data-title="Email"><img src="general_site_safety_inspection/download/safety_'.$assign_staff_id.'.png" width="150" height="70" border="0" alt=""></td>';
        $html_weekly .= '</tr>';
    }
    $html_weekly .= '</table>';
    ...
    // avs_near_miss = form name
    $pdf->Output('general_site_safety_inspection/download/hazard_'.$fieldlevelriskid.'.pdf', 'F');
    ...
    while($row_sa = mysqli_fetch_array( $sa )) {
        ...
        // avs_near_miss = form name
        unlink("general_site_safety_inspection/download/safety_".$assign_staff_id.".png");
    }
    echo '';
}
?>