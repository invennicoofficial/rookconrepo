 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        $query_insert_site = "INSERT INTO `safety_avs_hazard_identification` (`safetyid`, `today_date`, `contactid`, `location`, `hazard_rating`, `action_timeline`, `description`, `action`, `action_to`, `est_comp`, `date_comp`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$location', '$hazard_rating', '$action_timeline', '$description', '$action', '$action_to', '$est_comp', '$date_comp', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('avs_hazard_identification_pdf.php');
            echo avs_hazard_identification_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_avs_hazard_identification` SET `location` = '$location', `hazard_rating` = '$hazard_rating', `action_timeline` = '$action_timeline', `description` = '$description', `contactid` = '$contactid', `action` = '$action', `action_to` = '$action_to', `est_comp` = '$est_comp', `date_comp` = '$date_comp' WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'avs_hazard_identification/download/safety_'.$assign_staff_id.'.png');
