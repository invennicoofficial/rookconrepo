 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        ...
        imagepng($img_big, 'hazard_id_report/download/diagram_'.$safetyid.'.png');

        $query_insert_site = "INSERT INTO `safety_hazard_id_report` (`safetyid`, `today_date`, `contactid`, `fields`, `desc`, `desc1`, `all_task`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$fields', '$desc', '$desc1', '$all_task', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('hazard_id_report_pdf.php');
            echo hazard_id_report_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_hazard_id_report` SET `contactid` = '$contactid', `fields` = '$fields', `desc` = '$desc', `desc1` = '$desc1',`all_task` = CONCAT(all_task,'$all_task') WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'hazard_id_report/download/safety_'.$assign_staff_id.'.png');
