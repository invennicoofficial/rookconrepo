 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        $query_insert_site = "INSERT INTO `safety_toolbox_safety_meeting` (`safetyid`, `today_date`, `contactid`, `location`, `project_number`, `supervisor`, `item_discussed`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$location', '$project_number', '$supervisor', '$item_discussed', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('toolbox_safety_meeting_pdf.php');
            echo toolbox_safety_meeting_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_toolbox_safety_meeting` SET `location` = '$location', `project_number` = '$project_number', `supervisor` = '$supervisor', `item_discussed` = '$item_discussed', `contactid` = '$contactid' WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'toolbox_safety_meeting/download/safety_'.$assign_staff_id.'.png');
