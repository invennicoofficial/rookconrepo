<?php
function toolbox_safety_meeting_pdf($dbc,$safetyid, $fieldlevelriskid) {
    $form_by = $_SESSION['contactid'];

    $get_weekly = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM safety_toolbox_safety_meeting WHERE fieldlevelriskid='$fieldlevelriskid'"));
    ...
    $result_update_employee = mysqli_query($dbc, "UPDATE `safety_toolbox_safety_meeting` SET `status` = 'Done' WHERE fieldlevelriskid='$fieldlevelriskid'");
    ...
    $html_weekly = '<h2>Toolbox Safety Meeting</h2>';
    ...
    $html_weekly .= '<table border="1px" style="padding:3px; border:1px solid black;">
            ...
            <th>Date/Time</th><th>Project/Job Number</th></tr>';
    $html_weekly .= '<tr nobr="true"><td>'.$today_date.'</td><td>'.$project_number.'</td></tr>';
    $html_weekly .= '</table>';

    $html_weekly .= '<table border="1px" style="padding:3px; border:1px solid black;">
            <tr nobr="true" style="background-color:lightgrey; color:black;  width:22%;">
            <th>Location of Work</th><th>Supervisor</th></tr>';
    $html_weekly .= '<tr nobr="true"><td>'.$location.'</td><td>'.$supervisor.'</td></tr>';
    $html_weekly .= '</table>';

    $html_weekly .= "<h3>Items Discussed</h3><br>".html_entity_decode($item_discussed);
    ...
    while($row_sa = mysqli_fetch_array( $sa )) {
        ...
        $html_weekly .= '<td data-title="Email"><img src="toolbox_safety_meeting/download/safety_'.$assign_staff_id.'.png" width="150" height="70" border="0" alt=""></td>';
    ...
    $pdf->Output('toolbox_safety_meeting/download/hazard_'.$fieldlevelriskid.'.pdf', 'F');
    ...
    while($row_sa = mysqli_fetch_array( $sa )) {
        $assign_staff_id = $row_sa['safetyattid'];
        unlink("toolbox_safety_meeting/download/safety_".$assign_staff_id.".png");
    }
    echo '';
}
?>