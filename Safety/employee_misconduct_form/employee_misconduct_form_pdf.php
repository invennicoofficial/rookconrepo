<?php
function employee_misconduct_form_pdf($dbc,$safetyid, $fieldlevelriskid) {
	$get_field_level = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM safety_employee_misconduct_form WHERE fieldlevelriskid='$fieldlevelriskid'"));
	...
	$result_update_employee = mysqli_query($dbc, "UPDATE `safety_employee_misconduct_form` SET `status` = 'Done' WHERE fieldlevelriskid='$fieldlevelriskid'");
	...
	$html_weekly = '<h2>Employee Misconduct Form</h2>';
	...
	$html_weekly .= '<table border="1px" style="padding:3px; border:1px solid black;">
            ...
            <th>Date</th><th>Employee Name</th><th>Offense</th></tr>';
    $html_weekly .= '<tr nobr="true"><td>'.$today_date.'</td><td>'.$contactid.'</td><td>'.$offense.'</td></tr>';
    $html_weekly .= '</table>';

	$html_weekly .= "<h3>Reason For Warning</h3>".html_entity_decode($warning);
    ...
    while($row_sa = mysqli_fetch_array( $sa )) {
        ...
        $html_weekly .= '<td data-title="Email"><img src="employee_misconduct_form/download/safety_'.$assign_staff_id.'.png" width="150" height="70" border="0" alt=""></td>';
        $html_weekly .= '</tr>';
    }
    $html_weekly .= '</table>';
    ...
    $pdf->Output('employee_misconduct_form/download/hazard_'.$fieldlevelriskid.'.pdf', 'F');
    ...
    while($row_sa = mysqli_fetch_array( $sa )) {
        $assign_staff_id = $row_sa['safetyattid'];
        unlink("employee_misconduct_form/download/safety_".$assign_staff_id.".png");
    }
    echo '';
}
?>
