h <?php
    ...
    $offense = filter_var($_POST['offense'],FILTER_SANITIZE_STRING);
    $warning = filter_var(htmlentities($_POST['warning']),FILTER_SANITIZE_STRING);
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        $query_insert_site = "INSERT INTO `safety_employee_misconduct_form` (`safetyid`, `today_date`, `contactid`, `offense`, `warning`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$offense', '$warning', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('employee_misconduct_form_pdf.php');
            echo employee_misconduct_form_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_employee_misconduct_form` SET `contactid` = '$contactid', `offense` = '$offense',`warning` = '$warning' WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'employee_misconduct_form/download/safety_'.$assign_staff_id.'.png');
