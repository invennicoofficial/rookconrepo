 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        ...
        imagepng($img_big, 'journey_management_trip_tracking/download/diagram_'.$safetyid.'.png');

        $query_insert_site = "INSERT INTO `safety_journey_management_trip_tracking` (`safetyid`, `today_date`, `contactid`, `fields`, `desc`, `desc1`, `desc2`, `all_task`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$fields', '$desc', '$desc1', '$desc2', '$all_task', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('journey_management_trip_tracking_pdf.php');
            echo journey_management_trip_tracking_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_journey_management_trip_tracking` SET `contactid` = '$contactid', `fields` = '$fields', `desc` = '$desc', `desc1` = '$desc1', `desc2` = '$desc2', `all_task` = CONCAT(all_task,'$all_task') WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'journey_management_trip_tracking/download/safety_'.$assign_staff_id.'.png');
