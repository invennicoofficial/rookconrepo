 <?php
    ...
    $item_discussed = filter_var(htmlentities($_POST['item_discussed']),FILTER_SANITIZE_STRING);
    ...
    for($i=0; $i<$total_task; $i++) {
        if($_POST['task'][$i] != '') {
            $all_task .= $_POST['task'][$i].'**'.$_POST['hazard'][$i].'**'.$_POST['hazard1'][$i].'**'.$_POST['hazard_plan'][$i].'**'.$_POST['hazard_plan1'][$i].'**##**';
        }
    }
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        $query_insert_site = "INSERT INTO `safety_tailgate_safety_meeting` (`safetyid`, `today_date`, `contactid`, `location`, `project_number`, `supervisor`, `item_discussed`, `attendance_staff`, `attendance_extra`, `desc`, `all_task`, `fields`) VALUES	('$safetyid', '$today_date', '$contactid', '$location', '$project_number', '$supervisor', '$item_discussed', '$attendance_staff', '$attendance_extra', '$desc', '$all_task', '$fields')";
        ...
        if($tab == 'Form') {
            ...
            include ('tailgate_safety_meeting_pdf.php');
            echo tailgate_safety_meeting_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_tailgate_safety_meeting` SET `location` = '$location', `project_number` = '$project_number', `supervisor` = '$supervisor', `item_discussed` = '$item_discussed', `contactid` = '$contactid', `desc` = '$desc', `all_task` = CONCAT(all_task,'$all_task'), `fields` = '$fields' WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'tailgate_safety_meeting/download/safety_'.$assign_staff_id.'.png');
