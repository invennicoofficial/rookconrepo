 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        $query_insert_site = "INSERT INTO `safety_confined_space_entry_log` (`safetyid`, `today_date`, `contactid`, `fields`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$fields', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('confined_space_entry_log_pdf.php');
            echo confined_space_entry_log_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_confined_space_entry_log` SET `fields` = '$fields' WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                $timein = implode(',',$_POST['timein_'.$assign_staff_id]);
                $timeout = implode(',',$_POST['timeout_'.$assign_staff_id]);
                $runningtotal = implode(',',$_POST['runningtotal_'.$assign_staff_id]);

                $staffcheck = $timein.'*#*'.$timeout.'*#*'.$runningtotal.'*#*';

                $img = sigJsonToImage($sign);
                imagepng($img, 'confined_space_entry_log/download/safety_'.$assign_staff_id.'.png');
