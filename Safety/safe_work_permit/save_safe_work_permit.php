 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        ...
        imagepng($img_big, 'safe_work_permit/download/diagram_'.$safetyid.'.png');

        $query_insert_site = "INSERT INTO `safety_safe_work_permit` (`safetyid`, `today_date`, `contactid`, `fields`, `desc`, `desc1`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$fields', '$desc', '$desc1', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('safe_work_permit_pdf.php');
            echo safe_work_permit_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_safe_work_permit` SET `contactid` = '$contactid', `fields` = '$fields', `desc` = '$desc', `desc1` = '$desc1' WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'safe_work_permit/download/safety_'.$assign_staff_id.'.png');
