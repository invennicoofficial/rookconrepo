 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        $query_insert_site = "INSERT INTO `safety_general_office_safety_inspection` (`safetyid`, `today_date`, `contactid`, `fields`, `desc`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$fields', '$desc', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('general_office_safety_inspection_pdf.php');
            echo general_office_safety_inspection_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_general_office_safety_inspection` SET `contactid` = '$contactid', `fields` = '$fields',`desc` = '$desc' WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'general_office_safety_inspection/download/safety_'.$assign_staff_id.'.png');
