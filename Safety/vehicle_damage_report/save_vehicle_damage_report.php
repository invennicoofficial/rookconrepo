 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        ...
        imagepng($img_big, 'vehicle_damage_report/download/diagram_'.$safetyid.'.png');

        $query_insert_site = "INSERT INTO `safety_vehicle_damage_report` (`safetyid`, `today_date`, `contactid`, `fields`, `desc`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$fields', '$desc', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('vehicle_damage_report_pdf.php');
            echo vehicle_damage_report_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_vehicle_damage_report` SET `contactid` = '$contactid', `fields` = '$fields', `desc` = '$desc' WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'vehicle_damage_report/download/safety_'.$assign_staff_id.'.png');
