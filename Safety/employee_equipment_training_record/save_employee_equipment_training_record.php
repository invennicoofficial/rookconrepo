 <?php
    ...
    $fields_date = ',';
    for($i=1; $i<=19; $i++) {
        $fields_date .= $_POST['fields_date_'.$i].',';
    }
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        $query_insert_site = "INSERT INTO `safety_employee_equipment_training_record` (`safetyid`, `today_date`, `contactid`, `hire_date`, `fields`, `fields_value`, `fields_date`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$hire_date', '$fields', '$fields_value', '$fields_date', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('employee_equipment_training_record_pdf.php');
            echo employee_equipment_training_record_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_employee_equipment_training_record` SET `contactid` = '$contactid', `hire_date` = '$hire_date', `fields` = '$fields', `fields_value` = '$fields_value', `fields_date` = '$fields_date' WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'employee_equipment_training_record/download/safety_'.$assign_staff_id.'.png');
