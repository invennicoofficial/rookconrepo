 <?php
    ...
    $fields_option = ',,,,,,,,';
    for($i=8; $i<=44; $i++) {
        $fields_option .= $_POST['fields_option_'.$i].',';
    }
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        $query_insert_site = "INSERT INTO `safety_vehicle_inspection_checklist` (`safetyid`, `today_date`, `contactid`, `make`, `vehicle_type`, `serial_number`, `model`, `kilometers`, `fields_option`, `fields_value`, `attendance_staff`, `attendance_extra`, `fields`) VALUES	('$safetyid', '$today_date', '$contactid', '$make', '$vehicle_type', '$serial_number', '$model', '$kilometers', '$fields_option', '$fields_value', '$attendance_staff', '$attendance_extra', '$fields')";
        ...
        if($tab == 'Form') {
            ...
            include ('vehicle_inspection_checklist_pdf.php');
            echo vehicle_inspection_checklist_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_vehicle_inspection_checklist` SET `contactid` = '$contactid', `make` = '$make', `vehicle_type` = '$vehicle_type', `serial_number` = '$serial_number', `model` = '$model', `kilometers` = '$kilometers', `fields_option` = '$fields_option', `fields_value` = '$fields_value', `fields` = '$fields' WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'vehicle_inspection_checklist/download/safety_'.$assign_staff_id.'.png');
