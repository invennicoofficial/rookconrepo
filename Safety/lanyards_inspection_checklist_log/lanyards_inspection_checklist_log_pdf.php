<?php
	function lanyards_inspection_checklist_log_pdf($dbc,$safetyid, $fieldlevelriskid) {
    $form_by = $_SESSION['contactid'];
	$get_field_level = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM safety_lanyards_inspection_checklist_log WHERE fieldlevelriskid='$fieldlevelriskid'"));
	...
	$result_update_employee = mysqli_query($dbc, "UPDATE `safety_lanyards_inspection_checklist_log` SET `status` = 'Done' WHERE fieldlevelriskid='$fieldlevelriskid'");
	...
	$html_weekly = '<h2>Lanyards Inspection Checklist Log</h2>'; // Form nu heading
    ...
    $html_weekly .= '<tr nobr="true" style="background-color:lightgrey; color:black;">
            <th width="34%">Date</th><th width="33%">Lanyard Model</th><th width="33%">Manufacture Date</th></tr>';
    ...
    while($row_sa = mysqli_fetch_array( $sa )) {
        ...
        $html_weekly .= '<td data-title="Email"><img src="lanyards_inspection_checklist_log/download/safety_'.$assign_staff_id.'.png" width="150" height="70" border="0" alt=""></td>';
        $html_weekly .= '</tr>';
    }
    $html_weekly .= '</table>';
    ...
    // avs_near_miss = form name
    $pdf->Output('lanyards_inspection_checklist_log/download/hazard_'.$fieldlevelriskid.'.pdf', 'F');
    ...
    while($row_sa = mysqli_fetch_array( $sa )) {
        ...
        // avs_near_miss = form name
        unlink("lanyards_inspection_checklist_log/download/safety_".$assign_staff_id.".png");
    }
    echo '';
}
?>
