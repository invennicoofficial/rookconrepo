 <?php
    ...
    for($i=0; $i<$total_task; $i++) {
        if($_POST['task'][$i] != '') {
            $all_task .= $_POST['task'][$i].'**'.$_POST['hazard'][$i].'**'.$_POST['hazard1'][$i].'**'.$_POST['hazard_plan'][$i].'**'.$_POST['hazard_plan1'][$i].'**'.$_POST['hazard_plan2'][$i].'**'.$_POST['hazard_plan3'][$i].'**##**';
        }
    }
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        ...
        imagepng($img_big, 'dangerous_goods_shipping_document/download/diagram_'.$safetyid.'.png');

        $query_insert_site = "INSERT INTO `safety_dangerous_goods_shipping_document` (`safetyid`, `today_date`, `contactid`, `fields`, `desc`, `desc1`, `all_task`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$fields', '$desc', '$desc1', '$all_task', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('dangerous_goods_shipping_document_pdf.php');
            echo dangerous_goods_shipping_document_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_dangerous_goods_shipping_document` SET `contactid` = '$contactid', `fields` = '$fields', `desc` = '$desc', `desc1` = '$desc1',`all_task` = CONCAT(all_task,'$all_task') WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'dangerous_goods_shipping_document/download/safety_'.$assign_staff_id.'.png');
