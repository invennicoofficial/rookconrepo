 <?php
    ...
    for($i=0; $i<$total_task; $i++) {
        if($_POST['task'][$i] != '') {
            $all_task1 .= $_POST['task'][$i].'**'.$_POST['hazard'][$i].'**'.$_POST['plan'][$i].'**'.$_POST['hazard1'][$i].'**##**';
        }
    }
    $all_task = filter_var($all_task1,FILTER_SANITIZE_STRING);
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        ...
        imagepng($img_big, 'emergency_response_transportation_plan/download/diagram_'.$safetyid.'.png');

        $query_insert_site = "INSERT INTO `safety_emergency_response_transportation_plan` (`safetyid`, `today_date`, `contactid`, `fields`, `desc`, `desc1`, `desc2`, `desc3`, `desc4`, `all_task`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$fields', '$desc', '$desc1', '$desc2', '$desc3', '$desc4', '$all_task', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('emergency_response_transportation_plan_pdf.php');
            echo emergency_response_transportation_plan_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_emergency_response_transportation_plan` SET `contactid` = '$contactid', `fields` = '$fields', `desc` = '$desc', `desc1` = '$desc1', `desc2` = '$desc2', `desc3` = '$desc3', `desc4` = '$desc4', `all_task` = CONCAT(all_task,'$all_task') WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'emergency_response_transportation_plan/download/safety_'.$assign_staff_id.'.png');
