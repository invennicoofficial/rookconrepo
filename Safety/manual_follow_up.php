...
if (isset($_POST['send_follow_up_email'])) {
    for($i = 0; $i < count($_POST['check_send_email']); $i++) {
        ...
        $email_body .= 'Manual : <a target="_blank" href="'.$_SERVER['SERVER_NAME'].'/Safety/'.(strpos($_SERVER['script_name'],'index.php') !== FALSE ? 'index.php' : 'add_manual.php').'?safetyid='.$safetyid.'&type='.$manual_type.'&action=view">Click Here</a><br>';
...
if((!empty($_GET['action'])) && ($_GET['action'] == 'send_followup_email')) {
    ...
    $message .= 'Manual : <a target="_blank" href="'.$_SERVER['SERVER_NAME'].'/Safety/'.(strpos($_SERVER['script_name'],'index.php') !== FALSE ? 'index.php' : 'add_manual.php').'?safetyid='.$safetyid.'&type='.$manual_type.'&action=view">Click Here</a><br>';
    $message .= '</body></html>';
...
<div class="container">
	<div class="row">
        ...
        <a href='safety.php?tab=Toolbox'><button type="button" class="btn brand-btn mobile-block <?php echo $active_toolbox; ?>" >Toolbox</button></a>
        <a href='safety.php?tab=Tailgate'><button type="button" class="btn brand-btn mobile-block <?php echo $active_taligate; ?>" >Tailgate</button></a>
        <a href='safety.php?tab=Form'><button type="button" class="btn brand-btn mobile-block <?php echo $active_form; ?>" >Forms</button></a>
        <a href='safety.php?tab=Manual'><button type="button" class="btn brand-btn mobile-block <?php echo $active_manual; ?>" >Manuals</button></a>
        ...
        if(isset($_POST['reporting_client'])) {
            ...
            $heading = $_POST['heading'];
            $status = $_POST['status'];
            $query_check_credentials = "SELECT m.*, ms.*  FROM safety_staff ms, manuals m WHERE m.deleted=0 AND m.manual_type = '$type' AND m.safetyid = ms.safetyid AND (ms.staffid = '$contactid' OR m.category='$category' OR m.heading='$heading')";
            if($status == 'Deadline Past') {
                $query_check_credentials = "SELECT m.*, ms.*  FROM safety_staff ms, manuals m WHERE m.deleted=0 AND m.manual_type = '$type' AND m.safetyid = ms.safetyid AND ms.done=0 AND DATE(NOW()) > DATE(m.deadline)";
            }
            if($status == 'Deadline Today') {
                $query_check_credentials = "SELECT m.*, ms.*  FROM safety_staff ms, manuals m WHERE m.deleted=0 AND m.manual_type = '$type' AND m.safetyid = ms.safetyid AND ms.done=0 AND DATE(NOW()) = DATE(m.deadline)";
            }
        } else if(empty($_GET['action'])) {
            $query_check_credentials = "SELECT m.*, ms.*  FROM safety_staff ms, manuals m WHERE m.deleted=0 AND m.manual_type = '$type' AND m.safetyid = ms.safetyid";
        }
        ...
        while($row = mysqli_fetch_array( $result ))
        {
            ...
            if($row['done'] == 1) {
                ...
                echo '<td data-title="Code">';
                echo '<a href="manual_follow_up.php?staffid='.$row['staffid'].'&manual_type='.$row['manual_type'].'&safetyid='.$row['safetyid'].'&action=send_followup_email">Send</a>';
                echo '&nbsp;&nbsp;<input name="check_send_email[]" type="checkbox" value="'.$row['staffid'].'_'.$row['safetyid'].'" class="form-control check_send_email" style="width:25px;"/></td>';
            }
