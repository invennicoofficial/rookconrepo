 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        ...
        imagepng($img_big, 'confined_space_entry_permit/download/diagram_'.$safetyid.'.png');

        $query_insert_site = "INSERT INTO `safety_confined_space_entry_permit` (`safetyid`, `today_date`, `contactid`, `fields`, `desc`, `desc1`, `attendance_staff`, `attendance_extra`, `all_task`) VALUES	('$safetyid', '$today_date', '$contactid', '$fields', '$desc', '$desc1', '$attendance_staff', '$attendance_extra', '$all_task')";
        ...
        if($tab == 'Form') {
            ...
            include ('confined_space_entry_permit_pdf.php');
            echo confined_space_entry_permit_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_confined_space_entry_permit` SET `contactid` = '$contactid', `fields` = '$fields', `desc` = '$desc', `desc1` = '$desc1',`all_task` = CONCAT(all_task,'$all_task') WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'confined_space_entry_permit/download/safety_'.$assign_staff_id.'.png');
