<?php
function follow_up_incident_report_pdf($dbc,$safetyid, $fieldlevelriskid) {
    $form_by = $_SESSION['contactid'];

    $get_weekly = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM safety_follow_up_incident_report WHERE fieldlevelriskid='$fieldlevelriskid'"));
    ...
    $result_update_employee = mysqli_query($dbc, "UPDATE `safety_follow_up_incident_report` SET `status` = 'Done' WHERE fieldlevelriskid='$fieldlevelriskid'");
    ...
    $html_weekly = '<h2>Follow-Up Incident Report</h2>';
    ...
    if (strpos($form_config, ','."fields4".',') !== FALSE) {
        $html_weekly .= '<h3>Items to be followed up on</h3>'.html_entity_decode($action);
    }
    ...
    while($row_sa = mysqli_fetch_array( $sa )) {
        ...
        $html_weekly .= '<td data-title="Email"><img src="follow_up_incident_report/download/safety_'.$assign_staff_id.'.png" width="150" height="70" border="0" alt=""></td>';
        $html_weekly .= '</tr>';
    }
    $html_weekly .= '</table>';

    $pdf->writeHTML($html_weekly, true, false, true, false, '');
    $pdf->Output('follow_up_incident_report/download/hazard_'.$fieldlevelriskid.'.pdf', 'F');
    ...
    while($row_sa = mysqli_fetch_array( $sa )) {
        $assign_staff_id = $row_sa['safetyattid'];
        unlink("follow_up_incident_report/download/safety_".$assign_staff_id.".png");
    }
    echo '';
}
?>