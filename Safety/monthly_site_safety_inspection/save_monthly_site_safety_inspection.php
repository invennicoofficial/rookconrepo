 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        $query_insert_site = "INSERT INTO `safety_monthly_site_safety_inspection` (`safetyid`, `today_date`, `contactid`, `location`, `fields`, `fields_value`, `all_task`, `attendance_staff`, `attendance_extra`) VALUES	('$safetyid', '$today_date', '$contactid', '$location', '$fields', '$fields_value', '$all_task', '$attendance_staff', '$attendance_extra')";
        ...
        if($tab == 'Form') {
            ...
            include ('monthly_site_safety_inspection_pdf.php');
            echo monthly_site_safety_inspection_pdf($dbc,$safetyid, $fieldlevelriskid);
        ...
        $query_update_employee = "UPDATE `safety_monthly_site_safety_inspection` SET `contactid` = '$contactid', `location` = '$location',`fields` = '$fields', `fields_value` = '$fields_value', `all_task` = CONCAT(all_task,'$all_task') WHERE fieldlevelriskid='$fieldlevelriskid'";
        ...
        while($row_sa = mysqli_fetch_array( $sa )) {
            ...
            if($_POST['sign_'.$assign_staff_id] != '') {
                ...
                imagepng($img, 'monthly_site_safety_inspection/download/safety_'.$assign_staff_id.'.png');
