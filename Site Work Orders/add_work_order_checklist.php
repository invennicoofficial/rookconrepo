    ...
    $( ".connectedChecklist" ).sortable({
		...
		update: function( event, ui ) {
			...
			$.ajax({    //create an ajax request to load_page.php
				type: "GET",
				url: "site_work_orders_ajax.php?fill=checklist_priority&workorderid=<?= $workorderid ?>&id="+id+"&afterid="+afterid,
...
function addItem(textbox) {
	$.ajax({    //create an ajax request to load_page.php
		type: "POST",
		url: "site_work_orders_ajax.php?fill=add_checklist",
		data: { new_item: textbox.value, workorderid: '<?= $workorderid ?>' },
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function checklistChange(sel) {
    ...
    $.ajax({
        type: "GET",
        url: "site_work_orders_ajax.php?fill=checked&id="+sel.value+"&checked="+checked,
        dataType: "html",
        success: function(response){
        }
    });
...
function checklistAttach(item) {
	...
	$('[name=attach_'+id+']').change(function() {
		...
		$.ajax({
			...
			processData: false,
			type: "POST",
			url: "site_work_orders_ajax.php?fill=checklist_upload&id="+id,
			data: fileData,
...
function checklistReply(item) {
	...
	$('[name=reply_'+id+']').blur(function() {
		...
		if(reply != '') {
			...
			$.ajax({
				method: 'POST',
				url: 'site_work_orders_ajax.php?fill=checklist_reply',
...
function checklistArchive(item) {
	if(confirm("Are you sure you want to archive this item?")) {
		$.ajax({    //create an ajax request to load_page.php
			type: "GET",
			url: "site_work_orders_ajax.php?fill=delete_checklist&id="+$(item).parents('span').data('item'),
			dataType: "html",   //expect html to be returned
...
function checklistFlag(item) {
	$.ajax({
		method: "POST",
		url: "site_work_orders_ajax.php?fill=checklist_flag",
...
<ul id="sortable" class="connectedChecklist">
	...
	<?php $result = mysqli_query($dbc, "SELECT * FROM `site_work_checklist` WHERE workorderid='$workorderid' AND deleted = 0 ORDER BY sort");
