...
if (isset($_POST['purchase_order'])) {
	...
	if(empty($_POST['poid'])) {
		...
		$query_insert_po = "INSERT INTO `site_work_po` (`type`, `bill_to`, `vendorid`, `issue_date`, `description`, `qty`, `descript`, `grade`, `tag`, `detail`, `unit_price`, `unit_total`, `total_price`, `tax`, `final_total`, `history`) VALUES ('$type', '$bill_to', '$vendorid', '$issue_date', '$description', '$qty', '$descript', '$grade', '$tag', '$detail', '$unit_price', '$unit_total', '$total_price', '$tax', '$final_total', '$history')";
		...
		$poid = $_POST['poid'];
		$query_update_site = "UPDATE `site_work_po` SET `type` = '$type', `bill_to` = '$bill_to', `issue_date` = '$issue_date', `description`= '$description', `qty` = '$qty', `vendorid` = '$vendorid', `descript` = '$descript', `grade` = '$grade', `tag` = '$tag', `detail` = '$detail', `unit_price` = '$unit_price', `unit_total` = '$unit_total', `description` = '$description', `total_price` = '$total_price', `tax` = '$tax', `final_total`='$final_total', `revision` = '$revision', `history`=CONCAT(IFNULL(CONCAT(`history`,'<br />\n'),''),'$history') WHERE `poid` = '$poid'";
    ...
    echo '<script> if(window.self === window.top) {
		window.location.replace("site_work_orders.php?tab=po");
	} else {
...
<div class="container">
	...
	<?php
		...
		if(!empty($_GET['poid'])) {
			...
			$poid = $_GET['poid'];
			$get_po = mysqli_fetch_array(mysqli_query($dbc, "SELECT * FROM `site_work_po` WHERE `poid`='$poid'"));
		...
		<?php //if(strpos($edit_config,',site,') !== false): ?>
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    <select name="siteid" data-value="<?= $siteid ?>" class="form-control chosen-select-deselect">
                        <option></option><?php
            ...
            <div class="form-group">
                ...
                <div class="col-sm-8">
                    <select name="contactid" data-value="<?= $contactid ?>" class="form-control chosen-select-deselect"></select>
        ...
        <?php //if(strpos($edit_config,',work_orders,') !== false): ?>
            <div class="form-group vendor">
                ...
                <div class="col-sm-8">
                    <select <?php if ($type !== '') { echo "readonly";} ?>  data-placeholder="Choose a Work Order..." name="workorderid" class="chosen-select-deselect form-control" width="380">
                        <option value=""></option><?php
                        $wo_result = mysqli_query ( $dbc, "SELECT `workorderid`, `id_label` FROM `site_work_orders`" );
                        while ( $row_wo=mysqli_fetch_assoc($wo_result) ) {
                            echo "<option value='". $row_wo['workorderid'] ."'>". $row_wo['id_label'] .'</option>';
                        } ?>
                    </select>
                </div>
            </div>
    ...
    <div class="form-group">
        <div class="col-sm-4 clearfix">
			<a href="site_work_orders.php?tab=po" class="btn brand-btn pull-right">Back</a>
        </div>
        <div class="col-sm-8">
