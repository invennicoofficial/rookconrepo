...
if(isset($_POST['submit'])) {
	...
	if(!empty($pre_sign)) {
		...
		imagepng($img, 'download/pre_sign_'.$log_id.'.png');
	...
	if(!empty($post_sign)) {
		...
		imagepng($img, 'download/post_sign_'.$log_id.'.png');
	...
	$html .= '<table width="100%" cellspacing="2">
		...
		<tr>
			...
			<td><img width="190" src="download/pre_sign_'.$log_id.'.png"></td>
		</tr>
		<tr>
			<td>Post-Trip Signature:</td>
			<td>'.($_POST['safetyinspectid'] == 'PRE' || $_POST['include_post_trip'] != '1' ? '' : '<img width="190" src="download/post_sign_'.$log_id.'.png">').'</td>
		</tr></table>';
...
<div class="container">
	<div class="row">
		...
		<div class="form-group">
			<div class="col-sm-6">
				<a href="site_work_orders.php?tab=schedule" class="btn brand-btn">Back to Dashboard</a>
			</div>
					...
					<div class="panel-body">
						<div class="form-group"><label class="col-sm-4 control-label">Driver:</label>
							<div class="col-sm-8"><select name="staff" data-placeholder="Select a Driver" class="form-control chosen-select-deselect"><option></option>
								<?php $staff_list = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc, "SELECT `contactid`, `last_name`, `first_name` FROM `contacts` WHERE `category`='Staff' AND `deleted`=0 AND `status`=1 AND `show_hide_user`=1"),MYSQLI_ASSOC));
							...
							<div class="form-group"><label class="col-sm-4 control-label"><?= (empty($site_log_equip_cat) ? 'Equipment' : $site_log_equip_cat) ?>:</label>
								<div class="col-sm-8">
									<select name="equipment[]" multiple data-placeholder="Select <?= (empty($site_log_equip_cat) ? 'Equipment' : $site_log_equip_cat) ?>" class="form-control chosen-select-deselect"><option></option>
										<?php 
										$equip_list = mysqli_query($dbc, "SELECT `category`, `type`, `unit_number`, `make`, `model`, `label`, `equipmentid` FROM `equipment` WHERE `category`='$site_log_equip_cat' OR '$site_log_equip_cat'='' ORDER BY `category`, `type`, `unit_number`, `make`, `model`, `equipmentid`");
										$category = '';
				...
				<div id="collapse_safety" class="panel-collapse collapse">
					<div class="panel-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-3 text-center pretrip">
								<label class="pretrip show-on-mob">Pre-Trip Status</label>
								<label><input type="radio" name="safety29" value="Yes" style="height:1.5em;margin:0.5em;width:1.5em;" <?= ('pre' == $checklist ? '' : ($safety29 == 'Yes' ? ' checked tabindex="-1"' : 'disabled')) ?>> Yes</label>
								<label><input type="radio" name="safety29" value="No" style="height:1.5em;margin:0.5em;width:1.5em;" <?= ('pre' == $checklist ? '' : ($safety29 == 'No' ? ' checked tabindex="-1"' : 'disabled')) ?>> No</label>
							...
							<div class="col-sm-3 text-center posttrip"><?php if('pre' == $checklist) { echo '---'; } else { ?>
								<label class="posttrip show-on-mob">Post-Trip Status</label>
								<label><input type="radio" name="safety30" value="Yes" style="height:1.5em;margin:0.5em;width:1.5em;" <?= ('post' == $checklist ? '' : ($safety30 == 'Yes' ? ' checked tabindex="-1"' : 'disabled')) ?>> Yes</label>
								<label><input type="radio" name="safety30" value="No" style="height:1.5em;margin:0.5em;width:1.5em;" <?= ('post' == $checklist ? '' : ($safety30 == 'No' ? ' checked tabindex="-1"' : 'disabled')) ?>> No</label>
						...
						<div class="form-group">
							...
							<div class="col-sm-3 text-center pretrip">
								<label class="pretrip show-on-mob">Pre-Trip Status</label>
								<label><input type="radio" name="safety31" value="Yes" style="height:1.5em;margin:0.5em;width:1.5em;" <?= ('pre' == $checklist ? '' : ($safety31 == 'Yes' ? ' checked tabindex="-1"' : 'disabled')) ?>> Yes</label>
								<label><input type="radio" name="safety31" value="No" style="height:1.5em;margin:0.5em;width:1.5em;" <?= ('pre' == $checklist ? '' : ($safety31 == 'No' ? ' checked tabindex="-1"' : 'disabled')) ?>> No</label>
							...
							<div class="col-sm-3 text-center posttrip"><?php if('pre' == $checklist) { echo '---'; } else { ?>
								<label class="posttrip show-on-mob">Post-Trip Status</label>
								<label><input type="radio" name="safety32" value="Yes" style="height:1.5em;margin:0.5em;width:1.5em;" <?= ('post' == $checklist ? '' : ($safety32 == 'Yes' ? ' checked tabindex="-1"' : 'disabled')) ?>> Yes</label>
								<label><input type="radio" name="safety32" value="No" style="height:1.5em;margin:0.5em;width:1.5em;" <?= ('post' == $checklist ? '' : ($safety32 == 'No' ? ' checked tabindex="-1"' : 'disabled')) ?>> No</label>
						...
						<div class="form-group">
							...
							<div class="col-sm-8"><?php if('pre' == $checklist) {
									...
									<img src="download/pre_sign_<?= $log_id ?>.png">
								<?php } ?>
							</div>
						</div>
						<div class="form-group posttrip">
							...
							<div class="col-sm-8"><?php if('post' == $checklist) {
									...
									<img src="download/post_sign_<?= $log_id ?>.png">
								<?php } else {
									echo "---";
								} ?>
							</div>
		...
		<div class="form-group">
			<div class="col-sm-6">
				<a href="site_work_orders.php?tab=schedule" class="btn brand-btn btn-lg">Back</a>
			</div>
			<div class="col-sm-6">
