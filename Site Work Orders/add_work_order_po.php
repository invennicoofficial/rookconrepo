...
function new_po_added(id, total) {
	...
	$('.po_table').append('<tr><td data-title=""><input type="hidden" name="po_list[]" value="'+id+'"><label style="width:100%;"><input type="checkbox" name="attach_po_'+id+'" checked value="attach"></label></td>'+
...
<div id="no-more-tables">
	...
	<?php $pos = mysqli_query($dbc, "SELECT `site_work_po`.*, `site_work_orders`.`workorderid` FROM `site_work_po` LEFT JOIN `site_work_orders` ON CONCAT(',',`site_work_orders`.`po_id`,',') LIKE CONCAT('%,',`site_work_po`.`poid`,',%') WHERE `site_work_orders`.`workorderid` IS NULL OR `site_work_orders`.`workorderid`='$workorderid'");
	if(mysqli_num_rows($pos) > 0) {
		while($po = mysqli_fetch_array($pos)) { ?>
			<input type="hidden" name="po_list[]" value="<?= $po['poid'] ?>">
			<tr>
				<td data-title=""><label style="width:100%;"><input type="checkbox" name="attach_po_<?= $po['poid'] ?>" <?= ($po['workorderid'] > 0 ? 'checked' : '') ?> value="attach"></label></td>
