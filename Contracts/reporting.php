...
<div class="container triple-pad-bottom">
    <div class="row">
		...
		if(in_array('Reporting',$contract_tabs) && check_subtab_persmission($dbc, 'contracts', ROLE, 'Reporting') === TRUE) {
			echo "<a href='reporting.php' class='btn brand-btn mobile-block mobile-100 active_tab'>Reporting</a>";
		} ?>
        ...
        $heading = '';
        $status = 'Completed';
		$start_date = date('Y-m-d', strtotime("-2month"));
		$end_date = date('Y-m-d');
        ...
        <div class="col-lg-2 col-md-3 col-sm-4 col-xs-4">
          ...
          <div class="col-lg-4 col-md-3 col-sm-8 col-xs-8">
                <select data-placeholder="Select a Status" name="status" class="chosen-select-deselect form-control" width="380">
                  ...
                  <option <?php if ($status=='Completed') echo 'selected="selected"';?> value="Completed">Completed</option>
                </select>
          </div>
        ...
        if(isset($_POST['reporting_client'])) {
			$search = '';
			if($staffid != '') {
				$search .= " AND staff.`recipient`='%".get_email($dbc, $staffid)."%'";
			}
			if($contactid != '') {
			...
			if($start_date != '') {
				$search .= " AND (staff.`due_date` > '$start_date' OR staff.`today_date` > '$start_date')";
			}
			if($end_date != '') {
				$search .= " AND (staff.`due_date` > '$end_date' OR staff.`today_date` > '$end_date')";
			}
            if($status == 'Deadline Passed') {
                ...
                $query_check_credentials = "SELECT * FROM `contracts_staff` staff LEFT JOIN `contracts` ON staff.`contractid`=`contracts`.`contractid` WHERE staff.`deleted`=0 AND `contracts`.deleted=0 AND staff.done=1 $search";
            }
            else {
