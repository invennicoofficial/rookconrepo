...
while($ticket = mysqli_fetch_assoc($ticket_list)) {
	...
	$tickets[] = ['ticketid'=>$ticket['ticketid'],'staff'=>explode(',',$ticket['contactid'].','.$ticket['internal_qa_contactid'].','.$ticket['deliverable_contactid']),'internal_qa'=>explode(',',$ticket['internal_qa_contactid']),'deliverable_id'=>explode(',',$ticket['deliverable_contactid']),'file_namne'=>$file,'revision_namne'=>$ticket['revision'],'created_by'=>$ticket['created_by'],'po'=>$ticket['purchase_order'].'#*#'.$ticket['po_numbers'],'customer_orders'=>$ticket['customer_order_num'].'#*#'.$ticket['customer_orders'],'business'=>$ticket['businessid'],'contact_namne'=>explode(',',$ticket['clientid']),'project'=>$ticket['projecttype'],'status_namne'=>$ticket['status'],'key_namne'=>$ticket['to_do_date'].'-'.$label.' '.$ticket['purchase_order'].' '.$ticket['po_numbers'].' '.$ticket['customer_order_num'].' '.$ticket['customer_orders'],'label_namne'=>$label,'type_namne'=>$ticket_type];
}
