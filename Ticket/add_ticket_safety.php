...
<div class="safety_links">
	<?= (!empty($renamed_accordion) ? '<h3>'.$renamed_accordion.'</h3>' : '<h3>Safety Checklist</h3>') ?>
	<p>The following items need to be completed for this Work Order:</p>
	<?php include_once('../Safety/manual_checklist.php');
	manual_checklist($dbc, '35', '20', '20', 'ALL', 'ALL', "AND (CONCAT(',',`assign_sites`,',') LIKE '%,ALL,%' OR CONCAT(',',`assign_sites`,',') LIKE '%,{$get_ticket['siteid']},%' OR CONCAT(',',`assign_work_orders`,',') LIKE '%,ALL,%' OR CONCAT(',',`assign_work_orders`,',') LIKE '%,$ticketid,%')", 'iframe'); ?>
</div>