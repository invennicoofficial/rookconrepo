...
$query_check_credentials = "SELECT * FROM (SELECT 0 `id`, `tickettimerid`, `timer_type` `type`, `start_time`, `end_time`, CONCAT(IFNULL(`start_time`,''),' - ',IFNULL(`end_time`,''),' (',IFNULL(`timer`,''),')') `time`, `timer`, `created_date`, `created_by`, TIME_TO_SEC(`timer`) `second_count`, 0 `deleted`, 0 `deleted_by` FROM ticket_timer WHERE ticketid='$ticketid' AND `ticketid` > 0
	UNION SELECT `id`, 0 `tickettimerid`, `time_type` `type`, '00:00:00' `start_time`, '00:00:00' `end_time`, `time_length` `time`, '00:00:00' `timer`, `created_date`, `created_by`, TIME_TO_SEC(`time_length`) `second_count`, `deleted`, `deleted_by` FROM `ticket_time_list` WHERE `ticketid`='$ticketid' AND `ticketid` > 0) times ORDER BY `created_date` DESC, `tickettimerid` DESC";
...
if($num_rows > 0) { ?>
	...
	while($row = mysqli_fetch_array($result)) {
		...
		echo '<tr data-id="'.$row['id'].'" '.$deleted_styling.'>';
		$by = $row['created_by'];
