...
$sql = "SELECT * FROM `reminders` WHERE $query AND (`reminder_type` LIKE 'PROJECT%' OR `reminder_type` LIKE 'CLIENTPROJECT%') AND `deleted`=0 AND `sent`=0";
	...
	foreach($contacts as $contactid) {
		...
		if($email != '') {
			...
			try {
				...
				echo $row['subject']." sent to $email at $time. (Project Reminder: #".$row['reminderid'].")\n";
