...
// Check if the POS is in Edit mode
...
if (strpos($value_config, ','."Send Outbound Invoice".',') !== FALSE)
    ...
    if ( ( isset ( $_GET[ 'edit' ] ) && $_GET[ 'edit' ] == 'return' ) && ( isset ( $_GET[ 'posid' ] ) && $_GET[ 'posid' ] != '' )  ) {
		...
		$return_sub_total = $row['sub_total'] - $sub_total;
		$return_gst = $row['gst'] - $gst_total;
		$return_pst = $row['pst'] - $pst_total;
		$return_before_tax = $row['total_before_tax'] - $total_before_tax;
		$return_total = $row['total_price'] - $total_price;
		...
		$update_invoice_query = "UPDATE `point_of_sell` LEFT JOIN `contacts` ON `point_of_sell`.`contactid`=`contacts`.`contactid` SET `sub_total`='$sub_total', `total_after_discount`='$total_after_discount', `total_before_tax`='$total_before_tax', `returned_amt`=(`total_price` + `returned_amt` - '$total_price'), `contacts`.amount_credit=IF(IFNULL(`amount_owing`,0) > 0, IF(`amount_owing` > (`total_price` - '$total_price'), `amount_credit`, `total_price` - '$total_price' - `amount_owing` + `amount_credit`), `amount_credit` + `total_price` - '$total_price'), `amount_owing`=IF(IFNULL(`amount_owing`,0) > 0, IF(`amount_owing` > (`total_price` - '$total_price'), `amount_owing` - `total_price` + '$total_price', 0), 0), `total_price`='$total_price', `comment`=CONCAT(`comment`,'$comment'), `gst`='$gst_total', `pst`='$pst_total', `edit_id`=(`edit_id`+1), `return_ids`=CONCAT(IF(`return_ids`='','',CONCAT(`return_ids`,',')),'$return_invoice') WHERE `posid`='$posid'";
		...
		for ($i=0; $i<count($_POST['inventoryid']); $i++) {
			...
			if ( $inventoryid != '' ) {
				...
				$update_inventory_query = "UPDATE `inventory` SET `quantity`=(`quantity` - $return_diff) WHERE `inventoryid`='$inventoryid'";
		...
		for ($i=0; $i<count($_POST['misc_product']); $i++) {
			...
			if ($misc_product != '' AND $misc_product != '0') {
				$update_misc_results = mysqli_query ( $dbc, "UPDATE `point_of_sell_product` SET `returned`='$returned' WHERE `posid`='$posid' AND `type_category`='misc product'" );
    ...
    else if ( ( isset ( $_GET[ 'edit' ] ) && $_GET[ 'edit' ] == 'yes' ) && ( isset ( $_GET[ 'posid' ] ) && $_GET[ 'posid' ] != '' )  ) {
		// Edit POS
		$posid = trim ( $_GET['posid'] );
		$update_invoice_query = "UPDATE `point_of_sell` SET `invoice_date`='$invoice_date', `contactid`='$contactid', `productpricing`='$productpricing', `sub_total`='$sub_total', `discount_type`='$discount_type', `discount_value`='$discount_value', `total_after_discount`='$total_after_discount', `delivery`='$delivery', `assembly`='$assembly', `total_before_tax`='$total_before_tax', `client_tax_exemption`='$client_tax_exemption', `tax_exemption_number`='$tax_exemption_number', `total_price`='$total_price', `created_by`='$created_by', `comment`='$comment', `ship_date`='$ship_date', `due_date`='$due_date', `status`='$status', `gst`='$gst_total', `pst`='$pst_total', `delivery_type`='$delivery_type', `delivery_address`='$delivery_address', `contractorid`='$contractorid', `deposit_paid`='$dep_paid', `updatedtotal`='$updatedtotal', `edit_id`=(`edit_id`+1) WHERE `posid`='$posid'";
		$update_invoice_results = mysqli_query ( $dbc, $update_invoice_query );
		...
		for ($i=0; $i<count($_POST['inventoryid']); $i++) {
			...
			if ( $inventoryid != '' ) {
				$update_product_query = "UPDATE `point_of_sell_product` SET `quantity`='$quantity', `price`='$price' WHERE `posid`='$posid' AND `inventoryid`='$inventoryid' AND `type_category`='inventory'";
				$update_product_results = mysqli_query ( $dbc, $update_product_query );

				//Update Inventory table to adjust the quantity
				$update_inventory_query = "UPDATE `inventory` SET `quantity`=(`quantity` + $quantity_old - '$quantity') WHERE `inventoryid`='$inventoryid'";
				$update_inventory_results = mysqli_query ( $dbc, $update_inventory_query );
			}
		}
		...
		for ($i=0; $i<count($_POST['prodinventoryid']); $i++) {
			...
			if ( $inventoryid != '' ) {
				$query_insert_invoice = "UPDATE `point_of_sell_product` SET `inventoryid`='$inventoryid', `quantity`='$quantity', `price`='$price' WHERE `posid`='$posid' AND `inventoryid`='$inventoryid' AND `type_category`='product'";
				$results_are_in = mysqli_query($dbc, $query_insert_invoice);
			}
		}
		...
		for ($i=0; $i<count($_POST['servinventoryid']); $i++) {
			...
			if ( $inventoryid != '' ) {
				$update_service_results = mysqli_query ( $dbc, "UPDATE `point_of_sell_product` SET `quantity`='$quantity', `price`='$price' WHERE `posid`='$posid' AND `inventoryid`='$inventoryid' AND `type_category`='service'" );
			}
		}
		...
		for ($i=0; $i<count($_POST['misc_product']); $i++) {
			...
			if ($misc_product != '' AND $misc_product != '0') {
				$update_misc_results = mysqli_query ( $dbc, "UPDATE `point_of_sell_product` SET `misc_product`='$misc_product', `price`='$misc_price', `quantity`='$misc_quantity' WHERE `posid`='$posid' AND `type_category`='misc product'" );
			}
		}
	} else {
		// New POS
...
$(document).on('change', 'select[name="cuscateg1"]', function() { changeClient(this); });
...
$(document).on('change', 'select[name="couponid"]', function() { countPOSTotal(this); });
    ...
    <div class="row">
		...
		<?php if($return) { ?>
			<div class="notice double-gap-bottom popover-examples">
				...
				Because you are accessing this invoice in returns mode, you cannot change this invoice. You can only return inventory and products, and add comments. To modify the invoice, you will need to access it using an edit link to get to editing mode.</div>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-9">
				<select data-placeholder="Select Pricing..." id="productpricing" name="productpricing" class="chosen-select-deselect form-control" width="380">
                    ...
                    <?php $selected = ($productpricing == 'client_price') ? 'selected="selected"' : ''; ?>
					<option value="client_price" <?= $selected; ?>>Client Price</option>
                    <?php } ?>
                    <?php if (strpos($value_config, ','."Admin Price".',') !== FALSE) { ?>
                    <?php $selected = ($productpricing == 'admin_price') ? 'selected="selected"' : ''; ?>
                    <option value="admin_price" <?= $selected; ?>>Admin Price</option>
                    <?php } ?>
                    <?php if (strpos($value_config, ','."Commercial Price".',') !== FALSE) { ?>
                    <?php $selected = ($productpricing == 'commercial_price') ? 'selected="selected"' : ''; ?>
                    <option value="commercial_price" <?= $selected; ?>>Commercial Price</option>
                    <?php } ?>
                    <?php if (strpos($value_config, ','."Wholesale Price".',') !== FALSE) { ?>
                    <?php $selected = ($productpricing == 'wholesale_price') ? 'selected="selected"' : ''; ?>
                    <option value="wholesale_price" <?= $selected; ?>>Wholesale Price</option>
                    <?php } ?>
                    <?php if (strpos($value_config, ','."Final Retail Price".',') !== FALSE) { ?>
                    <?php $selected = ($productpricing == 'final_retail_price') ? 'selected="selected"' : ''; ?>
                    <option value="final_retail_price" <?= $selected; ?>>Final Retail Price</option>
                    <?php } ?>
                    <?php if (strpos($value_config, ','."Preferred Price".',') !== FALSE) { ?>
                    <?php $selected = ($productpricing == 'preferred_price') ? 'selected="selected"' : ''; ?>
                    <option value="preferred_price" <?= $selected; ?>>Preferred Price</option>
                    <?php } ?>
                    <?php if (strpos($value_config, ','."Web Price".',') !== FALSE) { ?>
                    <?php $selected = ($productpricing == 'web_price') ? 'selected="selected"' : ''; ?>
                    <option value="web_price" <?= $selected; ?>>Web Price</option>
                    <?php } ?>
					<?php if (strpos($value_config, ','."Purchase Order Price".',') !== FALSE) { ?>
                    <?php $selected = ($productpricing == 'purchase_order_price') ? 'selected="selected"' : ''; ?>
                    <option value="purchase_order_price" <?= $selected; ?>>Purchase Order Price</option>
                    <?php } ?>
					<?php if (strpos($value_config, ','."Sales Order Price".',') !== FALSE) { ?>
                    <?php $selected = ($productpricing == 'sales_order_price') ? 'selected="selected"' : ''; ?>
                    <option value="sales_order_price" <?= $selected; ?>><?= SALES_ORDER_NOUN ?> Price</option>
                    <?php } ?>
					<?php if (strpos($value_config, ','."Drum Unit Cost".',') !== FALSE) { ?>
                    <?php $selected = ($productpricing == 'drum_unit_cost') ? 'selected="selected"' : ''; ?>
                    <option value="drum_unit_cost" <?= $selected; ?>>Drum Unit Cost</option>
                    <?php } ?>
					<?php if (strpos($value_config, ','."Drum Unit Price".',') !== FALSE) { ?>
                    <?php $selected = ($productpricing == 'drum_unit_price') ? 'selected="selected"' : ''; ?>
                    <option value="drum_unit_price" <?= $selected; ?>>Drum Unit Price</option>
                    <?php } ?>
					<?php if (strpos($value_config, ','."Tote Unit Cost".',') !== FALSE) { ?>
                    <?php $selected = ($productpricing == 'tote_unit_cost') ? 'selected="selected"' : ''; ?>
                    <option value="tote_unit_cost" <?= $selected; ?>>Tote Unit Cost</option>
                    <?php } ?>
					<?php if (strpos($value_config, ','."Tote Unit Price".',') !== FALSE) { ?>
                    <?php $selected = ($productpricing == 'tote_unit_price') ? 'selected="selected"' : ''; ?>
                    <option value="tote_unit_price" <?= $selected; ?>>Tote Unit Price</option>
                    <?php } ?>
                </select>
            </div>
          </div>
          ...
          <div class="additional_positionserv">
            ...
            <div class="form-group clearfix" id="servservices_0" width="100%">
				...
				<label for="company_name" class="col-sm-4 show-on-mob control-label">Category:</label>
                    <select data-placeholder="Select a Category..."  id="servcategory_dd_0" name="servcategory[]" class="chosen-select-deselect form-control servcategory">
                        <option value=""></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT DISTINCT category FROM services WHERE include_in_pos != '' order by category");
                        while($row = mysqli_fetch_array($query)) {
                            ?><option id='<?php echo $row['category'];?>' value='<?php echo $row['category'];?>'><?php echo $row['category'];?></option><?php
                        }
                        ?>
				...
				<label for="company_name" class="col-sm-4 show-on-mob control-label">Price:</label>
                    <input <?php echo ($return ? 'readonly' : ''); ?> data-placeholder="Select a Product..." name="servprice[]" id="servprice_dd_0" value="0" style="" onkeyup="countPOSTotal(this);" type="text" class="form-control servprice" />
                </div>
                <?php } ?>
				...
				for ( $i=0; $i< ($return ? $count_misc : $count_misc + 1); $i++ ) { ?>
					<div class="additional_misc">
						...
						<div class="form-group clearfix" id="misc_<?= $i; ?>" width="100%">
							...
							<div class="col-sm-1 expand-mobile" id="quantity_<?= $i; ?>" style="width:10%; position:relative; display:inline-block;">
							<label for="company_name" class="col-sm-4 show-on-mob control-label">Returned:</label>
								<input data-placeholder="Quantity Returned" name="misc_returned[]" id="misc_returned_dd_<?= $i; ?>" value="<?= ( $misc_quantity[$i] ) ? $misc_quantity[$i] : 0; ?>" style="" onchange="returnFilter(this); countPOSTotal(this);" type="text" class="form-control misc_return" />
							</div>
							<?php } ?>
            ...
            <?php if (strpos($value_config, ','."Coupon".',') !== FALSE) { ?>
                <div class="form-group">
                    ...
                    <div class="col-sm-9">
						<select data-placeholder="Select a Coupon..." name="couponid" id="couponid" class="chosen-select-deselect form-control product" style="position:relative;">
                            <option value=""></option><?php
            ...
            <?php if (strpos($value_config, ','."Delivery".',') !== FALSE) { ?>
                ...
                <label for="site_name" class="col-sm-3 control-label">Delivery Option:</label>
                    <div class="col-sm-9">
						<select data-placeholder="Select a Type..." name="delivery_type" id="delivery_type" class="chosen-select-deselect form-control product" style="position:relative;">
                            <option value=""></option>
