<?php
/*
 * Voided Invoices / Accounts Receivable
*/
...
<style>
	.selectbutton { cursor:pointer; text-decoration:underline; }
...
<div class="container triple-pad-bottom">
    <div class="row">
		<div class="col-sm-10">
			<h1>Voided Invoicing Dashboard</h1>
		</div>
        ...
        <div class='gap-left tab-container mobile-100-container'>
			...
			<?php if ( check_subtab_persmission($dbc, 'pos', ROLE, 'voided') === TRUE ) { ?>
				<a href="voided.php"><button type="button" class="btn brand-btn mobile-block mobile-100 active_tab">Voided Invoices</button></a>
			<?php } else { ?>
					...
					<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
						...
						<div class="col-sm-8">
							<select name="contactid" data-placeholder="Select Customer..." class="chosen-select-deselect form-control width-me">
								<option value=''></option>
								<?php
									$query = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc,"SELECT contactid, first_name, last_name FROM contacts WHERE category NOT IN (".STAFF_CATS.") AND category != 'Employee' deleted=0 AND `status`=1"),MYSQLI_ASSOC));
									foreach($query as $id) {
										$selected = '';
		...
		<form method="POST" action="" name="send_email" class="form-horizontal">
				...
				<?php
					...
					if ( isset ( $_POST['display_all_invoice'] ) ) {
						$invoice_name = '';
						$query = "SELECT COUNT(inv.`posid`) AS numrows FROM `point_of_sell` inv WHERE inv.`deleted`=0 AND inv.`status`='Void' ORDER BY inv.`posid` DESC";
					}

					if ( isset ( $_POST['search_invoice_submit'] )) {
						$query_check_credentialss = "SELECT inv.*, c.`contactid`, c.`name`, c.`first_name`, c.`last_name` FROM `point_of_sell` inv, `contacts` c WHERE inv.`contactid` = c.`contactid` AND inv.`deleted` = 0 AND inv.`status` = 'Void' " . $searchbyany . " " . $searchbycustomer . " " . $searchbytype . " " . $starttimesql . " ORDER BY inv.`posid` DESC LIMIT $offset, $rowsPerPage";
						$query = "SELECT COUNT(inv.`posid`) as numrows FROM `point_of_sell` inv, `contacts` c WHERE inv.`contactid` = c.`contactid` AND inv.`deleted` = 0 AND inv.`status` = 'Void' " . $searchbyany . " " . $searchbycustomer . " " . $searchbytype . " " . $starttimesql . " ORDER BY inv.`posid` DESC";
					} else {
						$query_check_credentialss = "SELECT * FROM `point_of_sell` WHERE `deleted` = 0 AND `status` = 'Void' ORDER BY `posid` DESC LIMIT $offset, $rowsPerPage";
						$query = "SELECT COUNT(`posid`) AS numrows FROM `point_of_sell` WHERE `deleted` = 0 AND `status` = 'Void'";
					}
					...
					while ( $roww = mysqli_fetch_array ( $resultt ) ) {
						...
						if (strpos($value_config, ','."Edit POS".',') !== FALSE) {
							echo '<td data-title="Edit"><a href="add_point_of_sell.php?posid=' . $roww['posid'] . '&edit=yes&from_url=voided.php">Edit</a></td>';
						}
						if ( strpos ( $value_config, ',' . "Status" . ',' ) !== FALSE ) {
