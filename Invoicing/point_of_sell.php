	...
	<div class="row hide_on_iframe">
					...
					<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
						...
						<div class="col-sm-8">
							<select name="contactid" data-placeholder="Select Customer..." class="chosen-select-deselect form-control width-me">
								<option value=''></option>
							   <?php
								$query = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc,"SELECT contactid, first_name, last_name FROM contacts WHERE category NOT IN (".STAFF_CATS.") AND category != 'Employee' AND deleted=0 AND `status`=1"),MYSQLI_ASSOC));
								foreach($query as $id) {
									$selected = '';
		...
		<form method="POST" action="" name="send_email" class="form-horizontal">
            ...
            while($roww = mysqli_fetch_array( $resultt ))
            {
				...
				if (strpos($value_config, ','."Edit POS".',') !== FALSE) {
					echo '<td data-title="Edit"><a href="add_point_of_sell.php?posid=' . $roww['posid'] . '&edit=yes&from_url=point_of_sell.php">Edit</a></td>';
				}
                if (strpos($value_config, ','."Status".',') !== FALSE) {
