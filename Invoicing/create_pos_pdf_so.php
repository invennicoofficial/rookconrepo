<?php
// PDF CREATOR FOR POS WHEN CONVERTING FROM SALES ORDERS \\
// This file has been created because the original create_pos_pdf file was unsuitable for PDF creation from Sales Order data. \\
function create_pos3_so_pdf($dbc,$posid,$d_value,$comment, $gst_total, $pst_total,$company_software_name) {
	...
	?><?php
	$pdf->Output('../Point of Sale/download/invoice_'.$posid.'.pdf', 'F');

}
function create_pos2_so_pdf($dbc,$posid,$d_value,$comment, $gst_total, $pst_total) {
	include ('../database_connection.php');
    $point_of_sell = mysqli_fetch_assoc(mysqli_query($dbc, "SELECT * FROM point_of_sell WHERE posid='$posid'"));
    $contactid = $point_of_sell['contactid'];
	$zenornot = $point_of_sell['cross_software'];
	...
	class MYPDF extends TCPDF {
		...
		public function Header() {
			...
			if($point_of_sell['delivery_type'] !== '' && $point_of_sell['delivery_type'] !== NULL) {
				$footer_text = '<p style="text-align:right;">Date : ' .INVOICE_DATE.'<br>Invoice# : '.INVOICEID.'<br>Ship Date : ' .SHIP_DATE.'<br>Sales Person : ' .SALESPERSON.'<br>Payment Type : ' .PAYMENT_TYPE.'<br>Shipping Method : '.$point_of_sell['delivery_type'].$tdduedate.'</p>';
			} else {
				$footer_text = '<p style="text-align:right;">Date : ' .INVOICE_DATE.'<br>Invoice# : '.INVOICEID.'<br>Ship Date : ' .SHIP_DATE.'<br>Sales Person : ' .SALESPERSON.'<br>Payment Type : ' .PAYMENT_TYPE.$tdduedate.'</p>';
			}
            $this->writeHTMLCell(0, 0, 0 , 10, $footer_text, 0, 0, false, "R", true);
		}
...
function create_pos1_so_pdf($dbc,$posid,$d_value,$comment, $gst_total, $pst_total) {
	include ('../database_connection.php');
    $point_of_sell = mysqli_fetch_assoc(mysqli_query($dbc, "SELECT * FROM point_of_sell WHERE posid='$posid'"));
    $contactid = $point_of_sell['contactid'];
	$zenornot = $point_of_sell['cross_software'];
