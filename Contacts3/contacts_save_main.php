...
if(!empty($_POST['role'])) {
	...
	// if(strpos($_SERVER['HTTP_HOST'],'highland') !== FALSE) {
		// send_email('', 'jonathanhurdman@freshfocusmedia.com', '', '', 'Security Level Change: Contacts3', "$user changed the security level for $id to $role.<br />".WEBSITE_URL."<br />UPDATE `contacts` SET `role`='$role' WHERE `contactid` = '$contactid' AND `category` != 'Staff'", '');
	// }
}
