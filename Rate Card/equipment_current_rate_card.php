<div class='main_frame' id='no-more-tables'>
	...
	$db_conf = get_config($dbc, 'equip_db_rate_fields');
	if(str_replace(',','',$db_conf) == '') {
		$db_conf = ",card,annual,history,function,";
	}

    if(isset($_GET['t'])) {
        $cat_query = " AND `e`.`category`='".filter_var($_GET['t'],FILTER_SANITIZE_STRING)."'";
    } else {
        $cat_query = '';
    }
	$sql = "SELECT r.*, IFNULL(CONCAT(e.`make`,' ',e.`model`,', Unit: ',e.`unit_number`,', VIN:',`vin_number`),'N/A') equipment, e.`equipmentid` FROM `equipment` e LEFT JOIN `company_rate_card` r ON r.`item_id` = e.`equipmentid` AND r.`tile_name` LIKE 'Equipment' WHERE e.`deleted` = 0 AND IFNULL(r.`deleted`,0)=0 ".$cat_query." ORDER BY e.`make`, e.`model`, e.`unit_number`, e.`vin_number` LIMIT $offset, $rowsPerPage";
	$count_sql = "SELECT COUNT(*) numrows FROM `equipment` e LEFT JOIN `company_rate_card` r ON r.`item_id` = e.`equipmentid` AND r.`tile_name` LIKE 'Equipment' WHERE e.`deleted` = 0 AND IFNULL(r.`deleted`,0)=0 ".$cat_query;
	...
	if(mysqli_num_rows($result) > 0):
		...
		while($row = mysqli_fetch_array($result)) {
			echo '<tr>'; ?>
				...
				<td data-title="Category"><?= $row['equipment'] ?></td>
                <?php if(strpos($db_config, ',start_end_dates,') !== FALSE) { ?>
		...
		echo "<h2>No Equipment Rate Cards Found.</h2>";
	endif; ?>
</div>
...
function iframe_history() {
	var id = $(this).data('id');
	$('#iframe_instead_of_window').attr('src', 'rate_card.php?card=equipment&status=history&id='+id);
	$('#history_frame').show();
	$('.main_frame').hide();
