...
function selectPromotionService(sel) {
	...
	$.ajax({
		type: "GET",
		url: "ratecard_ajax_all.php?fill=promotion_service_config&value="+stage,
...
function selectPromotionCat(sel) {
	...
	$.ajax({
		type: "GET",
		url: "ratecard_ajax_all.php?fill=promotion_cat_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function selectPromotionHeading(sel) {
	...
	$.ajax({
		type: "GET",
		url: "ratecard_ajax_all.php?fill=promotion_head_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
<div class="form-group">
    <div class="col-sm-12">
        <div class="form-group clearfix hide-titles-mob">
            ...
            <?php } ?>
            <?php if (strpos($base_field_config, ','."Promotion Heading".',') !== FALSE) { ?>
        ...
        <?php if(!empty($_GET['ratecardid'])) {
            $each_promotion = explode('**', $promotion);
            $total_count = mb_substr_count($promotion,'**');
            $id_loop = 500;
            for($pid_loop=0; $pid_loop<$total_count; $pid_loop++) {

                $promotionid = '';

                if(isset($each_promotion[$pid_loop])) {
                    $each_val = explode('#', $each_promotion[$pid_loop]);
                    $promotionid = $each_val[0];
            ...
            <div class="form-group clearfix" id="<?php echo 'promotion_'.$id_loop; ?>">
                ...
                <?php } ?>
                <div class="col-sm-1" ><label for="company_name" class="col-sm-4 show-on-mob control-label">Rate Card Type:</label>
                    <input name="promotionfinalprice[]" value="<?php echo $ratecardprice;?>" id="<?php echo 'promotionfinalprice_'.$id_loop; ?>" type="text" class="form-control" />
                </div>
                <div class="col-sm-1" >
                    <a href="#" onclick="deleteRatecard(this,'promotion_','promotionheading_'); return false;" id="<?php echo 'deletepromotion_'.$id_loop; ?>" class="btn brand-btn">Delete</a>
                </div>
            </div>
        <?php  $id_loop++;
                }
        ...
        <div class="additional_promotion clearfix">
            ...
            <div class="form-group clearfix" id="promotion_0">
                ...
                <div class="col-sm-1" ><label for="company_name" class="col-sm-4 show-on-mob control-label">Rate Card Price:</label>
                    <input name="promotionfinalprice[]" id="promotionfinalprice_0" type="text" class="form-control" />
                </div>
                <div class="col-sm-1" >
                    <a href="#" onclick="deleteRatecard(this,'promotion_','promotionheading_'); return false;" id="deletepromotion_0" class="btn brand-btn">Delete</a>
                </div>
            </div>
