...
if (isset($_POST['submit'])) {
	...
	$history = 'Category rate card '.($id == '' ? 'Added' : 'Edited').' by '.get_contact($dbc, $_SESSION['contactid']).' on '.date('Y-m-d h:i:s');
	$sql = '';
	if($id == '') {
	...
	echo '<script type="text/javascript"> window.location.replace("?card=category&type=category"); </script>';
} ?>
<div class='main_frame' id='no-more-tables'><form id="category_rate" name="category_rate" method="post"	action="" enctype="multipart/form-data" class="form-horizontal" role="form">
	<?php
	if(isset($_GET['id'])) {
		$id = $_GET['id'];
