...
<div id='no-more-tables'>
	...
	$query_check_credentials = "SELECT r.*, c.contactid, c.name FROM rate_card r, contacts c WHERE r.clientid = c.contactid AND r.deleted = 0 AND r.on_off = 1 AND '$contact_category' IN (c.`category`,'') LIMIT $offset, $rowsPerPage";
	$query = "SELECT count(c.name) as numrows FROM rate_card r, contacts c WHERE r.clientid = c.contactid AND r.deleted = 0 AND r.on_off = 1 AND '$contact_category' IN (c.`category`,'')";
	...
	if(mysqli_num_rows($result) > 0):
		...
		while($row = mysqli_fetch_array( $result )) {
			echo '<tr>';
			echo '<td data-title="Client">' . (!empty(get_client($dbc, $row['contactid'])) ? get_client($dbc, $row['contactid']) : get_contact($dbc, $row['contactid'])) . '</td>';
