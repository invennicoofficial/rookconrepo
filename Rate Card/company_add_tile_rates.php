...
if($tile_name == 'Equipment' && $cat_name == 'category_rates') {
		...
		<div id="no-more-tables">
			<table width="100%" id="rate_table" class="table table-bordered">
				...
				<?php do {
					...
					$col_count = 0;
					$main_rate_row = $dbc->query("SELECT * FROM `company_rate_card` WHERE `rate_card_name`='$ref_card' AND `description`='$rate_desc' AND `heading`='$rate_heading' AND `rate_card_types`='$rate_type' AND `uom`='$rate_uom' AND `deleted`=0")->fetch_assoc();
					$main_rate = ($main_rate_row['profit'] / ($main_rate_row['margin'] / 100)) / ((($main_rate_row['profit'] / ($main_rate_row['margin'] / 100)) - $main_rate_row['profit']) / $main_rate_row['cost']);
					$main_rate_field = 'rate_est';
					$main_rate_field_name = 'estimateprice';
					if($rate_cust > 0) {
						$main_rate = $main_rate_row['cust_price'];
						$main_rate_field = 'rate_cust';
						$main_rate_field_name = 'cust_price';
					} else if($rate_hourly > 0) {
						$main_rate = $main_rate_row['hourly'];
						$main_rate_field = 'rate_hourly';
						$main_rate_field_name = 'hourly';
					} else if($rate_daily > 0) {
						$main_rate = $main_rate_row['daily'];
