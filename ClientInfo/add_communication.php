...
function selectContactCategory(sel) {
    ...
    <?php
        ...
        if(!empty($_GET['contactid']) && !empty($_GET['category'])) {
            $get_contact = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM social_story_communication WHERE support_contact='".$_GET['contactid']."'"));
            ...
            $communication_id = $get_contact['communication_id'];
        ...
        <?php   }      ?>
        <input type="hidden" id="submit_type" name="submit_type" value="communication" />
