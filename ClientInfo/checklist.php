...
<div style="width:95%" class="container">
	...
	<div class="notice double-gap-bottom popover-examples">
		...
		You must add a checklist in order to see your checklists in each sub tab. To do so, click Add Checklist and fill in all of the fields. Once you have clicked Submit, you will see the checklist in the specific sub tab you have assigned it to.</div>
		<div class="clearfix"></div>
	</div>
	<div class="notice double-gap-bottom popover-examples">
		<div class="col-sm-1 notice-icon"><img src="<?= WEBSITE_URL; ?>/img/info.png" class="wiggle-me" width="25"></div>
	...
	$list_count = mysqli_fetch_array(mysqli_query($dbc, "SELECT SUM(IF(`security`='My Checklist',1,0)) mylists, SUM(IF(`checklist_type`='ongoing',1,0)) myongoing, SUM(IF(`checklist_type`='daily',1,0)) mydaily, SUM(IF(`checklist_type`='weekly',1,0)) myweekly, SUM(IF(`checklist_type`='monthly',1,0)) mymonthly, SUM(IF(`security`='Company Checklist',1,0)) companylists, SUM(IF(`security`='Company Checklist' AND `checklist_type`='ongoing',1,0)) companyongoing, SUM(IF(`security`='Company Checklist' AND `checklist_type`='daily',1,0)) companydaily, SUM(IF(`security`='Company Checklist' AND `checklist_type`='weekly',1,0)) companyweekly, SUM(IF(`security`='Company Checklist' AND `checklist_type`='monthly',1,0)) companymonthly FROM `checklist` WHERE `deleted`=0 AND (`created_by` LIKE '%".$_GET['contactid']."%')"));
		...
		if(strpos($tab_config, 'my_ongoing') !== false) {
			echo "
				<div class='pull-left tab tab-nomargin'>
					...
					<a href='".addOrUpdateCurrentUrlParam(array('checklistid','update_type','subtab','type'), array('','','Checklists','ongoing'))."'><button type='button' class='btn brand-btn mobile-block ".$active_on."'>Ongoing (".$list_count['myongoing'].")</button></a>
				</div>";
		}
		if(strpos($tab_config, 'my_daily') !== false) {
			echo "
				<div class='pull-left tab tab-nomargin'>
					...
					<a href='".addOrUpdateCurrentUrlParam(array('checklistid','update_type','subtab','type'), array('','','Checklists','daily'))."'><button type='button' class='btn brand-btn mobile-block ".$active_daily."'>Daily (".$list_count['mydaily'].")</button></a>
				</div>";
		}
		if(strpos($tab_config, 'my_weekly') !== false) {
			echo "
				<div class='pull-left tab tab-nomargin'>
					...
					<a href='".addOrUpdateCurrentUrlParam(array('checklistid','update_type','subtab','type'), array('','','Checklists','weekly'))."'><button type='button' class='btn brand-btn mobile-block ".$active_weekly."'>Weekly (".$list_count['myweekly'].")</button></a>
				</div>";
		}
		if(strpos($tab_config, 'my_monthly') !== false) {
			echo "
				<div class='pull-left tab tab-nomargin'>
					...
					<a href='".addOrUpdateCurrentUrlParam(array('checklistid','update_type','subtab','type'), array('','','Checklists','monthly'))."'><button type='button' class='btn brand-btn mobile-block ".$active_monthly."'>Monthly (".$list_count['mymonthly'].")</button></a>
				</div>";
		}
	echo '</div><div class="clearfix"></div><br />';
	$contactid = $_GET['contactid'];
	$add_url = addOrUpdateCurrentUrlParam(array('checklistid','update_type','subtab','type','subtabid'), array('','add','Checklists','',''));
	echo '
		<div class="mobile-100-container pull-right tab tab-nomargin">
			<a href="'.$add_url.'" class="btn brand-btn mobile-block gap-bottom pull-right">Add Checklist</a>
    ...
    <form name="form_sites" method="post" action="" class="form-inline" role="form">
        ...
        $result = mysqli_query($dbc, "SELECT * FROM checklist WHERE deleted = 0 AND created_by = $contactid AND checklist_type='$type'");
        ...
        while($row = mysqli_fetch_array($result)) {
			...
			$set_checklistid = $row['checklistid'];
            echo "<a href='".addOrUpdateCurrentUrlParam(array('type','update_type','subtab','checklistid'),array($type,'','Checklists',$row['checklistid']))."'><button type='button' class='mobile-100 btn brand-btn mobile-block ".$active_daily."' >".$row['checklist_name']."</button></a>&nbsp;&nbsp;";
        }
        ?>
        <br><br>
		</div>
			...
			echo '<div class="tab-container">';
				...
				echo '
					<div class="pull-right tab">
						...
						<a href="'.addOrUpdateCurrentUrlParam(array('type','update_type','subtab','checklistid'),array($type,'edit','Checklists',$_GET['checklistid'])).'" class="btn brand-btn mobile-block mobile-100 gap-bottom pull-right">Edit</a>
					</div>';
