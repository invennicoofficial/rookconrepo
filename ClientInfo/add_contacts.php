...
*/
include ('../include.php');
if (isset($_POST['subtab']) && $_POST['subtab'] == 'Medical Charts') {
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        <?php if($_POST['subtab'] == 'Individual Service Plan (ISP)' && !check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_isp')) { ?>
            disableInputs();
        <?php } ?>
			<?php if($_POST['subtab'] == 'Checklists' && !check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_checklists')) { ?>
            disableInputs();
        <?php } ?>
        <?php if($_POST['subtab'] == 'Medical Details' && !check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_medical_details')) { ?>
            disableInputs();
        <?php } ?>
        <?php if($_POST['subtab'] == 'Medications' && !check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_medications')) { ?>
            disableInputs();
        <?php } ?>
        <?php if($_POST['subtab'] == 'Key Methodologies' && !check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_key_methodologies')) { ?>
            disableInputs();
        <?php } ?>
        <?php if($_POST['subtab'] == 'Protocols' && !check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_protocols')) { ?>
            disableInputs();
        <?php } ?>
        <?php if($_POST['subtab'] == 'Routine' && !check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_routine')) { ?>
            disableInputs();
        <?php } ?>
        <?php if($_POST['subtab'] == 'Communication' && !check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_communication')) { ?>
            disableInputs();
        <?php } ?>
        <?php if($_POST['subtab'] == 'Activities' && !check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_activities')) { ?>
            disableInputs();
        <?php } ?>
        <?php if($_POST['subtab'] == 'Medical Charts' && !check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_medical_charts')) { ?>
            disableInputs();
        <?php } ?>
        <?php if($_POST['subtab'] == 'Daily Log Notes' && !check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_daily_log_notes')) { ?>
            disableInputs();
        <?php } ?>
    });
...
<div class="container">
  <div class="row">
        ...
        while($row_main = mysqli_fetch_array($query_main)) {
            ...
            if ($subtab == 'Details') {
                if(check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_details_limited') && !check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_details_full')) {
                    if($accordion != 'Client Information' && $accordion != 'Client Address') {
						$visible = 'style="display:none;"';
                    }
                } else if(!check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_details_limited') && !check_subtab_persmission($dbc, 'client_info', ROLE, 'clients_details_full')) {
                    $visible = 'style="display:none;"';
                }
            }

            $client_info_subtabs = ['Individual Service Plan (ISP)', 'Medications', 'Key Methodologies', 'Protocols', 'Routine', 'Communication', 'Activities', 'Medical Charts', 'Checklists'];
            if($subtab == 'Individual Service Plan (ISP)') {
                include_once('add_individual_support_plan.php');
            } else if($subtab == 'Medications') {
                include_once('add_medications.php');
            } else if($subtab == 'Key Methodologies') {
                include_once('add_key_methodologies.php');
                $functions_included = true;
            } else if($subtab == 'Protocols') {
                include_once('add_protocols.php');
                $functions_included = true;
            } else if($subtab == 'Routine') {
                include_once('add_routines.php');
                $functions_included = true;
            } else if($subtab == 'Communication') {
                include_once('add_communication.php');
                $functions_included = true;
            } else if($subtab == 'Activities') {
                include_once('add_activities.php');
                $functions_included = true;
            } else if($subtab == 'Medical Charts') {
                include_once('add_medical_charts.php');
			} else if($subtab == 'Checklists') {
                include_once('add_checklists.php');
            } else {
                $client_info_subtabs = '';
            }

            if (strpos($value_config, ','."Injury".',') === FALSE && $accordion != '' && !in_array($row_main['subtab'], $client_info_subtabs)) {
            ?>
			<div class="panel panel-default" <?php echo ($subtab == $row_main['subtab'] || count($subtab_list) == 1 ? $visible : 'style="display:none;"'); ?>>
