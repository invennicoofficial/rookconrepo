...
if (isset($_POST['send_follow_up_email'])) {
    ...
    echo '<script type="text/javascript"> alert("Reactivation Email Sent."); window.location.replace(""); </script>';
...
<div class="container triple-pad-bottom">
    <div class="row">
        ...
        <div class="row">
            <div class="col-xs-11"><h1 class="single-pad-bottom">Cancelled Appointments Dashboard</div>
        ...
        <div class="mobile-100-container tab-container">
            ...
            <div class="tab pull-left">
                ...
                if (check_subtab_persmission($dbc, 'reactivation', ROLE, 'cancellations') === TRUE) { ?>
                    <a href='cancelled_appt.php'><button type="button" class="btn brand-btn mobile-block active_tab">Cancellations</button></a><?php
                } else { ?>
        ...
        Cancelled Appointments shows Patients that have an active injury, and their most recent appointment was cancelled.</div>
        <div class="clearfix"></div>
        </div>
        ...
        <center><div class="form-group">
            Cancelled Appointment From: <input name="starttime" type="text" class="datepicker" value="<?php echo $starttime; ?>">
            &nbsp;&nbsp;&nbsp;
            Cancelled Appointment Until: <input name="endtime" type="text" class="datepicker" value="<?php echo $endtime; ?>">
            &nbsp;&nbsp;&nbsp;
            Staff:
            <select data-placeholder="Select a Staff..." name="therapist" class="chosen-select-deselect form-control1" width="380">
                ...
                <?php foreach(sort_contacts_query(mysqli_query($dbc, "SELECT `contactid`, `first_name`, `last_name` FROM `contacts` WHERE `category` IN (".STAFF_CATS.") AND ".STAFF_CATS_HIDE_QUERY." AND `status` > 0 AND `deleted`=0")) as $row) { ?>
					<option <?= $row['contactid'] == $therapist ? 'selected' : '' ?> value="<?= $row['contactid'] ?>"><?= $row['first_name'].' '.$row['last_name'] ?></option>
				<?php } ?>
            </select>
        ...
        if($therapist == '') {
            $result = mysqli_query($dbc, "SELECT * FROM contacts WHERE category IN (".STAFF_CATS.") AND ".STAFF_CATS_HIDE_QUERY." AND `status`=1 AND deleted=0");
        } else {
        ...
        while($row = mysqli_fetch_array($result)) {
            ...
            $report_validation = mysqli_query($dbc,"SELECT pi.injuryid, pi.injury_name, pi.injury_type, pi.contactid, b.`appoint_date` cancelled_date, b.`cancellation`, b.`reactive_email_date` FROM patient_injury pi LEFT JOIN contacts c ON c.`contactid`=pi.`contactid` LEFT JOIN `booking` b ON c.`contactid`=b.`patientid` AND b.`bookingid` IN (SELECT MAX(bookingid) FROM `booking` WHERE LEFT(`appoint_date`,10) <= '$endtime' AND LEFT(`appoint_date`,10) >= '$starttime' GROUP BY `patientid`) WHERE pi.injury_therapistsid = '$therapistid' AND b.`follow_up_call_status`='Cancelled' AND pi.discharge_date IS NULL ORDER BY c.first_letter");
            ...
            while($row_report = mysqli_fetch_array($report_validation)) {
				...
				if(strtotime($get_arrived['last_arrived']) < strtotime(date('Y-m-d', strtotime('-7 days')))) {
					...
					$html_table .= '<td>' . $row_report['cancelled_date'] . '</td>';
					$html_table .= '<td>' . $row_report['cancellation'] . '</td>';
					$html_table .= '<td>' . $row_report['reactive_email_date'] . '</td>';
					$patient_email = get_email($dbc, $row_report['contactid']);
					if($patient_email == '-' || $patient_email == '') {
						$html_table .= '<td>-</td>';
					} else if($row_report['reactive_email_date'] != '') {
            ...
            if($data == 1) {
                ...
                <th style="width:8%">Cancelled Appointment</th>
                <th style="width:8%">Cancellation Reason</th>
