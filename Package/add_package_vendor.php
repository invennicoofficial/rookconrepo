...
$(document).ready(function() {
    ...
    $('#add_row_v').on( 'click', function () {
        ...
        clone.find('#packagevendor_0').attr('id', 'packagevendor_'+add_new_v);
        $('#vdelete_0').hide();
...
function selectVendor(sel) {
	...
	$.ajax({
		type: "GET",
		url: "package_ajax_all.php?fill=vendor_config&value="+stage,
...
function selectPricelist(sel) {
	...
	$.ajax({
		type: "GET",
		url: "package_ajax_all.php?fill=vpricelist_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function selectCategory(sel) {
	...
	$.ajax({
		type: "GET",
		url: "package_ajax_all.php?fill=vcat_config&value="+stage+"&pricelist="+pricelist,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function selectProduct(sel) {
	...
	$.ajax({
		type: "GET",
		url: "package_ajax_all.php?fill=vproduct_config&value="+stage+"&pricelist="+pricelist+"&ratecardid="+ratecardid,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
<div class="form-group">
    <div class="col-sm-12">
       ...
       <?php if(!empty($_GET['packageid'])) {
            ...
            ?>
                <div class="form-group clearfix" id="<?php echo 'packagevendor_'.$id_loop; ?>">
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Vendor:</label>
                    <select data-placeholder="Choose a Vendor..." id="<?php echo 'prvendorid_'.$id_loop; ?>" class="chosen-select-deselect form-control equipmentid vendor_onchange" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT contactid, name FROM contacts WHERE category='Vendor' AND deleted=0 order by name");
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Price List:</label>
                    <select data-placeholder="Choose a Price List Item..." id="<?php echo 'prpricelistid_'.$id_loop; ?>" class="chosen-select-deselect form-control equipmentid vendor_pricelist_onchange" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT pricelistid, pricelist_name FROM vendor_pricelist WHERE deleted=0 order by pricelist_name");
                        while($row = mysqli_fetch_array($query)) {
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Product:</label>
                    <select data-placeholder="Choose a Product..." id="<?php echo 'prproduct_'.$id_loop; ?>" name="assign_vendor[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT pricelistid, name FROM vendor_pricelist WHERE deleted=0 order by name");
                        while($row = mysqli_fetch_array($query)) {
        ...
        <div class="additional_v clearfix">
            ...
            <div class="form-group clearfix" id="packagevendor_0">
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Vendor:</label>
