...
$(document).ready(function() {
    ...
    $('#add_row_in').on( 'click', function () {
        ...
        clone.find('#packageinventory_0').attr('id', 'packageinventory_'+add_new_in);
        $('#indelete_0').hide();
...
function selectInventoryCategory(sel) {
	...
	$.ajax({
		type: "GET",
		url: "package_ajax_all.php?fill=in_cat_config&value="+stage,
	...
	$.ajax({
		type: "GET",
		url: "package_ajax_all.php?fill=in_cat_config_partno&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function selectInventoryCodePartName(sel) {
	...
	$.ajax({
		type: "GET",
		url: "package_ajax_all.php?fill=in_code_part_name_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
	...
	$.ajax({
		type: "GET",
		url: "package_ajax_all.php?fill=in_code_part_name_config_number&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
            $("#ininventorypart_"+arr[1]).html(response);
			$("#ininventorypart_"+arr[1]).trigger("change.select2");
...
function selectInventoryCodePartNo(sel) {
	...
	$.ajax({
		type: "GET",
		url: "package_ajax_all.php?fill=in_code_part_no_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
            var result = response.split('*FFM*');
            $("#inrp_"+arr[1]).val(result[0]);
	...
	$.ajax({
		type: "GET",
		url: "package_ajax_all.php?fill=in_code_part_no_config_name&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
            $("#ininventoryname_"+arr[1]).html(response);
			$("#ininventoryname_"+arr[1]).trigger("change.select2");
    ...
    <div class="col-sm-12">
        ...
        <?php if(!empty($_GET['packageid'])) {
            ...
            for($inventory_loop=0; $inventory_loop<$total_count; $inventory_loop++) {
                ...
                if($inventoryid != '') {
            ?>
                <div class="form-group clearfix" id="<?php echo 'packageinventory_'.$id_loop; ?>">
                    ...
                    <select data-placeholder="Choose a Product Name..." id="<?php echo 'ininventoryname_'.$id_loop; ?>" name="assign_inventory[]" class="chosen-select-deselect form-control inventoryid inv_partameo_onchange" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT inventoryid, name FROM inventory WHERE inventoryid = '$inventoryid' order by inventoryid");
                        while($row = mysqli_fetch_array($query)) {
        ...
        <div class="additional_in clearfix">
            ...
            <div class="form-group clearfix" id="packageinventory_0">
