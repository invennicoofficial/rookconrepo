...
$(document).ready(function() {
    ...
    $('#add_row_eq').on( 'click', function () {
        ...
        clone.find('#packageequipment_0').attr('id', 'packageequipment_'+add_new_eq);
        $('#eqdelete_0').hide();
...
function selectEquipmentCategory(sel) {
	...
	$.ajax({
		type: "GET",
		url: "package_ajax_all.php?fill=eq_cat_config&value="+stage,
...
function selectEquipmentUnSn(sel) {
	...
	$.ajax({
		type: "GET",
		url: "package_ajax_all.php?fill=eq_un_sn_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
    ...
    <div class="col-sm-12">
        ...
        <?php if(!empty($_GET['packageid'])) {
            ...
            for($equ_loop=0; $equ_loop<$total_count; $equ_loop++) {
                ...
                if($equipmentid != '') {
            ?>
                <div class="form-group clearfix" id="<?php echo 'packageequipment_'.$id_loop; ?>">
        ...
        <div class="additional_eq clearfix">
            ...
            <div class="form-group clearfix" id="packageequipment_0">
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Unit/Serial Number:</label>
                    <select data-placeholder="Choose a Number..." id="eqequipmentun_0" name="assign_equipment[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT equipmentid, unit_number, serial_number  FROM equipment order by serial_number");
                        while($row = mysqli_fetch_array($query)) {
