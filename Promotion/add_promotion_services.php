...
$(document).ready(function() {
    ...
    $('#add_row_s').on( 'click', function () {
        ...
        clone.find('#promotionservice_0').attr('id', 'promotionservice_'+add_new_s);
        $('#sdelete_0').hide();
...
function selectServiceService(sel) {
	...
	$.ajax({
		type: "GET",
		url: "promotion_ajax_all.php?fill=s_service_config&value="+stage,
...
function selectServiceCat(sel) {
	...
	$.ajax({
		type: "GET",
		url: "promotion_ajax_all.php?fill=s_cat_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function selectServiceHeading(sel) {
	...
	$.ajax({
		type: "GET",
		url: "promotion_ajax_all.php?fill=s_head_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
    ...
    <div class="col-sm-12">
        ...
        <?php if(!empty($_GET['promotionid'])) {
            ...
            for($services_loop=0; $services_loop<$total_count; $services_loop++) {
                ...
                if($serviceid != '') {
            ?>
                <div class="form-group clearfix" id="<?php echo 'promotionservice_'.$id_loop; ?>">
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Heading:</label>
                    <select data-placeholder="Choose a Heading..." id="<?php echo 'sheading_'.$id_loop; ?>" name="assign_services[]" class="chosen-select-deselect form-control" width="380">
                        <option value=''></option>
                        <?php
                ...
                <?php if (strpos($field_config_service, ','."Final Retail Price".',') !== FALSE) { ?>
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Final Retail Price:</label>
        ...
        <div class="additional_s clearfix">
            ...
            <div class="form-group clearfix" id="promotionservice_0">
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Service Type:</label>
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Category:</label>
                    <select data-placeholder="Choose a Category..." id="scategory_0" class="chosen-select-deselect form-control equipmentid serv_cat_onchange" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT serviceid, category FROM services WHERE deleted=0 order by category");
                        while($row = mysqli_fetch_array($query)) {
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Heading:</label>
                    <select data-placeholder="Choose a Heading..." id="sheading_0" name="assign_services[]" class="chosen-select-deselect form-control" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT serviceid, heading FROM services WHERE deleted=0");
                        while($row = mysqli_fetch_array($query)) {
