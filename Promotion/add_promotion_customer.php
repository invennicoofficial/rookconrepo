...
$(document).ready(function() {
    ...
    $('#add_row_cust').on( 'click', function () {
        ...
        clone.find('#promotioncustomer_0').attr('id', 'promotioncustomer_'+add_new_cust);
        $('#custdelete_0').hide();
...
function selectCustomer(sel) {
	...
	$.ajax({
		type: "GET",
		url: "promotion_ajax_all.php?fill=cust_config&value="+stage,
...
<div class="form-group">
    <div class="col-sm-12">
        ...
        <?php if(!empty($_GET['promotionid'])) {
            ...
            ?>
                <div class="form-group clearfix" id="<?php echo 'promotioncustomer_'.$id_loop; ?>">
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Customer:</label>
                    <select data-placeholder="Choose a Customer..." id="<?php echo 'custcustomerid_'.$id_loop; ?>" name="assign_customer[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
                ...
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Contact Person:</label>
                    <select data-placeholder="Choose a Contact..." id="<?php echo 'custcustomerperson_'.$id_loop; ?>" name="customerperson[]" class="chosen-select-deselect form-control" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT contactid, first_name, last_name FROM contacts WHERE category='Customer' AND deleted=0  order by name");
                        while($row = mysqli_fetch_array($query)) {
                            if ($contactid == $row['contactid']) {
                                $selected = 'selected="selected"';
                            } else {
        ...
        <div class="additional_cust clearfix">
            ...
            <div class="form-group clearfix" id="promotioncustomer_0">
                <div class="col-sm-2"><label for="company_name" class="col-sm-4 show-on-mob control-label">Customer:</label>
                    <select data-placeholder="Choose a Customer..." id="custcustomerid_0" name="assign_customer[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT contactid, name FROM contacts WHERE category='Customer' ORDER BY name");
                        while($row = mysqli_fetch_array($query)) {
