...
$result = mysqli_query($dbc, "SELECT * FROM (SELECT CONCAT('Timesheet for ',`date`) time_type, 'timesheet' `src`, 0 `srcid`, '' time_heading, `staff` time_staff, `date` time_date, `start_time` time_start, `end_time` time_end, SEC_TO_TIME(`total_hrs` * 3600) time_length, `time_cards_id` FROM `time_cards` LEFT JOIN `ticket_time_list` ON `ticket_time_list`.`created_date` LIKE CONCAT(`time_cards`.`date`,'%') AND `time_cards`.`ticketid`=`ticket_time_list`.`ticketid` AND `time_cards`.`staff`=`ticket_time_list`.`created_by` AND `time_cards`.`total_hrs` * 3600 = TIME_TO_SEC(`ticket_time_list`.`time_length`) AND `ticket_time_list`.`deleted`=0 WHERE `time_cards`.`deleted`=0 AND `projectid`='$projectid' AND `ticket_time_list`.`id` IS NULL UNION
	...
	SELECT CONCAT('Task #',`tasklist`.`tasklistid`) time_type, 'task' `src`, `tasklist`.`tasklistid` `srcid`, `tasklist`.`heading` time_heading, `tasklist`.`contactid` time_staff, `tasklist`.`task_tododate` time_date, '' time_start, '' time_end, `tasklist`.`work_time` time_length, '' `time_cards_id` FROM `tasklist` WHERE `projectid`='$projectid' UNION
...
echo '<h3>&nbsp;'.(vuaed_visible_function($dbc, 'time_cards') ? '<button class="btn brand-btn pull-right cursor-hand" onclick="overlayIFrameSlider(\'../Timesheet/add_time_cards.php?projectid='.$projectid.'\',\'auto\',true); return false;">Add Time</button>' : '').'</h3>';
if(mysqli_num_rows($result) > 0) {
    ...
    echo '<tr class="hidden-xs hidden-sm">
        ...
        <th>End Time</th>
        <th>Duration</th>
        '.(vuaed_visible_function($dbc, 'time_cards') ? '<th>Function</th>' : '').'
        </tr>';
	$total_time = 0;
	while($row = mysqli_fetch_array( $result )) {
		...
		if(vuaed_visible_function($dbc, 'time_cards')) {
			echo '<td data-title="Function">';
			if($row['time_cards_id'] > 0) {
