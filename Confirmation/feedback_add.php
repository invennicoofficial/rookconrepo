<?php
/*
 * Add follow up feedback manually.
 */
include ('../include.php');
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form name="form_clients" method="post" action="" class="form-horizontal" role="form">
                ...
                <?php $value_config = ','.get_config($dbc, 'email_confirmation').','; ?>
                ...
                <div class="gap-top">
                    ...
                    if ( check_subtab_persmission( $dbc, 'confirmation', ROLE, 'add_feedback' ) === true ) { ?>
                        <a href='feedback_add.php'><button type="button" class="btn brand-btn mobile-block active_tab">Add Follow Up Feedback</button></a><?php
                    } else { ?>
                ...
                $result_booking = mysqli_query($dbc, "SELECT `bookingid`, `patientid`, `therapistsid`, `appoint_date` FROM `booking` WHERE DATE_FORMAT(`appoint_date`,'%Y-%m-%d') BETWEEN '$starttime' AND '$endtime'");
                        ...
                        <table class="table">
                            ...
                            while ( $row=mysqli_fetch_assoc($result_booking) ) { ?>
                                <tr>
                                    ...
                                    <td data-title="Add Feedback"><a href="" onclick="overlayIFrameSlider('<?=WEBSITE_URL?>/Confirmation/feedback_add_manual.php?id=<?= $row['bookingid'] ?>', '50%', false, true, $('.iframe_overlay').closest('.container').outerHeight() + 20); return false;">Add Feedback</a></td>
                                </tr><?php
                            } ?>
                        </table>
                    </div><?php
