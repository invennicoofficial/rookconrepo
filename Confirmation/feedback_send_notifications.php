...
<div class="container">
    <div class="row">
        ...
        <div class="gap-top">
            ...
            if ( check_subtab_persmission( $dbc, 'confirmation', ROLE, 'followup' ) === true ) { ?>
                <a href='feedback_send_notifications.php'><button type="button" class="btn brand-btn mobile-block active_tab">Send Follow Up Requests</button></a><?php
            } else { ?>
            ...
            if ( check_subtab_persmission( $dbc, 'confirmation', ROLE, 'view_feedback' ) === true ) { ?>
                <a href='feedback_view.php'><button type="button" class="btn brand-btn mobile-block">View Follow Up Feedback</button></a><?php
            } else { ?>
        ...
        Send and track customer feedback for services rendered.</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-8">
				<input type="text" name="email_subject" class="form-control" value="<?= get_config($dbc, 'followup_email_subject') ?>">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-4 control-label">Email Body:</label>
			<div class="col-sm-8">
				<textarea name="email_body" class="form-control"><?= html_entity_decode(get_config($dbc, 'followup_email_body')) ?></textarea>
			</div>
		</div>
        <?php
        ...
        $inactive_patient =	mysqli_query($dbc, "SELECT bookingid, therapistsid, patientid, appoint_date, follow_up_call_status, confirmation_email_date FROM booking WHERE follow_up_call_status='Paid' AND type NOT IN ('Break', 'No Book Days', 'Vacation') AND DATE_FORMAT(appoint_date,'%Y-%m-%d') BETWEEN '".$starttime."' AND '".$endtime."' ORDER BY therapistsid, appoint_date");
        ...
        if ( $inactive_patient->num_rows > 0 ) {
            ...
            while($row = mysqli_fetch_assoc($inactive_patient)) {
                ...
                if($patient_email == '-' || $patient_email == '') {
                    ...
                    $followup_result = mysqli_query($dbc, "SELECT `feedback_method` FROM `followup_notifications` WHERE `bookingid`='{$row['bookingid']}'");
                    if ($followup_result->num_rows > 0) {
                        while($followup_row=mysqli_fetch_assoc($followup_result)) {
                            echo '<td>Followed up via '. $followup_row['feedback_method'] . '</td>';
                        }
                    }
                } else {
