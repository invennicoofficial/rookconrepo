<?php
/*
Send bulk emails to all Patient everyday for Confirmation.
*/
error_reporting(0);
...
if (isset($_POST['send_follow_up_email'])) {
    ...
    for($i = 0; $i < count($_POST['check_send_email']); $i++) {
        ...
        $confirmation = $_POST['email_body'];
        $email_body = str_replace("[Customer Name]", get_contact($dbc, $patientid), $confirmation);
        $email_body = str_replace("[Appointment Date]", get_patient_from_booking($dbc, $bookingid, 'appoint_date'), $email_body);

        $confirmation_link = '<a href="'.WEBSITE_URL.'/confirmation_booking.php?id='.$bookingid.'">Click to Confirm</a>';
    ...
    if($what == 2) {
        echo '<script type="text/javascript"> alert("Confirmation Email Sent."); window.location.replace("confirmation_email.php"); </script>';
    }
    if($what == 1) {
        echo '<script type="text/javascript"> window.location.replace("confirmation_email.php"); </script>';
    }
}
?>
...
<div class="container triple-pad-bottom">
    <div class="row">
        ...
        <?php
            echo '<a href="'.WEBSITE_URL.'/CRM/config_crm.php?category=confirmation&maintype='.$_GET['maintype'].'" class="btn mobile-block pull-right"><img style="width: 50px;" title="Tile Settings" src="../img/icons/settings-4.png" class="settings-classic wiggle-me"></a><br><br>';
        ?>
        </h1>
        ...
        <span class="popover-examples list-inline pull-right">
            <a href="#job_file" data-toggle="tooltip" data-placement="top" title="Once you send the confirmation email and get a reply from the customer, click on the star to hide it."><img src="<?php echo WEBSITE_URL;?>/img/info.png" width="25"></a>
        </span>
        ...
        if($confirmation_email_send_before == '') {
            $confirmation_email_send_before = '1 month';
        }

        echo '<h3>Appointments between '.date('Y-m-d').' AND '.date('Y-m-d', strtotime("+".$confirmation_email_send_before."s")).'</h3>';

        echo '<span class="pull-right"><h4>Select All&nbsp;<input type="checkbox" id="select_all" class="form-control" style="width:25px;"/></h4></span>';

        $result =	mysqli_query($dbc,"SELECT * FROM booking WHERE confirmation_email_reply_date IS NULL AND  (str_to_date(substr(appoint_date,1,10),'%Y-%m-%d')) BETWEEN CURDATE() AND CURDATE() + INTERVAL $confirmation_email_send_before ORDER BY `appoint_date`");
        ...
        <th>Confirmation Sent Date</th>
        <th>Reply From Customer</th>
        <th>Send Email</th>';
        echo "</tr>";
        ...
        while($row = mysqli_fetch_array($result)) {
            ...
            if($row['confirmation_email_date'] != '') {
                echo '<td><input name="reply_send_email[]" type="checkbox" value="'.$bookingid.'" class="form-control" style="width:25px;"/></td>';
            } else {
                echo '<td>-</td>';
            }
