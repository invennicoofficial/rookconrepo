...
if (isset($_POST['export'])) {
	if($num_rows > 0) {
		...
		header('Content-Disposition: attachment; filename=newsletter_contact.csv');
		// create a file pointer connected to the output stream
		$output = fopen('php://output', 'w');

		// output the column headings
		fputcsv($output, array('First Name', 'Last Name', 'Email', 'Cell Phone', 'Last Visit', 'Gender','Mailing Address','Postal Code'));
		foreach($result as $row)
		{
			...
			fputcsv($output, array($first_name, $last_name, $email, $cell_phone, $last_login, $gender, $mailing_address,$postal_code));	
		}
...
<div class="container triple-pad-bottom">
	...
	<div class="row hide_on_iframe">
            ...
            if (strpos($value_config, ',Newsletter,') !== false) { ?>
                <div class="tab pull-left">
                    ...
                    if ( check_subtab_persmission( $dbc, 'crm', ROLE, 'newsletter' ) === true ) { ?>
                        <a href='newsletter.php'><button type="button" class="btn brand-btn mobile-block gap_left active_tab">Newsletter</button></a><?php
                    } else { ?>
        ...
        Design what you want your newsletter to look like and send it out by email.</div>
        <div class="clearfix"></div>
        </div>
		<div>
		...
		<div>
			...
			<span class="col-sm-1"></span>
			<label for="last_visit_from" class="col-sm-2 control-label">Postal Code:</label>
			<div class="col-sm-4">
				<?php if(isset($_POST['search_client_submit'])) { ?>
