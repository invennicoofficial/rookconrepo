<?php if($_GET['type'] == 'docs' && !empty($_POST['submit'])) {
	foreach($_FILES['document']['name'] as $i => $basename) {
		...
		if($basename != '') {
			...
			if(!move_uploaded_file($_FILES['document']['tmp_name'][$i], 'download/'.$filename)) {
				...
				$query_insert_upload = "INSERT INTO `client_project_document` (`projectid`, `upload`, `label`) VALUES ('$projectid', '$filename', '$label')";
	...
	foreach($_POST['support_link'] as $i => $link) {
		...
		if($link != '') {
            $query_insert_client_doc = "INSERT INTO `client_project_document` (`projectid`, `link`, `label`) VALUES ('$projectid', '$link', '$label')";
...
<div class="form-group">
    ...
    <div class="col-sm-8">
        ...
        if(!empty($_GET['projectid'])) {
            ...
            $get_doc = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(uploadid) AS total_id FROM client_project_document WHERE projectid='$projectid' AND deleted = 0 AND upload IS NOT NULL"));

            if($get_doc['total_id'] > 0) {
                $result = mysqli_query($dbc, "SELECT upload, uploadid, `label` FROM client_project_document WHERE  projectid='$projectid' AND deleted = 0 AND upload IS NOT NULL");
...
<div class="form-group">
    ...
    <div class="col-sm-8">
        ...
        if(!empty($_GET['projectid'])) {
            ...
            $get_doc = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(uploadid) AS total_id FROM client_project_document WHERE projectid='$projectid' AND deleted = 0 AND link IS NOT NULL"));

            if($get_doc['total_id'] > 0) {
                $result = mysqli_query($dbc, "SELECT link, uploadid, label FROM client_project_document WHERE  projectid='$projectid' AND deleted = 0 AND link IS NOT NULL");
