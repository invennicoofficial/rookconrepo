...
if (isset($_POST['submit_general'])) {
    ...
    if($_POST['project_path_milestone'] == 'NEW') {
        $query_insert_config = "INSERT INTO `client_project_path_milestone` (`project_path`, `milestone`, `timeline`, `checklist`, `ticket`, `workorder`)
        ...
        $query_update_vendor = "UPDATE `client_project_path_milestone` SET `project_path` = '$project_path', `milestone` = '$milestone',`timeline` = '$timeline', `checklist` = '$checklist',
...
<div class="mobile-100-container">
	...
	<span class="nav-subtab no-popover"><a href="field_config_project.php?type=Client"><button type="button" class="btn brand-btn mobile-block mobile-100">Client Project</button></a>&nbsp;&nbsp;</span>
...
if(!empty($_GET['project_path_milestone'])) {
    ...
    $get_contact = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM client_project_path_milestone WHERE project_path_milestone='$project_path_milestone'"));
		...
		<div class="panel panel-default">
			<div class="panel-heading">
				<h4 class="panel-title">
					<a data-toggle="collapse" data-parent="#accordion2" href="#collapse_tilename" >Client Projects Template<span class="glyphicon glyphicon-plus"></span>
					</a>
				</h4>
			</div>
			<div id="collapse_tilename" class="panel-collapse collapse">
				<div class="panel-body">
					  <div class="form-group">
						<label for="fax_number"	class="col-sm-4	control-label">Project Template Name:</label>
						<div class="col-sm-8">
					...
					for($emp_loop=0; $emp_loop<=$total_count; $emp_loop++) {
						...
						<div class="form-group clearfix mt" id="mt_<?php echo $mt_id; ?>">
							...
							<div class="col-sm-7">
								...
								<?php $checklist_items = explode('*#*',$cl);
								foreach($checklist_items as $checklist_item):
									if($checklist_item != ''): ?>
										<div class="form-group">
											<label class="control-label col-sm-3">Checklist Item:</label>
											<div class="col-sm-8"><input type="text" name="checklist_item_<?php echo $mt_id; ?>[]" class="form-control" placeholder="Enter a Checklist Item" value="<?php echo $checklist_item; ?>"></div>
...
<?php else: ?>
	...
	<?php $query_check_credentials = "SELECT * FROM client_project_path_milestone ORDER BY `project_path`";
	$result = mysqli_query($dbc, $query_check_credentials);
	echo "<table class='table table-bordered'>";
	echo "<tr class='hidden-xs hidden-sm'>";
	echo "<th>Project Path</th>";
	echo "<th>Milestone & Timeline</th>
	<th>Function</th>";
	echo "</tr>";
	while($row = mysqli_fetch_array($result)) {
		echo '<tr>';
		echo '<td data-title="Project Path">' . $row['project_path']. '</td>';
