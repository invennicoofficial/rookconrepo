<?php $result = mysqli_query($dbc, "SELECT * FROM (SELECT CONCAT('Checklist ',`checklist`.`checklist_name`,' Item #',`checklist_name`.`checklistnameid`) time_type, `checklist_name`.`checklist` time_heading, `checklist_name_time`.`contactid` time_staff, `checklist_name_time`.`timer_date` time_date, '' time_start, '' time_end, `checklist_name_time`.`work_time` time_length FROM `checklist` RIGHT JOIN `checklist_name` ON `checklist`.`checklistid`=`checklist_name`.`checklistid` RIGHT JOIN `checklist_name_time` ON `checklist_name_time`.`checklist_id`=`checklist_name`.`checklistnameid` WHERE `projectid`='$projectid' UNION
	SELECT CONCAT('Ticket #',`tickets`.`ticketid`) time_type, `tickets`.`heading` time_heading, `ticket_timer`.`created_by` time_staff,  `ticket_timer`.`created_date` time_date, `ticket_timer`.`start_time` time_start, `ticket_timer`.`end_time` time_end, TIMEDIFF(`ticket_timer`.`end_time`,`ticket_timer`.`start_time`) time_length FROM `tickets` RIGHT JOIN `ticket_timer` ON `tickets`.`ticketid`=`ticket_timer`.`ticketid` WHERE `client_projectid`='$projectid' UNION
	SELECT CONCAT('Task #',`tasklist`.`tasklistid`) time_type, `tasklist`.`heading` time_heading, `tasklist`.`contactid` time_staff, `tasklist`.`task_tododate` time_date, '' time_start, '' time_end, `tasklist`.`work_time` time_length FROM `tasklist` WHERE `client_projectid`='$projectid' UNION
	SELECT CONCAT(`client_project_milestone_checklist`.`milestone`,' Milestone #',`client_project_milestone_checklist`.`checklistid`) time_type, `client_project_milestone_checklist`.`checklist` time_heading, `client_project_milestone_checklist_time`.`contactid` time_staff, `client_project_milestone_checklist_time`.`timer_date` time_date, '' time_start, '' time_end, `client_project_milestone_checklist_time`.`work_time` time_length FROM `client_project_milestone_checklist` RIGHT JOIN `client_project_milestone_checklist_time` ON `client_project_milestone_checklist`.`checklistid`=`client_project_milestone_checklist_time`.`checklist_id` WHERE `client_project_milestone_checklist`.`projectid`='$projectid') timers");
