...
$project = mysqli_fetch_array(mysqli_query($dbc, "SELECT * FROM `client_project` WHERE `projectid`='$projectid'"));
...
$purchase_orders = mysqli_query($dbc, "SELECT `posid`, `po_category`, `status`, `total_price` FROM `purchase_orders` WHERE `client_projectid`='$projectid' AND `deleted`=0");
...
$tasks = mysqli_query($dbc, "SELECT `tasklistid`, `heading`, `work_time`, `hourly` FROM `tasklist` LEFT JOIN `staff_rate_table` ON CONCAT(',',`staff_rate_table`.`staff_id`,',') LIKE CONCAT('%,',`tasklist`.`created_by`,',%') AND `staff_rate_table`.`deleted`=0 WHERE `tasklist`.`client_projectid`='$projectid' GROUP BY `tasklist`.`tasklistid`");
...
$checklists = mysqli_query($dbc, "SELECT `checklist_time_id`, checklist.`milestone`, checklist.`checklist`, `work_time`, `hourly` FROM `client_project_milestone_checklist_time` timer LEFT JOIN `client_project_milestone_checklist` checklist ON timer.`checklist_id`=checklist.`checklistid` LEFT JOIN `staff_rate_table` ON CONCAT(',',`staff_rate_table`.`staff_id`,',') LIKE CONCAT('%,',timer.`contactid`,',%') AND `staff_rate_table`.`deleted`=0 WHERE checklist.`projectid`='$projectid' GROUP BY timer.`checklist_time_id` ORDER BY `checklist_time_id`");
while($checklist = mysqli_fetch_array($checklists)) {
	$hours = explode(':',$checklist['work_time']);
	$hours = $hours[0] + ($hours[1] / 60) + ($hours[2] / 3600);
	$items[] = ['Checklist', $checklist['milestone'].' Milestone',html_entity_decode($checklist['checklist'])." (".round($hours,2)." hours)",round($hours * $checklist['hourly'],2)];
}
