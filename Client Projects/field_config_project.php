...
if (isset($_POST['submit_general'])) {
    ...
    $client_project_tabs = implode(',',$_POST['client_project_tabs']);
    $get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(configid) AS configid FROM general_configuration WHERE name='client_project_tabs'"));
    if($get_config['configid'] > 0) {
        $query_update_employee = "UPDATE `general_configuration` SET value = '$client_project_tabs' WHERE name='client_project_tabs'";
        $result_update_employee = mysqli_query($dbc, $query_update_employee);
    } else {
        $query_insert_config = "INSERT INTO `general_configuration` (`name`, `value`) VALUES ('client_project_tabs', '$client_project_tabs')";
        $result_insert_config = mysqli_query($dbc, $query_insert_config);
    }
    //Project Navigation Tabs

    //Next Step after Project
    $next_step_after_project = implode('/',$_POST['next_step_after_client_project']);
    $get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(configid) AS configid FROM general_configuration WHERE name='next_step_after_client_project'"));
    if($get_config['configid'] > 0) {
        $query_update_employee = "UPDATE `general_configuration` SET value = '$next_step_after_client_project' WHERE name='next_step_after_client_project'";
        $result_update_employee = mysqli_query($dbc, $query_update_employee);
    } else {
        $query_insert_config = "INSERT INTO `general_configuration` (`name`, `value`) VALUES ('next_step_after_client_project', '$next_step_after_client_project')";
        $result_insert_config = mysqli_query($dbc, $query_insert_config);
    }
    //Next Step after Project
    ...
    $get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(configid) AS configid FROM general_configuration WHERE name='client_project_service_price_or_hours'"));
    if($get_config['configid'] > 0) {
        $query_update_employee = "UPDATE `general_configuration` SET value = '$project_service_price_or_hours' WHERE name='client_project_service_price_or_hours'";
        $result_update_employee = mysqli_query($dbc, $query_update_employee);
    } else {
        $query_insert_config = "INSERT INTO `general_configuration` (`name`, `value`) VALUES ('client_project_service_price_or_hours', '$project_service_price_or_hours')";
        $result_insert_config = mysqli_query($dbc, $query_insert_config);
    }

    $project_service_qty_cost = $_POST['project_service_qty_cost'];
    $get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(configid) AS configid FROM general_configuration WHERE name='client_project_service_qty_cost'"));
    if($get_config['configid'] > 0) {
        $query_update_employee = "UPDATE `general_configuration` SET value = '$project_service_qty_cost' WHERE name='client_project_service_qty_cost'";
        $result_update_employee = mysqli_query($dbc, $query_update_employee);
    } else {
        $query_insert_config = "INSERT INTO `general_configuration` (`name`, `value`) VALUES ('client_project_service_qty_cost', '$project_service_qty_cost')";
        $result_insert_config = mysqli_query($dbc, $query_insert_config);
    }
...
if (isset($_POST['submit_all'])) {
    ...
    $get_field_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(fieldconfigprojectid) AS fieldconfigprojectid FROM field_config_client_project WHERE type='$type'"));
    if($get_field_config['fieldconfigprojectid'] > 0) {
        $query_update_employee = "UPDATE `field_config_client_project` SET config_fields = '$config_fields' WHERE `type` = '$type'";
        $result_update_employee = mysqli_query($dbc, $query_update_employee);
    } else {
        $query_insert_config = "INSERT INTO `field_config_client_project` (`type`, `config_fields`) VALUES ('$type', '$config_fields')";
        $result_insert_config = mysqli_query($dbc, $query_insert_config);
    }
    //Fields
...
$type = (!empty($_GET['type']) ? $_GET['type'] : 'Pending');
$nav_tabs = ','.get_config($dbc, 'client_project_tabs').','; ?>

<div class="container">
<div class="row">
<h1>Client Project</h1>
<div class="pad-left gap-top"><a href="project.php?tab=<?= array_filter(explode(',', $nav_tabs))[0] ?>" class="btn brand-btn">Back to Dashboard</a></div>
...
<div class="pad-left gap-top mobile-100-container">
	<span class="nav-subtab no-popover">
		<a href='field_config_project.php?type=Pending'><button type="button" class="btn brand-btn mobile-block mobile-100 <?php echo ($type == 'Pending' ? 'active_tab' : ''); ?>">General</button></a>&nbsp;&nbsp;
	</span>
	<span class="nav-subtab">
		<span class="popover-examples list-inline" style="margin:0 2px 0 0;"><a data-toggle="tooltip" data-placement="top" title="Click here to create a new Client Project Path and Milestone."><img src="<?= WEBSITE_URL; ?>/img/info.png" width="20"></a></span>
		<a href='field_config_project_path_milestone.php'><button type="button" class="btn brand-btn mobile-block mobile-100">Client Project Template</button></a>&nbsp;&nbsp;
	</span>
	<span class="nav-subtab no-popover">
		<a href='field_config_project.php?type=Client'><button type="button" class="btn brand-btn mobile-block mobile-100 <?php echo ($type == 'Client' ? 'active_tab' : ''); ?>">Client Projects</button></a>&nbsp;&nbsp;
	</span>
</div>
...
$config_sql = "SELECT `config_fields` FROM field_config_client_project WHERE type='$type' UNION
	SELECT `config_fields`  FROM `field_config_client_project` WHERE `fieldconfigprojectid` IN (SELECT MAX(`fieldconfigprojectid`) FROM `field_config_client_project` WHERE `type`='$type')";
...
<div class="panel-group" id="accordion2">
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4 class="panel-title">
                <a data-toggle="collapse" data-parent="#accordion2" href="#collapse_nav_tabs" >Client Project Tabs<span class="glyphicon glyphicon-plus"></span>
                </a>
            </h4>
        </div>
        ...
        <div id="collapse_nav_tabs" class="panel-collapse collapse">
            <div class="panel-body">
				<label><input type="checkbox" name="client_project_tabs[]" <?php echo (strpos(','.$nav_tabs.',', ',pending,') !== false ? 'checked' : ''); ?> value="pending" style="height: 1.5em; margin: 0.5em; width: 1.5em;"> Pending Projects</label>
				<label><input type="checkbox" name="client_project_tabs[]" <?php echo (strpos(','.$nav_tabs.',', ',active,') !== false ? 'checked' : ''); ?> value="active" style="height: 1.5em; margin: 0.5em; width: 1.5em;"> Active Projects</label>
				<label><input type="checkbox" name="client_project_tabs[]" <?php echo (strpos(','.$nav_tabs.',', ',archived,') !== false ? 'checked' : ''); ?> value="archived" style="height: 1.5em; margin: 0.5em; width: 1.5em;"> Archived Projects</label>
				<label><input type="checkbox" name="client_project_tabs[]" <?php echo (strpos(','.$nav_tabs.',', ',tickets,') !== false ? 'checked' : ''); ?> value="tickets" style="height: 1.5em; margin: 0.5em; width: 1.5em;"> Tickets</label>
				<label><input type="checkbox" name="client_project_tabs[]" <?php echo (strpos(','.$nav_tabs.',', ',daysheet,') !== false ? 'checked' : ''); ?> value="daysheet" style="height: 1.5em; margin: 0.5em; width: 1.5em;"> Day Sheet</label>
            </div>
        </div>
    </div>
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4 class="panel-title">
                <a data-toggle="collapse" data-parent="#accordion2" href="#collapse_next" >
                    Next Step After Client Project<span class="glyphicon glyphicon-plus"></span>
                </a>
            </h4>
        </div>
        ...
        <div id="collapse_next" class="panel-collapse collapse">
            <div class="panel-body">
                <?php
                $next_step_after_project = get_config($dbc, 'next_step_after_client_project');
                ?>

                <input type="checkbox" <?php if (strpos($next_step_after_project, "Ticket") !== FALSE) { echo " checked"; } ?> value="Ticket" style="height: 20px; width: 20px;" name="next_step_after_client_project[]">&nbsp;&nbsp;Ticket&nbsp;&nbsp;

                <input type="checkbox" <?php if (strpos($next_step_after_project, "Work Order") !== FALSE) { echo " checked"; } ?> value="Work Order" style="height: 20px; width: 20px;" name="next_step_after_client_project[]">&nbsp;&nbsp;Work Order&nbsp;&nbsp;
    ...
    <div class="panel panel-default">
        ...
        <div id="collapse_next12" class="panel-collapse collapse">
            <div class="panel-body">
                ...
                <?php
                $project_service_price_or_hours = get_config($dbc, 'client_project_service_price_or_hours');
                ...
                <div class="col-sm-8">
                    <input name="client_project_service_price_or_hours" type="text" value = "<?php echo $project_service_price_or_hours; ?>" class="form-control">
                </div>
                </div>

                <?php
                $project_service_qty_cost = get_config($dbc, 'client_project_service_qty_cost');
                ...
                <div class="col-sm-8">
                    <input name="client_project_service_qty_cost" type="text" value = "<?php echo $project_service_qty_cost; ?>" class="form-control">
                </div>
                </div>
...
<div class="form-group">
    <div class="col-sm-6">
        <a href="project.php?tab=pending" class="btn brand-btn btn-lg">Back</a>
    </div>
    <div class="col-sm-6">
...
<div class="form-group">
    <div class="col-sm-6">
        <a href="project.php?tab=active" class="btn brand-btn btn-lg">Back</a>
    </div>
    <div class="col-sm-6">
