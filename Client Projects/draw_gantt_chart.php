	...
	<div class="row hide_on_iframe">
        ...
        <div class="form-group">
		  ...
		  <div class="col-sm-8">
              ...
              <option value=""></option>
              <?php
                $query = mysqli_query($dbc,"SELECT c.contactid,c.name AS client_name FROM contacts c, estimated_gantt_chart t WHERE t.contactid=c.contactid");
                while($row = mysqli_fetch_array($query)) {
                ?><option <?php if ($row['contactid'] == $search_client) { echo " selected"; } ?> value='<?php echo  $row['contactid']; ?>' ><?php echo get_contact($dbc, $row['contactid']; ?></option>
            <?php	} ?>
            </select>
		  </div>
		</div>
        ...
        if($search_client != '') {
            $query_check_credentials = "SELECT t.*, c.name FROM estimated_gantt_chart t, contacts c WHERE t.contactid = c.contactid AND t.contactid = '$search_client' ORDER BY estimatedganttchartid DESC";
        } else {
        ...
        while($row = mysqli_fetch_array( $result )) {
            ...
            $get_tt = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT start_date,estimated_completed_date,completion_date FROM client_project WHERE clientid='$clientid'"));
            ...
            $result_ticket = mysqli_query($dbc, "SELECT * FROM tickets WHERE contactid = '$clientid'");
            ...
            $result_ticket = mysqli_query($dbc, "SELECT * FROM workorder WHERE clientid = '$clientid'");
            while($row_ticket = mysqli_fetch_array( $result_ticket )) {
                if($row_ticket['to_do_date'] != '0000-00-00') {
                    $class = 'current';
                    if($row_ticket['status'] == 'Sales/Estimate/RFP') {
        ...
        $gantti = new Gantti($data, array(
          'title'      => get_contact($dbc, $clientid, 'name'),
          'cellwidth'  => 25,
          'cellheight' => 35,
          'today'      => true
        ));
