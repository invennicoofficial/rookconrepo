...
<form name="invoice_table" method="post" action="" class="form-inline offset-top-20" role="form">
	...
	$query_check_credentialss = "SELECT inv.*, c.`contactid` FROM purchase_orders inv LEFT JOIN contacts c ON inv.contactid = c.contactid WHERE inv.`client_projectid`='$projectid' AND inv.deleted = 0 ".$search." ORDER BY inv.posid DESC";
