...
$contact_from_intake = 0;
if($row = mysqli_fetch_array(mysqli_query($dbc, "SELECT * FROM `intake` WHERE `contactid`='$contactid_intake'"))) {
	$contact_from_intake = $row['intakeid'];
}
...
if (isset($_POST['save'])) {
    ...
    //$budget_price = implode('*#*',$_POST['budget_price']).'*#*'.$_POST['total_budget'];
    ...
    $query_insert_customer = "INSERT INTO `client_project` (`clientid`, `ratecardid`, `projecttype`, `project_name`, `created_date`, `start_date`, `estimated_completed_date`, `completion_date`, `budget_price`, `project_path`, `status`, `history`) VALUES ('$clientid', '$ratecardid', 'Client', '$project_name', '$created_date', '$start_date', '$estimated_completed_date', '$completion_date', '$budget_price', '$project_path', 'Approve as Project', '$history')";
    $result_insert_customer = mysqli_query($dbc, $query_insert_customer);
    $projectid = mysqli_insert_id($dbc);

    echo insert_day_overview($dbc, $who_added, 'Client Project', $when_added, '', 'Added Client Project '.$project_name);
    ...
    for($i = 0; $i < count($_FILES['document']['name']); $i++) {
        if($document[$i] != '') {
            ...
            $query_insert_upload = "INSERT INTO `client_project_document` (`projectid`, `upload`) VALUES ('$projectid', '$document[$i]')";
    ...
    for($i = 0; $i < count($_POST['support_link']); $i++) {
        ...
        if($support_link != '') {
            $query_insert_client_doc = "INSERT INTO `client_project_document` (`projectid`, `link`) VALUES ('$projectid', '$support_link')";
	...
	if ( !empty($intakeid) ) {
		...
		$update_intake = mysqli_query ( $dbc, "UPDATE `intake` SET `assigned_date`='$assigned_date', `contactid`='$clientid' WHERE `intakeid`='$intakeid'" );
	}
	$intake_url_var = '&intakeid=' . $intakeid . '&clientid=' . $contactid_intake;
	if($row = mysqli_fetch_array(mysqli_query($dbc, "SELECT * FROM `intake` WHERE `contactid`='$clientid' AND `deleted`=0"))) {
		$label = $row['category'];
		$support_link = WEBSITE_URL . '/Intake/' . $row['intake_file'];
		$query_insert_client_doc = "INSERT INTO `client_project_document` (`projectid`, `link`, `label`) VALUES ('$projectid', '$support_link', '$label')";
		$result_insert_client_doc = mysqli_query($dbc, $query_insert_client_doc);
	}
	
	// Create the checklist from the default checklist for each milestone
    $milestone_list = explode('#*#', get_client_project_path_milestone($dbc, $project_path, 'milestone'));
    $checklist = explode('#*#', get_client_project_path_milestone($dbc, $project_path, 'checklist'));
    $tickets = explode('#*#', get_client_project_path_milestone($dbc, $project_path, 'ticket'));
    $workorders = explode('#*#', get_client_project_path_milestone($dbc, $project_path, 'workorder'));
	foreach($milestone_list as $key => $milestone) {
		$checklist_items = explode('*#*', $checklist[$key]);
		foreach($checklist_items as $sort => $item) {
			if($item != '') {
				$query = "INSERT INTO `client_project_milestone_checklist` (`projectid`, `milestone`, `checklist`, `sort`, `updated_date`) VALUES ('$projectid', '$milestone', '$item', '$sort', '".date('Y-m-d')."')";
		...
		foreach($ticket_list as $ticket) {
			if($ticket != '') {
				...
				$query = "INSERT INTO `tickets` (`projectid`, `service_type`, `service`, `sub_heading`, `heading`, `project_path`, `milestone_timeline`)
					VALUES ('$projectid', '".$service['service_type']."', '".$service['category']."', '".$service['heading']."', '$heading', '$project_path', '$milestone')";
				$result = mysqli_query($dbc, $query);
			}
		}
		...
		foreach($workorder_list as $workorder) {
			if($workorder != '') {
				$query = "INSERT INTO `workorders` (`projectid`, `heading`)
					VALUES ('$projectid', '$workorder')";
			}
		}
	}
	...
	if($ptype == 'Pending' && !in_array('pending',$nav_tabs)) {
		$status = 'Approve as Project';
		mysqli_query($dbc, "UPDATE `client_project` SET `status`='$status' WHERE `projectid`='$projectid'");
		$ptype = 'active';
	}

    echo '<script type="text/javascript">  window.location.replace("project.php?tab='.$ptype.'"); </script>';
}
...
if (isset($_POST['submit'])) {
    ...
    echo insert_day_overview($dbc, $who_added, 'Client Project', $when_added, '', 'Edited Client Project '.$project_name);

    $get_field_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT config_fields_quote FROM field_config_client_project"));
    ...
    foreach ($_POST['packageid'] as $packageid_all) {
        if($packageid_all != '') {
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`,  `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'Package', '$packageid_all', '$st', '$temp_ticket_desc')";
    ...
    foreach ($_POST['promotionid'] as $promotionid_all) {
        if($promotionid_all != '') {
            ...
            $st = $query['service_type'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`,  `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'Promotion', '$promotionid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['customid'] as $customid_all) {
        if($customid_all != '') {
            ...
            $st = $query['service_type'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`,  `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'Custom', '$customid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['materialid'] as $materialid_all) {
        if($materialid_all != '') {
            ...
            $st = $query['category'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'Material', '$materialid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['serviceid'] as $serviceid_all) {
        if($serviceid_all != '') {
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'Service', '$serviceid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['productid'] as $productid_all) {
        if($productid_all != '') {
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`, `clientid`, `category`, `itemid`, `product_type`,`desc`) VALUES ('$projectid', '$clientid', 'Product', '$productid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['sredid'] as $sredid_all) {
        if($sredid_all != '') {
            ...
            $st = $query['sred_type'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'SRED', '$sredid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['contactid'] as $contactid_all) {
        if($contactid_all != '') {
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'Staff', '$contactid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['contractorid'] as $contractorid_all) {
        if($contractorid_all != '') {
            ...
            $st = $query['first_name'].' '.$query['last_name'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'Contractor', '$contractorid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['clientid'] as $clientid_all) {
        if($clientid_all != '') {
            ...
            $st = $query['first_name'].' '.$query['last_name'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'Client', '$clientid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['vendorperson'] as $vendorperson_all) {
        if($vendorperson_all != '') {
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'Vendor Price List', '$vendorperson_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['customerid'] as $customerid_all) {
        if($customerid_all != '') {
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`,  `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'Customer', '$customerid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['inventoryid'] as $inventoryid_all) {
        if($inventoryid_all != '') {
            ...
            $st = $query['category'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`,  `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'Inventory', '$inventoryid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['equipmentid'] as $equipmentid_all) {
        if($equipmentid_all != '') {
            ...
            $st = $query['category'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`,  `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'Equipment', '$equipmentid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['labourid'] as $labourid_all) {
        if($labourid_all != '') {
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`,  `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$projectid', '$clientid', 'Labour', '$labourid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['expensetype'] as $expensetype_all) {
        if($expensetype_all != '') {
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`,  `clientid`, `category`, `desc`) VALUES ('$projectid', '$clientid', 'Expenses', '$desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
    ...
    foreach ($_POST['other_detail'] as $other_detail_all) {
        if($other_detail_all != '') {
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`projectid`,  `clientid`, `category`, `desc`) VALUES ('$projectid', '$clientid', 'Other', '$other_detail_all')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    $history = '';
    $get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(projectid) AS total_id FROM client_project WHERE `projectid` = '$projectid' AND status = 'Submitted'"));

    if($get_config['total_id'] == 1) {
        $get_project = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM client_project WHERE `projectid` = '$projectid'"));
    ...
    if($t_comment != '') {
        ...
        if($type != '') {
            $query_insert_ca = "INSERT INTO `client_project_comment` (`projectid`, `comment`, `email_comment`, `created_date`, `created_by`, `type`, `note_heading`) VALUES ('$projectid', '$t_comment', '$email_comment', '$created_date', '$who_added', '$type', '$note_heading')";
            $result_insert_ca = mysqli_query($dbc, $query_insert_ca);
        } else {
            $query_update_report = "UPDATE `client_project_detail` SET `$note_heading` = CONCAT($note_heading,'$t_comment') WHERE `projectid` = '$projectid'";
            $result_update_report = mysqli_query($dbc, $query_update_report);
        }
    ...
    for($i = 0; $i < count($_FILES['document']['name']); $i++) {
        if($document[$i] != '') {
            ...
            move_uploaded_file($_FILES["document"]["tmp_name"][$i], "download/" . $_FILES["document"]["name"][$i]) ;
            $query_insert_upload = "INSERT INTO `client_project_document` (`projectid`, `upload`, `label`) VALUES ('$projectid', '$document[$i]', '$label')";
            $result_insert_upload = mysqli_query($dbc, $query_insert_upload);
        }
    }
    ...
    $status = get_client_project($dbc, $projectid, 'status');
...
$(document).ready(function() {
	...
	$("#projectclientid").change(function() {
        ...
        window.location = 'add_project.php?type='+ptype+'&clientid='+clientid+"&intakeid="+intakeid;
	});
    ...
    $("#ratecardid").change(function() {
        ...
        window.location = 'add_project.php?type='+ptype+'&clientid='+clientid+'&ratecardid='+this.value+"&intakeid="+intakeid;
	});
...
<div class="container">
  <div class="row">
    ...
    ?>
    <h1><?php echo (empty($_GET['projectid']) ? 'Add ' : 'Edit '); ?> Project</h1>
	...
	<?php if ( !empty ($intakeid) ) { ?>
		<div class="notice">
			<img src="<?= WEBSITE_URL; ?>/img/info.png" width="30">&nbsp;&nbsp;Add a new Project and attach the Intake Form to it.
		</div><?php
	} else if ($contact_from_intake > 0) { ?>
		<div class="notice">
			<img src="<?= WEBSITE_URL; ?>/img/info.png" width="30">&nbsp;&nbsp;Add a new Project. (The Intake Form attached to the selected contact will be attached to this project.)
		</div><?php
	} ?>
    ...
    <?php
        ...
        if(!empty($_GET['projectid'])) {
            ...
            $get_contact = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM client_project WHERE projectid='$projectid'"));
        ...
        <?php }
		$config_sql = "SELECT `config_fields` FROM field_config_client_project WHERE type='$projecttype' UNION
			SELECT `config_fields`  FROM `field_config_client_project` WHERE `fieldconfigprojectid` IN (SELECT MAX(`fieldconfigprojectid`) FROM `field_config_client_project` WHERE `type` IN ('Client'))";
		...
		$config_sql = "SELECT `config_fields` FROM field_config_client_project WHERE type='$projecttype' UNION
			SELECT `config_fields`  FROM `field_config_client_project` WHERE `fieldconfigprojectid` IN (SELECT MAX(`fieldconfigprojectid`) FROM `field_config_project` WHERE `type` IN ('Client'))";
    ...
    <input type="hidden" id="ptype" name="ptype" value="Client" />
    ...
    <div class="panel-group" id="accordion2">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <h4 class="panel-title">
                    <a data-toggle="collapse" data-parent="#accordion2" href="#collapse_abi" >Project Info<span class="glyphicon glyphicon-minus"></span></a>
                </h4>
            </div>
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <h4 class="panel-title">
                    <a data-toggle="collapse" data-parent="#accordion2" href="#collapse_pm" >Project Template<span class="glyphicon glyphicon-plus"></span></a>
                </h4>
            </div>
        ...
        <div class="panel panel-default">
            ...
            <div id="collapse_doc" class="panel-collapse collapse">
                <div class="panel-body">
                    ...
                    if ( !empty ($intakeid) ) { ?>
						<div class="notice">
							<img src="<?php echo WEBSITE_URL; ?>/img/info.png" width="30">&nbsp;&nbsp;The Intake Form will be attached to this Project when you click the Save button.
						</div><?php
					} else if ( $contact_from_intake > 0 ) { ?>
						<div class="notice">
							<img src="<?php echo WEBSITE_URL; ?>/img/info.png" width="30">&nbsp;&nbsp;Becuase you selected a contact created from the Intake tile, the Intake Form will be attached to this Project when you click the Save button.
						</div><?php
					}
    ...
    <?php if(empty($_GET['projectid'])) { ?>
        <div class="form-group">
            ...
            <div class="col-sm-6">
                <button	type="submit" name="save" value="save" class="btn brand-btn btn-lg pull-right">Save</button>
