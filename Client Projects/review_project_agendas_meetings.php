...
if(vuaed_visible_function($dbc, 'agenda_meeting') == 1) {
		...
		echo '<a id="'.$projectid.'" href="'.WEBSITE_URL.'/Agenda Meetings/add_agenda.php?projectid=C'.$projectid.'&from='.urlencode(WEBSITE_URL.$_SERVER['REQUEST_URI']).'" class="btn brand-btn mobile-block">Add Agenda</a></div>';
		...
		echo '<a id="'.$projectid.'" href="'.WEBSITE_URL.'/Agenda Meetings/add_meeting.php?projectid=C'.$projectid.'&from='.urlencode(WEBSITE_URL.$_SERVER['REQUEST_URI']).'" class="btn brand-btn mobile-block">Add Meeting</a></div>';
}
...
$query_agenda = "SELECT * FROM `agenda_meeting` WHERE (`status`='$status' OR '$status' = '') AND `type`='Agenda' AND CONCAT(',',projectid,',') LIKE '%,C$projectid,%' ORDER BY `date_of_meeting` DESC";
$query_meeting = "SELECT * FROM `agenda_meeting` WHERE (`status`='$status' OR '$status' = '') AND `type`='Meeting' AND CONCAT(',',projectid,',') LIKE '%,C$projectid,%' ORDER BY `date_of_meeting` DESC";
