...
function countRCTotalCont(sel) {
	var stage = sel.value;
	var typeId = sel.id;

	var arr = typeId.split('_');
    var del = (jQuery('#crc_contractor_custprice_'+arr[3]).val() * jQuery('#crc_contractor_qty_'+arr[3]).val());
    jQuery('#crc_contractor_total_'+arr[3]).val(round2Fixed(del));
...
<?php
if(!empty($_GET['estimateid'])) {
    $query_rc = mysqli_query($dbc,"SELECT * FROM company_rate_card WHERE (rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories') AND tile_name='Contractor'");
    ...
    while($row_rc = mysqli_fetch_array($query_rc)) {
        ...
        <div class="form-group clearfix" width="100%">
            ...
            <div class="col-sm-1">
                <input value= "<?php echo $estimate_company_rate_card['cust_price']; ?>" onchange="countRCTotalCont(this)" name="crc_contractor_cust_price[]" type="text" id="crc_contractor_custprice_<?php echo $rc;?>" class="form-control" />
            </div>
            <div class="col-sm-1">
                <input name="crc_contractor_qty[]" value= "<?php echo $estimate_company_rate_card['qty']; ?>" type="text" onchange="countRCTotalCont(this)" id="crc_contractor_qty_<?php echo $rc;?>" class="form-control" />
            </div>
            <div class="col-sm-1">
