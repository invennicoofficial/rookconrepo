       ...
       <?php
        ...
        <?php if(!empty($get_staff)) {
            ...
            <div class="form-group clearfix" id="<?php echo 'staff_'.$id_loop; ?>" >
                ...
                <div class="col-sm-1" >
                    <input name="stestimateunit[]" id="<?php echo 'stestimateunit_'.$id_loop; ?>" value="<?php echo $unit; ?>" type="text" class="form-control" />
                </div>
                <div class="col-sm-1" >
...
<?php
if(!empty($_GET['estimateid'])) {
    $query_rc = mysqli_query($dbc,"SELECT * FROM company_rate_card WHERE (rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories') AND tile_name='Staff'");
