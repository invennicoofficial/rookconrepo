...
function countService(txb) {
    if(txb != 'delete') {
        ...
        document.getElementById('sestimatetotal_'+split_id[1]).value = parseFloat($('#sestimateprice_'+split_id[1]).val() * lbqty);
    }
...
function changeServiceTotal() {
    ...
    jQuery('[name="sprofit[]"]').each(function () {
        sum_profit += +$(this).val() || 0;
    });

    var count = 0;
    jQuery('[name="sprofitmargin[]"]').each(function () {
    ...
    jQuery('#services_profit').val(round2Fixed(sum_profit));
    jQuery('#services_profit_margin').val(round2Fixed(per_profit_margin));
}
...
<div class="form-group">
    <div class="col-sm-12">
        <div class="form-group clearfix">
            ...
            <!--<label class="col-sm-1 text-center"><?php echo get_config($dbc, 'estimate_service_price_or_hours'); ?></label>
            <label class="col-sm-1 text-center"><?php echo get_config($dbc, 'estimate_service_qty_cost'); ?></label>-->
       ...
       <?php
        ...
        <?php if(!empty($get_services)) {
            ...
            <div class="form-group clearfix" id="<?php echo 'services_'.$id_loop; ?>" >
                <?php if (strpos($base_field_config, ','."Services Service Type".',') !== FALSE) { ?>
                <div class="col-sm-2">
                    <select data-placeholder="Choose a Type..." id="<?php echo 'sservice_'.$id_loop; ?>" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
                ...
                <?php if (strpos($base_field_config, ','."Services Category".',') !== FALSE) { ?>
                <div class="col-sm-2">
                    <select data-placeholder="Choose a Category..." id="<?php echo 'scategory_'.$id_loop; ?>" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
...
<?php
if(!empty($_GET['estimateid'])) {
    $query_rc = mysqli_query($dbc,"SELECT * FROM company_rate_card WHERE (rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories') AND tile_name='Services'");
