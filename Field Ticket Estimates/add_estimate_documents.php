...
if(!empty($_GET['estimateid'])) {
    ...
    $get_doc = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(uploadid) AS total_id FROM bid_document WHERE estimateid='$estimateid'"));

    if($get_doc['total_id'] > 0) {
        $result = mysqli_query($dbc, "SELECT upload, uploadid FROM bid_document WHERE  estimateid='$estimateid'");
