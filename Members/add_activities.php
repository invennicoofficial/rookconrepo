...
function selectContactCategory(sel) {
    ...
    <?php
        ...
        if(!empty($_GET['contactid']) && !empty($_GET['category'])) {
            $get_contact = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM social_story_activities WHERE support_contact='".$_GET['contactid']."'"));
            ...
            $activities_id = $get_contact['activities_id'];
        ...
        <?php   }      ?>
        <input type="hidden" id="submit_type" name="submit_type" value="activities" />
