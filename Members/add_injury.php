...
if (isset($_POST['submit'])) {
    ...
    echo '<script type="text/javascript"> alert("Injury Added"); window.top.close(); window.opener.location.reload(); </script>';
...
function selectType(sel) {
    ...
    if(status == '') {
        alert("Please make sure you have filled in Type.");
        return false;
    }
	var typeId = sel.id;
	var arr = typeId.split('_');
	if (status.indexOf("MVA") >= 0) {
        $(".mva_type").show();
    } else {
        $(".mva_type").hide();
    }
...
function selectInsurer(sel) {
	var insurer = sel.value;
	var typeId = sel.id;
	var arr = typeId.split('_');
    var insurer = insurer.replace(/ /g,'');
    var insurer = insurer.replace("&", "__");
	$.ajax({    //create an ajax request to load_page.php
		type: "GET",
		url: "<?php echo WEBSITE_URL;?>/ajax_all.php?fill=injury&insurer="+insurer,
		dataType: "html",   //expect html to be returned
		success: function(response){
            $('#adjusterid').html(response);
            $("#adjusterid").trigger("change.select2");
        }
	});
}
</script>
  ...
  <div class="row">
		...
		<input type="hidden" name="contactid" value="<?php echo $contactid; ?>" />
         ...
         <div class="form-group">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-8">
                <select id="contactid" data-placeholder="Choose a Patient..." name="contactid" class="chosen-select-deselect form-control" width="380">
                    <option value=""></option>
                    <?php
                    $query = mysqli_query($dbc,"SELECT contactid, first_name, last_name FROM contacts WHERE contactid='$contactid'");
                    while($row = mysqli_fetch_array($query)) {
          ...
          <div class="form-group">
            <label for="company_name" class="col-sm-4 control-label">Date:</label>
            <div class="col-sm-8">
          ...
          <div class="form-group mva_type" <?php echo $style; ?>>
            ...
            <div class="col-sm-8">
                <select data-placeholder="Choose a Adjuster..." id="adjusterid" name="adjusterid" class="chosen-select-deselect form-control" width="380">
                  <option value=""></option>
				  <?php
					$query = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc,"SELECT contactid, first_name, last_name FROM contacts WHERE category='Adjuster' AND deleted=0 AND `status`=1"),MYSQLI_ASSOC));
					foreach($query as $id) {
						$selected = '';
						$selected = $id == $adjusterid ? 'selected = "selected"' : '';
