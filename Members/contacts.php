...
<div class="container">
	<div class="row">
		...
		<h1 class="single-pad-bottom">
			...
			<?php echo $_GET['contacts_tabs'] == 1 ? 'Contacts Dashboard' : 'Members Dashboard' ?><?php
			if(config_visible_function($dbc, 'members') == 1) {
		...
		<form name="form_search" method="post" action="" class="form-inline" role="form">
			...
			<div class="tab-container"><?php
                ...
                if(str_replace(',', '', $tabs) == '' && FOLDER_NAME == 'members') {
					$tabs = 'Business,Members';
					...
					$subtabs = "Details,Summary,Member Details,Medical Details,Key Methodologies,Activities,Daily Log Notes";
					...
					if($config_count['numrows'] == 0) {
						$sql_config = mysqli_query($dbc, "INSERT INTO `field_config_contacts` (`tile_name`, `tab`, `subtab`, `accordion`, `contacts`, `order`, `contacts_dashboard`) VALUES
							('".FOLDER_NAME."', 'Members', NULL, NULL, NULL, NULL, 'Category,First Name,Last Name,Cell Phone,Home Phone,Email Address,Status'),
							('".FOLDER_NAME."', 'Members', 'Summary', 'Checklist', 'Category,Checklist', 1, NULL),
							('".FOLDER_NAME."', 'Members', 'Member Details', 'Members Profile', 'Category,First Name,Last Name,Preferred Name,Cell Phone,Home Phone,Email Address,FSCD Number,Client Support Documents,Date of Birth,Insurance Alberta Health Care,School,Hear About', 2, NULL),
							('".FOLDER_NAME."', 'Members', 'Member Details', 'Address', 'Category,Address,City,Province,Postal Code', 3, NULL),
							('".FOLDER_NAME."', 'Members', 'Member Details', 'Parental Guardian Family Contact Information', 'Category,Guardians First Name,Guardians Last Name,Guardians Relationship,Guardians Home Phone,Guardians Cell Phone,Guardians Work Phone,Guardians Email Address,Guardians Multiple,Guardians Siblings', 4, NULL),
							('".FOLDER_NAME."', 'Members', 'Member Details', 'Emergency Contact', 'Category,Emergency Contact First Name,Emergency Contact Last Name,Emergency Contact Contact Number,Emergency Contact Relationship,Emergency Contact Multiple', 5, NULL),
							('".FOLDER_NAME."', 'Members', 'Member Details', 'Membership', 'Category,Membership Status,Membership Level,Membership Since,Membership Renewal Date', 6, NULL),
							('".FOLDER_NAME."', 'Members', 'Member Details', 'Programs', 'Category,Programs Female Only Programs,Programs Male Only Programs,Programs LAAFS Program,Programs AAFS Program,Programs Over 16 Program,Programs Over 18 Program', 7, NULL),
							('".FOLDER_NAME."', 'Members', 'Member Details', 'Funding', 'Category,Funding FSCD,Funding FSCD Worker Name,Funding FSCD File ID,Funding FSCD Renewal Date,Funding PDD,Funding Support Documents', 8, NULL),
							('".FOLDER_NAME."', 'Members', 'Member Details', 'Transportation', 'Category,Transportation Mode of Transportation,Transportation Transit Access,Transportation Access Password,Transportation Drivers License,Transportation Support Documents', 9, NULL),
							('".FOLDER_NAME."', 'Members', 'Medical Details', 'Diagnosis', 'Category,Medical Details Diagnosis,Diagnosis Support Documents', 10, NULL),
							('".FOLDER_NAME."', 'Members', 'Medical Details', 'Allergies', 'Category,Medical Details Allergies,Allergies Support Documents', 11, NULL),
							('".FOLDER_NAME."', 'Members', 'Medical Details', 'Equipment', 'Category,Medical Details Equipment,Equipment Support Documents', 12, NULL),
							('".FOLDER_NAME."', 'Members', 'Medical Details', 'Medications', 'Category,Medical Details Medications', 13, NULL),
							('".FOLDER_NAME."', 'Members', 'Medical Details', 'Seizure', 'Category,Medical Details Seizure', 14, NULL),
							('".FOLDER_NAME."', 'Members', 'Medical Details', 'Support Documents', 'Category,Medical Details Support Documents', 15, NULL),
							('".FOLDER_NAME."', 'Members', 'Key Methodologies', 'Key Methodologies', 'Category,Client Key Methodologies Social Story', 16, NULL),
							('".FOLDER_NAME."', 'Members', 'Activities', 'Activities', 'Category,Client Activities Social Story', 17, NULL),
							('".FOLDER_NAME."', 'Members', 'Daily Log Notes', 'Daily Log Notes', 'Category,Client Daily Log Notes', 18, NULL),
				...
				foreach ($each_tab as $cat_tab) {
					...
					$display = check_subtab_persmission( $dbc, 'members', $level_url, $subtab );
					...
					} elseif ( $cat_tab == 'Members' ) {
						$info_contact_type = 'These are all of your members.';
					} else {
					...
					$from_contacts = '';
					if ($_GET['contacts_tabs'] == 1) {
						$from_contacts = WEBSITE_URL.'/Contacts/';
					}
					...
					if (strpos(','.ROLE.',',',super,') !== false) {
						...
						$subtabs_nav .= "<a href='".$from_contacts."contacts.php?category=" . $cat_tab . "&filter=Top'><button type='button' class='btn brand-btn mobile-100 mobile-block ". $active_daily ."' >". $cat_label ."</button></a>";
				...
				// Other Tile tabs
				if ($_GET['contacts_tabs'] == 1) {
					$get_tabs_tiles_config = mysqli_fetch_array(mysqli_query($dbc,"SELECT `value` FROM `general_configuration` WHERE  `name` = 'contacts_tabs_tiles'"))['value'];
			...
			<?php
				if ( $category == 'Business' ) {
					$info_add_contact = 'Click to add a '.(get_software_name() == 'breakthebarrier' ? 'Program/Site' : ($rookconnect == 'highland' ? 'Customer' : 'Business')).'; you must do this before attaching multiple members.';
					...
					$info_add_contact = 'Click here to add a Member.';
				} else {
			...
			if ( vuaed_visible_function ( $dbc, 'members' ) == 1 ) { ?>
