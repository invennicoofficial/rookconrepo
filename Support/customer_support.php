...
$(document).ready(function() {
	$(window).resize(function() {
		...
		if($('.main-screen .standard-body-title').not('.show-on-mob .standard-body-title').is(':visible')) {
		...
		<div class="main-screen">
			<div class="tile-header standard-header" style="<?= IFRAME_PAGE ? 'display:none;' : '' ?>">
                ...
                <div class="scale-to-fill">
					<h1><a href="?">Customer Support: <?php switch($current_tab) {
						...
						case 'services' : echo "Services Dashboard"; break;
			...
			<div class="standard-collapsible tile-sidebar set-section-height hide-titles-mob">
				<ul class="sidebar">
					...
					<?php } else { ?>
						<a href='?tab=services'><li <?= $current_tab == 'services' ? 'class="active"' : '' ?>>Services</li></a>
						...
						<a href='?tab=requests&type=feedback'><li <?= $current_tab == 'requests' && $request_tab == 'new' ? 'class="active"' : '' ?>>Feedback & Ideas<span class="pull-right"><?= $count_row['count'] ?></span></li></a>
	            ...
	            <div class="scale-to-fill has-main-screen tile-content hide-titles-mob">
					...
					<?php } else { ?>
						<div id="no-more-tables">
							<div class="col-sm-12"><?php include($current_tab.'.php'); ?></div>
						</div>
					<?php } ?>
				</div>
			</div>
