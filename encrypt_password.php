...
while($col = mysqli_fetch_array($select_temp_data)) {
	...
	elseif($col['status'] == 'Active') {
		$status = 1;
	}


	$query_update_contacts = "UPDATE `contacts` SET name = '$name', first_name = '$first_name', last_name = '$last_name', password = '$password', office_phone = '$office_phone', cell_phone = '$cell_phone', home_phone = '$home_phone', email_address = '$email_address', business_street = '$business_street', business_city = '$business_city', business_state = '$business_state', business_country = '$business_country', business_zip = '$business_zip', health_care_no = '$health_care_no' , status = '$status' WHERE contactid = $contactid";
