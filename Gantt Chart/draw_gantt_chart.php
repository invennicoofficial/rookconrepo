...
<div class="container">
	...
	<div class="row hide_on_iframe">
		<div class="gap-left tab-container mobile-100-container padded">
			<?php if ( check_subtab_persmission($dbc, 'gantt_chart', ROLE, 'estimated') === TRUE ) { ?>
				<a href="estimated_gantt_chart.php"><button type="button" class="btn brand-btn mobile-block mobile-100">Estimated</button></a>&nbsp;&nbsp;
			<?php } else { ?>
			...
			<?php if ( check_subtab_persmission($dbc, 'gantt_chart', ROLE, 'chart') === TRUE ) { ?>
				<a href="draw_gantt_chart.php"><button type="button" class="btn brand-btn mobile-block active_tab mobile-100">Gantt Chart</button></a>&nbsp;&nbsp;
			<?php } else { ?>
					...
					if($row_ticket['to_do_date'] != '0000-00-00') {
						...
						$data[] = array(
						  //'label' => '#'.$row_ticket['ticketid'].' - '.str_replace("<br>",", ",$contactid),
							...
							'label' => '<a id="'.$row_ticket['ticketid'].'" href="'.WEBSITE_URL.'/Ticket/index.php?edit='.$row_ticket['ticketid'].'&from='.urlencode(WEBSITE_URL.$_SERVER['REQUEST_URI']).'" >#'.$row_ticket['ticketid'].'</a>',
