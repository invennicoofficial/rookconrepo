<form name="form_sites" method="post" action="" class="form-inline" role="form">
	<?php
		if(vuaed_visible_function($dbc, 'interactive_calendar') == 1) {
			echo '<a href="add_interactive_calendar.php?view=weekly" class="btn brand-btn mobile-block gap-bottom pull-right double-gap-top">Add Activity</a>';
		}
	?>
	 <?php
		$search_month = date('F');
...
function draw_calendar($dbc, $month,$year,$search_user,$search_client,$search_ticket,$search_status,$status_array){
	...
	for($list_day = 1; $list_day <= $days_in_month; $list_day++):
        ...
        $new_today_date = date_format(date_create_from_format('j-n-Y', $today_date), 'Y-m-d');
            ...
            $calendar.= '<div class="btn brand-btn ui-disabled pull-right '.$class.'">'.$list_day.'</div><br>';
