<script>
function send_invoice(address, invoice) {
	var message = $('[name=body]').val();
	var sender = $('[name=sender]').val();
	var subject = $('[name=subject]').val();
	$.ajax({
		method: "POST",
		url: "<?php echo WEBSITE_URL; ?>/Project Billing/billing_ajax.php?fill=send_email",
		data: { email: address, file: invoice, subject: subject, body: message, from: sender },
...
<div class="container">
	<div class="row">
		...
		if(empty($_GET['projectid'])) {
			$sql = "SELECT *, `project_invoice`.`status` `invoice_status` FROM `project_invoice` LEFT JOIN `project` ON `project_invoice`.`projectid`=`project`.`projectid` WHERE `project_invoice`.`deleted`=0";
			$query = "SELECT COUNT(*) `numrows` FROM `project_invoice` LEFT JOIN `project` ON `project_invoice`.`projectid`=`project`.`projectid` WHERE `project_invoice`.`deleted`=0";
		} else {
			$sql = "SELECT *, `project_invoice`.`status` `invoice_status` FROM `project_invoice` LEFT JOIN `project` ON `project_invoice`.`projectid`=`project`.`projectid` WHERE `projectid`='".$_GET['projectid']."' AND `project_invoice`.`deleted`=0";
			$query = "SELECT COUNT(*) `numrows` FROM `project_invoice` LEFT JOIN `project` ON `project_invoice`.`projectid`=`project`.`projectid` WHERE `projectid`='".$_GET['projectid']."' AND `project_invoice`.`deleted`=0";
		}
		...
		if($num_rows > 0) {
			...
			while($row = mysqli_fetch_array($result)) {
				...
				echo '<td data-title="Business">'.get_contact($dbc, $businessid, 'name').'</td>
					...
					<td data-title="Status">'.$row['invoice_status'].'</td>
					</tr>';
			} ?>
			</table>
			<div class="form-group">
				...
				<div class="col-sm-8">
					<input type="text" name="sender" class="form-control" value="<?php echo get_email($dbc, $_SESSION['contactid']); ?>">
				</div>
			</div>
			<div class="form-group">
