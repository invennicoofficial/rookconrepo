<?php
function swot_pdf($dbc,$infogatheringid, $fieldlevelriskid) {
	...
	$get_field_level = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM info_swot WHERE fieldlevelriskid='$fieldlevelriskid'"));
	...
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Problem/Issue</td></tr></table><br>'.html_entity_decode($business_1);
    }

    if (strpos($form_config, ','."fields2".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Strengths</td></tr></table><br>'.html_entity_decode($business_2);
    }

    if (strpos($form_config, ','."fields3".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Weaknesses</td></tr></table><br>'.html_entity_decode($business_3);
    }

    if (strpos($form_config, ','."fields4".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Opportunities</td></tr></table><br>'.html_entity_decode($business_4);
    }

    if (strpos($form_config, ','."fields5".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Next Steps/Action Plan</td></tr></table><br>'.html_entity_decode($business_5);
    }

    $pdf->writeHTML($html_weekly, true, false, true, false, '');

    $pdf->Output('swot/download/infogathering_'.$fieldlevelriskid.'.pdf', 'F');
