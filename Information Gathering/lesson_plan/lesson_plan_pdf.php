<?php
function lesson_plan_pdf($dbc,$infogatheringid, $fieldlevelriskid) {
	...
	$get_field_level = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM info_lesson_plan WHERE fieldlevelriskid='$fieldlevelriskid'"));
	...
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Topic/Main Idea</td></tr></table><br>'.html_entity_decode($business_1);
    }

    if (strpos($form_config, ','."fields2".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Objectives</td></tr></table><br>'.html_entity_decode($business_2);
    }

    if (strpos($form_config, ','."fields3".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Standards</td></tr></table><br>'.html_entity_decode($business_3);
    }

    if (strpos($form_config, ','."fields4".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Materials</td></tr></table><br>'.html_entity_decode($business_4);
    }

    if (strpos($form_config, ','."fields5".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Activity</td></tr></table><br>'.html_entity_decode($business_5);
    }

    if (strpos($form_config, ','."fields6".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Assessment</td></tr></table><br>'.html_entity_decode($business_6);
    }

    if (strpos($form_config, ','."fields7".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Next Steps</td></tr></table><br>'.html_entity_decode($business_7);
    }

    $pdf->writeHTML($html_weekly, true, false, true, false, '');

    $pdf->Output('lesson_plan/download/infogathering_'.$fieldlevelriskid.'.pdf', 'F');
