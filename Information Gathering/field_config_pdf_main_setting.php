...
?>
<?php
	$select_pdf_settings = mysqli_fetch_assoc(mysqli_query($dbc, "select count(pdfsettingid) as setting_count from infogathering_pdf_setting where style = '$style_settings' and setting_type='main'"));
	if(isset($_POST['pdf_setting'])) {
		...
		if($select_pdf_settings['setting_count'] <= 0) {	
			...
			$select_pdf = mysqli_fetch_assoc(mysqli_query($dbc, "select pdf_logo from infogathering_pdf_setting where style = '$style_settings' and setting_type='main'"));
			if($pdf_logo == '')
				$pdf_logo = $select_pdf['pdf_logo'];
			$update_pdf_settings = "UPDATE infogathering_pdf_setting SET `font_size`='$font_main_heading_size',`font_type`='$font_main_heading_type',`font`='$font_main_heading',`font_body_size`='$font_main_body_size',`font_body_type`='$font_main_body_type',`font_body`='$font_main_body',`pdf_color`='$pdf_heading_color',`pdf_body_color`='$pdf_body_color' where style = '$style_settings' and setting_type='main'";
			$result_pdf_settings = mysqli_query($dbc, $update_pdf_settings);
		}
		
		if($pdf_logo != '') {
			move_uploaded_file($_FILES["pdf_logo"]["tmp_name"],	"download/" . $pdf_logo);
			$update_pdf_settings = "UPDATE infogathering_pdf_setting SET `pdf_logo`='$pdf_logo' where style = '$style_settings' and setting_type='main'";
			$result_pdf_settings = mysqli_query($dbc, $update_pdf_settings);
		}
	}
	...
	if($select_pdf_settings['setting_count'] > 0) {
		$select_pdf_settings = mysqli_fetch_assoc(mysqli_query($dbc, "select * from infogathering_pdf_setting where style = '$style_settings' and setting_type='main'"));
