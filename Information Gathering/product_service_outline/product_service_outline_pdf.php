<?php
function product_service_outline_pdf($dbc,$infogatheringid, $fieldlevelriskid) {
	...
	$get_field_level = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM info_product_service_outline WHERE fieldlevelriskid='$fieldlevelriskid'"));
	...
	$html_weekly .= '<h2>Product-Service Outline</h2>';
    ...
    $html_weekly .= '</tr>';
    $html_weekly .= '<tr nobr="true"><td>'.$today_date.'</td>';
    if(strpos($form_config, ',Project,')) {
	...
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>What makes your company different?</td></tr></table><br>'.html_entity_decode($business_1);
    }

    if (strpos($form_config, ','."fields2".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>What makes you or your business valuable to your customers?</td></tr></table><br>'.html_entity_decode($business_2);
    }

    if (strpos($form_config, ','."fields3".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Who are your customers?</td></tr></table><br>'.html_entity_decode($business_3);
    }

    if (strpos($form_config, ','."fields4".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Top things your customers should say when they use your products or services</td></tr></table><br>'.html_entity_decode($business_4);
    }

    if (strpos($form_config, ','."fields5".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>What\'s your ongoing support plan?</td></tr></table><br>'.html_entity_decode($business_5);
    }

    if (strpos($form_config, ','."fields6".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>What calls to action have worked?</td></tr></table><br>'.html_entity_decode($business_6);
    }

    $pdf->writeHTML($html_weekly, true, false, true, false, '');

    $pdf->Output('product_service_outline/download/infogathering_'.$fieldlevelriskid.'.pdf', 'F');
