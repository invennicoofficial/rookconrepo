<?php
function marketing_plan_information_gathering_pdf($dbc,$infogatheringid, $fieldlevelriskid) {
	...
	$get_field_level = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM info_marketing_plan_information_gathering WHERE fieldlevelriskid='$fieldlevelriskid'"));
	...
	$html_weekly .= '<h2>Marketing Plan Information Gathering</h2>';
	...
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Targets</td></tr></table><br>'.html_entity_decode($business_2);
    }

    if (strpos($form_config, ','."fields3".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Audience</td></tr></table><br>'.html_entity_decode($business_3);
    }

    if (strpos($form_config, ','."fields4".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Current Plan</td></tr></table><br>'.html_entity_decode($business_4);
    }

    if (strpos($form_config, ','."fields5".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Strategy</td></tr></table><br>'.html_entity_decode($business_5);
    }

    if (strpos($form_config, ','."fields6".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Desired Outcome</td></tr></table><br>'.html_entity_decode($business_6);
    }

    $pdf->writeHTML($html_weekly, true, false, true, false, '');

    $pdf->Output('marketing_plan_information_gathering/download/infogathering_'.$fieldlevelriskid.'.pdf', 'F');
