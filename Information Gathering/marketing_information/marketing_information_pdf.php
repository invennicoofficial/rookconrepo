<?php
function marketing_information_pdf($dbc,$infogatheringid, $fieldlevelriskid) {
	...
	$get_field_level = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM info_marketing_information WHERE fieldlevelriskid='$fieldlevelriskid'"));
	...
	$html_weekly .= '<h2>Tactical Marketing</h2>';
	...
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Objectives</td></tr></table><br>'.html_entity_decode($business_1);
    }

    if (strpos($form_config, ','."fields2".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Product/Service Description</td></tr></table><br>'.html_entity_decode($business_2);
    }

    if (strpos($form_config, ','."fields3".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Customers/Clients</td></tr></table><br>'.html_entity_decode($business_3);
    }

    if (strpos($form_config, ','."fields4".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Competitors</td></tr></table><br>'.html_entity_decode($business_4);
    }

    if (strpos($form_config, ','."fields5".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Positioning</td></tr></table><br>'.html_entity_decode($business_5);
    }

    if (strpos($form_config, ','."fields6".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Pricing</td></tr></table><br>'.html_entity_decode($business_6);
    }

    if (strpos($form_config, ','."fields7".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Sales & Support</td></tr></table><br>'.html_entity_decode($business_7);
    }
    if (strpos($form_config, ','."fields8".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Promotion</td></tr></table><br>'.html_entity_decode($business_8);
    }
    if (strpos($form_config, ','."fields9".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Budget</td></tr></table><br>'.html_entity_decode($business_9);
    }
    if (strpos($form_config, ','."fields10".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Action Plan</td></tr></table><br>'.html_entity_decode($business_10);
    }

    $pdf->writeHTML($html_weekly, true, false, true, false, '');

    $pdf->Output('marketing_information/download/infogathering_'.$fieldlevelriskid.'.pdf', 'F');
