<?php
function client_reviews_pdf($dbc,$infogatheringid, $fieldlevelriskid) {
	...
	$get_field_level = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM info_client_reviews WHERE fieldlevelriskid='$fieldlevelriskid'"));
	...
	$html_weekly .= '<h2>Client Reviews</h2>';
	...
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>What\'s working</td></tr></table><br>'.html_entity_decode($business_1);
    }

    if (strpos($form_config, ','."fields2".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>What\'s not working</td></tr></table><br>'.html_entity_decode($business_2);
    }

    if (strpos($form_config, ','."fields3".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>What do we need to do better</td></tr></table><br>'.html_entity_decode($business_3);
    }

    if (strpos($form_config, ','."fields4".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>What\'s the Next Steps</td></tr></table><br>'.html_entity_decode($business_4);
    }

    if (strpos($form_config, ','."fields5".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Next Review Date</td></tr></table><br>'.html_entity_decode($business_5);
    }

    if (strpos($form_config, ','."fields5".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Reminder Date</td></tr></table><br>'.html_entity_decode($business_6);
    }

    $pdf->writeHTML($html_weekly, true, false, true, false, '');

    $pdf->Output('client_reviews/download/infogathering_'.$fieldlevelriskid.'.pdf', 'F');
