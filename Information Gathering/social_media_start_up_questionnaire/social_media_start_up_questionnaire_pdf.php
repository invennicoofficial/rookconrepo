<?php
function social_media_start_up_questionnaire_pdf($dbc,$infogatheringid, $fieldlevelriskid) {
	...
	$get_field_level = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM info_social_media_start_up_questionnaire WHERE fieldlevelriskid='$fieldlevelriskid'"));
	...
	$html_weekly .= '<h2>Social Media Start Up Questionnaire</h2>';
	...
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Describe your business in one sentence.</td></tr></table><br>'.html_entity_decode($business_1);
    }

    if (strpos($form_config, ','."fields2".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Who are your main competitors, and what do they do well?</td></tr></table><br>'.html_entity_decode($business_2);
    }

    if (strpos($form_config, ','."fields3".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>What separates you from your competitors? What do you do well by comparison?</td></tr></table><br>'.html_entity_decode($business_3);
    }

    if (strpos($form_config, ','."fields4".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>What kind of tone would you like to convey with your social media?</td></tr></table><br>'.html_entity_decode($business_4);
    }

    if (strpos($form_config, ','."fields5".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>What are 3-5 of the core values of your business? (e.g. innovation, humour, creativity).</td></tr></table><br>'.html_entity_decode($business_5);
    }

    if (strpos($form_config, ','."fields6".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>If you were to come up with 3 hashtags for your business, what might they be? Hashtags are keywords or word combinations used to label and search conversations in social media #yyc,#calgary,#yycweather, #calgaryweather, #yycstorm and #calgarystorm are all ways someone might find recent posts in social media about a current storm in Calgary.</td></tr></table><br>'.html_entity_decode($business_6);
    }

	$html_weekly .='<h3>Client</h3>';

	if (strpos($form_config, ','."fields7".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Who is your ideal customer? What do they typically come to your business for?</td></tr></table><br>'.html_entity_decode($client_1);
    }

    if (strpos($form_config, ','."fields8".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>What kind of topics and sources of information might your clients be interested in? What topics might be relevant to your clients?</td></tr></table><br>'.html_entity_decode($client_2);
    }

	$html_weekly .='<h3>Customer Service</h3>';

    if (strpos($form_config, ','."fields9".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Do you have a specific project/product you would want promoted in social media? Please provide a brief description of the project/product.</td></tr></table><br>'.html_entity_decode($customer_service_1);
    }

    if (strpos($form_config, ','."fields10".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>What type of content would you prefer to see on your social media (e.g. articles, images, text).</td></tr></table><br>'.html_entity_decode($customer_service_2);
    }

	if (strpos($form_config, ','."fields11".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Do you want to use social media to provide customer service (i.e. what should we do while acting on your behalf if someone has a question or concern? Alert you, reply and gather initial info and forward to you? Who should this information go to?)</td></tr></table><br>'.html_entity_decode($customer_service_3);
    }

	$html_weekly .='<h3>Social Media</h3>';

	if (strpos($form_config, ','."fields12".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Facebook</td></tr></table><br>'.html_entity_decode($social_1);
    }

	if (strpos($form_config, ','."fields13".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Twitter</td></tr></table><br>'.html_entity_decode($social_2);
    }

	if (strpos($form_config, ','."fields14".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>LinkedIn</td></tr></table><br>'.html_entity_decode($social_3);
    }

    $pdf->writeHTML($html_weekly, true, false, true, false, '');

    $pdf->Output('social_media_start_up_questionnaire/download/infogathering_'.$fieldlevelriskid.'.pdf', 'F');
