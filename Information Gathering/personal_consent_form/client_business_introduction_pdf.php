...
function client_business_introduction_pdf($dbc,$infogatheringid, $fieldlevelriskid) {
	...
	$html_weekly .= '<h2>Client Business Introduction</h2>';

	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Date:</td></tr></table><br>'.$today_date;

    if (strpos($form_config, ','."fields2".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Business Name:</td></tr></table><br>'.html_entity_decode($business_name);
    }

    if (strpos($form_config, ','."fields3".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Business Services:</td></tr></table><br>'.html_entity_decode($business_services);
    }

    if (strpos($form_config, ','."fields4".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Business Products:</td></tr></table><br>'.html_entity_decode($business_products);
    }

    if (strpos($form_config, ','."fields5".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Business Vision:</td></tr></table><br>'.html_entity_decode($business_vision);
    }

    if (strpos($form_config, ','."fields6".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Business Goals:</td></tr></table><br>'.html_entity_decode($business_goals);
    }

    if (strpos($form_config, ','."fields7".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Target Markets:</td></tr></table><br>'.html_entity_decode($target_markets);
    }

    if (strpos($form_config, ','."fields8".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Competitors:</td></tr></table><br>'.html_entity_decode($competitors);
    }

    if (strpos($form_config, ','."fields9".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Current Areas of Concern:</td></tr></table><br>'.html_entity_decode($current_areas_of_concern);
    }

    if (strpos($form_config, ','."fields10".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Estimated Project Timeline & Budget:</td></tr></table><br>'.html_entity_decode($estimated_project_timeline_budget);
    }

    if (strpos($form_config, ','."fields11".',') !== FALSE) {
	$html_weekly .= '<br><br><table><tr style="color:'.$active_color.'"><td>Communication Expectations & Methods:</td></tr></table><br>'.html_entity_decode($communication_expectations_methods);
    }
