<?php $accordions = explode('#*#',mysqli_fetch_array(mysqli_query($dbc,"SELECT `custom_accordions` FROM `field_config_cost_estimate`"))['custom_accordions']);
				...
				<div class="panel-body">
					...
					function count<?php echo $id; ?>(txb) {
						if(txb != 'delete') {
							...
							document.getElementById('<?php echo $id; ?>estimatetotal_'+split_id[1]).value = parseFloat(round2Fixed($('#<?php echo $id; ?>estimateprice_'+split_id[1]).val() * lbqty));
						}
					...
					<div class="form-group">
						<div class="col-sm-12">
							<?php $rc = 0;
								foreach(explode(',',$companyrcid) as $cri => $crid) {
									...
									foreach($field_order as $field_data):
										...
										switch($data[0]) {
											...
											case 'Total':
												...
												break;
											case 'Total Multiple':
												echo '<label class="col-sm-2 text-center" data-columns="'.$columns.'" data-width="1" data-base-title="'.$data[1].'">'.str_replace('[#]',1,$data[1]).'</label>';
												break;
										}
									endforeach; ?>
								</div>
							...
							<?php foreach(explode(',',$companyrcid) as $cri => $crid) {
								...
								$no_space_rate_card_types = str_replace('&', '', $no_space_rate_card_types);
								$no_space_rate_card_types = str_replace('-', '', $no_space_rate_card_types);
								$query_rc = mysqli_query($dbc,"SELECT * FROM company_rate_card WHERE rate_card_name IN ('$crname','') AND ',$company_rate_categories,' LIKE CONCAT('%,',IFNULL(`rate_categories`,''),',%') AND tile_name='$id' AND (',$crtype,' LIKE CONCAT('%,',`rate_card_types`,',%') OR (rate_card_name='' AND `rate_card_types`='')) AND `deleted`=0 ORDER BY IF(`sort_order`>0,`sort_order`,100)");
								...
								while($row_rc = mysqli_fetch_array($query_rc)) {
									...
									$estimate_company_rate_card = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM cost_estimate_company_rate_card WHERE companyrcid='$row_companyrcid' AND estimateid='$estimateid' AND `type_id` IN ('','$no_space_rate_card_types')"));
									...
									$crc_multiple = $estimate_company_rate_card['total_multiple'];
									...
									<div class="form-group clearfix all_<?php echo $id; ?> <?php echo $no_space_rate_card_types.$id; ?> rc_est_<?php echo $id; ?>_<?php echo $rc; ?>" width="100%">
										...
										<input type="hidden" name="crc_<?php echo $id; ?>_type_id_<?php echo $rc; ?>" value="<?= $no_space_rate_card_types ?>" />
										<input type="hidden" name="crc_<?php echo $id; ?>_type_label_<?php echo $rc; ?>" value="<?= $crlabel ?>" />
										<?php $columns = count($field_order) + 1;
										foreach($field_order as $field_data):
											$data = explode('***',$field_data);
											if($data[1] == '') {
											...
											switch($data[0]) {
												...
												case 'Total':
													...
													break;
												case 'Total Multiple':
													echo '<label class="col-sm-2 show-on-mob" data-columns="'.$columns.'" data-width="1" data-base-title="'.$data[1].'">'.str_replace('[#]',1,$data[1]).'</label>';
													break;
											}
											switch($data[0]) {
												...
												case 'Total Multiple': echo '<input name="crc_'.$id.'_total_multiple_'.$rc.'" value= "'.$crc_multiple.'"  id="crc_'.$id.'_total_multiple_'.$rc.'" type="text" class="form-control" data-columns="'.$columns.'" data-width="1" />'; break;
											}
											echo "</div>";
										endforeach; ?>
										...
										<?php if(!in_array_starts('Total Multiple', $field_order)): ?>
											<input name="crc_<?php echo $id; ?>_total_multiple_<?php echo $rc; ?>" value="<?php echo $crc_multiple; ?>"  id="crc_<?php echo $id; ?>_total_multiple_<?php echo $rc;?>" type="text" style="display:none;" />
										<?php endif; ?>
									</div>
							...
							<div class="form-group clearfix" style="margin-left:5px">
								...
								<div class="form-group clearfix additional_<?= $id ?>_misc" id="<?php echo $id; ?>misc_0">
									<?php foreach($field_order as $field_data):
										...
										switch($data[0]) {
											...
											case 'Total Multiple': echo '<input name="'.$id.'totalmiscmulti[]" id="'.$id.'totalmiscmulti_0" type="text" class="form-control" />'; break;
										}
										echo "</div>";
									endforeach; ?>
									...
									<?php endif; ?>
									<?php if(!in_array_starts('Total Multiple', $field_order)): ?>
										<input name="<?php echo $id; ?>totalmiscmulti[]" id="<?php echo $id; ?>totalmiscmulti_0" type="text" class="form-control" style="display:none;" />
									<?php endif; ?>
									
									<div class="col-sm-1" >
										<a href="#" onclick="deleteEstimate(this,'<?php echo $id; ?>misc_','lheadmisc_'); return false;" id="delete<?php echo $id; ?>misc_0" class="btn brand-btn">Delete</a>
									</div>
								</div>
								...
								<br>
								<?php
								$query_misc_rc = mysqli_query($dbc,"SELECT * FROM cost_estimate_misc WHERE REPLACE(accordion,' ','')='$id' AND `type_id` IN ('','$no_space_rate_card_types') AND estimateid=" . $_GET['estimateid']);
								//exit;
									...
									<div class="form-group clearfix">
										<?php foreach($field_order as $field_data):
											...
											switch($data[0]) {
												...
												case 'Total Multiple': echo '<input name="'.$id.'totalmiscmulti_display[]" id="'.$id.'totalmiscmulti" value="'.$misc_row_rc['total_multiple'].'" readonly type="text" class="form-control" />'; break;
											}
											echo "</div>";
										endforeach; ?>
										...
										<?php endif; ?>
										<?php if(!in_array_starts('Total', $field_order)): ?>
											<input name="<?php echo $id; ?>totalmisc_display[]" id="<?php echo $id; ?>totalmisc" value="<?php echo $misc_row_rc['total_multiple'] ?>" readonly type="text" class="form-control" style="display:none;" />
										<?php endif; ?>
										<?php if(!in_array_starts('TotalMultiple', $field_order)): ?>
											<input name="<?php echo $id; ?>totalmiscmulti_display[]" id="<?php echo $id; ?>totalmiscmulti" value="<?php echo $misc_row_rc['total_multiple'] ?>" readonly type="text" class="form-control" style="display:none;" />
										<?php endif; ?>
									</div>
									<?php
									$misc_rc++;
