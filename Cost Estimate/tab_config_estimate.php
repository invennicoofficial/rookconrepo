...
<?php if($_POST['submit_tab']): ?>
	...
	$tab = rtrim($tab, ",");
	$get_current_tabs = mysqli_fetch_all(mysqli_query($dbc,"DELETE FROM `cost_estimate_tab` WHERE `estimate_tab` NOT IN (" . $tab . ")"));
	foreach($tabNames as $tabName) {
		$get_tabs = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT `estimate_tab` FROM `cost_estimate_tab` WHERE `estimate_tab` = '" . $tabName . "'"));
		if(empty($get_tabs)) {
			$query_insert_tab = "INSERT INTO `cost_estimate_tab` (`estimate_tab`) VALUES ('" . $tabName . "')";
...
<?php endif; ?>
<div class="pad-left">
	<a href='?tab=add'><button type="button" class="btn brand-btn mobile-block <?php echo $_GET['tab'] == 'add' ? 'active_tab' : ''; ?>" >Add Cost Estimate Tabs</button></a>
	<?php $get_tabs = mysqli_query($dbc,"SELECT estimate_tab, estimate_tab_id FROM cost_estimate_tab");
