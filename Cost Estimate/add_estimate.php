...
$mandatory_details_quote_config = explode(',',get_config($dbc, 'mandatory_details_quote_config'));
$base_field_config_all = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM field_config_cost_estimate WHERE `fieldconfigestimateid` = 1"));
$base_field_config = ','.$base_field_config_all['config_fields'].',';
$pdf_first_header = $base_field_config_all['first_header'];
$pdf_first_logo = $base_field_config_all['first_logo'];
...
if (isset($_POST['save'])) {
    ...
    for($i = 0; $i < count($_FILES['document']['name']); $i++) {
        if($document[$i] != '') {
            ...
            $query_insert_upload = "INSERT INTO `cost_estimate_document` (`estimateid`, `upload`) VALUES ('$estimateid', '$document[$i]')";
...
if (isset($_POST['submit'])) {
    ...
    $businessid = get_cost_estimate($dbc, $estimateid, 'businessid');
    $clientid = get_cost_estimate($dbc, $estimateid, 'clientid');
    $siteid = $_POST['siteid'];
    $afe_number = filter_var($_POST['afe_number'],FILTER_SANITIZE_STRING);
    $payment_terms = filter_var($_POST['payment_terms'],FILTER_SANITIZE_STRING);
    $payment_due = filter_var($_POST['payment_due'],FILTER_SANITIZE_STRING);
    ...
    $get_field_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT config_fields_quote FROM field_config_cost_estimate"));
    $config_fields_quote = ','.$get_field_config['config_fields_quote'].',';

    $temp_ticket = mysqli_query($dbc, "DELETE FROM temp_ticket WHERE costestimateid='$estimateid'");
	$total_multi = false;
	$total_multiple = 0;
	$accordion_mandatory = [];
    ...
    foreach ($_POST['packageid'] as $packageid_all) {
        if($packageid_all != '') {
            ...
            $package .= $packageid_all.'#'.$_POST['packageestimateprice'][$j].'#'.$_POST['packageestimatepricemulti'][$j].'**';
            $total_price += $_POST['packageestimateprice'][$j];
            $total_package += $_POST['packageestimateprice'][$j];
            $total_multiple += $_POST['packageestimatepricemulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$package_html .= '<td>$'.number_format((float)$_POST['packageestimatepricemulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Package', '$packageid_all', '$st', '$temp_ticket_desc')";
    ...
    foreach ($_POST['promotionid'] as $promotionid_all) {
        if($promotionid_all != '') {
            ...
            $promotion .= $promotionid_all.'#'.$_POST['promotionestimateprice'][$j].'#'.$_POST['promotionestimatepricemulti'][$j].'**';
            $total_price += $_POST['promotionestimateprice'][$j];
            $total_multiple += $_POST['promotionestimatepricemulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$promotion_html .= '<td>$'.number_format((float)$_POST['promotionestimatepricemulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $st = $query['service_type'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Promotion', '$promotionid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['customid'] as $customid_all) {
        if($customid_all != '') {
            ...
            $custom .= $customid_all.'#'.$_POST['customestimateprice'][$j].'#'.$_POST['customestimatepricemulti'][$j].'**';
            $total_price += $_POST['customestimateprice'][$j];
            $total_multiple += $_POST['customestimatepricemulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$custom_html .= '<td>$'.number_format((float)$_POST['customestimatepricemulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $st = $query['service_type'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Custom', '$customid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['materialid'] as $materialid_all) {
        if($materialid_all != '') {
            ...
            $material .= $materialid_all.'#'.$_POST['mestimateprice'][$j].'#'.$_POST['mestimateqty'][$j].'#'.$_POST['mestimateunit'][$j].'#'.$_POST['mestimatetotal'][$j].'#'.$_POST['mestimatetotalmulti'][$j].'**';
            $total_price += $_POST['mestimateprice'][$j]*$_POST['mestimateqty'][$j];
            $total_multiple += $_POST['mestimatetotalmulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$m_html .= '<td>$'.number_format((float)$_POST['mestimatetotalmulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $st = $query['category'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Material', '$materialid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['serviceid'] as $serviceid_all) {
        if($serviceid_all != '') {
            ...
            $services .= $serviceid_all.'#'.$_POST['sestimateprice'][$j].'#'.$_POST['sestimateqty'][$j].'#'.$_POST['sestimateunit'][$j].'#'.$_POST['sestimatetotal'][$j].'#'.$_POST['sestimatetotalmulti'][$j].'**';
            $total_price += $_POST['sestimateprice'][$j]*$_POST['sestimateqty'][$j];
            $total_multiple += $_POST['sestimatetotalmulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$s_html .= '<td>$'.number_format((float)$_POST['sestimatetotalmulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Service', '$serviceid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['productid'] as $productid_all) {
        if($productid_all != '') {
            ...
            $products .= $productid_all.'#'.$_POST['pestimateprice'][$j].'#'.$_POST['pestimateqty'][$j].'#'.$_POST['pestimateunit'][$j].'#'.$_POST['peprofit'][$j].'#'.$_POST['peprofitmargin'][$j].'#'.$_POST['pestimatetotal'][$j].'#'.$_POST['pestimatetotalmulti'][$j].'**';
            $total_price += $_POST['pestimateprice'][$j]*$_POST['pestimateqty'][$j];
            $total_multiple += $_POST['pestimatetotalmulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$p_html .= '<td>$'.number_format((float)$_POST['pestimatetotalmulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `product_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Product', '$productid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['sredid'] as $sredid_all) {
        if($sredid_all != '') {
            ...
            $sred .= $sredid_all.'#'.$_POST['sredestimateprice'][$j].'#'.$_POST['sredestimateqty'][$j].'#'.$_POST['sredestimatetotal'][$j].'#'.$_POST['sredestimatetotalmulti'][$j].'**';
            $total_price += $_POST['sredestimateprice'][$j]*$_POST['sredestimateqty'][$j];
            $total_multiple += $_POST['sredestimatetotalmulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$sred_html .= '<td>$'.number_format((float)$_POST['sredestimatetotalmulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $st = $query['sred_type'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'SRED', '$sredid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['contactid'] as $contactid_all) {
        if($contactid_all != '') {
            ...
            $staff .= $contactid_all.'#'.$_POST['stestimateprice'][$j].'#'.$_POST['stestimateqty'][$j].'#'.$_POST['stestimateunit'][$j].'#'.$_POST['stestimatetotal'][$j].'#'.$_POST['stestimatetotalmulti'][$j].'**';
            $total_price += $_POST['stestimateprice'][$j]*$_POST['stestimateqty'][$j];
            $total_multiple += $_POST['stestimatetotalmulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$staff_html .= '<td>$'.number_format((float)$_POST['stestimatetotalmulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Staff', '$contactid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['contractorid'] as $contractorid_all) {
        if($contractorid_all != '') {
            ...
            $contractor .= $contractorid_all.'#'.$_POST['cntestimateprice'][$j].'#'.$_POST['cntestimateqty'][$j].'#'.$_POST['cntestimateunit'][$j].'#'.$_POST['cntestimatetotal'][$j].'#'.$_POST['cntestimatetotalmulti'][$j].'**';
            $total_price += $_POST['cntestimateprice'][$j]*$_POST['cntestimateqty'][$j];
            $total_multiple += $_POST['cntestimatetotalmulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$cont_html .= '<td>$'.number_format((float)$_POST['cntestimatetotalmulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $st = decryptIt($query['first_name']).' '.decryptIt($query['last_name']);
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Contractor', '$contractorid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['clientid'] as $clientid_all) {
        if($clientid_all != '') {
            $client .= $clientid_all.'#'.$_POST['clestimateprice'][$j].'#'.$_POST['clestimatepricemulti'][$j].'**';
            $total_price += $_POST['clestimateprice'][$j];
            $total_multiple += $_POST['clestimatepricemulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$c_html .= '<td>$'.number_format((float)$_POST['clestimatemulti'][$j] * 4, 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $st = decryptIt($query['first_name']).' '.decryptIt($query['last_name']);
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Client', '$clientid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['vestimateqty'] as $vendorperson_all) {
        if($vendorperson_all != '' && $vendorperson_all > 0  && $vendorperson_all !== NULL) {
            ...
            $total_multiple += $_POST['vestimatetotalmulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$v_html .= '<td>$'.number_format((float)$_POST['total_multi'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Vendor Price List', '$vendor_identification', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['customerid'] as $customerid_all) {
        if($customerid_all != '') {
            $customer .= $customerid_all.'#'.$_POST['custestimateprice'][$j].'#'.$_POST['custestimatepricemulti'][$j].'**';
            $total_price += $_POST['custestimateprice'][$j];
            $total_multiple += $_POST['custestimatepricemulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$cust_html .= '<td>$'.number_format((float)$_POST['custestimatemulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Customer', '$customerid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['inventoryid'] as $inventoryid_all) {
        if($inventoryid_all != '' && $_POST['inestimateqty'][$j] > 0) {
            $inventory .= $inventoryid_all.'#'.$_POST['inestimateprice'][$j].'#'.$_POST['inestimateqty'][$j].'#'.$_POST['inestimateunit'][$j].'#'.$_POST['inprofit'][$j].'#'.$_POST['inprofitmargin'][$j].'#'.$_POST['inestimatetotal'][$j].'#'.$_POST['inestimatetotalmulti'][$j].'**';
            $total_price += $_POST['inestimateprice'][$j]*$_POST['inestimateqty'][$j];
            $total_multiple += $_POST['inestimatetotalmulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$in_html .= '<td>$'.number_format((float)$_POST['inestimatetotalmulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $st = $query['category'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Inventory', '$inventoryid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['equipmentid'] as $equipmentid_all) {
        if($equipmentid_all != '') {
            $equipment .= $equipmentid_all.'#'.$_POST['eqestimateprice'][$j].'#'.$_POST['eqestimateqty'][$j].'#'.$_POST['eqestimateunit'][$j].'#'.$_POST['eqprofit'][$j].'#'.$_POST['eqprofitmargin'][$j].'#'.$_POST['eqestimatetotal'][$j].'#'.$_POST['eqestimatetotalmulti'][$j].'**';
            $total_price += $_POST['eqestimateprice'][$j]*$_POST['eqestimateqty'][$j];
            $total_multiple += $_POST['eqestimatetotalmulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$eq_html .= '<td>$'.number_format((float)$_POST['eqestimatetotalmulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $st = $query['category'];
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Equipment', '$equipmentid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['labourid'] as $labourid_all) {
        if($labourid_all != '') {
            $labour .= $labourid_all.'#'.$_POST['lestimateprice'][$j].'#'.$_POST['lestimateqty'][$j].'#'.$_POST['lestimateunit'][$j].'#'.$_POST['lprofit'][$j].'#'.$_POST['lprofitmargin'][$j].'#'.$_POST['lestimatetotal'][$j].'#'.$_POST['lestimatetotalmulti'][$j].'**';
            $total_price += $_POST['lestimateprice'][$j]*$_POST['lestimateqty'][$j];
            $total_multiple += $_POST['lestimatetotalmulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$l_html .= '<td>$'.number_format((float)$_POST['lestimatetotalmulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `itemid`, `service_type`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Labour', '$labourid_all', '$st', '$temp_ticket_desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['expensetype'] as $expensetype_all) {
        if($expensetype_all != '') {
            $expense .=     $expensetype_all.'#'.$_POST['expensecategory'][$j].'#'.$_POST['expestimateprice'][$j].'#'.$_POST['eestimatetotal'][$j].'#'.$_POST['eestimatetotalmulti'][$j].'**';
            $total_price += $_POST['expestimateprice'][$j];
            $total_multiple += $_POST['expestimatetotalmulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$ex_html .= '<td>$'.number_format((float)$_POST['expestimatemulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`,`desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Expenses', '$desc')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    foreach ($_POST['other_detail'] as $other_detail_all) {
        if($other_detail_all != '') {
            $other .=     $other_detail_all.'#'.$_POST['otherestimateprice'][$j].'#'.$_POST['otherestimatepricemulti'][$j].'**';
            $total_price += $_POST['otherestimateprice'][$j];
            $total_multiple += $_POST['otherestimatepricemulti'][$j];
			...
			foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total':
						...
						break;
					case 'Total Multiple':
						$other_html .= '<td>$'.number_format((float)$_POST['otherestimatemulti'][$j], 2, '.', '').'</td>';
						$total_multi = true;
						break;
				}
			}
            ...
            $query_insert_ticket = "INSERT INTO `temp_ticket` (`costestimateid`, `businessid`, `clientid`, `category`, `desc`) VALUES ('$estimateid', '$businessid', '$clientid', 'Other', '$other_detail_all')";
            $result_insert_ticket = mysqli_query($dbc, $query_insert_ticket);
        }
        $j++;
    }
    ...
    if($total_package != 0) {
		...
		foreach($field_order as $field_data) {
			...
			switch($data[0]) {
				...
				case 'Total Multiple': $category_totals .= '<td>'.number_format((float)$current_total_multi, 2, '.', '').'</td>'; $total_multi = true; break;
			}
		}
		$category_totals .= '</tr>';
    }
    ...
    if($total_inventory != 0) {
		...
		foreach($field_order as $field_data) {
			...
			switch($data[0]) {
				case 'Type': $category_totals .= '<td>Total for Inventory</td>'; break;
				case 'Description': $category_totals .= '<td></td>'; break;
				case 'UOM': $category_totals .= '<td></td>'; break;
				case 'Quantity': $category_totals .= '<td></td>'; break;
				case 'Price': $category_totals .= '<td></td>'; break;
	...
	// Details
	$detail_config = mysqli_fetch_array(mysqli_query($dbc,"SELECT `config_fields_quote` FROM `field_config_cost_estimate`"));
    $detail_detail = filter_var(htmlentities($_POST['detail_detail']),FILTER_SANITIZE_STRING);
    ...
    $html .= '<tr nobr="true">
            <td style="width:70%;">';
    $html .= get_client($dbc, $businessid);
	...
	if(strpos($base_field_config, ',AFE Number,') !== FALSE) {
		$html .= '<br />AFE #: '.($afe_number == '' ? '___________________________' : '<u> '.$afe_number.' </u>');
	}
    $html .= '</td></tr>';
    $html .= '</table><br><br>';
	...
	if($accordion_type_count > 0) {
		foreach($accordion_types as $type_key => $accordion_type_html) {
			$html_arr[$type_key] = $html.$accordion_type_html;
		}
	}
	$html .= $accordion_html;
	...
	foreach($field_order as $field_data) {
		...
		switch($data[0]) {
			case 'Type':
				$table_head .= '<th style="text-align:center; width:'.$width_factor.'%;">'.$data[1].'</th>';
				break;
			case 'Description':
				$table_head .= '<th style="text-align:center; width:'.($width_factor*5).'%;">'.$data[1].'</th>';
				break;
			case 'UOM':
				$table_head .= '<th style="text-align:center; width:'.$width_factor.'%;">'.$data[1].'</th>';
				break;
			...
			case 'Total':
				...
				break;
			case 'Total Multiple':
				$table_head .= '<th style="text-align:center; width:'.$width_factor.'%;">'.str_replace('[#]',$quote_multiple,$data[1]).'</th>';
				break;
		}
	}
	$table_head .= '</tr>';
	...
	$table_foot = '<tr><td colspan="'.($col_count-1-($total_multi ? 1 : 0)).'" border="0px" style="border-left:0px white hidden; border-bottom:0px white hidden;"><p style="text-align:right;">Sub Total</p></td><td>$'.number_format((float)$total_price - $accordion_total, 2, '.', '').'</td>'.($total_multi ? '<td>$'.number_format((float)$total_multiple - $accordion_total_multi, 2, '.', '').'</td>' : '').'</tr></table>';
    ...
    for($eq_loop=0; $eq_loop<=$total_count; $eq_loop++) {
        ...
        if($quote_tax_name_rate[1] != '0') {
            $html_close .= '<tr><td style="width:'.($total_multi ? '80' : '90').'%;"><p style="text-align:right;">'.$quote_tax_name_rate[0].'<br>';
            if($quote_tax_name_rate[2] != '') {
                $html_close .= '<em>['.$quote_tax_name_rate[2].']</em>';
            }
            $html_close .= '</p></td><td style="width:10%">'.$quote_tax_name_rate[1].'%</td>';
			if($total_multi) {
				$html_close .= '<td style="width:10%">'.$quote_tax_name_rate[1].'%</td>';
			}
			$html_close .= '</tr>';
        }
    }
	...
	foreach($html_arr as $key => $html_types) {
		...
		$type_multiple = $total_multiple - $accordion_total_multi;
		$type_total += $accordion_type_totals[$key];
		$type_multiple += $accordion_type_totals_multi[$key];
		$total_line = '<tr><td border="0px" style="border-left:0px white hidden; border-top:0px white hidden; border-bottom:0px white hidden; width:'.($total_multi ? '80' : '90').'%;">';
		$total_line .= '<p style="text-align:right;">Total</p></td><td style="width:10%">$'.number_format((float)$type_total, 2, '.', '').'</td>';
		$total_line .= ($total_multi ? '<td style="width:10%">$'.number_format((float)$type_multiple, 2, '.', '').'</td>' : '');
		$total_line .= '</tr></table>'.html_entity_decode(get_config($dbc, 'cost_estimate_sign_notes')).html_entity_decode($detail_note);
		if(in_array('Total',explode(',',$quote_mode))) {
			$html_arr[$key] = $quote_html.$accordion_mandatory[$key].$html_close.$total_line;
			$quote_arr[] = $quote_html.$accordion_mandatory[$key].$html_close.$total_line;
		} else if(in_array('Category',explode(',',$quote_mode))) {
			$html_arr[$key] = $quote_html.$accordion_mandatory[$key].'<table border="1px" style="padding:3px; border:1px solid black; width:100%;">'.$category_totals.'</table>'.$html_close.$total_line;
			$quote_arr[] = $quote_html.$accordion_mandatory[$key].'<table border="1px" style="padding:3px; border:1px solid black; width:100%;">'.$category_totals.'</table>'.$html_close.$total_line;
		} else {
			$html_arr[$key] = $html_types.$html_close.$total_line;
			$quote_arr[] = $html_types.$html_close.$total_line;
		}
	}
	
    $final = ($total_price*$tax_rate)/100;
    $final_total = ($total_price+$final);
	$final_multi = $total_multiple + ($total_multiple * $tax_rate / 100);

    $html_close .= '<tr><td border="0px" style="border-left:0px white hidden; border-top:0px white hidden; border-bottom:0px white hidden; width:'.($total_multi ? '80' : '90').'%;"><p style="text-align:right;">Total</p></td><td style="width:10%">$'.number_format((float)$final_total, 2, '.', '').'</td>'.($total_multi ? '<td style="width:10%">$'.number_format((float)$final_multi, 2, '.', '').'</td>' : '').'</tr>';

    $html_close .= '</table>'.html_entity_decode(get_config($dbc, 'cost_estimate_sign_notes')).html_entity_decode($detail_note);

    $html_1 = htmlentities($html.$html_close,ENT_QUOTES);
	$html_arr_1 = [];
	foreach($html_arr as $html_types) {
		$html_arr_1[] = $html_types;
	}
	$html_split = htmlentities(implode('**#**', $html_arr_1),ENT_QUOTES);
    ...
    $history = '';
    $get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(estimateid) AS total_id FROM cost_estimate WHERE `estimateid` = '$estimateid' AND status = 'Submitted'"));

    if($get_config['total_id'] == 1) {
        $get_estimate = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM cost_estimate WHERE `estimateid` = '$estimateid'"));
    ...
    if($t_comment != '') {
        ...
        if($type != '') {
            $query_insert_ca = "INSERT INTO `cost_estimate_comment` (`estimateid`, `comment`, `email_comment`, `created_date`, `created_by`, `type`, `note_heading`) VALUES ('$estimateid', '$t_comment', '$email_comment', '$created_date', '$created_by', '$type', '$note_heading')";
            $result_insert_ca = mysqli_query($dbc, $query_insert_ca);
        } else {
            $query_update_report = "UPDATE `cost_estimate_detail` SET `$note_heading` = CONCAT($note_heading,'$t_comment') WHERE `estimateid` = '$estimateid'";
            $result_update_report = mysqli_query($dbc, $query_update_report);
        }
    ...
    insert_day_overview($dbc, $who_added, 'Cost Estimate', $when_added, '', 'Edited Cost Estimate '.$estimate_name);
...
function deleteEstimate(sel, hide, blank) {
    ...
    if(estimateid == 0) {
        ...
        if(arr[0] == 'deletecustom') {
            alert('If you Delete a Custom item, then all data Related to this Custom item will be gone.');
...
function toggleMultiples(count, no_update) {
	$('label').filter(function() { return $(this).data('base-title') != undefined; }).each(function() {
		$(this).text($(this).data('base-title').replace('[#]',count));
	});
	if(no_update !== true) {
		$('[name*=multi]').not('[name=quote_multiple]').not('readonly').each(function() {
			$(this).val(round2Fixed($('[name="'+this.name.replace('_multiple','').replace('multi','')+'"]').val()*count));
		});
	}
}
</script>
...
$info_config = mysqli_fetch_array(mysqli_query($dbc, "SELECT `field_info` FROM `field_config_cost_estimate`"));
  ...
  <div class="row">
    ...
    <form id="form1" name="form1" method="post"	action="" enctype="multipart/form-data" class="form-horizontal" role="form">
		<div class="pad-left double-gap-bottom">
			...
			<?php foreach($field_order as $i => $value) {
				if(strpos($value, 'Total X 4') !== FALSE) {
					unset($field_order[$i]);
				}
			}
			$field_order = array_values($field_order);
			if($_GET['estimateid']):
				$get_field_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM field_config_cost_estimate"));
				$value_config = ','.$get_field_config['config_fields'].',';
				$get_tabs = mysqli_query($dbc,"SELECT * FROM `cost_estimate_tab` WHERE `estimate_tab` != ''");
        ...
        if(!empty($_GET['estimateid'])) {
            ...
            $get_contact = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM cost_estimate WHERE estimateid='$estimateid'"));
