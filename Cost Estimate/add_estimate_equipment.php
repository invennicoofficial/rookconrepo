...
function countEquipment(txb) {
    if(txb != 'delete') {
        ...
        document.getElementById('eqestimatetotal_'+split_id[1]).value = round2Fixed(parseFloat($('#eqestimateprice_'+split_id[1]).val() * lbqty));
    }
...
function changeEquipmentTotal() {
    ...
    jQuery('[name=equipment_summary_margin]').val(round2Fixed(per_profit_margin));
       ...
       <?php
        ...
        <?php if(!empty($get_equipment_field)) {
            ...
            <div class="form-group clearfix" id="<?php echo 'equipment_'.$id_loop; ?>" >
				...
				<?php if(in_array_starts('Total Multiple', $field_order)) { ?>
                <div class="col-sm-1" >
                    <input name="eqestimatetotalmulti[]" value="<?php echo $totalmulti; ?>" id="<?php echo 'eqestimatetotalmulti_'.$id_loop; ?>" type="text" class="form-control" />
                </div>
				<?php } ?>
        ...
        <div class="additional_eq clearfix">
            ...
            <div class="form-group clearfix" id="equipment_0">
				...
				<?php if(in_array_starts('Total Multiple', $field_order)) { ?>
                <div class="col-sm-1" >
                    <input name="eqestimatetotalmulti[]" id='eqestimatetotalmulti_0' type="text" class="form-control" />
                </div>
				<?php } ?>
		...
		if(!empty($_GET['estimateid'])) {
			...
			$querxy = mysqli_query ($dbc, "SELECT DISTINCT(rate_card_types) FROM company_rate_card WHERE ((rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories') OR rate_card_name='') AND tile_name='Equipment' $types");
			...
			$query_rc = mysqli_query($dbc,"SELECT * FROM company_rate_card WHERE ((rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories') OR $universal_rc_search) AND tile_name='Equipment'");
					...
					<?php foreach($field_order as $field_data) {
						...
						switch($data[0]) {
							...
							case 'Total Multiple': ?><div class="col-sm-1">
									<input name="crc_equipment_total_multiple_<?php echo $rc; ?>" value= "<?php echo $estimate_company_rate_card['total_multiple']; ?>"  id="crc_equipment_total_multiple_<?php echo $rc;?>" type="text" class="form-control" />
								</div><?php
								break;
						}
					} ?>
        ...
        <div class="form-group clearfix" style="margin-left:5px">
					...
					<?php foreach($field_order as $field_data) {
						...
						switch($data[0]) {
							...
							case 'Total': ?><div class="col-sm-1">
								...
								break;
							case 'Total Multiple': ?><div class="col-sm-1">
									<input name="eqtotalmiscmulti[]" id="eqtotalmiscmulti_0" type="text" class="form-control" />
								</div><?php
								break;
						}
					} ?>
            ...
            <br>
            <?php
            $query_misc_rc = mysqli_query($dbc,"SELECT * FROM cost_estimate_misc WHERE accordion='Equipment' AND estimateid=" . $_GET['estimateid']);
            //exit;
					...
					<?php foreach($field_order as $field_data) {
						...
						switch($data[0]) {
							...
							case 'Total': ?><div class="col-sm-1">
								...
								break;
							case 'Total Multiple': ?><div class="col-sm-1">
									<input name="eqtotalmiscmulti_display[]" id="eqtotalmiscmulti" value="<?php echo $misc_row_rc['total_multiple'] ?>" readonly type="text" class="form-control" />
								</div><?php
								break;
						}
					} ?>
