...
function countService(txb) {
    if(txb != 'delete') {
        ...
        document.getElementById('sestimatetotal_'+split_id[1]).value = parseFloat(round2Fixed($('#sestimateprice_'+split_id[1]).val() * lbqty));
    }
...
function changeServiceTotal() {
    ...
    $('#services_profit_margin').val(round2Fixed(margin));
    ...
    $('[name=services_summary_margin]').val(round2Fixed(margin));
       ...
       <?php
        ...
        <?php if(!empty($get_services)) {
            ...
            <div class="form-group clearfix" id="<?php echo 'services_'.$id_loop; ?>" >
				...
				<?php if(in_array_starts('Total Multiple', $field_order)) { ?>
                <div class="col-sm-1" data-columns="<?php echo $columns; ?>" data-width="1">
                    <input name="sestimatetotalmulti[]" value="<?php echo $totalmulti; ?>" id="<?php echo 'sestimatetotalmulti_'.$id_loop; ?>" type="text" class="form-control" />
                </div>
				<?php } ?>
        ...
        <div class="additional_s clearfix">
            ...
            <div class="form-group clearfix" id="services_0">
				...
				<?php if(in_array_starts('Total Multiple', $field_order)) { ?>
                <div class="col-sm-1" data-columns="<?php echo $columns; ?>" data-width="1">
                    <input name="sestimatetotalmulti[]" id="sestimatetotalmulti_0" type="text" class="form-control" />
                </div>
				<?php } ?>
...
<?php
if(!empty($_GET['estimateid'])) {
    $query_rc = mysqli_query($dbc,"SELECT * FROM company_rate_card WHERE ((rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories') OR $universal_rc_search) AND tile_name='Services' AND `deleted`=0");
        ...
        <div class="form-group clearfix" width="100%" <?php echo $load_tab != 'Master' && strpos($estimateConfigValue,',Services'.$row_rc['rate_card_types'].',') === false ? 'style="display:none;"' : ''; ?>>
			<?php foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total Multiple': ?>
						<div class="col-sm-1">
							<input name="crc_services_total_multiple[]" value= "<?php echo $estimate_company_rate_card['total_multiple']; ?>"  id="crc_services_total_multiple_<?php echo $rc;?>" type="text" class="form-control" />
						</div>
						<?php break;
				}
			} ?>
