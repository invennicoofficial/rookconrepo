...
if(!empty($_GET['estimateid'])) {
    ...
    $get_doc = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(uploadid) AS total_id FROM cost_estimate_document WHERE estimateid='$estimateid'"));

    if($get_doc['total_id'] > 0) {
        $result = mysqli_query($dbc, "SELECT upload, uploadid FROM cost_estimate_document WHERE  estimateid='$estimateid'");
