...
if (isset($_POST['submit_tab_config'])) {
	...
	$security_levels = mysqli_fetch_assoc(mysqli_query ( $dbc, "SELECT * FROM  security_level" ));
	$on_security = [];
	foreach($security_levels as $level => $status) {
		if($status != 'super' && strpos($status,'*turn_on*') !== FALSE && strpos(','.ROLE.',',','.$level.',') === FALSE) {
			$on_security[] = $level;
		}
	}
	foreach($project_tab_list as $tab_name) {
		if(strpos(','.$old_types.',',','.$tab_name.',') === FALSE && $tab_name != '') {
			foreach($on_security as $level) {
    ...
    if($get_field_config['fieldconfigestimateid'] > 0) {
        $query_update_employee = "UPDATE `field_config_cost_estimate` SET config_fields = '$config_fields', config_fields_dashboard = '$config_fields_dashboard', `field_info` = '$label_config', `estimate_groups` = '$staff_groups', `custom_accordions` = '$accordion_config' WHERE `fieldconfigestimateid` = 1";
        $result_update_employee = mysqli_query($dbc, $query_update_employee);
    } else {
        $query_insert_config = "INSERT INTO `field_config_cost_estimate` (`config_fields`, `config_fields_dashboard`, `field_info`, `estimate_groups`, `custom_accordions`) VALUES ('$config_fields', '$config_fields_dashboard', '$label_config', '$staff_groups', '$accordion_config')";
    ...
    $get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(configid) AS configid FROM general_configuration WHERE name='cost_estimate_service_price_or_hours'"));
    if($get_config['configid'] > 0) {
        $query_update_employee = "UPDATE `general_configuration` SET value = '$estimate_service_price_or_hours' WHERE name='cost_estimate_service_price_or_hours'";
        $result_update_employee = mysqli_query($dbc, $query_update_employee);
    } else {
        $query_insert_config = "INSERT INTO `general_configuration` (`name`, `value`) VALUES ('cost_estimate_service_price_or_hours', '$estimate_service_price_or_hours')";
        $result_insert_config = mysqli_query($dbc, $query_insert_config);
    }
    //Tile Name

    $estimate_service_qty_cost = $_POST['estimate_service_qty_cost'];
    $get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(configid) AS configid FROM general_configuration WHERE name='cost_estimate_service_qty_cost'"));
    if($get_config['configid'] > 0) {
        $query_update_employee = "UPDATE `general_configuration` SET value = '$estimate_service_qty_cost' WHERE name='cost_estimate_service_qty_cost'";
        $result_update_employee = mysqli_query($dbc, $query_update_employee);
    } else {
        $query_insert_config = "INSERT INTO `general_configuration` (`name`, `value`) VALUES ('cost_estimate_service_qty_cost', '$estimate_service_qty_cost')";
        $result_insert_config = mysqli_query($dbc, $query_insert_config);
    }
...
<div class="pad-left">
	...
	<?php endif; ?>
	<a href='field_config_estimate.php'><button type="button" class="btn brand-btn mobile-block <?php echo $active_tab; ?>" >Internal Cost Estimate Settings</button></a>
	<a href='field_config_quote.php'><button type="button" class="btn brand-btn mobile-block" >Customer Cost Estimate Settings</button></a>
...
<div class="panel-group" id="accordion2">
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h4 class="panel-title">
                <a data-toggle="collapse" data-parent="#accordion2" href="#collapse_dash" >
                    Internal Estimate Dashboard Settings<span class="glyphicon glyphicon-plus"></span>
                </a>
            </h4>
        </div>
        ...
        <div id="collapse_dash" class="panel-collapse collapse">
            <div class="panel-body">
                ...
                <input type="checkbox" <?php if (strpos($config_fields_dashboard, ','."Review Quote".',') !== FALSE) { echo " checked"; } ?> value="Review Quote" style="height: 20px; width: 20px;" name="config_fields_dashboard[]">&nbsp;&nbsp;Review Customer Cost Estimate&nbsp;&nbsp;
        ...
        <div class="panel-heading">
            <h4 class="panel-title">
                <a data-toggle="collapse" data-parent="#accordion2" href="#collapse_basic" >
                    Internal Estimate Settings<span class="glyphicon glyphicon-plus"></span>
                </a>
            </h4>
        </div>
    ...
    <div class="panel panel-default">
        ...
        <div id="collapse_Detail" class="panel-collapse collapse">
            <div class="panel-body">
                ...
                <input type="checkbox" <?php if (strpos($value_config, ','."Details Notes".',') !== FALSE) { echo " checked"; } ?> value="Details Notes" style="height: 20px; width: 20px;" name="config_fields[]">&nbsp;&nbsp;Notes for Customer Cost Estimate&nbsp;&nbsp;
    ...
    <div class="panel panel-default">
        ...
        <div id="collapse_Services" class="panel-collapse collapse">
            <div class="panel-body">
                ...
                <?php
                $estimate_service_price_or_hours = get_config($dbc, 'cost_estimate_service_price_or_hours');
              ...
              <div class="form-group">
                ...
                <?php
                $estimate_service_qty_cost = get_config($dbc, 'cost_estimate_service_qty_cost');
    ...
    <div class="panel panel-default">
        ...
        <div id="collapse_custom" class="panel-collapse collapse">
            <div class="panel-body">
				<?php $accordion_result = mysqli_fetch_array(mysqli_query($dbc, "SELECT `custom_accordions` FROM `field_config_cost_estimate`"));
				...
				foreach($accordions as $acc_order => $accordion): ?>
					<div class="custom-accordion">
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input name="accordion[]" type="text" value="<?php echo $accordion; ?>" class="form-control">
							</div>
