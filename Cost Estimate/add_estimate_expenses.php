...
function countExpense(txb) {
    ...
    $('[name="expense_summary"]').val('$'+round2Fixed(sum_fee)).change();
...
function fillmarginexpensevalue(est) {
    ...
    var profitid = 'eeprofit_' + idarray[1];
    var profitmarginid = 'eeprofitmargin_' + idarray[1];
    var qtyid = 'eestimateqty_' + idarray[1];
    var pcid = 'ec_' + idarray[1];
    var del = (est.value - jQuery('#'+pcid).val()) * jQuery('#'+qtyid).val();
    var delper = (del / (jQuery('#'+qtyid).val() * est.value)) * 100;
...
function changeExpenseTotal() {
    ...
    jQuery('[name="eestimatetotal[]"]').each(function () {
        sum_total += +$(this).val() || 0;
    });

    sum_profit_margin = sum_profit / sum_total * 100;
	if(isNaN(sum_profit_margin)) {
		sum_profit_margin = 'N/A';
	}
	else {
		sum_profit_margin = round2Fixed(sum_profit_margin) + '%';
	}

    jQuery('#expense_profit').val('$'+round2Fixed(sum_profit));
    jQuery('#expense_profit_margin').val(sum_profit_margin);
    jQuery('#expense_cost').val('$'+round2Fixed(sum_total - sum_profit));
    jQuery('#expense_total').val('$'+round2Fixed(sum_total));
    jQuery('[name=expense_summary_profit]').val('$'+round2Fixed(sum_profit));
    jQuery('[name=expense_summary_margin]').val(sum_profit_margin);
    jQuery('[name=expense_summary_cost]').val('$'+round2Fixed(sum_total - sum_profit));
    jQuery('[name=expense_summary]').val('$'+round2Fixed(sum_total)).change();
}
       ...
       <?php
        ...
        <?php if(!empty($get_expenses)) {
            ...
            <div class="form-group clearfix" id="<?php echo 'expenses_'.$id_loop; ?>" >
                ...
                <div class="col-sm-2">
                    <select onChange='selectExpenseHeading(this)' data-placeholder="Choose a Expense..." id="<?php echo 'eexpense_'.$id_loop; ?>" class="chosen-select-deselect form-control exepnseid" width="380">
                        <option value=''></option>
                        <?php
				...
				<?php if(in_array_starts('Total Multiple', $field_order)) { ?>
                <div class="col-sm-1" >
                    <input name="eestimatetotalmulti[]" value="<?php echo $totalmulti; ?>" id="<?php echo 'eestimatetotalmulti_'.$id_loop; ?>" type="text" class="form-control" />
                </div>
				<?php } ?>
                <div class="col-sm-1" >
        ...
        <div class="additional_e clearfix">
            ...
            <div class="form-group clearfix" id="expenses_0">
                <?php if (strpos($field_config_expense, ','."Expense Heading".',') !== FALSE) { ?>
                <div class="col-sm-2">
                    <select onChange='selectExpenseHeading(this)' data-placeholder="Choose a Type..." id="eexpense_0" class="chosen-select-deselect form-control etype" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT distinct(staff), title FROM expense WHERE deleted=0 order by title");
                        while($row = mysqli_fetch_array($query)) {
				...
				<?php if(in_array_starts('Total Multiple', $field_order)) { ?>
                <div class="col-sm-1" >
                    <input name="eestimatetotalmulti[]" id='eestimatetotalmulti_0' type="text" class="form-control" />
                </div>
				<?php } ?>
                <div class="col-sm-1" >
...
<div class="form-group">
    ...
    <div class="col-sm-8">
      <input name="expense_cost" id="expense_cost" value="" readonly="" type="text" class="form-control">
    </div>
</div>
