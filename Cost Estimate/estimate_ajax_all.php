...
if($_GET['fill'] == 'rate_card_types') {
	...
	$query = mysqli_query($dbc,"SELECT distinct(rate_card_types) FROM company_rate_card WHERE rate_card_name IN (SELECT `rate_card_name` FROM `company_rate_card` WHERE `companyrcid`='$rate_card')");
    while($row = mysqli_fetch_array($query)) {
