...
if(!empty($_GET['estimateid']) && ($action == 'approve' || $action == 'draft')) {
    ...
    $get_estimate_data = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM cost_estimate WHERE `estimateid` = '$estimateid'"));
	...
	//for($i = 0; $i < 100; $i++) {
	//	$r = $s[0] - round(($s[0]-$e[0])/100*$i);
	//	$g = $s[1] - round(($s[1]-$e[1])/100*$i);
	//	$b = $s[2] - round(($s[2]-$e[2])/100*$i);
	...
	if($action == 'approve' && $html_quote != '') {
		...
		foreach($html_arr as $i => $html) {
			$html_arr[$i] = html_entity_decode($html);
		}
	} else if($html_split != '') {
	...
	if($logo != '' && !file_exists('download/'.$logo)) {
		exit("<script>alert('Logo file not found! Please upload a new logo on the Customer Cost Estimates Config page.'); window.location.replace('estimate.php'); </script>");
	}
    ...
    class MYPDF extends TCPDF {
            ...
            if($front_client_info != '') {
                if ($this->PageNo() > 1) {
                    ...
                    if(HEADER_TEXT != '') {
						...
						if(!defined(HEADER_HEIGHT)) {
							DEFINE('HEADER_HEIGHT', $this->getStringHeight(150, $footer_text));
						}
                    }
                }
            } else {
        ...
        public function Footer() {
            ...
            $this->writeHTMLCell(0, 0, '', '', '<p style="text-align:right;">'.$this->getAliasNumPage().'</p>', 0, 0, false, "R", true);
            ...
            if(FOOTER_LOGO != '') {
                ...
                $this->Image($image_file, 0, 285, '', 10, '', '', '', false, 300, 'C', false, false, 0, false, false, false);
            }

            // Finish at 30 mm from bottom
            if(TERM_CONDITION != '') {
				//Position to finish at 30mm from bottom
                $this->SetFont('helvetica', 'I', 8);
                $this->SetY(-($pdf->getStringHeight(180, TERM_CONDITION)) - 30);
                $this->writeHTMLCell(0, 0, '', '', TERM_CONDITION, 0, 0, false, "L", true);
            }
        }
    }
    ...
    $pdf->SetMargins(PDF_MARGIN_LEFT, (defined(HEADER_HEIGHT) ? HEADER_HEIGHT : 0), PDF_MARGIN_RIGHT);
	...
	foreach($html_arr as $html) {
		...
		if($front_content_pages != '') {
			$pdf->SetMargins(PDF_MARGIN_LEFT, (defined(HEADER_HEIGHT) ? HEADER_HEIGHT : 0) + 10, PDF_MARGIN_RIGHT);
			$pdf->AddPage();
			$pdf->SetFont('helvetica', '', 8);
	...
	if($action == 'approve') {
		...
		$pdf->Output('download/cost_estimate_'.$estimateid.'.pdf', 'F');

		$message = 'Estimate Approved and move to Customer Cost Estimates.';
		$url = 'quotes.php';
...
if((!empty($_GET['estimateid'])) && ($_GET['type'] == 'reject')) {
    ...
    $contactid = $_SESSION['contactid'];
    echo insert_day_overview($dbc, $contactid, 'Cost Estimate', date('Y-m-d'), '', 'Rejected Cost Estimate '.$estimate_name);

    $message = 'Estimate Rejected and Removed from Cost Estimates.';
    $url = 'estimate.php';
    echo '<script type="text/javascript"> alert("'.$message.'"); window.location.replace("'.$url.'"); </script>';
}
...
if((!empty($_GET['estimateid'])) && (!empty($_GET['status']))) {
    $estimateid = $_GET['estimateid'];
    $status = $_GET['status'];
    $query_update_report = "UPDATE `cost_estimate` SET `status` = '$status' WHERE `estimateid` = '$estimateid'";
    $result_update_report = mysqli_query($dbc, $query_update_report);

    if($status == 'Approve') {
        echo '<script type="text/javascript"> alert("Estimate Approved and Moved to Project."); window.location.replace("estimate.php"); </script>';
    } else {
        echo '<script type="text/javascript"> alert("Estimate Denied and Removed from Cost Estimates."); window.location.replace("estimate.php"); </script>';
    }
}
...
$('.iframe_open').click(function(){
	   ...
	   $('.iframe_title').text('Cost Estimate History');
	   $('.iframe_holder').show();
	   $('.hide_on_iframe').hide();
});
...
<div class="container">
	...
	<div class="row hide_on_iframe">
        ...
        <h1>Internal Cost Estimates Dashboard</h1>
		<?php
        if(config_visible_function($dbc, 'cost_estimate') == 1) {
		...
		<div class="tab-container">
			<?php if ( check_subtab_persmission($dbc, 'cost_estimate', ROLE, 'internal_cost_estimate') === true ) { ?>
                <a href="estimate.php" class="btn brand-btn mobile-100 active_tab">Internal Cost Estimates</a>
                <?php } else { ?>
                    <button type="button" class="btn disabled-btn mobile-block mobile-100">Internal Cost Estimates</button>
                <?php } ?>
			<?php if ( check_subtab_persmission($dbc, 'cost_estimate', ROLE, 'customer_cost_estimates') === true ) { ?>
                <a href="quotes.php" class="btn brand-btn mobile-100">Customer Cost Estimates</a>
            <?php } else { ?>
        ...
        if(vuaed_visible_function($dbc, 'cost_estimate') == 1) {
			echo '<div class="pull-right mobile-100-pull-right">';
				echo '<span class="popover-examples list-inline" style="margin:0 5px 0 0;"><a data-toggle="tooltip" data-placement="top" title="Click here to add a new Internal Cost Estimate."><img src="' . WEBSITE_URL . '/img/info.png" width="20"></a></span>';
				echo '<a href="add_estimate.php" class="btn brand-btn">Add Internal Estimate</a>';
			echo '</div>';
        }
		echo '</div>';
        ...
        if(!empty($_GET['businessid'])) {
            ...
            $query_check_credentials = "SELECT r.*, c.name FROM cost_estimate r, contacts c WHERE r.businessid = c.contactid AND r.deleted = 0 AND r.businessid = '$businessid' LIMIT $offset, $rowsPerPage";
            $query = "SELECT count(c.name) as numrows FROM cost_estimate r, contacts c WHERE r.businessid = c.contactid AND r.deleted = 0 AND r.businessid = '$businessid'";
        } else {
            $query_check_credentials = "SELECT r.*, c.name FROM cost_estimate r, contacts c WHERE r.businessid = c.contactid AND r.deleted = 0 AND (r.status='Saved' OR r.status='Submitted') ORDER BY estimateid DESC LIMIT $offset, $rowsPerPage";
            $query = "SELECT count(c.name) as numrows FROM cost_estimate r, contacts c WHERE r.businessid = c.contactid AND r.deleted = 0 AND (r.status='Saved' OR r.status='Submitted') ORDER BY estimateid DESC";
        }


        $base_field_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT config_fields_dashboard FROM field_config_cost_estimate WHERE `fieldconfigestimateid` = 1 LIMIT $offset, $rowsPerPage"));
        ...
        if($num_rows > 0) {
            ...
            echo '<th>
				<span class="popover-examples list-inline" style="margin:15px 2px 0 0;"><a data-toggle="tooltip" data-placement="top" title="Click \'Approve\' to send your estimate to Customer Cost Estimates."><img src="' . WEBSITE_URL . '/img/info-w.png" width="20"></a></span>
				Status
				</th>';
            }
