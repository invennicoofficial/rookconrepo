...
function selectPackageService(sel) {
	...
	$.ajax({
		type: "GET",
		url: "estimate_ajax_all.php?fill=package_service_config&value="+stage,
...
function selectPackageCat(sel) {
	...
	$.ajax({
		type: "GET",
		url: "estimate_ajax_all.php?fill=package_cat_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function selectPackageHeading(sel) {
	...
	$.ajax({
		type: "GET",
		url: "estimate_ajax_all.php?fill=package_head_config&value="+stage+"&ratecardid="+ratecardid,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function countPackage() {
    var sum_fee = 0;
    $('[name="packageestimateprice[]"]').each(function () {
        sum_fee += +$(this).val() || 0;
    });

    $('[name="package_total"]').val('$'+round2Fixed(sum_fee));
    $('[name="package_summary"]').val('$'+round2Fixed(sum_fee)).change();
...
<div class="form-group">
    <div class="col-sm-12">
        ...
        <?php if((!empty($_GET['pid'])) || (!empty($_GET['estimateid']))) {
            ...
            $each_pvalmulti = explode(',',trim($pvalmulti,","));
            ...
            for($pid_loop=0; $pid_loop<$total_count; $pid_loop++) {
                ...
                if(isset($each_pval[$pid_loop])) {
                    $pestvalmulti = $each_pvalmulti[$pid_loop];
                }
            ...
            ?>
                ...
                <div class="col-sm-2">
                    <select onChange='selectPackageService(this)' data-placeholder="Choose a Type..." id="<?php echo 'packageservice_'.$id_loop; ?>" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT distinct(service_type) FROM package WHERE deleted=0 order by service_type");
                ...
                <div class="col-sm-2">
                    <select onChange='selectPackageCat(this)' data-placeholder="Choose a Category..." id="<?php echo 'packagecategory_'.$id_loop; ?>" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT distinct(category) FROM package WHERE deleted=0 order by category");
                        while($row = mysqli_fetch_array($query)) {
                ...
                <div class="col-sm-2">
                    <select onChange='selectPackageHeading(this)' data-placeholder="Choose a Heading..." id="<?php echo 'packageheading_'.$id_loop; ?>" name="packageid[]" class="chosen-select-deselect form-control package_head" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT packageid, heading FROM package WHERE deleted=0 order by heading");
                        while($row = mysqli_fetch_array($query)) {
                ...
                <div class="col-sm-1" >
                    <input name="packageestimateprice[]"  value="<?php echo $pestval; ?>"  onchange="countPackage()" type="text" id="<?php echo 'packageest_'.$id_loop; ?>" class="form-control" />
                </div>
				<?php if(in_array_starts('Total Multiple', $field_order)) { ?>
                <div class="col-sm-1" >
                    <input name="packageestimatepricemulti[]"  value="<?php echo $pestvalmulti; ?>"  type="text" id="<?php echo 'packageestmulti_'.$id_loop; ?>" class="form-control" />
                </div>
				<?php } ?>
                <div class="col-sm-1" >
                    <a href="#" onclick="deleteEstimate(this,'package_','packageheading_'); return false;" id="<?php echo 'deletepackage_'.$id_loop; ?>" class="btn brand-btn">Delete</a>
                </div>
            </div>
        <?php  $id_loop++;
                }
        ...
        <div class="additional_package clearfix">
            ...
            <div class="form-group clearfix" id="package_0">
                ...
                <div class="col-sm-1" >
                    <input name="packageestimateprice[]" id="packageest_0" onchange="countPackage()" type="text" class="form-control" />
                </div>
                <div class="col-sm-1" >
                    <input name="packageestimatepricemulti[]" id="packageestmulti_0" type="text" class="form-control" />
                </div>
                <div class="col-sm-1" >
                    <a href="#" onclick="deleteEstimate(this,'package_','packageheading_'); return false;" id="deletepackage_0" class="btn brand-btn">Delete</a>
                </div>
            </div>
