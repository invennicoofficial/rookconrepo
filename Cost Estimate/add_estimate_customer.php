...
function countCustomer() {
    ...
    $('[name="customer_summary"]').val('$'+round2Fixed(sum_fee)).change();
    ...
    <div class="col-sm-12">
        ...
        <?php if(!empty($get_customer)) {
            ...
            <div class="form-group clearfix" id="<?php echo 'customer_'.$id_loop; ?>" >
                ...
                <div class="col-sm-2">
                    <select onChange='selectCustomer(this)' data-placeholder="Choose a Customer..." id="<?php echo 'custcustomerperson_'.$id_loop; ?>" name="customerperson[]" class="chosen-select-deselect form-control" width="380">
                        <option value=''></option>
                        <?php
				...
				<?php if(in_array_starts('Total Multiple', $field_order)) { ?>
                <div class="col-sm-1" >
                    <input name="custestimatepricemulti[]" value="<?php echo $pricemulti; ?>" id="<?php echo 'customerestmulti_'.$id_loop; ?>" type="text" class="form-control" />
                </div>
				<?php } ?>
                <div class="col-sm-1" >
        ...
        <div class="additional_cust clearfix">
            ...
            <div class="form-group clearfix" id="customer_0">
                ...
                <div class="col-sm-2">
                    <select onChange='selectCustomer(this)' data-placeholder="Choose a Customer..." id="custcustomerperson_0" name="customerperson[]" class="chosen-select-deselect form-control" width="380">
                        <option value=''></option>
						<?php
				...
				<?php if(in_array_starts('Total Multiple', $field_order)) { ?>
                <div class="col-sm-1" >
                    <input name="custestimatepricemulti[]" id="customerestmulti_0" type="text" class="form-control" />
                </div>
				<?php } ?>
                <div class="col-sm-1" >
...
<?php
if(!empty($_GET['estimateid'])) {
    $query_rc = mysqli_query($dbc,"SELECT * FROM company_rate_card WHERE ((rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories') OR $universal_rc_search) AND tile_name='Customer'");
			...
			<?php foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total Multiple': ?><div class="col-sm-1">
							<input name="crc_customer_total_multiple[]" value= "<?php echo $estimate_company_rate_card['total_multiple']; ?>"  id="crc_customer_total_multiple_<?php echo $rc;?>" type="text" class="form-control" />
						</div><?php
						break;
				}
			} ?>
