       ...
       <?php
            ...
            <div class="form-group clearfix" id="<?php echo 'staff_'.$id_loop; ?>" >
                <div class="col-sm-2">
                    <select onChange='selectStaff(this)' data-placeholder="Choose a Staff Member..." id="<?php echo 'stcontactid_'.$id_loop; ?>" name="contactid[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT contactid, first_name, last_name FROM contacts WHERE category='Staff' order by first_name");
				...
				<?php if(in_array_starts('Total Multiple', $field_order)) { ?>
                <div class="col-sm-1" >
                    <input name="stestimatetotalmulti[]" value="<?php echo $totalmulti; ?>" id="<?php echo 'stestimatetotalmulti_'.$id_loop; ?>" type="text" class="form-control" />
                </div>
				<?php } ?>
                <div class="col-sm-1" >
        ...
        <div class="additional_st clearfix">
            ...
            <div class="form-group clearfix" id="staff_0">
				...
				<?php if(in_array_starts('Total Multiple', $field_order)) { ?>
                <div class="col-sm-1" >
                    <input name="stestimatetotalmulti[]" id='stestimatetotalmulti_0' type="text" class="form-control" />
                </div>
				<?php } ?>
                <div class="col-sm-1" >
...
<?php
if(!empty($_GET['estimateid'])) {
    $query_rc = mysqli_query($dbc,"SELECT * FROM company_rate_card WHERE ((rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories') OR $universal_rc_search) AND tile_name='Staff' AND `deleted`=0");
			...
			<?php foreach($field_order as $field_data) {
				...
				switch($data[0]) {
					...
					case 'Total': ?>
						...
						<?php break;
					case 'Total Multiple': ?>
						<input name="crc_staff_total_multiple[]" value= "<?php echo $estimate_company_rate_card['total_multiple']; ?>"  id="crc_staff_total_multiple_<?php echo $rc;?>" type="text" class="form-control" />
						<?php break;
				}
			} ?>
