...
if(!empty($_GET['estimateid'])) {
    ...
    $get_detail = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM cost_estimate_detail WHERE estimateid='$estimateid'"));

    $detail_detail = $get_detail['detail_detail'];
    ...
    $detail_note = $get_detail['detail_quote_note'];
}
?>

<?php if (strpos($base_field_config, ','."Details Detail".',') !== FALSE) { ?>
<div class="form-group">
    <label for="fax_number"	class="col-sm-4	control-label">Detail:</label>
    <div class="col-sm-8">
        <textarea name="detail_detail" rows="5" cols="50" class="form-control"><?php echo $detail_detail; ?></textarea>
    </div>
</div>
<?php } ?>
...
<?php } ?>
<?php if (strpos($base_field_config, ','."Details Notes".',') !== FALSE) { ?>
<div class="form-group">
    <label for="fax_number"	class="col-sm-4	control-label">Note to be Displayed on Customer Cost Estimate PDF:</label>
    <div class="col-sm-8">
        <textarea name="detail_note" rows="5" cols="50" class="form-control"><?php echo $detail_note; ?></textarea>
    </div>
</div>
<?php } ?>