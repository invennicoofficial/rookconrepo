...
$(document).ready(function() {
    ...
    $('#add_row_sred').on( 'click', function () {
		...
		clone.find('#sredestimateprice_0').attr('id', 'sredestimateprice_'+add_new_sred);
		clone.find('#sredestimateqty_0').attr('id', 'sredestimateqty_'+add_new_sred);
		clone.find('#sredestimatetotal_0').attr('id', 'sredestimatetotal_'+add_new_sred);
...
function selectSrEd(sel) {
	...
	$.ajax({
		type: "GET",
		url: "estimate_ajax_all.php?fill=sred_config&value="+stage,
...
function selectSrEdCat(sel) {
	...
	$.ajax({
		type: "GET",
		url: "estimate_ajax_all.php?fill=sred_cat_config&value="+stage,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function selectSrEdHeading(sel) {
	...
	$.ajax({
		type: "GET",
		url: "estimate_ajax_all.php?fill=sred_head_config&value="+stage+"&ratecardid="+ratecardid,
		dataType: "html",   //expect html to be returned
		success: function(response){
...
function countSrEd(txb) {
    if(txb != 'delete') {
        ...
        document.getElementById('sredestimatetotal_'+split_id[1]).value = parseFloat($('#sredestimateprice_'+split_id[1]).val() * $('#sredestimateqty_'+split_id[1]).val());
    }

    var sum_fee = 0;
    $('[name="sredestimatetotal[]"]').each(function () {
        sum_fee += +$(this).val() || 0;
    });

    $('[name="sred_total"]').val('$'+round2Fixed(sum_fee));
    $('[name="sred_summary"]').val('$'+round2Fixed(sum_fee)).change();
...
<div class="form-group">
    <div class="col-sm-12">
        <div class="form-group clearfix">
            ...
            <label class="col-sm-1 text-center">Total X 1</label>
            <?php } ?>
        </div>
       ...
       <?php
            ...
            <div class="form-group clearfix" id="<?php echo 'sred_'.$id_loop; ?>" >
                ...
                <div class="col-sm-2">
                    <select onChange='selectSrEd(this)' data-placeholder="Choose a Type..." id="<?php echo 'sredservice_'.$id_loop; ?>" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT distinct(sred_type) FROM sred WHERE deleted=0 order by sred_type");
                ...
                <div class="col-sm-2">
                    <select onChange='selectSrEdCat(this)' data-placeholder="Choose a Category..." id="<?php echo 'sredcategory_'.$id_loop; ?>" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT distinct(category) FROM sred WHERE deleted=0 order by category");
                        while($row = mysqli_fetch_array($query)) {
                ...
                <div class="col-sm-2">
                    <select onChange='selectSrEdHeading(this)' data-placeholder="Choose a Heading..." id="<?php echo 'sredheading_'.$id_loop; ?>" name="sredid[]" class="chosen-select-deselect form-control equipmentid" width="380">
                        <option value=''></option>
                        <?php
                        $query = mysqli_query($dbc,"SELECT sredid, heading FROM sred WHERE deleted=0 order by heading");
                        while($row = mysqli_fetch_array($query)) {
                ...
                <div class="col-sm-1" >
                    <input name="sredestimateqty[]" id="<?php echo 'sredestimateqty_'.$id_loop; ?>" onchange="countSrEd(this)" value="<?php echo $qty; ?>" type="text" class="form-control" />
                </div>
                <div class="col-sm-1" >
                    <input name="sredestimateprice[]" id="<?php echo 'sredestimateprice_'.$id_loop; ?>" onchange="countSrEd(this)" value="<?php echo $est; ?>" type="text" class="form-control" />
                </div>
                <div class="col-sm-1" >
                    <input name="sredestimatetotal[]" value="<?php echo $total; ?>" id="<?php echo 'sredestimatetotal_'.$id_loop; ?>" type="text" class="form-control" />
                </div>
                <?php if (in_array_starts('Total Multiple', $field_order)) { ?>
                <div class="col-sm-1" >
                    <input name="sredestimatetotalmulti[]" value="<?php echo $totalmulti; ?>" id="<?php echo 'sredestimatetotalmulti_'.$id_loop; ?>" readonly type="text" class="form-control" />
                </div>
                <?php } ?>
                <div class="col-sm-1" >
                    <a href="#" onclick="deleteEstimate(this,'sred_','sredheading_'); return false;" id="<?php echo 'deletesred_'.$id_loop; ?>" class="btn brand-btn">Delete</a>
                </div>
            </div>
            <?php  $id_loop++;
                    }
        ...
        <div class="additional_sred clearfix">
            ...
            <div class="form-group clearfix" id="sred_0">
                ...
                <div class="col-sm-1" >
                    <input name="sredestimateqty[]" id='sredestimateqty_0' onchange="countSrEd(this)" type="text" class="form-control" />
                </div>
                <div class="col-sm-1" >
                    <input name="sredestimateprice[]" id='sredestimateprice_0' onchange="countSrEd(this)" type="text" class="form-control" />
                </div>
                <div class="col-sm-1" >
                    <input name="sredestimatetotal[]" id='sredestimatetotal_0' type="text" class="form-control" />
                </div>
				<?php if(in_array_starts('Total Multiple', $field_order)) { ?>
                <div class="col-sm-1" >
                    <input name="sredestimatetotalmulti[]" id='sredestimatetotalmulti_0' type="text" class="form-control" />
                </div>
				<?php } ?>
                <div class="col-sm-1" >
                    <a href="#" onclick="deleteEstimate(this,'sred_','sredheading_'); return false;" id="deletesred_0" class="btn brand-btn">Delete</a>
                </div>
            </div>
