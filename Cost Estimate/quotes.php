...
if (isset($_POST['quote_email'])) {
    ...
    echo insert_day_overview($dbc, $contactid, 'Customer Cost Estimate', date('Y-m-d'), '', 'Sent Customer Cost Estimate '.$estimate_name.' To Client');

    $clientid = get_estimate($dbc, $estimateid, 'clientid');
    $clientname = get_client($dbc, $clientid);
    $email = get_email($dbc, $clientid);

    $promo = html_entity_decode(get_config($dbc, 'send_cost_quote_client_body'));
    $email_body = str_replace("[Client Name]", $clientname, $promo);

    $subject = get_config($dbc, 'send_quote_client_subject');
    $attachment = '';
    $attachment = WEBSITE_URL.'/Cost Estimate/download/cost_estimate_'.$estimateid.'.pdf';

    send_email('', $email, '', '', $subject, $email_body, $attachment);

    $history = decryptIt($_SESSION['first_name']).' '.decryptIt($_SESSION['last_name']).' Sent Customer Cost Estimate on '.date('Y-m-d H:i:s').'<br>';

    $start_date = date('Y-m-d');
    $query_update_report = "UPDATE `cost_estimate` SET `status` = 'Sent To Client', `history` = CONCAT(history,'$history'), `quote_send_date` = '$start_date' WHERE `estimateid` = '$estimateid'";
    $result_update_report = mysqli_query($dbc, $query_update_report);

    echo '<script type="text/javascript"> alert("Sent to Customer."); window.location.replace("quotes.php"); </script>';
}
...
function followupDate(sel) {
	...
	$.ajax({    //create an ajax request to load_page.php
		type: "GET",
		url: "<?php echo WEBSITE_URL;?>/ajax_all.php?fill=cost_quote_followup&id="+arr[1]+'&name='+action+'&fname='+fname+'&lname='+lname+'&contactid='+contactid,
...
function selectStatus(sel) {
	...
	$.ajax({    //create an ajax request to load_page.php
		type: "GET",
		url: "<?php echo WEBSITE_URL;?>/ajax_all.php?fill=cost_quote_status&estimateid="+arr[1]+'&status='+status+'&fname='+fname+'&lname='+lname+'&contactid='+contactid,
		dataType: "html",   //expect html to be returned
		success: function(response){
            if(status == 'Approved Quote') {
                alert("Customer Cost Estimate Approved and move to Project.");
                window.location.replace("<?php echo WEBSITE_URL;?>/Project/project.php?type=Pending");
            } else if(status == 'Move To Estimate') {
                alert("Estimate Move back to Internal Cost Estimates.");
                window.location.replace("<?php echo WEBSITE_URL;?>/Cost Estimate/estimate.php");
            } else {
			    location.reload();
            }
		}
...
$(document).ready(function() {
	$('.iframe_open').click(function(){
			var id = $(this).attr('id');
		   $('#iframe_instead_of_window').attr('src', '<?php echo WEBSITE_URL; ?>/Cost Estimate/estimate_history.php?estimateid='+id);
...
<div class="container">
	...
	<div class="row hide_on_iframe">
        ...
        <h1>Customer Cost Estimates Dashboard
        <?php
		...
		<div class="tab-container">
			<?php if ( check_subtab_persmission($dbc, 'cost_estimate', ROLE, 'internal_cost_estimate') === true ) { ?>
                <a href="estimate.php" class="btn brand-btn">Internal Cost Estimates</a>
                <?php } else { ?>
			...
			<?php if ( check_subtab_persmission($dbc, 'cost_estimate', ROLE, 'customer_cost_estimates') === true ) { ?>
                <a href="quotes.php" class="btn brand-btn active_tab">Customer Cost Estimates</a>
            <?php } else { ?>
        ...
        if(!empty($_GET['quoteid'])) {
            ...
            $query_check_credentials = "SELECT * FROM cost_estimate WHERE estimateid = '$quoteid'";
        } elseif(!empty($_GET['businessid'])) {
            $businessid = $_GET['businessid'];
            $query_check_credentials = "SELECT r.*, c.name FROM cost_estimate r, contacts c WHERE r.businessid = c.contactid AND r.deleted = 0 AND r.businessid = '$businessid' AND (r.status!='Saved' AND r.status!='Submitted' AND r.status!='Approved Quote')";
        } else {
            $query_check_credentials = "SELECT r.*, c.name FROM cost_estimate r, contacts c WHERE r.businessid = c.contactid AND r.deleted = 0 AND (r.status!='Saved' AND r.status!='Submitted' AND r.status!='Approved Quote') ORDER BY estimateid DESC";
        }

        $result = mysqli_query($dbc, $query_check_credentials);

        $base_field_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT config_fields_quote_dashboard FROM field_config_cost_estimate WHERE `fieldconfigestimateid` = 1"));
        ...
        if($num_rows > 0) {
            ...
            echo '<th>
				<span class="popover-examples list-inline" style="margin:15px 2px 0 0;"><a data-toggle="tooltip" data-placement="top" title="Select \'Move To Internal\' in order to edit as an Internal Cost Estimate."><img src="' . WEBSITE_URL . '/img/info-w.png" width="20"></a></span>
				Status
				</th>';
            }
            if (strpos($config_fields_quote_dashboard, ','."Send To Client".',') !== FALSE) {
            echo '<th>Send To Customer</th>';
            }
        ...
        while($row = mysqli_fetch_array( $result )) {
            ...
            if (strpos($config_fields_quote_dashboard, ','."Client".',') !== FALSE) {
            echo '<td data-title="Customer">' . get_contact($dbc, $businessid, 'name').'<br>'.get_contact($dbc, $clientid, 'first_name').' '.get_contact($dbc, $clientid, 'last_name') . '</td>';
            }

            //echo '<td data-title="Serial Number">' . $row['name'] . '</td>';
            if (strpos($config_fields_quote_dashboard, ','."Quote Name".',') !== FALSE) {
            echo '<td data-title="Estimate Name">' . $row['estimate_name'] . '</td>';
            }
            if (strpos($config_fields_quote_dashboard, ','."Total Cost".',') !== FALSE) {
            ...
            if (strpos($config_fields_quote_dashboard, ','."Quote".',') !== FALSE) {
		    echo '<td data-title="Estimate"><a href="'.WEBSITE_URL.'/Cost Estimate/download/cost_estimate_'.$row['estimateid'].'.pdf" target="_blank" ><img src="'.WEBSITE_URL.'/img/pdf.png" width="16" height="16" border="0" alt="View">View</a></td>';
            }
            ...
            <div class="form-group">
                <select id="status_<?php echo $row['estimateid']; ?>"  data-placeholder="Choose a Status..." name="status[]" class="chosen-select-deselect form-control input-sm">
                  ...
                  <option value="Move To Estimate" <?php if($row['status'] == "Move To Estimate")  { echo " selected"; } ?> >Move To Internal</option>
                  <option value="Sent To Client" <?php if($row['status'] == "Sent To Client")  { echo " selected"; } ?> >Sent To Customer</option>
            ...
            if (strpos($config_fields_quote_dashboard, ','."Send To Client".',') !== FALSE) {
            echo '<td data-title="Send To Customer"><button type="submit" name="quote_email" value="'.$row['estimateid'].'" class="btn brand-btn">Send</button></td>';
            }
