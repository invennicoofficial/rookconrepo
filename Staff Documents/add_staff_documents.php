...
if(!empty($_GET['certuploadid'])) {
    ...
    echo '<script type="text/javascript"> window.location.replace("add_staff_documents.php?staff_documentsid='.$staff_documentsid.'"); </script>';
}
...
if (isset($_POST['add_staff_documents'])) {
    ...
    echo '<script type="text/javascript"> window.location.replace("staff_documents.php"); </script>';
...
<div class="container">
  <div class="row">
    ...
    <form id="form1" name="form1" method="post"	action="add_staff_documents.php" enctype="multipart/form-data" class="form-horizontal" role="form">
       ...
       <div class="form-group">
        ...
        <div class="col-sm-8">
            <select id="category" name="category" class="chosen-select-deselect form-control" width="380">
                <option value=''></option>
                <?php
                $query = mysqli_query($dbc,"SELECT distinct(category) FROM staff_documents WHERE `deleted` = 0 order by category");
                while($row = mysqli_fetch_array($query)) {
            ...
            if(!empty($_GET['staff_documentsid'])) {
                ...
                if($num_rows > 0) {
                    while($row = mysqli_fetch_array($result)) {
                        ...
                        echo '<ul>';
                        echo '<li><a href="download/'.$row['document_link'].'" target="_blank">'.$row['document_link'].'</a> - <a href="add_staff_documents.php?certuploadid='.$certuploadid.'&staff_documentsid='.$staff_documentsid.'"> Delete</a></li>';
                        echo '</ul>';
                    }
                }
            }
            ...
            if(!empty($_GET['staff_documentsid'])) {
                ...
                if($num_rows > 0) {
                    ...
                    while($row = mysqli_fetch_array($result)) {
                        $certuploadid = $row['certuploadid'];
                        echo '<ul>';
                        echo '<li><a target="_blank" href=\''.$row['document_link'].'\'">Link '.$link_no.'</a> - <a href="add_staff_documents.php?certuploadid='.$certuploadid.'&staff_documentsid='.$staff_documentsid.'"> Delete</a></li>';
                        echo '</ul>';
                        $link_no++;
                    }
                }
  ...
  <?php } ?>
        ...
        <div class="form-group">
            <div class="col-sm-6">
                ...
                <a href="staff_documents.php" class="btn brand-btn btn-lg">Back</a>
