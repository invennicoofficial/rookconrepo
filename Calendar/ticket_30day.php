...
<div class="hide_on_iframe ticket-calendar" class="ticket-calendar style="padding: 1em;">

	<form name="form_sites" method="post" action="" class="form-inline" role="form">
	<a href='?type=ticket&view=daily'><button type="button" class="btn brand-btn mobile-block" >Day</button></a>
	<a href='?type=ticket&view=weekly'><button type="button" class="btn brand-btn mobile-block" >Week</button></a>
	...
	<a href='?type=ticket&view=30day'><button type="button" class="btn brand-btn mobile-block active_tab" >30 Days</button></a>
...
function draw_calendar($dbc, $month,$year,$search_user,$search_client,$search_ticket,$search_status,$status_array){
	...
	for($list_day = 1; $list_day <= $remaining_days; $list_day++):
        ...
        $new_today_date = date_format(date_create_from_format('j-n-Y', $today_date), 'Y-m-d');
            ...
            if($search_user != '') {
                $result = mysqli_query($dbc,"SELECT * FROM contacts WHERE category IN (".STAFF_CATS.") AND contactid ='$search_user' AND deleted=0");
            } else {
                $result = mysqli_query($dbc,"SELECT * FROM contacts WHERE category IN (".STAFF_CATS.") AND deleted=0");
            }
