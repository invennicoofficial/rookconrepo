...
if(!empty($_POST['all_contacts'])) {
	$all_contacts = array_filter($_POST['all_contacts']);
} else if($_GET['mode'] == 'client') {
	$all_contacts = array_filter(explode(',',get_user_settings()['appt_calendar_contacts']));
} else {
...
<?php $calendar_table = [];
$day_start = get_config($dbc, 'staff_schedule_day_start');
$day_end = get_config($dbc, 'staff_schedule_day_end');
$day_period = get_config($dbc, 'staff_schedule_increments');
$current_row = strtotime($day_start);
$calendar_table[0][0]['title'] = "Time";
if(get_config($dbc, 'staff_schedule_calendar_notes') == '1') { $calendar_table[0][0]['notes'] = "Notes"; }
if(get_config($dbc, 'staff_schedule_reminders') == '1') { $calendar_table[0][0]['reminders'] = "Reminders"; }
