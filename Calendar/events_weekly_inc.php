...
$all_contacts = array_filter(explode(',',get_user_settings()['events_calendar_projects']));
...
<?php $calendar_table = [];
$day_start = get_config($dbc, 'event_day_start');
$day_end = get_config($dbc, 'event_day_end');
$day_period = get_config($dbc, 'event_increments');
$current_row = strtotime($day_start);
$appointment_calendar = 'daily';
...
for($i = 1; $i <= 7; $i++) {
	foreach ($all_contacts as $contact_id) {
		$project = mysqli_fetch_assoc(mysqli_query($dbc, "SELECT * FROM `project` WHERE `projectid` = '$contact_id'"));
