...
<div class="hide_on_iframe ticket-calendar calendar-screen" style="padding-bottom: 0px;">
			...
			<div class="panel panel-default">
				...
				<div id="collapse_staff" class="panel-collapse collapse in">
					<div class="panel-body panel-body-height" style="overflow-y: auto; padding: 0;">
						...
						if($_GET['type'] == 'my') {
							$active_contacts = [$_SESSION['contactid']];
							$contact_list = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc, "SELECT `contactid`, `first_name`, `last_name` FROM `contacts` WHERE `contactid` = '".$_SESSION['contactid']."'"),MYSQLI_ASSOC));
						} else {
	...
	<div class="scale-to-fill">
		...
		<a href="?type=<?= $_GET['type'] ?>&view=weekly<?= $region_url ?>"><div class="block-button">Week</div></a>
		<a href="?type=<?= $_GET['type'] ?>&view=monthly<?= $region_url ?>"><div class="block-button active blue">Month</div></a>
