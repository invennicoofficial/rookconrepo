...
<?php $calendar_table = [];
$day_start = get_config($dbc, 'appt_day_start');
$day_end = get_config($dbc, 'appt_day_end');
$day_period = get_config($dbc, 'appt_increments');
$current_row = strtotime($day_start);
$appointment_calendar = 'weekly';
$calendar_table[0][0]['title'] = "Time";
if(get_config($dbc, 'appt_calendar_notes') == '1') { $calendar_table[0][0]['notes'] = "Notes"; }
if(get_config($dbc, 'appt_reminders') == '1') { $calendar_table[0][0]['reminders'] = "Reminders"; }
