...
<?php $calendar_table = [];
$day_start = get_config($dbc, 'estimates_day_start');
$day_end = get_config($dbc, 'estimates_day_end');
$day_period = get_config($dbc, 'estimates_increments');
if ( empty($day_period) ) { $day_period = 15; }
$current_row = strtotime($day_start);
$appointment_calendar = 'weekly';
$calendar_table[0][0]['title'] = "Time";
if(get_config($dbc, 'estimates_calendar_notes') == '1') { $calendar_table[0][0]['notes'] = "Notes"; }
if(get_config($dbc, 'estimates_reminders') == '1') { $calendar_table[0][0]['reminders'] = "Reminders"; }
...
for($i = 1; $i <= 7; $i++) {
	foreach ($all_contacts as $contact_id) {
	    ...
	    if(in_array($day_of_week, $weekly_days)) {
			$column_id++;
		    include('estimates_blocks.php');
		}
	}
}
include('estimates_display.php');