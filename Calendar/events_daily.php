...
<div class="calendar-screen set-height">
	...
	<div class="scale-to-fill"><!--col-sm-<?= ($_GET['unbooked'] || $_GET['teamid'] || $_GET['equipment_assignmentid'] || $_GET['shiftid'] || $_GET['bookingid']) ? '6' : '9' ?> col-xs-12">-->
		...
		<a href="?type=event&view=daily&date=<?= date('Y-m-d') ?><?= $region_url ?>"><div class="block-button pull-right">Today</div></a>
		<input value="<?= $calendar_start ?>" type="text" style="border: 0; width: 0;" class="pull-right datepicker set_date" onchange="window.location.href = '?date='+this.value+'&<?= http_build_query($page_query) ?>';">
