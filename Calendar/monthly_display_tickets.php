...
while($row = mysqli_fetch_array( $result )) {
	...
	if($_GET['block_type'] == 'team') {
	    ...
	    $tickets = mysqli_query($dbc,"SELECT * FROM tickets WHERE `deleted`=0 AND '$new_today_date' BETWEEN `to_do_date` AND `to_do_end_date` AND `status` NOT IN ('Archive', 'Done', 'Internal QA', 'Customer QA')".$contacts_query);
