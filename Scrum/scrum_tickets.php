...
foreach ($each_tab as $cat_tab) {
	...
	if($cat_tab == 'Yesterday'){
		$result = mysqli_query($dbc,"SELECT * FROM tickets WHERE (internal_qa_date='$yesterday' OR deliverable_date='$yesterday' OR '$yesterday' BETWEEN to_do_date AND to_do_end_date) AND status != 'Archive' $query_clause ORDER BY status");
	} else if($cat_tab == 'Doing Today') {
		$result = mysqli_query($dbc,"SELECT * FROM tickets WHERE (internal_qa_date='$today_date' OR deliverable_date='$today_date' OR '$today_date' BETWEEN to_do_date AND to_do_end_date) AND status != 'Archive' $query_clause ORDER BY status");
	} else {
		$result = mysqli_query($dbc, "SELECT * FROM tickets WHERE status='$cat_tab' ORDER BY status");
	}
	...
	while($row = mysqli_fetch_array( $result )) {
		...
		echo '</span><div class="clearfix"></div><a href="'.WEBSITE_URL.'/Ticket/index.php?edit='.$row['ticketid'].'&from='.urlencode(WEBSITE_URL.$_SERVER['REQUEST_URI']).'">'.TICKET_NOUN.' #'.$row['ticketid'].' : '.get_client($dbc, $row['businessid']).' - '.limit_text($row['heading'], 5 ).'<br><em>'.get_multiple_contact($dbc, $row['contactid']).$status_date. '</em></a>';
