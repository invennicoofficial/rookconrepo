    ...
    <div class="row">
		...
		<form method="POST" action="" name="send_email" class="form-horizontal">
			...
			if ( isset ( $_POST['search_invoice_submit'] )) {
                ...
                $query_check_credentialss = "SELECT * FROM point_of_sell inv WHERE deleted = 0 AND status != 'Completed' AND status != 'Void' AND inv.status != 'Returns' AND inv.status != 'Archived' ORDER BY posid DESC LIMIT $offset, $rowsPerPage";
				$query = "SELECT COUNT(posid) as numrows FROM point_of_sell inv WHERE deleted = 0 AND status != 'Completed' AND status != 'Void' AND inv.status != 'Returns' AND inv.status != 'Archived'";
            }
