...
function countMaterial(txb) {
    if(txb != 'delete') {
        ...
        document.getElementById('mestimatetotal_'+split_id[1]).value = parseFloat($('#mestimateprice_'+split_id[1]).val() * meqty).toFixed(2);
    }
...
function changeMaterialTotal() {
    ...
    $('#material_profit_margin').val(per_profit_margin);
    ...
    $('[name=material_summary_margin]').val(per_profit_margin);
...
<?php
if(!empty($_GET['estimateid'])) {
    $query_rc = mysqli_query($dbc,"SELECT * FROM company_rate_card WHERE ((rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories' AND DATE(NOW()) BETWEEN `start_date` AND IFNULL(NULLIF(`end_date`,'0000-00-00'),'9999-12-31')) OR $universal_rc_search) AND tile_name='Material' AND `deleted`=0");
