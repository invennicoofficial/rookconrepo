		...
		<div class="form-group clearfix">
			<!--<?php if(strpos(get_config($dbc,'company_rate_fields'),',category,') !== FALSE): ?>
				...
				<div class="col-sm-8">
					<select name="companyrccat[]" <?php echo $disable_rc; ?> data-placeholder="Select a Rate Card" onchange="filter_categories(this);" class="chosen-select-deselect form-control" width="380">
						<option value=''></option>
						<?php $query = mysqli_query($dbc,"SELECT companyrcid, IFNULL(`rate_categories`,'') rate_categories FROM company_rate_card WHERE `rate_card_name` != '' AND `deleted`=0 GROUP BY rate_categories");
			...
			<div class="col-sm-8">
				<select name="companyrcid[]" <?php echo $disable_rc; ?> data-placeholder="Select a Rate Card" class="chosen-select-deselect form-control" width="380">
					<option value=''></option>
					<?php
					$query = mysqli_query($dbc,"SELECT companyrcid, rate_card_name, IFNULL(`rate_categories`,'') rate_categories FROM company_rate_card WHERE `rate_card_name` != '' AND `deleted`=0 AND DATE(NOW()) BETWEEN `start_date` AND IFNULL(NULLIF(`end_date`,'0000-00-00'),'9999-12-31') GROUP BY rate_card_name, rate_categories");
					while($row = mysqli_fetch_array($query)) {
	...
	<div class="form-group clearfix">
		...
		<div class="col-sm-8">
			<select name="ratecardid" <?php echo $disable_rc; ?> id="ratecardid" data-placeholder="Select a Customer Rate Card..." class="chosen-select-deselect form-control" width="380">
				<option value=''></option>
				<?php
				$query = mysqli_query($dbc,"SELECT ratecardid, rate_card_name FROM rate_card WHERE on_off=1 AND `deleted`=0 AND DATE(NOW()) BETWEEN `start_date` AND IFNULL(NULLIF(`end_date`,'0000-00-00'),'9999-12-31') ORDER BY rate_card_name");
				while($row = mysqli_fetch_array($query)) {
