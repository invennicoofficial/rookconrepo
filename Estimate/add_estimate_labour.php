...
function countLabour(txb) {
    if(txb != 'delete') {
        ...
        document.getElementById('lestimatetotal_'+split_id[1]).value = parseFloat($('#lestimateprice_'+split_id[1]).val() * lbqty).toFixed(2);
    }
...
function changelabourTotal() {
    ...
    jQuery('#labour_profit_margin').val(per_profit_margin);
    ...
    jQuery('[name=labour_summary_margin]').val(per_profit_margin);
    ...
    <div class="col-sm-12">
       ...
       <?php
		...
		if(!empty($_GET['estimateid'])) {
			...
			$querxy = mysqli_query ($dbc, "SELECT DISTINCT(rate_card_types) FROM ((rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories' AND DATE(NOW()) BETWEEN `start_date` AND IFNULL(NULLIF(`end_date`,'0000-00-00'),'9999-12-31')) OR rate_card_name='') AND tile_name='Labour' $types");
			...
			$query_rc = mysqli_query($dbc,"SELECT * FROM company_rate_card WHERE ((rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories' AND DATE(NOW()) BETWEEN `start_date` AND IFNULL(NULLIF(`end_date`,'0000-00-00'),'9999-12-31')) OR $universal_rc_search) AND tile_name='Labour'");
        ...
        <div class="form-group clearfix" style="margin-left:5px">
            ...
            <br>
            <?php
            $query_misc_rc = mysqli_query($dbc,"SELECT * FROM estimate_misc WHERE accordion='Labour' AND estimateid=" . $_GET['estimateid']);
            //exit;
