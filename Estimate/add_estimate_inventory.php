...
function countInventory(txb) {
    if(txb != 'delete') {
        ...
        document.getElementById('inestimatetotal_'+split_id[1]).value = parseFloat($('#inestimateprice_'+split_id[1]).val() * lbqty).toFixed(2);
    }
...
function changeInventoryTotal() {
    ...
    jQuery('#inventory_profit_margin').val(per_profit_margin);
...
<div class="form-group">
    <div class="col-sm-12">
		...
		if(!empty($_GET['estimateid'])) {
			...
			$query_rc = mysqli_query($dbc,"SELECT * FROM company_rate_card WHERE ((rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories' AND DATE(NOW()) BETWEEN `start_date` AND IFNULL(NULLIF(`end_date`,'0000-00-00'),'9999-12-31')) OR $universal_rc_search) AND tile_name='Inventory'");
		...
		<div class="form-group clearfix" style="margin-left:5px">
            ...
            <br>
            <?php
            $query_misc_rc = mysqli_query($dbc,"SELECT * FROM estimate_misc WHERE accordion='Inventory' AND estimateid=" . $_GET['estimateid']);
