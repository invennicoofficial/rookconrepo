...
function filterInventory() {
	...
	$.ajax({
		url: 'estimates_ajax.php?action=inventory_list&category='+category,
		method: 'GET',
		dataType: 'html',
...
<div class="form-group">
	...
	<div class="col-sm-8">
		<select name="category" data-placeholder="Select a Category..." class="chosen-select-deselect form-control">
			<option></option>
			<?php $categories = $dbc->query("SELECT `category` FROM `inventory` WHERE `deleted`=0 GROUP BY `category`");
