...
if (isset($_POST['submit'])) {
    ...
    if($get_field_config['fieldconfigid'] > 0) {
        $query_update_or_insert = "UPDATE `field_config_agendas_meetings` SET field_config='$agenda_meeting', `email_from`='$email_from', `email_subject`='$email_subject', `email_body`='$email_body' WHERE `fieldconfigid` = 1";
    } else {
        $query_update_or_insert = "INSERT INTO `field_config_agendas_meetings` (`field_config`, `email_from`, `email_subject`, `email_body`)
			VALUES ('$agenda_meeting', '$email_from', '$email_subject', '$email_body')";
    }
