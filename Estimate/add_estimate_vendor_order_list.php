...
$(".vndr_estime_price_changer").focusout(function() {
	...
	if(!isNaN(thisval)) {
		...
		$('#total_vndr_'+arr[1]).val(total.toFixed(2));
		...
		$('[name^=vendor_summary]').val('$'+grand_total.toFixed(2)).change();
		$('[name=vendor_summary_usd]').val('$'+grand_total_usd.toFixed(2));
		if(grand_total_usd > 0) {
			$('[name=grand_total_usd]').val('$'+grand_total_usd.toFixed(2));
...
$(".vndr_qty_changer").focusout(function() {
	...
	if(!isNaN(thisval) && thisval > 0) {
		...
		total = Math.round(total * 100) / 100;
		$('#total_vndr_'+arr[2]).val((total+usd).toFixed(2));
