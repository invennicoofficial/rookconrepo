...
function filterInventory() {
	...
	$.ajax({
		url: 'estimates_ajax.php?action=labour_list&category='+category,
		method: 'GET',
		dataType: 'html',
...
<div class="form-group">
	...
	<div class="col-sm-8">
		<select name="category" data-placeholder="Select a Category..." class="chosen-select-deselect form-control">
			<option></option>
			<?php $categories = $dbc->query("SELECT `labour_type` FROM `labour` WHERE `deleted`=0 GROUP BY `category`");
			while($category = $categories->fetch_assoc()) { ?>
				<option value="<?= $category['labour_type'] ?>"><?= $category['labour_type'] ?></option>
			<?php } ?>
		</select>
	</div>
</div>
