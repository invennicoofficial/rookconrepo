...
if($_GET['fill'] == 'in_code_part_no_config') {
    ...
    $get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM inventory WHERE inventoryid='$value'"));
	$cost = $get_config['average_cost'] > 0 ? $get_config['average_cost'] : $get_config['cost'];
    echo $get_config['final_retail_price'].'*FFM*'.$get_config['admin_price'].'*FFM*'.$get_config['wholesale_price'].'*FFM*'.$get_config['commercial_price'].'*FFM*'.$get_config['client_price'].'*FFM*'.$get_config['msrp'].'*FFM*'.$rate_card_price[1].'*FFM*'.$cost;
}
...
//Rate Card Types
if($_GET['fill'] == 'rate_card_types') {
	echo "<option></option>";
    $rate_card = $_GET['rate_card'];
	$query = mysqli_query($dbc,"SELECT distinct(rate_card_types) FROM company_rate_card WHERE rate_card_name IN (SELECT `rate_card_name` FROM `company_rate_card` WHERE `companyrcid`='$rate_card') AND DATE(NOW()) BETWEEN `start_date` AND IFNULL(NULLIF(`end_date`,'0000-00-00'),'9999-12-31')");
    while($row = mysqli_fetch_array($query)) {
		echo "<option value='".$row['rate_card_types']."'>".$row['rate_card_types']."</option>";
	}
}
?>
