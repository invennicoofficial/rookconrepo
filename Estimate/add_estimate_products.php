...
function changeProfitPrice(profit)
{
    ...
    var split_id = get_id.split('_');
    qty = jQuery('#pestimateqty_' + split_id[1]).val();
...
function changeProfitRCPrice(profit)
{
    var get_id = profit.id;
    var split_id = get_id.split('_');
    qty = jQuery('#crc_products_qty_' + split_id[3]).val();
...
function changeProfitMiscPrice(profit)
{
    var get_id = profit.id;
    var split_id = get_id.split('_');
    qty = jQuery('#pqtymisc_' + split_id[1]).val();
    ...
    <div class="col-sm-12">
       ...
       <?php
		...
		if(!empty($_GET['estimateid'])) {
			...
			$querxy = mysqli_query ($dbc, "SELECT DISTINCT(rate_card_types) FROM company_rate_card WHERE ((rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories') OR rate_card_name='' AND DATE(NOW()) BETWEEN `start_date` AND IFNULL(NULLIF(`end_date`,'0000-00-00'),'9999-12-31')) AND tile_name='Products' $types");
			...
			$query_rc = mysqli_query($dbc,"SELECT * FROM company_rate_card WHERE ((rate_card_name='$company_rate_card_name' AND IFNULL(`rate_categories`,'')='$company_rate_categories' AND DATE(NOW()) BETWEEN `start_date` AND IFNULL(NULLIF(`end_date`,'0000-00-00'),'9999-12-31')) OR $universal_rc_search) AND tile_name='Products'");
