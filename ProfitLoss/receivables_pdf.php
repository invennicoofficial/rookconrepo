<?php 
error_reporting(1);
function profit_loss_receivable_pdf($dbc) {
	DEFINE('PDF_HEADER', "");
    DEFINE('PDF_FOOTER', "");
...
$table_name = 'receivables_profit_loss';
...
mysqli_query($dbc, "INSERT INTO `$table_name` (`invoice_date`, `type`, `heading`, `category`, `total`) SELECT pos.`invoice_date`, posp.`type_category`,
	...
	WHERE `invoice_date` >= '$search_start' AND `invoice_date` <= '$search_end' AND pos.`status` NOT IN ('Completed', 'Void')
...
mysqli_query($dbc, "INSERT INTO `$table_name` (`invoice_date`, `type`, `category`, `heading`, `total`)
	...
	WHERE `invoice`.`invoice_date` >= '$search_start' AND `invoice`.`invoice_date` <= '$search_end' AND (`invoice_patient`.`paid`='On Account' OR `invoice_patient`.`paid`='' OR `invoice_patient`.`paid` IS NULL) AND `invoice_patient`.`service_category` != ''");
mysqli_query($dbc, "INSERT INTO `$table_name` (`invoice_date`, `type`, `category`, `heading`, `total`)
...
mysqli_query($dbc, "INSERT INTO `$table_name` (`invoice_date`, `type`, `category`, `heading`, `total`)
	...
	WHERE `invoice`.`invoice_date` >= '$search_start' AND `invoice`.`invoice_date` <= '$search_end' AND (`invoice_patient`.`paid`='On Account' OR `invoice_patient`.`paid`='' OR
		`invoice_patient`.`paid` IS NULL) AND `invoice_patient`.`service_category` = '' GROUP BY `invoice_patient`.`invoicepatientid`");
mysqli_query($dbc, "INSERT INTO `$table_name` (`invoice_date`, `type`, `category`, `heading`, `total`)
...
for ($year = $startyear; $year <= $endyear; $year++) {
	//Pull receivables amounts by category
	...
	$receivables_sql = "SELECT LOWER(`type`) type, `heading`, `category`,
		SUM(IF(`invoice_date` LIKE '".$year."-01%', `total`, 0)) `JAN`,
		SUM(IF(`invoice_date` LIKE '".$year."-02%', `total`, 0)) `FEB`,
		SUM(IF(`invoice_date` LIKE '".$year."-03%', `total`, 0)) `MAR`,
		SUM(IF(`invoice_date` LIKE '".$year."-04%', `total`, 0)) `APR`,
	...
	$receivables = mysqli_query($dbc, $receivables_sql);
	...
	$html = '<table style="border:1px solid black;border-collapse:collapse;height:2px">
		<thead>
			<tr style="border:1px solid black;border-collapse:collapse;height:2px">
				<th style="border:1px solid black;border-collapse:collapse;height:2px">Receivable Items</th>';
	...
	$html .= '</tr>
		...
		<tbody>';
			...
			while($row = mysqli_fetch_array($receivables)) {
				...
				$html .= '<tr><td style="border:1px solid black;border-collapse:collapse;height:2px" data-title="Receivable Item">'.$row['heading'].'</td>';
				for($month = 0; $month < 12; $month++) {
					$dateObj = DateTime::createFromFormat('!m', $month+1);
					$amt = $row[strtoupper($dateObj->format('M'))];
					$html .= '<td style="border:1px solid black;border-collapse:collapse;height:2px" data-title="'.$dateObj->format('F').'">'.($month < $startmonth || $month > $endmonth ? '-' : '$'.number_format($amt, 2, '.', ',')).'</td>';
				}
				$html .=  '</tr>';
			}
			...
			$html .= '</tr>';
			$html .= '<tr>
				<td style="border:1px solid black;border-collapse:collapse;height:2px" colspan="10">Total Receivables for ';
			$html .= $year == $startyear ? $search_start : $year;
			$html .= '-01-01 to ';
			$html .= $year == $endyear ? $search_end : $year;
			$html .= '-12-31 </td>
				<td style="border:1px solid black;border-collapse:collapse;height:2px" data-title="Total" colspan="3">$' . number_format(array_sum($totals), 2, '.', ',') . '</td></tr>';
			$html .= '</tbody>
	</table>';
	}

    $pdf->writeHTML($html, true, false, true, false, '');
    $pdf->Output('download/receivables_'.time().'.pdf', 'F');
    echo '<script type="text/javascript" language="Javascript">window.location.replace("profit_loss.php?tab=receivables");
    window.open("download/receivables_'.time().'.pdf", "fullscreen=yes");
    </script>';
}
?>
