...
include ('../include.php');
include 'config.php';
$from_url = (!empty($_GET['from_url']) ? $_GET['from_url'] : 'blood_glucose.php');

$value = $config['settings']['Choose Fields for Blood Glucose'];

if(isset($_GET['action']) && $_GET['action'] == 'delete') {
    $date_of_archival = date('Y-m-d');
    mysqli_query($dbc, "UPDATE blood_glucose set deleted = 1, `date_of_archival` = '$date_of_archival' WHERE blood_glucose_id=".$_GET['blood_glucose_id']);
...
if (isset($_POST['submit'])) {
    ...
    if(empty($_POST['blood_glucose_id'])) {
        $query_insert_vendor = prepare_insert($inputs, 'blood_glucose');
        $result_insert_vendor = mysqli_query($dbc, $query_insert_vendor);
        $seizure_record_id = mysqli_insert_id($dbc);
        $url = 'Added';
    } else {
        $blood_glucose_id = $_POST['blood_glucose_id'];
        $query_update_vendor = prepare_update($inputs, 'blood_glucose', 'blood_glucose_id', $blood_glucose_id);
...
<div class="container">
  <div class="row">
    ...
    <?php
        ...
        if(!empty($_GET['blood_glucose_id'])) {
