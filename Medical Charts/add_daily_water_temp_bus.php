...
include ('../include.php');
include 'config.php';
$from_url = (!empty($_GET['from_url']) ? $_GET['from_url'] : 'daily_water_temp_bus.php');

$value = $config['settings']['Choose Fields for Daily Water Temp (Business)'];

if(isset($_GET['action']) && $_GET['action'] == 'delete') {
    $date_of_archival = date('Y-m-d');
    mysqli_query($dbc, "UPDATE daily_water_temp_bus set deleted = 1, `date_of_archival` = '$date_of_archival' WHERE daily_water_temp_bus_id=".$_GET['daily_water_temp_bus_id']);
...
if (isset($_POST['submit'])) {
    ...
    if(empty($_POST['daily_water_temp_bus_id'])) {
        $query_insert_vendor = prepare_insert($inputs, 'daily_water_temp_bus');
        $result_insert_vendor = mysqli_query($dbc, $query_insert_vendor);
        $daily_water_temp_bus = mysqli_insert_id($dbc);
        $url = 'Added';
    } else {
        $daily_water_temp_bus_id = $_POST['daily_water_temp_bus_id'];
        $query_update_vendor = prepare_update($inputs, 'daily_water_temp_bus', 'daily_water_temp_bus_id', $daily_water_temp_bus_id);
...
<div class="container">
  <div class="row">
    ...
    <h1>Daily Water Temp (Business)</h1>
    ...
    <?php
        ...
        if(!empty($_GET['daily_water_temp_bus_id'])) {
            ...
            $get_contact = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM daily_water_temp_bus WHERE daily_water_temp_bus_id='$daily_water_temp_bus_id'"));
        ...
        ?>
        <input type="hidden" id="daily_water_temp_bus_id" name="daily_water_temp_bus_id" value="<?php echo $daily_water_temp_bus_id ?>" />
        <?php   }      ?>
