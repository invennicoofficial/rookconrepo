...
$html .= '<h1>Water Temperature Chart - '.get_client($dbc, $contactid).' - '.date('F Y', strtotime($date)).'</h1>';
$html .= '</p>';

$date_start = date('Y-m-01', strtotime($date));
$date_end = date('Y-m-t', strtotime($date));
$wtcb_record_query = "SELECT * FROM `daily_water_temp_bus` WHERE `business`='$contactid' AND `date` BETWEEN '$date_start' AND '$date_end' AND `deleted`=0 ORDER BY `date`, STR_TO_DATE(`time`, '%h:%m %p') ASC, IFNULL(STR_TO_DATE(`time`, '%l:%i %p'),STR_TO_DATE(`time`, '%H:%i')) ASC";
...
if (strpos($value_config, ','."water_temp".',') !== FALSE) {
	$html .= '<th><b>Water Temperature</b></th>';
}
if (strpos($value_config, ','."note".',') !== FALSE) {
	$html .= '<th><b>Note</b></th>';
}
if (strpos($value_config, ','."ai_done".',') !== FALSE) {
	$html .= '<th><b>A/I Done</b></th>';
}
...
foreach ($wtcb_record_result as $chart) {
	...
	$html .= '<tr>';
	$html .= '<td>'.$wtcb_date.'</td>';
	if (strpos($value_config, ','."time".',') !== FALSE) {
		$html .= '<td>'.$time.'</td>';
	}
	if (strpos($value_config, ','."location".',') !== FALSE) {
		$html .= '<td>'.$wtcb_location.'</td>';
	}
	if (strpos($value_config, ','."water_temp".',') !== FALSE) {
		$html .= '<td>'.$water_temp.'</td>';
	}
	if (strpos($value_config, ','."note".',') !== FALSE) {
...
$file_name = 'water_temp_chart_bus_'.$contactid.'_'.$today_date.'.pdf';
