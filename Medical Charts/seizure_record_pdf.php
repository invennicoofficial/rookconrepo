...
if(strpos($value_config, ',pdf_days_column,') !== FALSE) {
	$pdf_times = mysqli_fetch_all(mysqli_query($dbc, "SELECT * FROM `field_config_charts_pdf_times` WHERE `chart` = 'seizure_record' ORDER BY `fieldconfigid`"),MYSQLI_ASSOC);
		...
		foreach($pdf_times as $pdf_time) {
			...
			for($i = $start_day; $i <= $end_day; $i++) {
				...
				foreach($seizure_record_result as $chart) {
					...
					if($seizure_date == date('Y-m-'.sprintf('%02d',$i), strtotime($date)) && strtotime(date('Y-m-d').' '.$start_time) >= $start_seconds && strtotime(date('Y-m-d').' '.$start_time) <= $end_seconds) {
						if (strpos($value_config, ','."start_time".',') !== FALSE) {
							$html .= 'Start Time: '.$start_time.'<br>';
						}
						if (strpos($value_config, ','."end_time".',') !== FALSE) {
							$html .= 'End Time: '.$end_time.'<br>';
						}
						if (strpos($value_config, ','."form".',') !== FALSE) {
							$html .= 'Form: '.$seizure_type.'<br>';
						}
	...
	$html .= '<th><b>Date</b></th>';
	if (strpos($value_config, ','."start_time".',') !== FALSE) {
		$html .= '<th><b>Start Time</b></th>';
	}
	if (strpos($value_config, ','."end_time".',') !== FALSE) {
		$html .= '<th><b>End Time</b></th>';
	}
	if (strpos($value_config, ','."form".',') !== FALSE) {
		$html .= '<th><b>Type of Seizure</b></th>';
	}
	if (strpos($value_config, ','."note".',') !== FALSE) {
		$html .= '<th><b>Note</b></th>';
	}
...
$file_name = 'seizure_record_'.$contactid.'_'.$today_date.'.pdf';
