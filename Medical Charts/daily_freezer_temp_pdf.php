...
$html .= '<h1>Daily Freezer Temp Chart - '.get_client($dbc, $contactid).' - '.date('F Y', strtotime($date)).'</h1>';
$html .= '</p>';

$date_start = date('Y-m-01', strtotime($date));
$date_end = date('Y-m-t', strtotime($date));
$freezer_record_query = "SELECT * FROM `daily_freezer_temp` WHERE `business`='$contactid' AND `date` BETWEEN '$date_start' AND '$date_end' AND `deleted`=0 ORDER BY `date`, IFNULL(STR_TO_DATE(`time`, '%l:%i %p'),STR_TO_DATE(`time`, '%H:%i')) ASC";
...
if (strpos($value_config, ','."freezer".',') !== FALSE) {
	$html .= '<th><b>Freezer</b></th>';
}
...
$file_name = 'daily_freezer_temp_'.$contactid.'_'.$today_date.'.pdf';
