...
include ('../include.php');
include 'config.php';
$from_url = (!empty($_GET['from_url']) ? $_GET['from_url'] : 'daily_freezer_temp.php');

$value = $config['settings']['Choose Fields for Daily Freezer Temp'];

if(isset($_GET['action']) && $_GET['action'] == 'delete') {
    $date_of_archival = date('Y-m-d');
    mysqli_query($dbc, "UPDATE daily_freezer_temp set deleted = 1, `date_of_archival` = '$date_of_archival' WHERE daily_freezer_temp_id=".$_GET['daily_freezer_temp_id']);
...
if (isset($_POST['submit'])) {
    ...
    if(empty($_POST['daily_freezer_temp_id'])) {
        $query_insert_vendor = prepare_insert($inputs, 'daily_freezer_temp');
        $result_insert_vendor = mysqli_query($dbc, $query_insert_vendor);
        $daily_freezer_temp = mysqli_insert_id($dbc);
        $url = 'Added';
    } else {
        $daily_freezer_temp_id = $_POST['daily_freezer_temp_id'];
        $query_update_vendor = prepare_update($inputs, 'daily_freezer_temp', 'daily_freezer_temp_id', $daily_freezer_temp_id);
  ...
  <div class="row">
    ...
    <?php
        ...
        if(!empty($_GET['daily_freezer_temp_id'])) {
            ...
            $get_contact = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM daily_freezer_temp WHERE daily_freezer_temp_id='$daily_freezer_temp_id'"));
        ...
        ?>
        <input type="hidden" id="daily_freezer_temp_id" name="daily_freezer_temp_id" value="<?php echo $daily_freezer_temp_id ?>" />
        <?php   }      ?>
