...
include ('../include.php');
include 'config.php';
$from_url = (!empty($_GET['from_url']) ? $_GET['from_url'] : 'daily_water_temp.php');

$value = $config['settings']['Choose Fields for Daily Water Temp (Client)'];

if(isset($_GET['action']) && $_GET['action'] == 'delete') {
    $date_of_archival = date('Y-m-d');
    mysqli_query($dbc, "UPDATE daily_water_temp set deleted = 1, `date_of_archival` = '$date_of_archival' WHERE daily_water_temp_id=".$_GET['daily_water_temp_id']);
...
if (isset($_POST['submit'])) {
    ...
    if(empty($_POST['daily_water_temp_id'])) {
        $query_insert_vendor = prepare_insert($inputs, 'daily_water_temp');
        $result_insert_vendor = mysqli_query($dbc, $query_insert_vendor);
        $daily_water_temp = mysqli_insert_id($dbc);
        $url = 'Added';
    } else {
        $daily_water_temp_id = $_POST['daily_water_temp_id'];
        $query_update_vendor = prepare_update($inputs, 'daily_water_temp', 'daily_water_temp_id', $daily_water_temp_id);
