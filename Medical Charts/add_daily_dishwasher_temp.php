...
include ('../include.php');
include 'config.php';
$from_url = (!empty($_GET['from_url']) ? $_GET['from_url'] : 'daily_dishwasher_temp.php');
...
if (isset($_POST['submit'])) {
    ...
    if(empty($_POST['daily_dishwasher_temp_id'])) {
        $query_insert_vendor = prepare_insert($inputs, 'daily_dishwasher_temp');
        $result_insert_vendor = mysqli_query($dbc, $query_insert_vendor);
        $daily_dishwasher_temp = mysqli_insert_id($dbc);
        $url = 'Added';
    } else {
        $daily_dishwasher_temp_id = $_POST['daily_dishwasher_temp_id'];
        $query_update_vendor = prepare_update($inputs, 'daily_dishwasher_temp', 'daily_dishwasher_temp_id', $daily_dishwasher_temp_id);
  ...
  <div class="row">
    ...
    <?php
        ...
        if(!empty($_GET['daily_dishwasher_temp_id'])) {
            ...
            $get_contact = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM daily_dishwasher_temp WHERE daily_dishwasher_temp_id='$daily_dishwasher_temp_id'"));
        ...
        ?>
        <input type="hidden" id="daily_dishwasher_temp_id" name="daily_dishwasher_temp_id" value="<?php echo $daily_dishwasher_temp_id ?>" />
        <?php   }      ?>
