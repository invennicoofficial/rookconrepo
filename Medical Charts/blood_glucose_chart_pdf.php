<?php
/*
 * Blood Glucose Chart Export PDF
 * Called from edit_addition_blood_glucose_chart.php
 */
error_reporting(0);
include('../include.php');
...
$html .= '<h1>Blood Glucose Chart - '.get_contact($dbc, $contactid).' - '.date('F Y', strtotime($date)).'</h1>';
$html .= '</p>';

$date_start = date('Y-m-01', strtotime($date));
$date_end = date('Y-m-t', strtotime($date));
$bg_record_query = "SELECT * FROM `blood_glucose` WHERE `client`='$contactid' AND `date` BETWEEN '$date_start' AND '$date_end' AND `deleted`=0 ORDER BY `date`, IFNULL(STR_TO_DATE(`time`, '%l:%i %p'),STR_TO_DATE(`time`, '%H:%i')) ASC";
...
if(strpos($value_config, ',pdf_days_column,') !== FALSE) {
	$pdf_times = mysqli_fetch_all(mysqli_query($dbc, "SELECT * FROM `field_config_charts_pdf_times` WHERE `chart` = 'blood_glucose' ORDER BY `fieldconfigid`"),MYSQLI_ASSOC);
		...
		foreach($pdf_times as $pdf_time) {
			...
			for($i = $start_day; $i <= $end_day; $i++) {
				...
				foreach($bg_record_result as $chart) {
					...
					if($bg_date == date('Y-m-'.sprintf('%02d',$i), strtotime($date)) && strtotime(date('Y-m-d').' '.$time) >= $start_seconds && strtotime(date('Y-m-d').' '.$time) <= $end_seconds) {
						if (strpos($value_config, ','."time".',') !== FALSE) {
							$html .= 'Time: '.$time.'<br>';
						}
						if (strpos($value_config, ','."bg".',') !== FALSE) {
							$html .= 'BG: '.$bm.'<br>';
						}
...
$file_name = 'blood_glucose_chart_'.$contactid.'_'.$today_date.'.pdf';
