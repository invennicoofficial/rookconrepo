...
include ('../include.php');
include 'config.php';
$from_url = (!empty($_GET['from_url']) ? $_GET['from_url'] : 'bowel_movement.php');

$value = $config['settings']['Choose Fields for Bowel Movement'];

if(isset($_GET['action']) && $_GET['action'] == 'delete') {
    $date_of_archival = date('Y-m-d');
    mysqli_query($dbc, "UPDATE bowel_movement set deleted = 1, `date_of_archival` = '$date_of_archival' WHERE bowel_movement_id=".$_GET['bowel_movement_id']);
...
if (isset($_POST['submit'])) {
    ...
    if(empty($_POST['bowel_movement_id'])) {
        $query_insert_vendor = prepare_insert($inputs, 'bowel_movement');
        $result_insert_vendor = mysqli_query($dbc, $query_insert_vendor);
        $bowel_movement_id = mysqli_insert_id($dbc);
        $url = 'Added';
    } else {
        $bowel_movement_id = $_POST['bowel_movement_id'];
        $query_update_vendor = prepare_update($inputs, 'bowel_movement', 'bowel_movement_id', $bowel_movement_id);
