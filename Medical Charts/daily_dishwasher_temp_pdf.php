...
$html .= '<h1>Daily Dishwasher Temp Chart - '.get_client($dbc, $contactid).' - '.date('F Y', strtotime($date)).'</h1>';
$html .= '</p>';

$date_start = date('Y-m-01', strtotime($date));
$date_end = date('Y-m-t', strtotime($date));
$dishwasher_record_query = "SELECT * FROM `daily_dishwasher_temp` WHERE `business`='$contactid' AND `date` BETWEEN '$date_start' AND '$date_end' AND `deleted`=0 ORDER BY `date`, IFNULL(STR_TO_DATE(`time`, '%l:%i %p'),STR_TO_DATE(`time`, '%H:%i')) ASC";
...
$file_name = 'water_temp_chart_'.$contactid.'_'.$today_date.'.pdf';
