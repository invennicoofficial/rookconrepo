<?php $value = $config['settings']['Choose Fields for Daily Water Temp (Client) Dashboard'];
	$search_staff = '';
	$search_client = '';
	$search_date = '';
	...
	if(isset($display_contact)) {
		...
		<form id="form1" name="form1" method="get" enctype="multipart/form-data" class="form-horizontal" role="form">
		  ...
		  <div class="col-lg-4 col-md-3 col-sm-8 col-xs-12 gap-bottom">
			  ...
			  <option value=""></option>
			  <?php
			  	$query = sort_contacts_array(mysqli_fetch_all(mysqli_query($dbc, "SELECT * FROM `contacts` WHERE `category` = 'Staff' AND `deleted` = 0 AND `status` > 0 AND `show_hide_user` = 1"),MYSQLI_ASSOC));
			  	foreach ($query as $id) { ?>
		...
		<div class="form-group">
		  ...
		  <div class="col-sm-8">
			...
			<button type="button" onclick="window.location='daily_water_temp.php'" name="display_all_inventory" value="Display All" class="btn brand-btn mobile-block">Display All</button>
		  </div>
		</div>
	</form>
	<?php }
...
if(vuaed_visible_function_custom($dbc)) {
	echo '<a href="../Medical Charts/add_daily_water_temp.php?from_url='.$return_url.'" class="btn brand-btn mobile-block pull-right mobile-full-width">Add Daily Water Temp</a>';
	echo '<a href="../Medical Charts/monthly_chart.php?type=daily_water_temp&edit='.$search_client.'" class="btn brand-btn mobile-block pull-right mobile-full-width">Daily Water Temp Chart</a>';
}
?>
<br><br><br>
	...
	$query_check_credentials = 'SELECT * FROM daily_water_temp WHERE 1=1 and deleted=0'.$filter.' ORDER BY `date` DESC, IFNULL(STR_TO_DATE(`time`, "%l:%i %p"),STR_TO_DATE(`time`, "%H:%i")) ASC';
	...
	{
		echo "<tr>";
		$daily_water_temp_id = $row['daily_water_temp_id'];
		$clientid = $row['client'];
		...
		if(vuaed_visible_function_custom($dbc)) {
		echo '<a href=\'../Medical Charts/add_daily_water_temp.php?daily_water_temp_id='.$daily_water_temp_id.'&from_url='.$return_url.'\'>Edit</a> | ';
		echo '<a href=\'../Medical Charts/monthly_chart.php?type=daily_water_temp&edit='.$clientid.'\'>Client Monthly Chart</a> | ';
		echo '<a href=\'../Medical Charts/add_daily_water_temp.php?action=delete&daily_water_temp_id='.$daily_water_temp_id.'&from_url='.$return_url.'\' onclick="return confirm(\'Are you sure?\')">Archive</a>';
		}
		echo '</td>';
