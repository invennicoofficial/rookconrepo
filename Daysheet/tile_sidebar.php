...
<?php } else {
    ...
    <ul class="sidebar">
        ...
        <?php } ?>
        <?php if (in_array('My Sales', $daysheet_button_config)) { ?>
            <a href="?tab=sales&daily_date=<?= date('Y-m-d') ?>&side_content=my_sales&date_display=daily"><li class="<?= $_GET['tab'] == 'sales' ? 'active' : '' ?>">My <?= SALES_TILE ?></li></a>
        <?php } ?>
