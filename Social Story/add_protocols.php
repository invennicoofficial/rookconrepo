...
if(isset($_GET['action']) && $_GET['action'] == 'delete') {
    mysqli_query($dbc, "DELETE FROM social_story_protocols WHERE protocol_id=".$_GET['protocol_id']);
...
if (isset($_POST['submit'])) {
    ...
    if(empty($_POST['protocol_id'])) {
        $query_insert_vendor = prepare_insert_social($inputs, 'social_story_protocols');
        $result_insert_vendor = mysqli_query($dbc, $query_insert_vendor);
        $protocol_id = mysqli_insert_id($dbc);
        $url = 'Added';
    } else {
        $protocol_id = $_POST['protocol_id'];
        $query_update_vendor = prepare_update_social($inputs, 'social_story_protocols', 'protocol_id', $protocol_id);
...
<div class="container">
  <div class="row">
    ...
    <?php
        ...
        if(!empty($_GET['protocol_id'])) {

            $protocol_id = $_GET['protocol_id'];
            $get_contact = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM social_story_protocols WHERE protocol_id='$protocol_id'"));
        ...
        ?>
        <input type="hidden" id="protocol_id" name="protocol_id" value="<?php echo $protocol_id ?>" />
        <?php   }      ?>
