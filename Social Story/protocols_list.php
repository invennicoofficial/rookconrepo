...
if(vuaed_visible_function_social($dbc)) {
	echo '<a href="../Social Story/add_protocols.php?from_url='.$from_url.'" class="btn brand-btn mobile-block pull-right">Add Protocols</a>';
}
?>
<br><br><br>
	...
	$query_check_credentials = 'SELECT * FROM social_story_protocols WHERE 1=1'.$filter;
	...
	while($row = mysqli_fetch_array( $result ))
	{
		...
		echo '<td>';
		if(vuaed_visible_function_social($dbc)) {
		echo '<a href=\'../Social Story/add_protocols.php?protocol_id='.$protocol_id.'&from_url='.$from_url.'\'>Edit</a> | ';
		echo '<a href=\'../Social Story/add_protocols.php?action=delete&protocol_id='.$protocol_id.'&from_url='.$from_url.'\' onclick="return confirm(\'Are you sure?\')">Delete</a>';
		}
		echo '</td>';
