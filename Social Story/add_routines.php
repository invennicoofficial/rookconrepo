...
if (isset($_POST['submit'])) {
    ...
    if(empty($_POST['routine_id'])) {
        $query_insert_vendor = prepare_insert_social($inputs, 'social_story_routines');
        $result_insert_vendor = mysqli_query($dbc, $query_insert_vendor);
        $protocol_id = mysqli_insert_id($dbc);
        $url = 'Added';
    } else {
        $routine_id = $_POST['routine_id'];
        $query_update_vendor = prepare_update_social($inputs, 'social_story_routines', 'routine_id', $routine_id);
  ...
  <div class="row">
    ...
    <?php
        ...
        if(!empty($_GET['routine_id'])) {
            ...
            $get_contact = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM social_story_routines WHERE routine_id='$routine_id'"));
