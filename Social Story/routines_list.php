...
if(vuaed_visible_function_social($dbc)) {
	echo '<a href="../Social Story/add_routines.php?from_url='.$from_url.'" class="btn brand-btn mobile-block pull-right">Add Routine</a>';
}
?>
<br><br><br>
	...
	$query_check_credentials = 'SELECT * FROM social_story_routines WHERE 1=1'.$filter;
	...
	while($row = mysqli_fetch_array( $result ))
	{
		...
		echo '<td>';
		if(vuaed_visible_function_social($dbc)) {
		echo '<a href=\'../Social Story/add_routines.php?routine_id='.$routine_id.'&from_url='.$from_url.'\'>Edit</a> | ';
		echo '<a href=\'../Social Story/add_routines.php?action=delete&routine_id='.$routine_id.'&from_url='.$from_url.'\' onclick="return confirm(\'Are you sure?\')">Delete</a>';
		}
		echo '</td>';
