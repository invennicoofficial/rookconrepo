...
if(isset($_GET['action']) && $_GET['action'] == 'delete') {
    mysqli_query($dbc, "DELETE FROM social_story_communication WHERE communication_id=".$_GET['communication_id']);
...
if (isset($_POST['submit'])) {
    ...
    if(empty($_POST['communication_id'])) {
        $query_insert_vendor = prepare_insert_social($inputs, 'social_story_communication');
        $result_insert_vendor = mysqli_query($dbc, $query_insert_vendor);
        $protocol_id = mysqli_insert_id($dbc);
        $url = 'Added';
    } else {
        $communication_id = $_POST['communication_id'];
        $query_update_vendor = prepare_update_social($inputs, 'social_story_communication', 'communication_id', $routine_id);
...
<div class="container">
  <div class="row">
    ...
    <?php
        ...
        if(!empty($_GET['communication_id'])) {

            $communication_id = $_GET['communication_id'];
            $get_contact = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM social_story_communication WHERE communication_id='$communication_id'"));
        ...
        ?>
        <input type="hidden" id="communication_id" name="communication_id" value="<?php echo $communication_id ?>" />
        <?php   }      ?>
