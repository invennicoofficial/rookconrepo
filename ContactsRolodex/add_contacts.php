...
<div class="container">
  <div class="row">
            ...
            ?>
                    <div class="panel panel-default">
                        ...
                        <?php
                            ...
                            echo '<td><a href=\''.WEBSITE_URL.'/ContactsRolodex/send_injury_drop_off.php?patientid='.$contactid.'&type='.$drop_off.'\' onclick="return confirm(\'Are you sure you want to send drop of analysis email to patient?\')">Send</a></td>';
		...
		<?php if($url_category == 'Business' || $url_category == 'Sites' || $url_category == 'Corporation') : ?>
		...
		$.ajax({
			...
			success: function(result) {
				...
				categories.forEach(function(cat) {
					...
					option.value = cat;
					if((cat != 'Business' && cat != 'Corporation' && cat != 'Staff' && cat != '<?= $url_category ?>') || ('<?= $url_category ?>' == 'Corporation' && cat == 'Business')) {
		...
		function add_business() {
			...
			$('#accordion2').hide();
			$('[name=iframe_business]').attr('src', '<?php echo WEBSITE_URL; ?>/ContactsRolodex/add_contacts.php?category=<?= ($url_category == 'Business' ? 'Corporation' : 'Business') ?>&name='+encodeURI(business));
		...
		function close_related_contacts() {
			$('[name=iframe_related_contacts]').attr('src', '<?php echo WEBSITE_URL; ?>/ContactsRolodex/contacts.php');
		}
		$('[name=contact-category]').change(function() {
			...
			$('[name=iframe_related_contacts]').attr('src', '<?php echo WEBSITE_URL; ?>/ContactsRolodex/add_contacts.php?category='+category);
		...
		function update_business_list(id = '') {
			$.ajax({
				url: 'get_list.php',
				method: 'POST',
				data: { target: 'businessid', category: '<?= ($url_category == 'Business' ? 'Corporation' : 'Business') ?>' },
				success: function(result) {
					var option = document.createElement("option");
					option.text = "Add a new <?= ($url_category == 'Business' ? 'Corporation' : 'Business') ?>";
