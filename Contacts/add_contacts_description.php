		    <?php if (strpos($value_config, ','."BIO".',') !== FALSE) { ?>
				...
				<label for="first_name[]" class="col-sm-4 control-label">
                    <span class="popover-examples"><a data-toggle="tooltip" data-placement="top" title="Input a short <?= strtolower(FOLDER_NAME) ?> bio."><img src="../img/info.png" width="20"></a></span>
                    BIO:
                </label>
				<div class="col-sm-8">
			...
			<?php if (strpos($value_config, ','."Medical Details Goals of Care".',') !== FALSE) { ?>
				<div class="form-group">
				<label for="medical_details_goals_of_care" class="col-sm-4 control-label">Goals of Care:</label>
				<div class="col-sm-8">
				<textarea name="medical_details_goals_of_care" rows="5" cols="50" <?php echo (strpos($edit_config, ','."Medical Details Goals of Care".',') === false ? 'class="form-control noMceEditor" readonly style="display:none;"' : 'class="form-control"'); ?>><?php echo $medical_details_goals_of_care; ?></textarea>
				<div <?php echo (strpos($edit_config, ','."Medical Details Goals of Care".',') === false ? 'class="col-sm-12"' : 'style="display:none;"'); ?>><?php echo html_entity_decode($medical_details_goals_of_care); ?></div>
				</div>
				</div>
			<?php } ?>
			...
			<?php if (strpos($value_config, ',Signature,') !== FALSE) { ?>
				<label class="col-sm-4 control-label">Signature:</label>
				<div class="col-sm-8">
					<div class="sign_view" <?= $get_contact_desc['stored_signature'] != '' ? '' : 'style="display:none;"' ?>>
						<?php if($contactid > 0 && $get_contact_desc['stored_signature'] != '') {
							if(!file_exists('../Contacts/signatures')) {
								mkdir('../Contacts/signatures', 0777, true);
							}
							include_once('../phpsign/signature-to-image.php');
							$signature = sigJsonToImage(html_entity_decode($get_contact_desc['stored_signature']));
						...
						<button class="btn brand-btn" onclick="$(this).closest('.col-sm-8').find('.sign_new').show(); $(this).closest('.sign_view').hide(); return false;">Update Signature</button>
					</div>
					<div class="sign_new" <?= $get_contact_desc['stored_signature'] != '' ? 'style="display:none;"' : '' ?>>
						<?php $output_name = 'stored_signature';
						$output_value = $get_contact_desc['stored_signature'];
