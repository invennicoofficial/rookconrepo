...
if(isset($_POST["submitty"]))
{
	...
	while( ( $csv = fgetcsv( $handle, 0, "," ) ) !== false )
	{
		...
		$sql = "INSERT INTO `contacts` (`tile_name`, `category`, `businessid`, `first_name`, `last_name`, `office_phone`, `cell_phone`, `home_phone`, `fax`, `customer_address`, `email_address`, `position`, `mailing_address`, `postal_code`, `zip_code`, `city`, `province`, `country`, `deleted`, `status`)
			VALUES ('".FOLDER_NAME."', 'Customer', $businessid, '{$values['first_name']}', '{$values['last_name']}', '{$values['office_phone']}', '{$values['cell_phone']}', '{$values['home_phone']}', '{$values['fax']}', '{$values['customer_address']}', '{$values['email_address']}', '{$values['position']}', '{$values['mailing_address']}', '{$values['postal_code']}', '{$values['zip_code']}', '{$values['city']}', '{$values['province']}', '{$values['country']}', 0, 1)";
...
if(isset($_POST["submitty2"]))
{
	...
	while(($row = fgetcsv($handle, 0, ",")) !== false)
	{
		...
		if($contactid > 0) {
			$original = mysqli_fetch_assoc(mysqli_query($dbc, "SELECT * FROM `contacts` WHERE `contactid`='$contactid'"));
			$updates = [];
			$history = '';
			...
			$sql = "UPDATE `contacts` SET ".implode(',',$updates)." WHERE `contactid`='$contactid'";
...
if(isset($_POST["exporter"]))
{
	...
	//$sql = "SELECT * FROM `contacts` LEFT JOIN `contacts_dates` ON `contacts`.`contactid`=`contacts_dates`.`contactid` LEFT JOIN `contacts_cost` ON `contacts`.`contactid`=`contacts_cost`.`contactid` LEFT JOIN `contacts_description` ON `contacts`.`contactid`=`contacts_description`.`contactid` LEFT JOIN `contacts_medical` ON `contacts`.`contactid`=`contacts_medical`.`contactid` WHERE `contacts`.`deleted`=0 AND `contacts`.`tile_name`='".FOLDER_NAME."'";
	$sql = "SELECT * FROM `contacts` WHERE `contacts`.`deleted`=0 AND `contacts`.`tile_name`='".FOLDER_NAME."'";
