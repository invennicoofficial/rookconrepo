...
<?php if (strpos($value_config, ','."Client Activities Social Story".',') !== FALSE) { ?>
	<h3>Activities</h3>
	<div class="form-group">
		<?php if (!$functions_included) {include_once('../Social Story/config.php'); }
		$search_client = $contactid;
		$from_url = urlencode(WEBSITE_URL.'/'.FOLDER_URL.'/add_contacts.php?category='.$url_category.'&contactid='.$contactid.'&subtab='.$subtab);
		if (!$functions_included) {include_once('../Social Story/activities_list.php'); } ?>
	</div>
<?php } ?>
<?php if (strpos($value_config, ','."Client Communication Social Story".',') !== FALSE) { ?>
	...
	<div class="form-group">
		...
		$search_client = $contactid;
		$from_url = urlencode(WEBSITE_URL.'/'.FOLDER_URL.'/add_contacts.php?category='.$url_category.'&contactid='.$contactid.'&subtab='.$subtab);
		if (!$functions_included) {include_once('../Social Story/communication_list.php'); } ?>
	</div>
<?php } ?>
<?php if (strpos($value_config, ','."Client Routines Social Story".',') !== FALSE) { ?>
	...
	<div class="form-group">
		...
		$search_client = $contactid;
		$from_url = urlencode(WEBSITE_URL.'/'.FOLDER_URL.'/add_contacts.php?category='.$url_category.'&contactid='.$contactid.'&subtab='.$subtab);
		if (!$functions_included) {include_once('../Social Story/routines_list.php'); } ?>
	</div>
<?php } ?>
<?php if (strpos($value_config, ','."Client Protocols Social Story".',') !== FALSE) { ?>
	...
	<div class="form-group">
		...
		$search_client = $contactid;
		$from_url = urlencode(WEBSITE_URL.'/'.FOLDER_URL.'/add_contacts.php?category='.$url_category.'&contactid='.$contactid.'&subtab='.$subtab);
		if (!$functions_included) {include_once('../Social Story/protocols_list.php'); } ?>
	</div>
<?php } ?>
<?php if (strpos($value_config, ','."Client Key Methodologies Social Story".',') !== FALSE) { ?>
	...
	<div class="form-group">
		...
		$search_client = $contactid;
		$from_url = urlencode(WEBSITE_URL.'/'.FOLDER_URL.'/add_contacts.php?category='.$url_category.'&contactid='.$contactid.'&subtab='.$subtab);
		if (!$functions_included) {include_once('../Social Story/key_methodologies_list.php'); } ?>
	</div>
<?php } ?>
...
<?php if (strpos($value_config, ','."Incident Reports".',') !== FALSE) { ?>
	<?php
	$display_contact = $contactid;
	$query_check_credentials = "SELECT * FROM incident_report WHERE (CONCAT(',',`contactid`,',') LIKE '%,$display_contact,%' AND `contactid` != '') OR (CONCAT(',',`clientid`,',') LIKE '%,$display_contact,%' AND `clientid` != '') ORDER BY incidentreportid DESC";
        ...
        if($num_rows > 0) { ?>
			...
			<?php while($row = mysqli_fetch_array($result)) {
				...
				<tr>
					<td><?= get_staff($dbc, $row['clientid']) ?></td>
					<td><?= get_staff($dbc, $row['contactid']) ?></td>
					<td><?= $row['today_date'] ?></td>
					<td><?= $row['location'] ?></td>
					<td><a href="<?= WEBSITE_URL ?>/Incident Report/download/<?= $name_of_file ?>" target="_blank" >PDF</a></td>
				</tr>
			<?php } ?>
	...
	?>
<?php } ?>
<?php if (strpos($value_config, ','."Strategies Levels of Communication".',') !== FALSE) { ?>
	<div class="form-group">
		<label class="control-label col-sm-4">Levels of Communication:</label>
		<div class="col-sm-8">
			<textarea name="strategies_communication" rows="5" cols="50" class="form-control"><?= $strategies_communication ?></textarea>
		</div>
	</div>
<?php } ?>
<?php if (strpos($value_config, ','."Strategies Types of Supports".',') !== FALSE) { ?>
	<div class="form-group">
		<label class="control-label col-sm-4">Types of Supports:</label>
		<div class="col-sm-8">
			<textarea name="strategies_supports" rows="5" cols="50" class="form-control"><?= $strategies_supports ?></textarea>
		</div>
	</div>
<?php } ?>
<?php if (strpos($value_config, ','."Strategies Likes".',') !== FALSE) { ?>
	<div class="form-group">
		<label class="control-label col-sm-4">Likes:</label>
		<div class="col-sm-8">
			<textarea name="strategies_likes" rows="5" cols="50" class="form-control"><?= $strategies_likes ?></textarea>
		</div>
	</div>
<?php } ?>
<?php if (strpos($value_config, ','."Strategies Dislikes".',') !== FALSE) { ?>
	<div class="form-group">
		<label class="control-label col-sm-4">Dislikes:</label>
		<div class="col-sm-8">
			<textarea name="strategies_dislikes" rows="5" cols="50" class="form-control"><?= $strategies_dislikes ?></textarea>
		</div>
	</div>
<?php } ?>