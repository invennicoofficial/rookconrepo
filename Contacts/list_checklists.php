...
switch($_GET['subtabid']) {
	default:
		echo "<h2 style='padding-left:2em;'>".get_contact($dbc, $_GET['edit'])."'s Checklists".($_GET['subtabid'] > 0 ? "<span class='pull-right'><a href='?edittab=".$_GET['subtabid']."'><img src='".WEBSITE_URL."/img/icons/ROOK-edit-icon.png' style='height:1em;'></a></span>" : "")."</h2>";
}
$subtab_list = mysqli_query($dbc, "SELECT `checklist`.*, `checklist_subtab`.`name` FROM `checklist` LEFT JOIN `checklist_subtab` ON `checklist`.`subtabid`=`checklist_subtab`.`subtabid` AND `checklist_subtab`.`deleted`=0 WHERE `assign_staff` LIKE '%,{$_GET['edit']},%' AND `checklist`.`deleted`=0");
...
while($checklist = mysqli_fetch_array($subtab_list)) {
	...
	case 'favourites':
		...
		break;
	case 'private':
		if($checklist['assign_staff'] == ",{$_GET['edit']},") {
			$visibility = "visible";
		}
		break;
	case 'shared':
	...
	echo "<a href='".$_SERVER['REQUEST_URI']."&view_checklist=".$checklist['checklistid']."' class='col-sm-6' data-subtab='{$checklist['name']}' data-users='$users' data-name='{$checklist['checklist_name']}' data-visible='$visibility'><li>";
