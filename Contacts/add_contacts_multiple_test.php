	...
	while( ( $csv = fgetcsv( $handle, 1024, "," ) ) !== false )
	{
		...
		if($values['businessid'] != '') {
			...
			if ( mysqli_num_rows ( $result_business ) > 0 ) {
                ...
                echo $query_insert_business;
			}
		}
var_dump($values['first_name']);
        ...
        if ( $c==0 ) {
            ...
            for ( $x=0; $x<count($fields); $x++ ) {
                if ($x==0) {
                    ...
                    /*$row['name'] = encryptIt($row['name']);
                    $row['first_name'] = encryptIt($row['first_name']);
                    $row['last_name'] = encryptIt($row['last_name']);
                    $row['email_address'] = encryptIt($row['email_address']);*/
