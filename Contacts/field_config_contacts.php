...
if (isset($_POST['add_tab'])) {
	...
	$contacts_tabs_tiles = implode(',', $_POST['contacts_tabs_tiles']);

	$get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(configid) AS configid FROM general_configuration WHERE name='".FOLDER_NAME."_tabs_tiles'"));

	if($get_config['configid'] > 0) {
		$query_update_employee = "UPDATE `general_configuration` SET value = '$contacts_tabs_tiles' WHERE name='".FOLDER_NAME."_tabs_tiles'";
		$result_update_employee = mysqli_query($dbc, $query_update_employee);
	} else {
		$query_insert_config = "INSERT INTO `general_configuration` (`name`, `value`) VALUES ('".FOLDER_NAME."_tabs_tiles', '$contacts_tabs_tiles')";
		$result_insert_config = mysqli_query($dbc, $query_insert_config);
	}
...
if (isset($_POST['add_general'])) {
    ...
    */
	...
	$get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(`configid`) AS `configid` FROM `general_configuration` WHERE name='".FOLDER_NAME."_".$field_category_lower."_field_subtabs'"));
	if($get_config['configid'] > 0) {
		$query_update_employee = "UPDATE `general_configuration` SET value='$field_tabs' WHERE name='".FOLDER_NAME."_".$field_category_lower."_field_subtabs'";
		$result_update_employee = mysqli_query($dbc, $query_update_employee);
	} else {
		$query_insert_config = "INSERT INTO `general_configuration` (`name`, `value`) VALUES ('".FOLDER_NAME."_".$field_category_lower."_field_subtabs', '$field_tabs')";
		$result_insert_config = mysqli_query($dbc, $query_insert_config);
	}
...
if (isset($_POST['con_filters'])) {
	...
	$get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT configcontactid,con_regions FROM field_config_contacts WHERE con_regions IS NOT NULL"));
	if($get_config['con_regions'] != '') {
		$configcontactid = $get_config['configcontactid'];
		$query_update_cont_config = "UPDATE `field_config_contacts` SET con_regions = '$con_regions' WHERE configcontactid=$configcontactid";
		$result_cont_config = mysqli_query($dbc, $query_update_cont_config);
	} else {
		$query_insert_config = "INSERT INTO field_config_contacts (`con_regions`) VALUES ('$con_regions')";
		$result_insert_config = mysqli_query($dbc, $query_insert_config);
	}

	$get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT configcontactid,con_locations FROM field_config_contacts WHERE con_locations IS NOT NULL"));
	if($get_config['con_locations'] != '') {
		$configcontactid = $get_config['configcontactid'];
		$query_update_cont_config = "UPDATE `field_config_contacts` SET con_locations = '$con_locations' WHERE configcontactid=$configcontactid";
		$result_cont_config = mysqli_query($dbc, $query_update_cont_config);
	} else {
		$query_insert_config = "INSERT INTO field_config_contacts (`con_locations`) VALUES ('$con_locations')";
		$result_insert_config = mysqli_query($dbc, $query_insert_config);
	}

	$get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT configcontactid,con_classifications FROM field_config_contacts WHERE con_classifications IS NOT NULL"));
	if($get_config['con_classifications'] != '') {
		$configcontactid = $get_config['configcontactid'];
		$query_update_cont_config = "UPDATE `field_config_contacts` SET con_classifications = '$con_classifications' WHERE configcontactid=$configcontactid";
		$result_cont_config = mysqli_query($dbc, $query_update_cont_config);
	} else {
		$query_insert_config = "INSERT INTO field_config_contacts (`con_classifications`) VALUES ('$con_classifications')";
		$result_insert_config = mysqli_query($dbc, $query_insert_config);
	}

	$get_config = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT configcontactid,con_title FROM field_config_contacts WHERE con_title IS NOT NULL"));
	if($get_config['con_title'] != '') {
		$configcontactid = $get_config['configcontactid'];
		$query_update_cont_config = "UPDATE `field_config_contacts` SET con_title = '$con_title' WHERE configcontactid=$configcontactid";
		$result_cont_config = mysqli_query($dbc, $query_update_cont_config);
	} else {
		$query_insert_config = "INSERT INTO field_config_contacts (`con_title`) VALUES ('$con_title')";
		$result_insert_config = mysqli_query($dbc, $query_insert_config);
	}

	echo '<script type="text/javascript"> window.location.replace("field_config_contacts.php?type=filters"); </script>';
}
...
<div class="panel-group" id="accordion2">
	...
	if($_GET['type'] == 'filters') {
		$filters = 'active_tab';
	}
	?>

	<div class="tab-container"><?php
		echo "<div class='pull-left tab'><span class='popover-examples list-inline'><a data-toggle='tooltip' data-placement='top' title='Remember to select the tab you would like to make changes to.'><img src='". WEBSITE_URL ."/img/info.png' width='20'></a></span><a href='field_config_contacts.php?type=tab&category=". $category ."'><button type='button' class='btn brand-btn mobile-block ".$active_tab."' >Tabs</button></a></div>";

		echo "<div class='pull-left tab'><span class='popover-examples list-inline'><a data-toggle='tooltip' data-placement='top' title='Click this to add the sub tabs in Contacts. This will help to organize your contacts by a specific business.'><img src='". WEBSITE_URL ."/img/info.png' width='20'></a></span><a href='field_config_contacts.php?type=general&category=". $category ."'><button type='button' class='btn brand-btn mobile-block ".$active_general."' >Sub Tabs</button></a></div>";

		echo "<div class='pull-left tab'><span class='popover-examples list-inline'><a data-toggle='tooltip' data-placement='top' title='Click this to add or remove so you can organize your own tabs.'><img src='". WEBSITE_URL ."/img/info.png' width='20'></a></span><a href='field_config_contacts.php?type=dashboard&category=". $category ."'><button type='button' class='btn brand-btn mobile-block ".$active_dashboard."' >Dashboard</button></a></div>";

		echo "<div class='pull-left tab'><span class='popover-examples list-inline'><a data-toggle='tooltip' data-placement='top' title='Click to add fields within the tabs, you must add a tab before adding fields. These determine which fields you want when creating a new type of contact.'><img src='". WEBSITE_URL ."/img/info.png' width='20'></a></span><a href='field_config_contacts.php?type=field&category=". $category ."'><button type='button' class='btn brand-btn mobile-block ".$active_field."' >Fields</button></a></div>";
		...
		echo "<div class='pull-left tab'><span class='popover-examples list-inline'><a data-toggle='tooltip' data-placement='top' title='Click this to enable/disable the ability to export a spreadsheet of your Contacts. This functionality also allows you to edit/add multiple contacts at a time by importing a spreadsheet into the software.'><img src='". WEBSITE_URL ."/img/info.png' width='20'></a></span><a href='field_config_contacts.php?type=filters&category=". $category ."'><button type='button' class='btn brand-btn mobile-block ".$filters."' >Filters</button></a></div>";
		?>
	</div>
	...
	if($_GET['type'] == 'tab') { ?>
		...
		<div class="panel-group" id="accordion_tabs">
			...
			<!-- Add Tabs From Tiles -->
			<?php
				$other_tile_names = mysqli_query($dbc, "SELECT distinct `tile_name` FROM `field_config_contacts` WHERE `tile_name` = 'clientinfo' OR `tile_name` = 'members'");
				$count_tile_names = mysqli_num_rows($other_tile_names);
				...
				$contacts_tabs_tiles = ','.$get_tabs_tiles_config.',';
				if ($count_tile_names > 0) {
			?>
			<div class="panel panel-default">
				<div class="panel-heading">
					<h4 class="panel-title">
						...
						<a data-toggle="collapse" data-parent="#accordion_tabs" href="#collapse_3">
							Add Tabs From Tiles<span class="glyphicon glyphicon-plus"></span>
						</a>
					</h4>
				</div>
				<div id="collapse_3" class="panel-collapse collapse">
					<div class="panel-body">
						Check off the Tiles that you want displayed as a Tab in the Contacts tile:<br />
						<br />
						<?php 
							while ($row = mysqli_fetch_array($other_tile_names)) {
								$tile = '';
								$tile_name = '';
								if ($row['tile_name'] == 'members') {
									$tile_name = $row['tile_name'];
									$tile = 'Members Tile';
								}
								if ($tile_name != '') {
									echo $tile . '<br />';
									$tile_tabs = mysqli_query($dbc, "SELECT distinct `tab` FROM `field_config_contacts` WHERE `tile_name` = '$tile_name'");
									while ($row_tab = mysqli_fetch_array($tile_tabs)) {
										$tab_name = $row_tab['tab'];
										echo '<input type="checkbox"'.(strpos($contacts_tabs_tiles, ','.$tile_name.'_'.$tab_name.',') !== FALSE ? ' checked' : '').' value="'.$tile_name.'_'.$tab_name.'"  style="height: 20px; width: 20px;" name="contacts_tabs_tiles[]">&nbsp;&nbsp;'.$tab_name.'&nbsp;&nbsp;';
									}
									echo '<br /><br />';
								}
							}
	...
	if($_GET['type'] == 'field') {
		...
		<div class="form-group">
            ...
            $subtab_config = get_config($dbc, FOLDER_NAME.'_'.$category.'_field_subtabs');
			...
			<div class="panel panel-default">
				...
				<div id="collapse_1" class="panel-collapse collapse">
					<div class="panel-body">
						...
						<input type="checkbox" <?php if (strpos($contacts_dashboard_config, ','."Prefix".',') !== FALSE) { echo " checked"; } ?> value="Prefix" style="height: 20px; width: 20px;" name="contacts_dashboard[]">&nbsp;&nbsp;Prefix
						...
						<input type="checkbox" <?php if (strpos($contacts_dashboard_config, ','."Staff Category".',') !== FALSE) { echo " checked"; } ?> value="Staff Category" style="height: 20px; width: 20px;" name="contacts_dashboard[]">&nbsp;&nbsp;Staff Category
			...
			<div class="panel panel-default">
				...
				<div id="collapse_3" class="panel-collapse collapse">
					<div class="panel-body"><?php
                        ...
                        Select Contact Category<br />
                        <select name="contact_field_category" class="chosen-select-deselect" data-placeholder="Choose Contact Category..." id="contact_field_category">
                            <option></option><?php
                            foreach ( $each_tab as $tab ) {
                                $selected = ( $category==$tab ) ? 'selected="selected"' : ''; ?>
                                <option value="<?= $tab ?>" <?= $selected; ?>><?= $tab ?></option><?php
                            } ?>
                        </select><br />
                        <br />
						...
						<br />
						<input name="contact_field_subtabs" id="contact_field_subtabs" type="text" value="<?php echo get_config($dbc, FOLDER_NAME.'_'.$category_lower.'_field_subtabs'); ?>" class="form-control"/><br />
...
<?php if($_GET['type'] == 'filters') { ?>
	<br><br>
	<div class="panel panel-default">
			<div class="panel-heading">
				<h4 class="panel-title">
					<span class="popover-examples list-inline"><a data-toggle="tooltip" data-placement="top" title="Click here to set the filters."><img src=" <?= WEBSITE_URL; ?>/img/info.png" width="20"></a></span>
					<a data-toggle="collapse" data-parent="#accordion_tabs" href="#collapse_2">
			...
			<div id="collapse_2" class="panel-collapse collapse in">
				<div class="panel-body">
					Add Regions separated by a comma in the order you want them on the regions listing:<br />
					<br />
					<?php
						$con_regions = $_POST['con_regions'];
						$get_config_values = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT con_regions FROM field_config_contacts WHERE `con_regions` IS NOT NULL"));
					?>
					<input name="con_regions" type="text" value="<?php echo str_replace(',,',',',str_replace('Staff','',$get_config_values['con_regions'])); ?>" class="form-control"/><br />
					Add Locations separated by a comma in the order you want them on the locations listing:<br />
					<br />
					<?php
						$con_locations = $_POST['con_locations'];
						$get_config_values = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT con_locations FROM field_config_contacts WHERE `con_locations` IS NOT NULL"));
					?>
					<input name="con_locations" type="text" value="<?php echo str_replace(',,',',',str_replace('Staff','',$get_config_values['con_locations'])); ?>" class="form-control"/><br />
					Add Classifications separated by a comma in the order you want them on the classifications listing:<br />
					<br />
					<?php
						$con_classifications = $_POST['con_classifications'];
						$get_config_values = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT con_classifications FROM field_config_contacts WHERE `con_classifications` IS NOT NULL"));
					?>
					<input name="con_classifications" type="text" value="<?php echo str_replace(',,',',',str_replace('Staff','',$get_config_values['con_classifications'])); ?>" class="form-control"/><br />
					Add Title separated by a comma in the order you want them on the title listing:<br />
					<br />
					<?php
						$con_title = $_POST['con_title'];
						$get_config_values = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT con_title FROM field_config_contacts WHERE `con_title` IS NOT NULL"));
					?>
					<input name="con_title" type="text" value="<?php echo str_replace(',,',',',str_replace('Staff','',$get_config_values['con_title'])); ?>" class="form-control"/><br />
				</div>
			</div>
			<div class="form-group">
				<div class="col-sm-6">
				...
				<div class="col-sm-6">
					<button	type="submit" name="con_filters" value="con_filters" class="btn config-btn btn-lg pull-right">Submit</button>
				</div>
			</div>
		</div><!-- .panel .panel-default -->
