...
function get_tabs_mc($tab = '')
{
	global $config_mc;

	$html = '';
	foreach($config_mc['tabs'] as $title => $url) {
		$active = '';
		if($title == $tab) {
			$active = 'active_tab';
		}
...
function get_all_inputs_mc($data) {
	global $config_mc;
	$fields = array();
...
function get_post_inputs_mc($data) {
	global $config_mc;
	$fields = array();
	foreach($data as $tabs) {
		foreach($tabs as $field) {
...
function get_post_uploads_mc($data) {
	global $config_mc;
	$fields = array();
...
function move_files_mc($files) {
	foreach($files as $file => $name) {
		move_uploaded_file($_FILES[$file]["tmp_name"], "download/". $_FILES[$file]["name"]);
	}
}

function prepare_insert_mc($ins_data = array(), $table = '') {
...
function prepare_update_mc($up_data = array(), $table = '', $key = '', $value = '') {
	$fields = array();
...
function get_field_mc($field, $value, $dbc = '')
{
	$html = '';
...
function call_staff_mc($dbc, $field, $value, $type = '', $disabled = '') {
