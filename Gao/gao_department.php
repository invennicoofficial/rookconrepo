...
<?php
    ...
    foreach ($each_tab as $cat_tab) {
        ...
        echo "<a href='gao.php?maintype=department&status=".$cat_tab."'><button type='button' class='btn brand-btn mobile-block  mobile-100 ".$active_to_be."'>".$cat_tab."</button></a>&nbsp;&nbsp";
        $totalCount++;
...
$status = $_GET['status'];
$query_check_credentials = "SELECT * FROM goals where type='department' and goal_timeline='$status' LIMIT $offset, $rowsPerPage";
$query = "SELECT count(*) as numrows FROM goals where type='department' and goal_timeline='$status'";
...
<div class="mobile-100-container">
	<div class="col-sm-12 col-xs-12 col-lg-12 pad-top offset-xs-top-20">
		<div class="pull-right">
			...
			<a href="add_gao.php?type=department" class="btn brand-btn mobile-block gap-bottom">Add Goal</a>
		</div>
	</div>
	<?php
	if($num_rows > 0) {
						...
						<div id="collapse_prep_<?php echo $row['goalid']; ?>" class="panel-collapse collapse <?php echo $in; $in = ''; ?>">
							<div class="panel-body">
								<div class="form-group">
									...
									<div class="col-sm-8">
										<strong><?php echo get_contact($dbc, $row['goal_setter'], 'name'); ?></strong>
									</div>
									<?php endif; ?>
									...
									<div class="col-sm-8">
										<strong>
											<?php $set_fors = explode(',', $row['goal_set_for']); ?>
											<?php foreach($set_fors as $set_for): ?>
												<?php if($row['goal_set_for'] == 0) echo "For All"; else echo get_contact($dbc, $set_for, 'name') . ', '; ?>
											<?php endforeach; ?>
										</strong>
									</div>
									<?php endif; ?>
