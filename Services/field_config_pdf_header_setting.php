...
?>
<?php
	$select_pdf_settings = mysqli_fetch_assoc(mysqli_query($dbc, "select count(pdfsettingid) as setting_count from services_pdf_setting where style = '$style_settings' and setting_type = 'header'"));
	...
	if(isset($_POST['pdf_setting'])) {
		...
		if($select_pdf_settings['setting_count'] <= 0) {	
			$insert_pdf_settings = "INSERT INTO services_pdf_setting(`style`,`font_size`,`font_type`,`font`,`pdf_logo`,`pdf_color`,`setting_type`,`text`,`alignment`) VALUES('$style_settings','$font_size','$font_type','$font','$pdf_logo','$pdf_color','header','$header_text','$alignment')";
			$result_pdf_settings = mysqli_query($dbc, $insert_pdf_settings);
		}
		else {
			$select_pdf = mysqli_fetch_assoc(mysqli_query($dbc, "select pdf_logo from services_pdf_setting where style = '$style_settings' and setting_type = 'header'"));
			if($pdf_logo == 'header_')
				$pdf_logo = $select_pdf['pdf_logo'];
			$update_pdf_settings = "UPDATE services_pdf_setting SET `style`='$style_settings',`font_size`='$font_size',`font_type`='$font_type',`font`='$font',`pdf_logo`='$pdf_logo',`pdf_color`='$pdf_color',`text`='$header_text',`alignment`='$alignment' where style = '$style_settings' and setting_type='header'";
	...
	$alignment = '';
	if($select_pdf_settings['setting_count'] > 0) {
		$select_pdf_settings = mysqli_fetch_assoc(mysqli_query($dbc, "select * from services_pdf_setting where style = '$style_settings' and setting_type='header'"));
