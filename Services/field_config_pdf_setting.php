...
?>
<?php
	$select_pdf_settings = mysqli_fetch_assoc(mysqli_query($dbc, "select count(pdfsettingid) as setting_count from services_pdf_setting where style = '$style_settings'"));
	if(isset($_POST['pdf_setting'])) {
		...
		if($select_pdf_settings['setting_count'] <= 0) {	
			$insert_pdf_settings = "INSERT INTO services_pdf_setting(`style`,`file_name`,`font_size`,`font_type`,`font`,`pdf_logo`,`pdf_size`,`page_ori`,`units`,`left_margin`,`right_margin`,`top_margin`,`header_margin`,`bottom_margin`,`pdf_color`) VALUES('$style_settings','$file_name','$font_size','$font_type','$font','$pdf_logo','$pdf_size','$page_ori','$units','$left_margin','$right_margin','$top_margin','$header_margin','$bottom_margin','$pdf_color')";
			$result_pdf_settings = mysqli_query($dbc, $insert_pdf_settings);
		}
		else {
			$select_pdf = mysqli_fetch_assoc(mysqli_query($dbc, "select pdf_logo from services_pdf_setting where style = '$style_settings'"));
			if($pdf_logo == '')
				$pdf_logo = $select_pdf['pdf_logo'];
			$update_pdf_settings = "UPDATE services_pdf_setting SET `style`='$style_settings',`file_name`='$file_name',`font_size`='$font_size',`font_type`='$font_type',`font`='$font',`pdf_logo`='$pdf_logo',`pdf_size`='$pdf_size',`page_ori`='$page_ori',`units`='$units',`left_margin`='$left_margin',`right_margin`='$right_margin',`top_margin`='$top_margin',`header_margin`='$header_margin',`bottom_margin`='$bottom_margin',`pdf_color`='$pdf_color' where style = '$style_settings'";
