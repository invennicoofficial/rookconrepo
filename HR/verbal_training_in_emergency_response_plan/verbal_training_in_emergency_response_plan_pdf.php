<?php
function verbal_training_in_emergency_response_plan_pdf($dbc,$hrid, $fieldlevelriskid) {
    ...
    $html = '<h2>PPE Requirements</h2>';
    $html .= html_entity_decode($hr_description);
    if($document != '') {
        $html .= 'Document Name(s)<br>';
        ...
        for($client_loop=0; $client_loop<=$total_count; $client_loop++) {
            ...
            $hazard = $task_item[1];
            if($task != '') {
                $html .= '<img src="../img/checkmark.png" width="10" height="10" border="0" alt="">&nbsp;&nbsp;'.$task . '<br>';
            }
        }
    }
    ...
    $html .= '<img src="verbal_training_in_emergency_response_plan/download/hr_'.$_SESSION['contactid'].'.png" width="150" height="70" border="0" alt="">';

    $pdf->writeHTML($html, true, false, true, false, '');
    $pdf->Output('verbal_training_in_emergency_response_plan/download/hr_'.$fieldlevelriskid.'.pdf', 'F');
    echo '<script type="text/javascript" language="Javascript">window.location.replace("?tile_name='.$tile.'");
    window.open("verbal_training_in_emergency_response_plan/download/hr_'.$fieldlevelriskid.'.pdf", "fullscreen=yes");
    </script>';

    unlink("verbal_training_in_emergency_response_plan/download/hr_".$_SESSION['contactid'].".png");
    echo '';
}
?>