...
function employment_agreement_pdf($dbc,$hrid, $fieldlevelriskid) {
    ...
    $html = '<h2>Employment Agreement</h2>';
    ...
    $email_body = str_replace("[Company Province]", $config_extra_fields[2], $email_body);
    $email_body = str_replace("[Company Plan Name]", $config_extra_fields[3], $email_body);
    $email_body = str_replace("[Employee Name]", $fields[0], $email_body);
    $email_body = str_replace("[Employee Position]", $fields[1], $email_body);
    $email_body = str_replace("[Employee Salary]", $fields[2], $email_body);
    $email_body = str_replace("[Employee Salary Type]", $fields[3], $email_body);
    $email_body = str_replace("[Employee Vacation]", $fields[4], $email_body);
    $email_body = str_replace("[Today Date]", $fields[5], $email_body);
    ...
    $html .= '<img src="employment_agreement/download/hr_'.$_SESSION['contactid'].'.png" width="150" height="70" border="0" alt="">';
    */
