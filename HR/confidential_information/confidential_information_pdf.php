<?php
function confidential_information_pdf($dbc,$hrid, $fieldlevelriskid) {
	...
	$get_field_level = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM hr_confidential_information WHERE fieldlevelriskid='$fieldlevelriskid'"));
    ...
    $html = '<h2>Confidential Information</h2>';
    ...
    $html .= '<br>Person : '.decryptIt($_SESSION['first_name']).' '.decryptIt($_SESSION['last_name']).'<br>';
	...
	<tr>
		<td>'.$fields[0].'</td>
		<td><img src="confidential_information/download/hr_'.$_SESSION['contactid'].'_sign2_.png" width="150" height="70" border="0" alt=""></td>
	</tr>
</table>';

	$html .= 'Person Signature<br>';
    $html .= '<img src="confidential_information/download/hr_'.$_SESSION['contactid'].'.png" width="150" height="70" border="0" alt="">';

    $pdf->writeHTML($html, true, false, true, false, '');
    $pdf->Output('confidential_information/download/hr_'.$fieldlevelriskid.'.pdf', 'F');
    echo '<script type="text/javascript" language="Javascript">window.location.replace("?tile_name='.$tile.'");
    window.open("confidential_information/download/hr_'.$fieldlevelriskid.'.pdf", "fullscreen=yes");
    </script>';

    unlink("confidential_information/download/hr_".$_SESSION['contactid'].".png");
    echo '';
}
?>