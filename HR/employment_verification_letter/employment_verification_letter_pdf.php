<?php
function employment_verification_letter_pdf($dbc,$hrid, $fieldlevelriskid) {
	...
	$get_field_level = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM hr_employment_verification_letter WHERE fieldlevelriskid='$fieldlevelriskid'"));
    ...
    $html = '<h2>Employment Verification Letter</h2>';
    ...
    $email_body = str_replace("[Today Date]", 'Date : '.$fields[3], $email_body);
    $email_body = str_replace("[Amount]", $fields[4], $email_body);
    $email_body = str_replace("[Work Type]", $fields[5], $email_body);

    $html .= $email_body;

    $html .= '<br><br><br><br><br>________________________________<br>Signature';
