 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        $query_insert_site = "INSERT INTO `hr_hr_complaint` (`hrid`, `today_date`, `contactid`, `fields`, `desc`, `desc1`, `desc2`) VALUES	('$hrid', '$today_date', '$contactid', '$fields', '$desc', '$desc1', '$desc2')";
        $result_insert_site	= mysqli_query($dbc, $query_insert_site);
        $fieldlevelriskid = mysqli_insert_id($dbc);
    } else {
        $fieldlevelriskid = $_POST['fieldlevelriskid'];
        $query_update_employee = "UPDATE `hr_hr_complaint` SET `fields` = '$fields', `desc` = '$desc', `desc1` = '$desc1', `desc2` = '$desc2' WHERE fieldlevelriskid='$fieldlevelriskid'";
        $result_update_employee = mysqli_query($dbc, $query_update_employee);
    }

    $img = sigJsonToImage($_POST['output']);
    imagepng($img, 'hr_complaint/download/hr_'.$_SESSION['contactid'].'.png');
    ...
    if($tab == 'Form') {
        ...
        include ('hr_complaint_pdf.php');
        echo hr_complaint_pdf($dbc,$hrid, $fieldlevelriskid);
