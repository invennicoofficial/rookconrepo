<?php
	function hr_complaint_pdf($dbc,$hrid, $fieldlevelriskid) {
	...
	//$result_update_employee = mysqli_query($dbc, "UPDATE `hr_hr_complaint` SET `status` = 'Done' WHERE fieldlevelriskid='$fieldlevelriskid'");

	$get_field_level = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM hr_hr_complaint WHERE fieldlevelriskid='$fieldlevelriskid'"));
	...
	$html = '<h2>HR Complaint</h2>'; // Form nu heading

	$html .= '<table border="1px" style="padding:3px; border:1px solid black;">';

    if (strpos(','.$form_config.',', ',fields1,') !== FALSE) {
	$html .= '<tr nobr="true">
            <th width="30%" style="background-color:lightgrey; color:black;">Date of filing of HR Complaint:</th>
            <td width="70%">'.$fields[0].'</td></tr>';
    }

    if (strpos(','.$form_config.',', ',fields2,') !== FALSE) {
	$html .= '<tr nobr="true">
            <th width="30%" style="background-color:lightgrey; color:black;">Full name of the person filing the complaint:</th>
            <td width="70%">'.$fields[1].'</td></tr>';
    }

    if (strpos(','.$form_config.',', ',fields4,') !== FALSE) {
	$html .= '<tr nobr="true">
            <th width="30%" style="background-color:lightgrey; color:black;">Nature of the complaint</th>
            <td width="70%">'.$fields[3].'</td></tr>';
    }

	if (strpos(','.$form_config.',', ',fields5,') !== FALSE) {
	$html .= '<tr nobr="true">
            <th width="30%" style="background-color:lightgrey; color:black;">Name of the person against whom the complaint is made:</th>
	...
	$html .= '<tr nobr="true">
            <th width="30%" style="background-color:lightgrey; color:black;">Main points of the allegation</th>
            <td width="70%">'.html_entity_decode($desc).'</td></tr>';


	$html .= '<tr nobr="true">
            <th width="30%" style="background-color:lightgrey; color:black;">The effect on the person filing the complaint</th>
            <td width="70%">'.html_entity_decode($desc1).'</td></tr>';

	$html .= '<tr nobr="true">
            <th width="30%" style="background-color:lightgrey; color:black;">Any other relevant information</th>
    ...
    $html .= '<img src="hr_complaint/download/hr_'.$_SESSION['contactid'].'.png" width="150" height="70" border="0" alt="">';

    $pdf->writeHTML($html, true, false, true, false, '');
    $pdf->Output('hr_complaint/download/hr_'.$fieldlevelriskid.'.pdf', 'F');
    echo '<script type="text/javascript" language="Javascript">window.location.replace("?tile_name='.$tile.'");
    window.open("hr_complaint/download/hr_'.$fieldlevelriskid.'.pdf", "fullscreen=yes");
    </script>';

    unlink("hr_complaint/download/hr_".$_SESSION['contactid'].".png");
