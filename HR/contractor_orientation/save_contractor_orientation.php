 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        $query_insert_site = "INSERT INTO `hr_contractor_orientation` (`hrid`, `today_date`, `contactid`, `fields`) VALUES	('$hrid', '$today_date', '$contactid', '$fields')";
        $result_insert_site	= mysqli_query($dbc, $query_insert_site);
        $fieldlevelriskid = mysqli_insert_id($dbc);
    } else {
        $fieldlevelriskid = $_POST['fieldlevelriskid'];
        $query_update_employee = "UPDATE `hr_contractor_orientation` SET `fields` = '$fields' WHERE fieldlevelriskid='$fieldlevelriskid'";
        $result_update_employee = mysqli_query($dbc, $query_update_employee);
    }

    $img = sigJsonToImage($_POST['output']);
    imagepng($img, 'contractor_orientation/download/hr_'.$_SESSION['contactid'].'.png');
    ...
    if($tab == 'Form') {
        ...
        include ('contractor_orientation_pdf.php');
        echo contractor_orientation_pdf($dbc,$hrid, $fieldlevelriskid);
