 <?php
    ...
    if(empty($_POST['fieldlevelriskid'])) {
        $query_insert_site = "INSERT INTO `hr_employee_accident_report_form` (`hrid`, `today_date`, `contactid`, `fields`, `desc`, `desc1`, `desc2`, `desc3`, `desc4`) VALUES	('$hrid', '$today_date', '$contactid', '$fields', '$desc', '$desc1', '$desc2', '$desc3', '$desc4')";
        $result_insert_site	= mysqli_query($dbc, $query_insert_site);
        $fieldlevelriskid = mysqli_insert_id($dbc);
    } else {
        $fieldlevelriskid = $_POST['fieldlevelriskid'];
        $query_update_employee = "UPDATE `hr_employee_accident_report_form` SET `fields` = '$fields', `desc` = '$desc', `desc1` = '$desc1', `desc2` = '$desc2', `desc3` = '$desc3', `desc4` = '$desc4' WHERE fieldlevelriskid='$fieldlevelriskid'";
        $result_update_employee = mysqli_query($dbc, $query_update_employee);
    }

    $img = sigJsonToImage($_POST['output']);
    imagepng($img, 'employee_accident_report_form/download/hr_'.$_SESSION['contactid'].'.png');
    ...
    if($tab == 'Form') {
        ...
        include ('employee_accident_report_form_pdf.php');
        echo employee_accident_report_form_pdf($dbc,$hrid, $fieldlevelriskid);
