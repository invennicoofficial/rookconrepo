<?php
function work_hours_policy_pdf($dbc,$hrid, $fieldlevelriskid) {
    ...
    $html .= '<img src="work_hours_policy/download/hr_'.$_SESSION['contactid'].'.png" width="150" height="70" border="0" alt="">';

    $pdf->writeHTML($html, true, false, true, false, '');
    $pdf->Output('work_hours_policy/download/hr_'.$fieldlevelriskid.'.pdf', 'F');
    echo '<script type="text/javascript" language="Javascript">window.location.replace("?tile_name='.$tile.'");
    window.open("work_hours_policy/download/hr_'.$fieldlevelriskid.'.pdf", "fullscreen=yes");
    </script>';

    unlink("work_hours_policy/download/hr_".$_SESSION['contactid'].".png");
    echo '';
}
?>