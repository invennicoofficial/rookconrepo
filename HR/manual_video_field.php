          <?php if($action == 'view') { ?>
           <div class="form-group">
            ...
            <div class="col-sm-8">
            <?php
                $get_doc = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(uploadid) AS total_id FROM hr_upload WHERE type='video' AND hrid='$hrid'"));

                if((!empty($_GET['hrid'])) && ($get_doc['total_id'] > 0)) {
                    $result = mysqli_query($dbc, "SELECT upload, uploadid FROM hr_upload WHERE type='video' AND hrid='$hrid'");
                ...
                <?php
                    ...
                    if($get_doc['total_id'] > 0) {
                        ...
                        while($row = mysqli_fetch_array($result)) {
                            $video = $row['upload'];
                            if($video != '') {
                                echo '<li><a href="download/'.$video.'" target="_blank">'.$video.'</a> - <a href="add_manual.php?action=delete&uploadid='.$row['uploadid'].'&hrid='.$hrid.'&type='.$type.'" onclick="return confirm(\'Are you sure?\')">Delete</a></li>';
                            }
                        }
                        echo '</ul>';
                    }
