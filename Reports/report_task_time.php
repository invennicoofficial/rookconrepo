...
if (isset($_POST['printpdf'])) {
	...
	class MYPDF extends TCPDF {
		...
		public function Header() {
            ...
            $footer_text = 'Task Time Tracking';
...
function report_receivables($dbc, $starttime, $endtime, $table_style, $table_row_style, $grand_total_style, $staffid) {
	...
	$result = mysqli_query($dbc, "SELECT * FROM (SELECT CONCAT('Project #',`tasklist`.`projectid`,': ',`tasklist`.`task_milestone_timeline`,' Task #',`tasklist`.`tasklistid`) time_type, `tasklist`.`heading` time_heading, `tasklist_time`.`contactid` time_staff, `tasklist_time`.`timer_date` time_date, '' time_start, '' time_end, `tasklist_time`.`work_time` time_length FROM `tasklist` RIGHT JOIN `tasklist_time` ON `tasklist`.`tasklistid`=`tasklist_time`.`tasklistid`) timers WHERE `time_length` != '' $clause ORDER BY `time_date`, `time_type`");
    $report_data = '';
