...
if (isset($_POST['printpdf'])) {
	...
	class MYPDF extends TCPDF {
		...
		public function Header() {
            ...
            $footer_text = 'Vendor Price List History';
	...
	//$pdf->writeHTML($html, true, false, true, false, '');
	$pdf->Output('Download/vpl_history_'.$today_date.'.pdf', 'F');
    track_download($dbc, 'history_vpl_history', 0, WEBSITE_URL.'/Reports/Download/vpl_history_'.$today_date.'.pdf', 'Vendor Price List History Report');

    ?>

	<script type="text/javascript" language="Javascript">
	window.open('Download/vpl_history_<?php echo $today_date;?>.pdf', 'fullscreen=yes');
	</script>
    <?php
...
function report_receivables($dbc, $starttime, $endtime, $table_style, $table_row_style, $grand_total_style) {
	...
	$query = mysqli_fetch_all(mysqli_query($dbc, "SELECT * FROM vendorpl_history where updated_at >= '$starttime' and updated_at < '$endtime'"));
    $odd_even = 0;
