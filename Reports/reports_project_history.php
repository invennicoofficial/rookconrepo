...
if (isset($_POST['printpdf'])) {
	...
	class MYPDF extends TCPDF {
		...
		public function Footer() {
			...
			// Projectition at 15 mm from bottom
			$this->SetY(-15);
            $this->SetFont('helvetica', 'I', 9);
	...
	//$pdf->writeHTML($html, true, false, true, false, '');
	$pdf->Output('Download/project_history_'.$today_date.'.pdf', 'F');
    track_download($dbc, 'history_project_history', 0, WEBSITE_URL.'/Reports/Download/project_history_'.$today_date.'.pdf', 'Point of Sale History Report');

    ?>

	<script type="text/javascript" language="Javascript">
	window.open('Download/project_history_<?php echo $today_date;?>.pdf', 'fullscreen=yes');
	</script>
    <?php
...
function report_receivables($dbc, $starttime, $endtime, $table_style, $table_row_style, $grand_total_style) {
	...
	$query = mysqli_fetch_all(mysqli_query($dbc, "SELECT * FROM project_history where updated_at >= '$starttime' and updated_at < '$endtime'"));
    $odd_even = 0;
...
function AddPlayTime($times) {
    // loop tprojectought all the times
    foreach ($times as $time) {
