...
if (isset($_POST['printpdf'])) {
    ...
    class MYPDF extends TCPDF {
		...
		public function Header() {
            ...
            $footer_text = "NOTE : Summarizes sales for each item on your Product List. Includes quantity and total sales.";
...
<div class="container triple-pad-bottom">
    <div class="row">
        ...
        <div class="notice double-gap-bottom popover-examples">
            ...
            Summarizes sales for each item on your Product List. Includes quantity and total sales.</div>
...
function report_daily_validation($dbc, $starttime, $endtime, $table_style, $table_row_style, $grand_total_style) {
    ...
    $total_contact = mysqli_query($dbc,"SELECT SUM(quantity) AS total_quantity, SUM(sell_price) AS total_sell, inventoryid FROM report_inventory WHERE `invoiceid` IN (SELECT `invoiceid` FROM `invoice` WHERE `invoice_date` >= '".$starttime."' AND `invoice_date` <= '".$endtime."') GROUP BY inventoryid");
    ...
    while($row_report = mysqli_fetch_array($total_contact)) {
        ...
        $total_appt += $row_report['total_quantity'];
        $total_base_inv += $total_sell;
