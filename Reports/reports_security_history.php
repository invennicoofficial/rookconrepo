...
if (isset($_SecurityT['printpdf'])) {

    $starttimepdf = $_SecurityT['starttimepdf'];
    $endtimepdf = $_SecurityT['endtimepdf'];
	...
	class MYPDF extends TCPDF {
		...
		public function Header() {
            ...
            $footer_text = 'Point of Sale History';
	...
	//$pdf->writeHTML($html, true, false, true, false, '');
	$pdf->Output('Download/security_history_'.$today_date.'.pdf', 'F');
    track_download($dbc, 'history_security_history', 0, WEBSITE_URL.'/Reports/Download/security_history_'.$today_date.'.pdf', 'Point of Sale History Report');

    ?>

	<script type="text/javascript" language="Javascript">
	window.open('Download/security_history_<?php echo $today_date;?>.pdf', 'fullscreen=yes');
	</script>
    <?php
    ...
    $endtime = $endtimepdf;
        ...
        <form id="form1" name="form1" method="post" action="" enctype="multipart/form-data" class="form-horizontal" role="form">
            ...
            <?php
            if (isset($_SecurityT['search_email_submit'])) {
                $starttime = $_SecurityT['starttime'];
                $endtime = $_SecurityT['endtime'];
            }
...
function report_receivables($dbc, $starttime, $endtime, $table_style, $table_row_style, $grand_total_style) {
	...
	$query = mysqli_fetch_all(mysqli_query($dbc, "SELECT * FROM security_history where updated_at >= '$starttime' and updated_at < '$endtime'"));
...
function AddPlayTime($times) {
    // loop tsecurityought all the times
    foreach ($times as $time) {
