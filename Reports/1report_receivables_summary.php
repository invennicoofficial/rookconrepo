...
if (isset($_POST['printpdf'])) {
	...
	class MYPDF extends TCPDF {
		...
		public function Header() {
            ...
            $footer_text = 'View Receivables by Insurer till <b>'.START_DATE.'</b>';
...
<div class="container triple-pad-bottom">
    <div class="row">
        ...
        <a href='1report_receivables_summary.php?type=Daily'><button type="button" class="btn brand-btn mobile-block active_tab" >Insurer Receivable Summary</button></a>&nbsp;&nbsp;
...
function report_receivables($dbc, $starttime, $endtime, $table_style, $table_row_style, $grand_total_style) {
    ...
    $report_service = mysqli_query($dbc,"SELECT DISTINCT(insurerid) FROM invoice_insurer ORDER BY insurerid");
    ...
    while($row_report = mysqli_fetch_array($report_service)) {
	    ...
	    $total_invoiced = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT SUM(insurer_price) AS total_invoiced FROM invoice_insurer WHERE insurerid = '$insurerid'"));
	    $total_paid = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT SUM(insurer_price) AS total_paid FROM invoice_insurer WHERE insurerid = '$insurerid' AND paid='Yes'"));
	    $total_due = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT SUM(insurer_price) AS total_due FROM invoice_insurer WHERE insurerid = '$insurerid' AND paid != 'Yes'"));
        ...
        $total_last30 = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT SUM(insurer_price) AS total_last30 FROM invoice_insurer WHERE (DATE(invoice_date) >= '".$last30."' AND DATE(invoice_date) <= '".$today_date."') AND paid != 'Yes' AND insurerid = '$insurerid'"));

        $total_last3059 = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT SUM(insurer_price) AS total_last3059 FROM invoice_insurer WHERE (DATE(invoice_date) >= '".$last60."' AND DATE(invoice_date) < '".$last30."') AND paid != 'Yes' AND insurerid = '$insurerid'"));

        $total_last6089 = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT SUM(insurer_price) AS total_last6089 FROM invoice_insurer WHERE (DATE(invoice_date) >= '".$last90."' AND DATE(invoice_date) < '".$last60."') AND paid != 'Yes' AND insurerid = '$insurerid'"));

        $total_last90119 = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT SUM(insurer_price) AS total_last90119 FROM invoice_insurer WHERE (DATE(invoice_date) >= '".$last120."' AND DATE(invoice_date) < '".$last90."') AND paid != 'Yes' AND insurerid = '$insurerid'"));

        $total_last120 = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT SUM(insurer_price) AS total_last120 FROM invoice_insurer WHERE (DATE(invoice_date) <= '".$last120."') AND paid != 'Yes' AND insurerid = '$insurerid'"));

        $insurerid = $row_report['insurerid'];

        $report_data .= '<tr nobr="true">';
        $report_data .= '<td>'.get_all_form_contact($dbc, $insurerid, 'name').'</td>';
        $report_data .= '<td>$'.$total_invoiced['total_invoiced'].'</td>';
        $report_data .= '<td>$'.$total_paid['total_paid'].'</td>';
        $report_data .= '<td>$'.$total_due['total_due'].'</td>';
        $report_data .= '<td>$'.$total_last30['total_last30'].'</td>';
