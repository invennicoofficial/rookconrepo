...
if (isset($_POST['printpdf'])) {
	...
	//$pdf->writeHTML($html, true, false, true, false, '');
	$pdf->Output('Download/sales_history_'.$today_date.'.pdf', 'F');
    track_download($dbc, 'history_sales_history', 0, WEBSITE_URL.'/Reports/Download/sales_history_'.$today_date.'.pdf', 'Sales History Report');

    ?>

	<script type="text/javascript" language="Javascript">
	window.open('Download/sales_history_<?php echo $today_date;?>.pdf', 'fullscreen=yes');
	</script>
    <?php
...
$query = mysqli_fetch_all(mysqli_query($dbc, "SELECT * FROM sales_history where updated_at >= '$starttime' and updated_at < '$endtime'"));
foreach($query as $rowid) {
      $changed_by = $rowid[6];
      $changed_date = $rowid[2];
      $change_details = $rowid[5];
      $cid = $row['contactid'];

      $report_data .= '<tr nobr="true">';
      $report_data .= '<td>'.$rowid[7].'</td>';
