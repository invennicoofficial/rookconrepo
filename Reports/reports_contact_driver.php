...
function report_receivables($dbc, $starttime, $endtime, $staff, $table_style, $table_row_style, $grand_total_style) {
    ...
    $report_validation = mysqli_query($dbc, "SELECT t.created_by, t.created_date, AVG(ta.rate) AS avg_rate FROM tickets t, ticket_attached ta WHERE (DATE(created_date) >= '".$starttime."' AND DATE(created_date) <= '".$endtime."') AND ta.rating > 0 GROUP BY `created_by`");
    ...
    while($row1 = mysqli_fetch_array($report_validation)) {
				...
				$report_data .= '<td>'.$row1['avg_rate'].'</td>';
                $report_data .= '<tr>';
    }
