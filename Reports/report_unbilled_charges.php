...
if (isset($_POST['printpdf'])) {
	...
	class MYPDF extends TCPDF {
		...
		public function Header() {
            ...
            $footer_text = 'Unbilled Invoices From <b>'.START_DATE.'</b> To <b>'.END_DATE.'</b>';
            $this->writeHTMLCell(0, 0, 0 , 35, $footer_text, 0, 0, false, "R", true);

            $this->setCellHeightRatio(1.30);
            $this->SetFont('helvetica', '', 10);
            $footer_text = "NOTE : Lists Customer Invoices/Transactions that have been started but not yet invoiced.";
	...
	$pdf->Output('Download/unbilled_charges_'.$today_date.'.pdf', 'F');
    track_download($dbc, 'report_unbilled_charges', 0, WEBSITE_URL.'/Reports/Download/unbilled_charges_'.$today_date.'.pdf', 'Unbilled Invoices Report');
    ?>

	<script type="text/javascript" language="Javascript">
	window.open('Download/unbilled_charges_<?php echo $today_date;?>.pdf', 'fullscreen=yes');
	</script>
    <?php
...
<div class="container triple-pad-bottom">
    <div class="row">
        ...
        <div class="notice double-gap-bottom popover-examples">
            ...
            Lists Customer Invoices/Transactions that have been started but not yet invoiced.</div>
...
function report_sales_summary($dbc, $starttime, $endtime, $table_style, $table_row_style, $grand_total_style) {
    ...
    $result5 = mysqli_query($dbc, "SELECT patientid,invoiceid,final_price, invoice_date, injuryid FROM invoice WHERE invoice_date >= '".$starttime."' AND invoice_date <= '".$endtime."' AND paid='Saved'");
