...
if (isset($_POST['printpdf'])) {
	...
	class MYPDF extends TCPDF {
		...
		public function Header() {
            ...
            $footer_text = 'View Receivables by Customer as at '.AS_AT_DATE.' Including Invoices '.(START_DATE > '0000-00-00' ? ' From <b>'.START_DATE.'</b> Until <b>'.END_DATE.'</b>' : 'Until <b>'.END_DATE.'</b>');
    ...
    track_download($dbc, 'report_receivables_patient_summary', 0, WEBSITE_URL.'/Reports/Download/ar_aging_summary_'.$today_date.'.pdf', 'View Receivables by Customer Report');
    ?>
    ...
    $as_at_date = $as_at_datepdf;
        ...
        <div class="notice double-gap-bottom popover-examples">
            ...
            This displays how much has been Invoiced, Paid and is Due from each Customer and how old it is (Current [under 30 days], 30-59 days, 60-89 days, 90-119 days, 120 + days) relative to the Report As At date. Clicking the customer name will redirect you to their Profile/Injury/Account dashboard. Clicking the outstanding $ amount will redirect you to their Accounts Receivable dashboard, where payment can be applied to the outstanding invoice.</div>
...
function report_receivables($dbc, $starttime, $endtime, $as_at_date, $table_style, $table_row_style, $grand_total_style) {
    ...
    while($row_report = mysqli_fetch_array($report_service)) {
        ...
        $total_30 = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT SUM(patient_price) AS `all_payment` FROM invoice_patient WHERE (DATE(invoice_date) >= '".$starttime."' AND DATE(invoice_date) <= '".$endtime."') AND (DATE(invoice_date) >= '".$last29."' AND DATE(invoice_date) <= '".$as_at_date."') AND patientid = '$patientid' AND (paid_date > '$as_at_date' OR `paid_date > '$as_at_date' OR IFNULL(`paid`,'') IN ('On Account',''))"));
    ...
    $report_data .= '<td><b>Total</b></td><td align="right"><b>$'.number_format($total1, 2).'</b></td><td align="right"><b>$'.number_format($total2, 2).'</b></td><td align="right"><b>$'.number_format($total3, 2).'</b></td><td align="right"><b>$'.number_format($total4, 2).'</b></td><td align="right"><b>$'.number_format($total5, 2).'</b></td><td align="right"><b>$'.number_format($total6, 2).'</b></td><td align="right"><b>$'.number_format($total7, 2).'</b></td><td><b>$'.number_format($total8, 2).'</b></td>';
    $report_data .= "</tr>";
    $report_data .= '</table>';
