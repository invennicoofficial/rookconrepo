...
if (isset($_POST['printpdf'])) {
	...
	//$pdf->writeHTML($html, true, false, true, false, '');
	$pdf->Output('Download/hr_history_'.$today_date.'.pdf', 'F');
    track_download($dbc, 'history_hr_history', 0, WEBSITE_URL.'/Reports/Download/hr_history_'.$today_date.'.pdf', 'HR History Report');

    ?>

	<script type="text/javascript" language="Javascript">
	window.open('Download/hr_history_<?php echo $today_date;?>.pdf', 'fullscreen=yes');
	</script>
    <?php
...
function report_receivables($dbc, $starttime, $endtime, $table_style, $table_row_style, $grand_total_style) {
	...
	$query = mysqli_fetch_all(mysqli_query($dbc, "SELECT * FROM hr_history where updated_at >= '$starttime' and updated_at < '$endtime'"));
