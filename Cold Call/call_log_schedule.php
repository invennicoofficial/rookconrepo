<?php
    ...
    foreach ($each_tab as $cat_tab) {
        ...
        // echo "<a href='call_log.php?maintype=schedule&status=".$cat_tab."'><button type='button' class='btn brand-btn mobile-block  mobile-100 ".$active_to_be."'>".$cat_tab."</button></a>&nbsp;&nbsp";
        $totalCount++;
        ...
        $notes = mysqli_fetch_assoc(mysqli_query($dbc, "SELECT note FROM notes_setting WHERE subtab='cc_schedule'"));
        $note = $notes['note'];
        if ( !empty($note) ) { ?>
        ...
        <form name="form_sites" method="post" action="" class="form-inline" role="form">
            <div class="pad-top pad-bottom clearfix">
                ...
                <?php if($_GET['status'] == 'Custom') { ?>
                    <div class="col-lg-2 col-md-3 col-sm-4 col-xs-4" style='max-width:200px;'>
                    <label for="search_site" style='width:100%; text-align: center;'>By Reminder Date:</label>
                    </div>
                ...
                <select data-placeholder="Choose a Status..." name="search_status" class="chosen-select-deselect form-control" width="380">
                  <option value=""></option>
                  <?php
                    $tabs = get_config($dbc, 'calllog_schedule_status');
                    $each_tab = explode(',', $tabs);
                    foreach ($each_tab as $cat_tab) {
            ...
            if($_GET['status'] == 'Today') {
                $query_check_credentials = "SELECT * FROM calllog_pipeline WHERE new_reminder=DATE(NOW()) $add_query LIMIT $offset, $rowsPerPage";
                $query = "SELECT count(*) as numrows FROM calllog_pipeline WHERE new_reminder=DATE(NOW()) $add_query";
            }
            if($_GET['status'] == 'This Week') {
                $query_check_credentials = "SELECT * FROM calllog_pipeline WHERE WEEKOFYEAR(new_reminder)=WEEKOFYEAR(NOW()) $add_query  LIMIT $offset, $rowsPerPage";
                $query = "SELECT count(*) as numrows FROM calllog_pipeline WHERE WEEKOFYEAR(new_reminder)=WEEKOFYEAR(NOW()) $add_query";
            }
            if($_GET['status'] == 'This Month') {
                $query_check_credentials = "SELECT * FROM calllog_pipeline WHERE YEAR(new_reminder) = YEAR(NOW()) AND MONTH(created_date)=MONTH(NOW()) $add_query LIMIT $offset, $rowsPerPage";
                $query = "SELECT count(*) as numrows FROM calllog_pipeline WHERE YEAR(new_reminder) = YEAR(NOW()) AND MONTH(created_date)=MONTH(NOW()) $add_query";
            }
            if($_GET['status'] == 'Custom') {
                if($search_date == '') {
                    $query_check_credentials = "SELECT * FROM calllog_pipeline WHERE new_reminder=DATE(NOW()) $add_query LIMIT $offset, $rowsPerPage";
                    ...
                    $query_check_credentials = "SELECT * FROM calllog_pipeline WHERE new_reminder='$search_date' $add_query LIMIT $offset, $rowsPerPage";
                    $query = "SELECT count(*) as numrows FROM calllog_pipeline WHERE new_reminder='$search_date' $add_query";
                }
            }
