...
<form name="form_sites" method="post" action="" class="form-inline" role="form">
    ...
    if(!empty($_GET['status'])) {
        ...
        $days5ago = date('Y-m-d', time() - 3 * SECONDS_PER_DAY) . ' ' . date('H:m:s');
        $query_check_credentials = "SELECT * FROM `calllog_pipeline` WHERE updated_at < '$days5ago' $add_query LIMIT $offset, $rowsPerPage";
        $query = "SELECT count(*) as numrows FROM calllog_pipeline WHERE updated_at < '$days5ago' $add_query";
    } else {
    ...
    if($num_rows > 0) {
        ...
        echo "</tr>";
    } else { echo $query_check_credentials;
        echo "<h2>No Record Found.</h2>";
    }

    while($row = mysqli_fetch_array( $result ))
    {
        $query_update_calllog = "UPDATE `calllog_pipeline` set status = 'Abandoned' where calllogid = " . $row['calllogid'];
        $result_update_calllog = mysqli_query($dbc, $query_update_calllog);
