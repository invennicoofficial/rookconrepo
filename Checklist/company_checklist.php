...
<div class="container">
    ...
    <h1 class="single-pad-bottom pull-left">Company Checklists</h1>
    ...
    <div class="notice double-gap-bottom popover-examples">
        <div class="col-sm-1 notice-icon"><img src="<?= WEBSITE_URL; ?>/img/info.png" class="wiggle-me" width="25"></div>
        <div class="col-sm-11"><span class="notice-name">NOTE:</span>
        You will find the checklists that have been shared with you in this sub tab. You must share the checklist with someone in order for them to view it under their Company Checklists sub tab.</div>
        <div class="clearfix"></div>
    </div>
	...
	if(config_visible_function($dbc, 'checklist') == 1) {
		echo '<br /><div class="pull-right">';
			...
			echo '<a href="field_config.php?from_url=company_checklist.php" class="mobile-block"><img style="width: 50px;" title="Tile Settings" src="../img/icons/settings-4.png" class="settings-classic wiggle-me"></a>';
		echo '</div>';
	}
		...
		if(strpos($tab_config, 'my_') !== false) {
			echo "
				<div class='pull-left tab tab-nomargin'>
					...
					<a href='my_checklist.php'><button type='button' class='btn brand-btn mobile-block mobile-100'>My Checklists (".$list_count['mylists'].")</button></a>
				</div>";
		}
		if(strpos($tab_config, 'company_') !== false) {
			echo "
				<div class='pull-left tab tab-nomargin'>
					...
					<a href='company_checklist.php'><button type='button' class='btn brand-btn mobile-block active_tab mobile-100'>Company Checklists (".$list_count['companylists'].")</button></a>
				</div>";
		} else {
			echo "<script> window.location.replace('project_checklist.php'); </script>";
		}
    ...
    $active_monthly = '';
	if($type == '') {
		$type = trim(explode('company_',$tab_config)[1],',');
	}
	if($type == 'ongoing') {
	...
	echo '<div class="tab-container">';
		if(strpos($tab_config, 'company_ongoing') !== false) {
			echo "
				<div class='pull-left tab tab-nomargin'>
					...
					<a href='company_checklist.php?type=ongoing'><button type='button' class='btn brand-btn mobile-block ".$active_on."'>Ongoing (".$list_count['companyongoing'].")</button></a>
				</div>";
		}
		if(strpos($tab_config, 'company_daily') !== false) {
			echo "
				<div class='pull-left tab tab-nomargin'>
					...
					<a href='company_checklist.php?type=daily'><button type='button' class='btn brand-btn mobile-block ".$active_daily."'>Daily (".$list_count['companydaily'].")</button></a>
				</div>";
		}
		if(strpos($tab_config, 'company_weekly') !== false) {
			echo "
				<div class='pull-left tab tab-nomargin'>
					...
					<a href='company_checklist.php?type=weekly'><button type='button' class='btn brand-btn mobile-block ".$active_weekly."'>Weekly (".$list_count['companyweekly'].")</button></a>
				</div>";
		}
		if(strpos($tab_config, 'company_monthly') !== false) {
			echo "
				<div class='pull-left tab tab-nomargin'>
					...
					<a href='company_checklist.php?type=monthly'><button type='button' class='btn brand-btn mobile-block ".$active_monthly."'>Monthly (".$list_count['companymonthly'].")</button></a>
				</div>";
		}
	echo '</div><div class="clearfix"></div><br />';
    ...
    <form name="form_sites" method="post" action="" class="form-inline" role="form">
        ...
        $result = mysqli_query($dbc, "SELECT * FROM checklist WHERE deleted = 0 AND security = 'Company Checklist' AND checklist_type='$type' AND assign_staff LIKE '%," . $contactid . ",%'");
        ...
        while($row = mysqli_fetch_array($result)) {
            ...
            echo "<a href='company_checklist.php?type=".$type."&checklistid=".$row['checklistid']."'><button type='button' class='mobile-100 btn brand-btn mobile-block ".$active_daily."' >".$row['checklist_name']."</button></a>&nbsp;&nbsp;";
        }
        ?>
        <br><br>
		</div>
        ...
        if(!empty($_GET['checklistid'])) {
            ...
            echo '<a href="add_checklist.php?checklistid='.$checklistid.'" class="btn brand-btn mobile-block gap-bottom pull-right">Edit</a>';

            echo '<a href=\'../delete_restore.php?action=delete&remove_checklist=all&subtab=company&type='.$_GET['type'].'&checklistid='.$checklistid.'\' onclick="return confirm(\'Are you sure?\')" class="btn brand-btn mobile-block gap-bottom pull-right">Archive</a>';
            ...
            echo '<button type="submit" name="export_pdf" value="Submit" class="btn brand-btn pull-right">Export</button>';
            ...
            while($row = mysqli_fetch_array( $result )) {
                //echo '<li id="'.$row['tasklistid'].'" class="ui-state-default '.$class_on.'"><span class="iframe_open_add" id="'.$row['status'].'_'.$row['tasklistid'].'" style="cursor:pointer;"><input type="checkbox" onclick="checklistChange(this);" value="'.$row['checklistnameid'].'" style="height: 20px; width: 20px;" name="checklistnameid[]">&nbsp;&nbsp;'.$row['checklist']. '</span>';

                echo '<li id="'.$row['checklistnameid'].'" class="ui-state-default" '.($row['flag_colour'] == '' ? '' : 'style="background-color: #'.$row['flag_colour'].';"').'>';
				echo '<span style="cursor:pointer; font-size: 25px;"><input type="checkbox" onclick="checklistChange(this);" value="'.$row['checklistnameid'].'" style="height: 30px; width: 30px;" name="checklistnameid[]">';
				...
				echo '<img class="drag_handle pull-right" src="'.WEBSITE_URL.'/img/icons/hold.png" style="height:30px; width:30px;" /></span>';
            ...
            while($row = mysqli_fetch_array( $result )) {
                ...
                echo '<li id="'.$row['checklistnameid'].'" class="ui-state-default no-sort"><span style="cursor:pointer; font-size: 20px;"><input type="checkbox" onclick="checklistChange(this);" checked value="'.$row['checklistnameid'].'" style="height: 30px; width: 30px;" name="checklistnameid[]">&nbsp;&nbsp;'.html_entity_decode($row['checklist']).$info;
				$documents = mysqli_query($dbc, "SELECT * FROM checklist_name_document WHERE checklistnameid='".$row['checklistnameid']."'");
				while($doc = mysqli_fetch_array($documents)) {
					echo '<br /><a href="download/'.$doc['document'].'">'.$doc['document'].' (Uploaded by '.get_staff($dbc, $doc['created_by']).' on '.$doc['created_date'].')</a>';
				}
