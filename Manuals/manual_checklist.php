...
<?php
function manual_checklist($dbc, $td_height, $img_height, $img_width, $type, $category) {
    ?>
	...
	if(strpos(ROLE,',super,') !== false || strpos(ROLE,',staff,') !== false || strpos(ROLE,',staff,') !== false || strpos(ROLE,',staff,') !== false || strpos(ROLE,',admin,') !== false)
		$status = '';
	else
		$status = 'status > 0 AND ';		
				
    $result = mysqli_query($dbc, "SELECT * FROM manuals WHERE deleted = 0 AND manual_type='$type' AND $status category='$category' ORDER BY category, INET_ATON(heading_number), INET_ATON(sub_heading_number)");
    ...
    if(mysqli_num_rows($result) > 0) {
        ...
        <tr>
            ...
            <td height="<?php echo $td_height;?>" width="">
                <?php
                    echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='add_manual.php?manualtypeid=".$manualtypeid."&type=".$type."&action=view'>".$row['sub_heading_number'].'&nbsp;&nbsp;'.$row['sub_heading']."</a>";
                ?>&nbsp;&nbsp;
            </td>
            <?php } else { ?>
            <td height="<?php echo $td_height;?>" width="">
                <?php
                    echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='add_manual.php?manualtypeid=".$manualtypeid."&type=".$type."&action=view'>".$row['third_heading_number'].'&nbsp;&nbsp;'.$row['third_heading']."</a>";
                ?>&nbsp;&nbsp;
            </td>
            <?php } ?>
            ...
            <td height="<?php echo $td_height;?>" width="2%">
                <?php
                    echo "<a href='add_manual.php?manualtypeid=".$manualtypeid."&type=".$type."&action=config'>Configure/Edit</a> | <a href=\"".WEBSITE_URL."/delete_restore.php?action=delete&type=".$type."&category=".$_GET['category']."&manualtypeid=".$manualtypeid."\" onclick=\"return confirm('Are you sure?')\">Archive</a>";
                ?>&nbsp;&nbsp;
            </td>
    		 
                <?php
    				if(strpos(ROLE,',super,') !== false || strpos(ROLE,',staff,') !== false || strpos(ROLE,',staff,') !== false || strpos(ROLE,',staff,') !== false || strpos(ROLE,',admin,') !== false) { ?>
    					<td height="<?php echo $td_height;?>" width="2%" style="padding-top: 22px !important;">
    					<?php
    						if($row['status'] == 1) {
    							$inactive = "<a href='add_manual.php?manualtypeid=".$manualtypeid."&type=".$type."&action=status&value=0'>Deactivate</a>";
    							$active = 'Active';
    						}
    						else {
    							$active = "<a href='add_manual.php?manualtypeid=".$manualtypeid."&type=".$type."&action=status&value=1'>Active</a>";
