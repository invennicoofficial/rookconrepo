...
<?php
function manual_checklist($dbc, $td_height, $img_height, $img_width, $category, $tab = '') {
    ?>
    ...
    $result = mysqli_query($dbc, "SELECT * FROM patientform WHERE deleted = 0 AND IFNULL(`tab`,'')='$tab' AND category='$category' ORDER BY category, lpad(heading_number, 100, 0), lpad(sub_heading_number, 100, 0)");
	...
	if(mysqli_num_rows($result) > 0) {
		while($row = mysqli_fetch_array($result)) {
			...
			<tr>
				...
				<td height="<?php echo $td_height;?>" width="20%">
					<?php
						echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='add_manual.php?patientformid=".$patientformid."&action=view'>".$row['sub_heading_number'].'&nbsp;&nbsp;'.$row['sub_heading']."</a>";
					?>&nbsp;&nbsp;
				</td>
				<?php } else { ?>
				<td height="<?php echo $td_height;?>" width="20%">
					<?php
						echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='add_manual.php?patientformid=".$patientformid."&action==view'>".$row['third_heading_number'].'&nbsp;&nbsp;'.$row['third_heading']."</a>";
					?>&nbsp;&nbsp;
				</td>
				<?php } ?>
				...
				<td height="<?php echo $td_height;?>" width="7%">
					<?php
						echo "<a href='add_manual.php?patientformid=".$patientformid."&action=config'>Configure/Edit</a> |";
						echo '<a href="'.WEBSITE_URL.'/delete_restore.php?action=delete&patientformid='.$patientformid.'" onclick="return confirm(\'Are you sure?\')">Archive</a></li>';
					?>&nbsp;&nbsp;
				</td>
			</tr>
		<?php }
		...
		echo "<h3 class='triple-gap-bottom'>No forms have been configured for this section.</h3></table><br /><br />";
	} ?>
<?php }
?>