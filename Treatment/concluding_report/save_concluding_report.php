 <?php
    ...
    $query_insert_site = "INSERT INTO `patientform_concluding_report` (`patientformid`, `today_date`, `fields_value`) VALUES	('$patientformid', '$today_date', '$fields_value')";

    $result_insert_site	= mysqli_query($dbc, $query_insert_site);
    $fieldlevelriskid = mysqli_insert_id($dbc);

    $form_name = get_formid_from_patientform($dbc, $patientformid, 'form');
    $pdf_path = 'concluding_report/download/patientform_'.$fieldlevelriskid.'.pdf';
    ...
    imagepng($img, 'concluding_report/download/sign_'.$fieldlevelriskid.'.png');

    include ('concluding_report_pdf.php');
    echo concluding_report_pdf($dbc,$patientformid, $fieldlevelriskid);

    echo '<script type="text/javascript">
        window.location.replace("patientform.php?tab=Form");
        window.open("concluding_report/download/patientform_'.$fieldlevelriskid.'.pdf", "fullscreen=yes"); </script>';
