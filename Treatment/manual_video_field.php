          <?php if($action == 'view') { ?>
           <div class="form-group">
            ...
            <div class="col-sm-8">
            <?php
                $get_doc = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT COUNT(uploadid) AS total_id FROM patientform_upload WHERE type='video' AND patientformid='$patientformid'"));

                if((!empty($_GET['patientformid'])) && ($get_doc['total_id'] > 0)) {
                    $result = mysqli_query($dbc, "SELECT upload, uploadid FROM patientform_upload WHERE type='video' AND patientformid='$patientformid'");
                ...
                <?php
                    ...
                    if($get_doc['total_id'] > 0) {
                        ...
                        while($row = mysqli_fetch_array($result)) {
                            $video = $row['upload'];
                            if($video != '') {
                                echo '<li><a href="download/'.$video.'" target="_blank">'.$video.'</a> - <a href="add_manual.php?action=delete&uploadid='.$row['uploadid'].'&patientformid='.$patientformid.'&type='.$type.'" onclick="return confirm(\'Are you sure?\')">Delete</a></li>';
                            }
                        }
                        echo '</ul>';
                    }
                ...
                ?>
                    ...
                    <div class="form-group triple-gapped clearfix">
                        <div class="col-sm-offset-4 col-sm-8">
                            <button id="add_row_videos" class="btn brand-btn pull-left">Add More Video</button>
                        </div>
                    </div>
                </div>
            </div>
