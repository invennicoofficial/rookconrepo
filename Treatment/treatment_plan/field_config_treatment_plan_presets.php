...
<?php if (strpos(','.$fields.',', ',fields12,') !== FALSE) { ?>
	<p>Diagnosis</p>
	<?php $result = mysqli_query($dbc, "SELECT * FROM `field_config_treatment_presets` WHERE `form`='$form' AND `field`='fields12' UNION SELECT '', '', '', ''");
...
<?php } ?>
<?php if (strpos(','.$fields.',', ',fields13,') !== FALSE) { ?>
	...
	<button class="btn brand-btn" onclick="add_line(this);return false;">Add Line</button>
<?php } ?>
<?php if (strpos(','.$fields.',', ',fields14,') !== FALSE) { ?>
	<p>C/O</p>
	<?php $result = mysqli_query($dbc, "SELECT * FROM `field_config_treatment_presets` WHERE `form`='$form' AND `field`='fields14' UNION SELECT '', '', '', ''");
	while($row = mysqli_fetch_array($result)) { ?>
		<div class="form-group"><input type="hidden" name="preset_id[]" value=""><input type="hidden" name="preset_field[]" value="fields14"><input type="text" name="preset_value[]" value="<?php echo html_entity_decode($row['preset_text']); ?>" class="form-control">
			<button onclick="$(this).closest('.form-group').remove();return false;" class="btn brand-btn">Delete</button></div>
	<?php } ?>
	<button class="btn brand-btn" onclick="add_line(this);return false;">Add Line</button>
<?php } ?>
<?php if (strpos(','.$fields.',', ',fields15,') !== FALSE) { ?>
	<p>O/E</p>
	<?php $result = mysqli_query($dbc, "SELECT * FROM `field_config_treatment_presets` WHERE `form`='$form' AND `field`='fields15' UNION SELECT '', '', '', ''");
	while($row = mysqli_fetch_array($result)) { ?>
		<div class="form-group"><input type="hidden" name="preset_id[]" value=""><input type="hidden" name="preset_field[]" value="fields15"><input type="text" name="preset_value[]" value="<?php echo html_entity_decode($row['preset_text']); ?>" class="form-control">
			<button onclick="$(this).closest('.form-group').remove();return false;" class="btn brand-btn">Delete</button></div>
	<?php } ?>
	<button class="btn brand-btn" onclick="add_line(this);return false;">Add Line</button>
<?php } ?>