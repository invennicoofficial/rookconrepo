				...
				<tr>
					...
					<?php if(count($region_list) > 0) { ?>
						<td data-title="Allowed Regions">
							<select name="role[]" multiple id="<?= $row['contactid'] ?>" data-placeholder="Select Allowed Regions" class="role chosen-select-deselect form-control">
								<option value=""></option><?php foreach($region_list as $value) { ?>
									<option <?= empty($region_access) || in_array($value, $region_access) ? 'selected' : '' ?> value="<?= $value ?>"><?= $value ?></option>
								<?php } ?>
							</select>
						</td>
					<?php } ?>
					<?php if(count($location_list) > 0) { ?>
						<td data-title="Allowed Locations">
							<select name="role[]" multiple id="<?= $row['contactid'] ?>" data-placeholder="Select Allowed Locations" class="role chosen-select-deselect form-control">
								<option value=""></option><?php foreach($location_list as $value) { ?>
									<option <?= empty($location_access) || in_array($value, $location_access) ? 'selected' : '' ?> value="<?= $value ?>"><?= $value ?></option>
								<?php } ?>
							</select>
						</td>
					<?php } ?>
					<?php if(count($classification_list) > 0) { ?>
						<td data-title="Allowed Classifications">
							<select name="role[]" multiple id="<?= $row['contactid'] ?>" data-placeholder="Select Allowed Classifications" class="role chosen-select-deselect form-control">
								<option value=""></option><?php foreach($classification_list as $value) { ?>
									<option <?= empty($classification_access) || in_array($value, $classification_access) ? 'selected' : '' ?> value="<?= $value ?>"><?= $value ?></option>
								<?php } ?>
							</select>
						</td>
					<?php } ?>
					<?php if(count($position_list) > 0) { ?>
						<td data-title="Allowed Positions">
							<select name="role[]" multiple id="<?= $row['contactid'] ?>" data-placeholder="Select Allowed Positions" class="role chosen-select-deselect form-control">
								<option value=""></option><?php foreach($position_list as $value) { ?>
									<option <?= empty($allowed_positions) || in_array($value['position_id'], $allowed_positions) ? 'selected' : '' ?> value="<?= $value['position_id'] ?>"><?= $value['name'] ?></option>
								<?php } ?>
							</select>
						</td>
					<?php } ?>
