				...
				<div class="form-group  mobile-100 cate_fitter" style='width:100%; '>
					<select name="search_category" class="chosen-select-deselect form-control category_actual" onchange="location=this.value;">
                        ...
                        while($row = mysqli_fetch_assoc($sql)){
                            ...
                            echo '<option value="inventory.php?category='.$cat_tab.$order_list.'&contactid='.$contactid.'&subtab=Price Lists" '.$active_daily.'>'.$cat_tab.'</option>';
                        }
                      ?>
                    </select>
				</div><?php
                ...
                */
				...
				if (isset($_GET['order_list']) ) {
                    if ($currentlist == 'on' && $inventoryidorder !== '') {
                        ...
                        $query = "SELECT COUNT(*) AS `numrows` FROM `vendor_price_list` WHERE `vendorid`='$contactid' AND `deleted`=0 ORDER BY `inventoryid` DESC";
                    } else {
                        $category = $_GET['category'];
