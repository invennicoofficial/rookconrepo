...
if (isset($_POST['submit_general'])) {
    ...
    if($_POST['jobs_path_milestone'] == 'NEW') {
        $query_insert_config = "INSERT INTO `jobs_path_milestone` (`project_path`, `milestone`, `timeline`, `checklist`, `ticket`, `workorder`)
			VALUES ('$project_path', '$milestone', '$timeline', '$checklist', '$ticket', '$workorder')";
        $result_insert_config = mysqli_query($dbc, $query_insert_config);
    } else {
        $jobs_path_milestone = $_POST['jobs_path_milestone'];
        $query_update_vendor = "UPDATE `jobs_path_milestone` SET `project_path` = '$project_path', `milestone` = '$milestone',`timeline` = '$timeline', `checklist` = '$checklist',
...
<div class="mobile-100-container">
	...
	<span class="nav-subtab">
		...
		<a href="field_config_jobs_path_milestone.php"><button type="button" class="btn brand-btn mobile-block mobile-100 active_tab">Project Template</button></a>&nbsp;&nbsp;
	</span>
	<?php foreach($jobs_tabs as $key => $tab_name): ?>
...
if(!empty($_GET['jobs_path_milestone'])) {
    $jobs_path_milestone = $_GET['jobs_path_milestone'];
    $get_contact = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM jobs_path_milestone WHERE jobs_path_milestone='$jobs_path_milestone'"));
...
?>
<input type="hidden" id="jobs_path_milestone" name="jobs_path_milestone" value="<?php echo $jobs_path_milestone ?>" />
<?php   }      ?>
<?php if(!empty($_GET['jobs_path_milestone'])): ?>
	<div class="panel-group" id="accordion2">
		<div class="panel panel-default">
			<div class="panel-heading">
				<h4 class="panel-title">
					<a data-toggle="collapse" data-parent="#accordion2" href="#collapse_tilename" >
						<?php if (JOBS_TILE=='Projects') { echo "Project"; } else { echo JOBS_TILE; } ?> Template<span class="glyphicon glyphicon-plus"></span>
					</a>
				</h4>
			</div>
			<div id="collapse_tilename" class="panel-collapse collapse">
				<div class="panel-body">
					  <div class="form-group">
						<label for="fax_number"	class="col-sm-4	control-label"><?php if (JOBS_TILE=='Projects') { echo "Project"; } else { echo JOBS_TILE; } ?> Template Name:</label>
						<div class="col-sm-8">
				...
				<div class="additional_row">
					...
					<div class="form-group clearfix mt" id="mt_0">
						...
						<div class="col-sm-2 m-top-mbl" >
							...
							<a href="#" onclick="addChecklistItem(this); return false;" id="add_checklist_item_0" class="btn brand-btn">Add Checklist</a>
						</div>
					</div>
				</div>
...
<?php else:  ?>
	...
	<?php $query_check_credentials = "SELECT * FROM jobs_path_milestone ORDER BY `project_path`";
	$result = mysqli_query($dbc, $query_check_credentials);
	echo "<table class='table table-bordered'>";
	echo "<tr class='hidden-xs hidden-sm'>";
	echo "<th>".(JOBS_TILE=='Projects' ? "Project" : JOBS_TILE)." Path</th>";
	echo "<th>Milestone & Timeline</th>
	<th>Function</th>";
	echo "</tr>";
	while($row = mysqli_fetch_array($result)) {
		echo '<tr>';
		echo '<td data-title="'.(JOBS_TILE=='Projects' ? "Project" : JOBS_TILE).' Path">' . $row['project_path']. '</td>';
				...
				if(!empty($checklist[$j]) || !empty($ticket[$j]) || !empty($workorder[$j])) {
					...
					foreach(explode('*#*', $ticket[$j]) as $item) {
						if($item != '' && $item != 'FFMSPLIT') {
							...
							echo "<small><li>Ticket: ".$item[0]." (Service: ".$service.")</li></small>";
						}
					}
		...
		echo '<a href=\'field_config_jobs_path_milestone.php?jobs_path_milestone='.$row['jobs_path_milestone'].'\'>Edit</a>';
		echo '</td>';
		echo '</tr>';
	}
	echo '</table>'; ?>
	<div class="form-group">
		...
		<div class="col-sm-8">
			<a href="?jobs_path_milestone=NEW" class="btn brand-btn btn-lg	pull-right">Add Template</a>
		</div>
	</div>
<?php endif; ?>
