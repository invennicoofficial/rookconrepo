...
<div class="container">
	<div class="row hide_on_iframe">
        ...
        <?php
        if($search_client != '') {
            $each_tab = explode('#*#', get_jobs_path_milestone($dbc, $search_client, 'milestone'));
            $timeline = explode('#*#', get_jobs_path_milestone($dbc, $search_client, 'timeline'));

            $i=0;
            foreach ($each_tab as $cat_tab) {
                $query_check_credentials = "select * from jobs WHERE project_path='$search_client' AND milestone_timeline='$cat_tab' AND projecttype = '$type' ORDER BY project_path";
