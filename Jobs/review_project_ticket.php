...
if(vuaed_visible_function($dbc, 'ticket') == 1) {
        //echo '<a href="#"  class="btn brand-btn mobile-block pull-right" onclick="wwindow.open(\''.WEBSITE_URL.'/Ticket/add_tickets.php?projectid='.$projectid.'\', \'newwindow\', \'width=900, height=900\'); return false;">Add Ticket</a>';
		//echo '<li style="background: white;" id="'.$row['ticketid'].'" class="ui-state-default ui-state-ticket"><span class="iframe_open" id="'.$row['ticketid'].'">'.limit_text($row['heading'], 5 ). '</a></li>';
		echo '<a id="'.$projectid.'" href="'.WEBSITE_URL.'/Ticket/add_tickets.php?projectid='.$projectid.'&from='.urlencode(WEBSITE_URL.$_SERVER['REQUEST_URI']).'" class="btn brand-btn mobile-block pull-right">Add Ticket</a>';
}
...
echo "<a href='review_project.php?type=ticket&projectid=".$projectid."&category=Active&from_url=".urlencode($_GET['from_url'])."'><button type='button' class='btn brand-btn mobile-block ".$internal_active_tab."'>Active</button></a>&nbsp;&nbsp;";
echo "<a href='review_project.php?type=ticket&projectid=".$projectid."&category=Archive&from_url=".urlencode($_GET['from_url'])."'><button type='button' class='btn brand-btn mobile-block ".$external_active_tab."'>Archive</button></a>&nbsp;&nbsp;";
...
if($_GET['category'] == 'Archive') {
    $query_check_credentials = "SELECT t.*, c.name FROM tickets t, contacts c WHERE t.businessid = c.contactid AND t.status = 'Archive' AND projectid='$projectid' ORDER BY ticketid DESC LIMIT $offset, $rowsPerPage";
    $query = "SELECT count(t.ticketid) as numrows FROM tickets t, contacts c WHERE t.businessid = c.contactid AND t.status = 'Archive' AND projectid='$projectid'";
} else {
    $query_check_credentials = "SELECT t.*, c.name FROM tickets t, contacts c WHERE t.businessid = c.contactid AND t.status != 'Archive' AND projectid='$projectid' ORDER BY ticketid DESC LIMIT $offset, $rowsPerPage";
    $query = "SELECT count(t.ticketid) as numrows FROM tickets t, contacts c WHERE t.businessid = c.contactid AND t.status != 'Archive' AND projectid='$projectid'";
}
