...
foreach ($each_tab as $cat_tab) {
    //$result = mysqli_query($dbc, "SELECT * FROM tickets WHERE projectid='$projectid' AND milestone_timeline = '$cat_tab' AND (`status` != 'Archive' OR DATE_ADD(`status_date`, INTERVAL 7 DAY) > CURDATE())");
	...
	$result = mysqli_query($dbc, "SELECT * FROM `jobs_milestone_checklist` WHERE `projectid`='$projectid' AND `milestone` = '$cat_tab' AND `deleted` = 0 ORDER BY `sort`");
	while($row = mysqli_fetch_array($result)) {
		...
		echo '<input type="checkbox" onclick="checklistChange(this);" '.($row['checked'] == 1 ? 'checked' : '').' value="'.$row['checklistid'].'" style="height: 1.5em; width: 1.5em;" name="checklistid[]">';
			...
			$documents = mysqli_query($dbc, "SELECT * FROM project_milestone_document WHERE checklistid='".$row['checklistid']."'");
