...
if($_GET['fill'] == 'jobs_path_milestone') {
	if(empty($_GET['project_path'])) {
		exit("(Select a Template above)");
	}
    $project_path = $_GET['project_path'];
    $each_tab = explode('#*#', get_jobs_path_milestone($dbc, $project_path, 'milestone'));
    $timeline = explode('#*#', get_jobs_path_milestone($dbc, $project_path, 'timeline'));
...
if($_GET['fill'] == 'path_milestone') {
	...
	$query_update_project = "UPDATE `jobs` SET  milestone_timeline='$milestone_timeline' WHERE `projectid` = '$projectid'";
...
if($_GET['fill'] == 'checklist_path_milestone') {
	...
	echo $query_update_project = "UPDATE `jobs_milestone_checklist` SET  `milestone`='$milestone' WHERE `checklistid` = '$checklistid'";
	$result_update_project = mysqli_query($dbc, $query_update_project);
}
...
if($_GET['fill'] == 'add_milestone_item') {
	...
	$query = "INSERT INTO `jobs_milestone_checklist` (`projectid`, `milestone`, `checklist`, `sort`, `updated_date`, `updated_by`)
		SELECT '$projectid', '$milestone', '$item', IFNULL(MAX(`sort`),0), '$updated_date', '$updated_by' FROM `jobs_milestone_checklist` WHERE `projectid`='$projectid' AND `milestone`='$milestone'";
...
if($_GET['fill'] == 'delete_milestone_item') {
	...
	$query = "UPDATE `jobs_milestone_checklist` SET `deleted`=1, `date_of_archival` = '$date_of_archival', `updated_by`='$updated_by', `updated_date`='$updated_date' WHERE `checklistid`='$item'";
	$result = mysqli_query($dbc, $query);
}
...
if($_GET['fill'] == 'reply_milestone_item') {
	...
	$query = "UPDATE `jobs_milestone_checklist` SET `checklist`=CONCAT(`checklist`,'$reply'), `updated_by`='$updated_by', `updated_date`='$updated_date' WHERE `checklistid`='$item'";
	$result = mysqli_query($dbc, $query);
}
if($_GET['fill'] == 'milestone_item_check') {
	...
	$query = "UPDATE `jobs_milestone_checklist` SET `checklist`=CONCAT(`checklist`,'$note'), `checked`='$status', `updated_by`='$updated_by', `updated_date`='$updated_date' WHERE `checklistid`='$item'";
	$result = mysqli_query($dbc, $query);
}
if($_GET['fill'] == 'milestone_item_alert') {
	$item_id = $_POST['id'];
	$user = $_POST['user'];
	$result = mysqli_fetch_array(mysqli_query($dbc, "SELECT * FROM `jobs_milestone_checklist` WHERE `checklistid`='$item_id'"));
	$id = $result['projectid'];
...
if($_GET['fill'] == 'milestone_item_flag') {
	$item_id = $_POST['id'];
	$colour = mysqli_fetch_array(mysqli_query($dbc, "SELECT `flag_colour` FROM jobs_milestone_checklist WHERE checklistid = '$item_id'"))['flag_colour'];
	...
	$result = mysqli_query($dbc, "UPDATE `jobs_milestone_checklist` SET `flag_colour`='$new_colour' WHERE `checklistid` = '$item_id'");
	echo $new_colour;
}
if($_GET['fill'] == 'milestone_item_upload') {
	...
	$query_insert = "INSERT INTO `jobs_milestone_document` (`checklistid`, `type`, `document`, `created_date`, `created_by`) VALUES ('$id', 'Support Document', '$filename', '".date('Y/m/d')."', '".$_SESSION['contactid']."')";
...
if($_GET['fill'] == 'milestone_quick_time') {
	...
	$projectid = $_POST['projectid'];
	$time = $_POST['time'];
	$query_time = "INSERT INTO `jobs_milestone_checklist_time` (`checklist_id`, `work_time`, `contactid`, `timer_date`) VALUES ('$checklistid', '$time', '".$_SESSION['contactid']."', '".date('Y-m-d')."')";
	$result = mysqli_query($dbc, $query_time);
	insert_day_overview($dbc, $_SESSION['contactid'], 'Checklist', date('Y-m-d'), '', "Updated Job Checklist Item #$checklistid on Job #$projectid - Added Time : $time");
}
if($_GET['fill'] == 'assignchecklist') {
	$checklistid = $_GET['id'];
	$status = $_GET['assign'];
	$query_update = "UPDATE `jobs_milestone_checklist` SET `assign_client`='$status' WHERE `checklistid`='$checklistid'";
...
if($_GET['fill'] == 'review_project') {
	...
	$query_review = "UPDATE `jobs` SET `review_date` = CURRENT_TIMESTAMP, `reviewer_id` = '$contactid' WHERE `projectid`='$projectid'";
	$result_review = mysqli_query($dbc, $query_review);

	$query_history = "UPDATE `jobs` SET `history`=CONCAT(IFNULL(CONCAT(`history`,'<br />'),''),'Reviewed by ".get_contact($dbc,$contactid)." at ".date('Y-m-d H:i')."') WHERE `projectid`='$projectid'";
