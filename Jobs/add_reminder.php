...
if(isset($_POST['save_reminder'])) {
	...
	if($reminderid > 0) {
		...
		$sql = "INSERT INTO `reminders` (`contactid`, `reminder_type`, `reminder_date`, `reminder_time`, `subject`, `body`, `sender`)
			VALUES ('$contacts', 'PROJECT".$projectid."', '$reminder_date', '$reminder_time', '$subject', '$body', '$sender')";
	}
...
$project_info = mysqli_fetch_array(mysqli_query($dbc, "SELECT * FROM `jobs` WHERE `projectid`='$projectid'"));
