<?php $result = mysqli_query($dbc, "SELECT certificate.*, `jobs`.`project_name` FROM certificate LEFT JOIN `jobs` ON `certificate`.`jobid`=`jobs`.`projectid` WHERE `certificate`.`jobid`='$projectid' ORDER BY `certificate`.expiry_date ASC");

if(vuaed_visible_function($dbc, 'certificate') == 1) {
	echo '<a href="../Certificate/add_certificate.php?jobid='.$projectid.'&from_url='.urlencode(WEBSITE_URL.$_SERVER['REQUEST_URI']).'" class="btn brand-btn mobile-block pull-right">Add Certificate</a>';
