...
if (isset($_POST['save'])) {
    ...
    $query_insert_customer = "INSERT INTO `jobs` (`businessid`, `clientid`, `ratecardid`, `projecttype`, `project_name`, `created_date`, `start_date`, `estimated_completed_date`, `completion_date`, `budget_price`, `project_path`, `history`) VALUES ('$businessid', '$clientid', '$ratecardid', '$projecttype', '$project_name', '$created_date', '$start_date', '$estimated_completed_date', '$completion_date', '$budget_price', '$project_path', '$history')";
    ...
    for($i = 0; $i < count($_FILES['document']['name']); $i++) {
        if($document[$i] != '') {
            ...
            $query_insert_upload = "INSERT INTO `jobs_document` (`projectid`, `upload`, `label`) VALUES ('$projectid', '$document[$i]', '$label')";
    ...
    for($i = 0; $i < count($_POST['support_link']); $i++) {
        ...
        if($support_link != '') {
            $query_insert_client_doc = "INSERT INTO `jobs_document` (`projectid`, `link`, `label`) VALUES ('$projectid', '$support_link', '$label')";
	...
	if ( !empty($intakeid) ) {
		...
		$query_insert_client_doc	= "INSERT INTO `jobs_document` (`projectid`, `link`) VALUES ('$projectid', '$support_link')";
		$result_insert_client_doc	= mysqli_query($dbc, $query_insert_client_doc);
    ...
    $milestone_list = explode('#*#', get_jobs_path_milestone($dbc, $project_path, 'milestone'));
    $checklist = explode('#*#', get_jobs_path_milestone($dbc, $project_path, 'checklist'));
    $tickets = explode('#*#', get_jobs_path_milestone($dbc, $project_path, 'ticket'));
    $workorders = explode('#*#', get_jobs_path_milestone($dbc, $project_path, 'workorder'));
	foreach($milestone_list as $key => $milestone) {
		$checklist_items = explode('*#*', $checklist[$key]);
		foreach($checklist_items as $sort => $item) {
			if($item != '') {
				$query = "INSERT INTO `jobs_milestone_checklist` (`projectid`, `milestone`, `checklist`, `sort`, `updated_date`) VALUES ('$projectid', '$milestone', '$item', '$sort', '".date('Y-m-d')."')";
...
if (isset($_POST['submit'])) {
    ...
    if($get_config['total_id'] == 1) {
        $get_project = mysqli_fetch_assoc(mysqli_query($dbc,"select * from jobs WHERE `projectid` = '$projectid'"));
    ...
    $jobs_comment = htmlentities($_POST['jobs_comment']);
    $t_comment = filter_var($jobs_comment,FILTER_SANITIZE_STRING);
    if($t_comment != '') {
        $email_comment = $_POST['email_comment'];

        if($type != '') {
            $query_insert_ca = "INSERT INTO `jobs_comment` (`projectid`, `comment`, `email_comment`, `created_date`, `created_by`, `type`, `note_heading`) VALUES ('$projectid', '$t_comment', '$email_comment', '$created_date', '$who_added', '$type', '$note_heading')";
            $result_insert_ca = mysqli_query($dbc, $query_insert_ca);
        } else {
            $query_update_report = "UPDATE `jobs_detail` SET `$note_heading` = CONCAT($note_heading,'$t_comment') WHERE `projectid` = '$projectid'";
            $result_update_report = mysqli_query($dbc, $query_update_report);
        }
        ...
        if ($_POST['send_email_on_comment'] == 'Yes') {
			...
			$email_body = str_replace(['[NOTE]','[PROJECTID]'], [$_POST['jobs_comment'],$projectid], $_POST['email_body']);
    ...
    for($i = 0; $i < count($_FILES['document']['name']); $i++) {
        if($document[$i] != '') {
            move_uploaded_file($_FILES["document"]["tmp_name"][$i], "download/" . $_FILES["document"]["name"][$i]) ;
            $query_insert_upload = "INSERT INTO `jobs_document` (`projectid`, `upload`) VALUES ('$projectid', '$document[$i]')";
            $result_insert_upload = mysqli_query($dbc, $query_insert_upload);
        }
    }
...
$(document).ready(function() {
	...
	$("#project_path").change(function() {
		...
		$.ajax({
			type: "GET",
			url: "project_ajax_all.php?fill=jobs_path_milestone&project_path="+project_path,
...
<div class="container">
  <div class="row">
    ...
    ?>
    <h1><?php echo (empty($_GET['projectid']) ? 'Add ' : 'Edit ').JOBS_TILE; ?></h1>
    ...
    <div class="panel-group" id="accordion2">
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <h4 class="panel-title">
                    <a data-toggle="collapse" data-parent="#accordion2" href="#collapse_abi" ><?php echo (JOBS_TILE == 'Projects' ? 'Project' : (JOBS_TILE == 'Jobs' ? 'Job' : JOBS_TILE)); ?> Info<span class="glyphicon glyphicon-minus"></span></a>
                </h4>
            </div>
        ...
        <div class="panel panel-default">
            ...
            <div id="collapse_pm" class="panel-collapse collapse">
                <div class="panel-body">
                    <?php
                    include ('add_jobs_path_milestone.php');
                    ?>
                </div>
            </div>
        </div>
        ...
        <div class="panel panel-default">
            ...
            <div id="collapse_notes" class="panel-collapse collapse <?php echo $note_add_view; ?>">
                <div class="panel-body">
                 <?php include ('add_view_jobs_comment.php'); ?>
                </div>
            </div>
        </div>
        <?php } ?>
        ...
        <div class="panel panel-default">
            ...
            <div id="collapse_doc" class="panel-collapse collapse">
                <div class="panel-body">
					...
					include ('add_jobs_documents.php');
                    ?>
                </div>
            </div>
        </div>
