...
<div class="form-group">
  ...
  <div class="col-sm-8">
    <select data-placeholder="Choose a Project..." name="projectid" id="projectid"  class="chosen-select-deselect form-control" width="380">
      <option value=""></option>
      <?php
        $query = mysqli_query($dbc,"SELECT * FROM (SELECT projectid, project_name FROM project WHERE `deleted`=0 UNION SELECT CONCAT('C',`projectid`), `project_name` FROM `client_project` WHERE `deleted`=0) PROJECTS order by project_name");
        while($row = mysqli_fetch_array($query)) {
...
<div class="form-group">
  ...
  <div class="col-sm-8">
    <select data-placeholder="Choose a <?= TICKET_NOUN ?>..." name="ticketid" id="ticketid"  class="chosen-select-deselect form-control" width="380">
      <option value=""></option>
      <?php
        $query = mysqli_query($dbc,"SELECT ticketid, service_type, heading FROM tickets WHERE status!='Archived' order by service_type");
        while($row = mysqli_fetch_array($query)) {
...
<div class="form-group">
  ...
  <div class="col-sm-8">
    <select data-placeholder="Choose a Work Order..." name="workorderid" id="workorderid"  class="chosen-select-deselect form-control" width="380">
      <option value=""></option>
      <?php
        $query = mysqli_query($dbc,"SELECT workorderid, service_type, heading FROM workorder WHERE status!='Archived' order by service_type");
        while($row = mysqli_fetch_array($query)) {
