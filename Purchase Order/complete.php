...
if (isset($_POST['send_drive_log_noemail'])) {
	...
	window.location.replace("index.php?tab=complete&subtab='.$_GET['subtab'].'&projectid='.$_GET['projectid'].'&businessid='.$_GET['businessid'].'&siteid='.$_GET['siteid'].'&vendorid='.$_GET['vendorid'].'"); </script>';
}
	...
	if($num_rows > 0) {
		...
		echo "<tr class='hidden-xs hidden-sm'>";
			...
			if (strpos($value_config, ','."Invoice PDF".',') !== FALSE) {
				echo '<th width="6%"><div class="popover-examples list-inline" style="margin:2px 5px 5px 0"><a data-toggle="tooltip" data-placement="top" title="Purchase Order created into a PDF document. This opens in a new tab on your computer."><img src="'. WEBSITE_URL .'/img/info-w.png" width="20"></a></div>P.O. PDF</th>';
			}
			if (strpos($value_config, ','."View Spreadsheet".',') !== FALSE) {
				echo '<th width="6%"><div class="popover-examples list-inline" style="margin:2px 5px 5px 0"><a data-toggle="tooltip" data-placement="top" title="Purchase Order created into a Spreadsheet. This opens in a new tab on your computer."></a></div>P.O. Spreadsheet</th>';
			}
		...
		for($i = $offset; $i < $num_rows && $i < $rowsPerPage * $pageNum; $i++)
		{
			...
			if (strpos($value_config, ','."Invoice PDF".',') !== FALSE) {
				echo '<td data-title="P.O. PDF"><a target="_blank" href="'.$software_url_get.'/Purchase Order/download/invoice_'.$roww['posid'].'.pdf">PDF <img src="'.WEBSITE_URL.'/img/pdf.png" title="PDF"></a></td>';
			}
			if (strpos($value_config, ','."View Spreadsheet".',') !== FALSE) {
			...
			if (strpos($value_config, ','."Send to Client".',') !== FALSE) {
				if($roww['status'] == "Void") {
					...
					echo '<td data-title="Send to Client"><a href="?tab=send_pos&posid='.$roww['posid'].'&from='.urlencode(WEBSITE_URL.$_SERVER['REQUEST_URI']).'" id="'.$roww['posid'].'" style="cursor:pointer">Send</a></td>';
				}
			}
			 if (strpos($value_config, ','."Send to Anyone".',') !== FALSE) {
