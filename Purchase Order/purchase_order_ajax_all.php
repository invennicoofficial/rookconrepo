...
if($_GET['fill'] == 'projectname') {
	...
	$query = mysqli_query($dbc,"SELECT projectid, project_name FROM project WHERE (clientid = '$businessid' OR businessid='$businessid') AND `deleted`=0 UNION SELECT CONCAT('C',`projectid`), `project_name` FROM `client_project` WHERE `clientid`='$businessid' AND `deleted`=0");
