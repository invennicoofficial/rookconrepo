<?php
// PO PDF - Design 3
function create_pos3_pdf($dbc, $posid, $d_value, $comment, $gst_total, $pst_total, $company_software_name, $entered_tax_rate) {
	include ('../database_connection.php');
    $point_of_sell = mysqli_fetch_assoc(mysqli_query($dbc, "SELECT * FROM purchase_orders WHERE posid='$posid'"));
    ...
    //Tax
    $point_of_sell_product = mysqli_fetch_assoc(mysqli_query($dbc, "SELECT SUM(gst) AS total_gst, SUM(pst) AS total_pst FROM purchase_orders_product WHERE posid='$posid'"));
        ...
        for($eq_loop=0; $eq_loop<=$total_count; $eq_loop++) {
            ...
            if($pos_tax_name_rate[3] == 'Yes' && $point_of_sell['client_tax_exemption'] == 'Yes') {
                ...
                if ( $tax == '0' ) {
					...
					$pdf_tax .= '<tr><td style="text-align:right;" width="75%"><strong>Tax[' . $entered_tax_rate . '%]</strong></td><td border="1" width="25%" style="text-align:right;background-color: rgb(232,238,238);">$'.$taxrate_value.'</td></tr>';
				}
            }
	...
	class MYPDF extends TCPDF {
		...
		public function Header() {
            ...
            $this->SetFont('helvetica', '', 9);
				...
				$footer_text = '<table border="0"><tr><td style="width:50%;padding:10px;"><br><br><br><br><br></td><td  style="width:50%;"><p style="text-align:right;"><h1><em>Purchase Order</em></h1>Date: '.INVOICE_DATE.'<br>P.O. #: '.INVOICEID.'<br></p></td></tr></table>';
	...
	?><?php
	$pdf->Output('../Purchase Order/download/purchase_order_'.$posid.'.pdf', 'F');
	$pdf->Output('../Purchase Order/download/invoice_'.$posid.'.pdf', 'F');

}


// PO PDF - Design 2
function create_pos2_pdf($dbc, $posid, $d_value, $comment, $gst_total, $pst_total, $entered_tax_rate) {
	include ('../database_connection.php');
    $point_of_sell = mysqli_fetch_assoc(mysqli_query($dbc, "SELECT * FROM purchase_orders WHERE posid='$posid'"));
    $contactid = $point_of_sell['contactid'];
	$zenornot = $point_of_sell['cross_software'];
        ...
        for($eq_loop=0; $eq_loop<=$total_count; $eq_loop++) {
            ...
            if($pos_tax_name_rate[3] == 'Yes' && $point_of_sell['client_tax_exemption'] == 'Yes') {
                ...
                if ( $tax == '0' ) {
					...
					$pdf_tax .= '<tr><td style="text-align:right;" width="75%"><strong>Tax[' . $entered_tax_rate . '%]</strong></td><td border="1" width="25%" style="text-align:right;">$'.$taxrate_value.'</td></tr>';
				}
            }
	...
	class MYPDF extends TCPDF {
		...
		public function Header() {
			...
			if($point_of_sell['delivery_type'] !== '' && $point_of_sell['delivery_type'] !== NULL) {
				$footer_text = '<p style="text-align:right;"><h1><em>Purchase Order</em></h1>Date : ' .INVOICE_DATE.'<br>P.O. # : '.INVOICEID.'<br>Ship Date : ' .SHIP_DATE.'<br>Sales Person : ' .SALESPERSON.'<br>Payment Type : ' .PAYMENT_TYPE.'<br>Shipping Method : '.$point_of_sell['delivery_type'].$tdduedate.'</p>';
			} else {
				$footer_text = '<p style="text-align:right;"><h1><em>Purchase Order</em></h1>Date : ' .INVOICE_DATE.'<br>P.O. # : '.INVOICEID.'<br>Ship Date : ' .SHIP_DATE.'<br>Sales Person : ' .SALESPERSON.'<br>Payment Type : ' .PAYMENT_TYPE.$tdduedate.'</p>';
			}
            $this->writeHTMLCell(0, 0, 0 , 10, $footer_text, 0, 0, false, "R", true);
		}
...
// PO PDF - Design 1
function create_pos1_pdf($dbc, $posid, $d_value, $comment, $gst_total, $pst_total, $entered_tax_rate) {
	include ('../database_connection.php');
    $point_of_sell = mysqli_fetch_assoc(mysqli_query($dbc, "SELECT * FROM purchase_orders WHERE posid='$posid'"));
    $contactid = $point_of_sell['contactid'];
	$zenornot = $point_of_sell['cross_software'];
        ...
        for($eq_loop=0; $eq_loop<=$total_count; $eq_loop++) {
            ...
            if($pos_tax_name_rate[3] == 'Yes' && $point_of_sell['client_tax_exemption'] == 'Yes') {
                ...
                if ( $tax == '0' ) {
					...
					$pdf_tax .= 'Tax: ' . $entered_tax_rate . '% : $' . $taxrate_value . '<br>';
				}
            }
    ...
    DEFINE('INVOICE_FOOTER', $invoice_footer);
		...
		public function Header() {
            ...
            $header_text = '<h1><em>Purchase Order</em></h1>';
	...
	$html = '<br><br><br /><br /><center><div style="margin-top:10px; text-align:center;"><h1>Purchase Order  #'.$posid.'</h1></div></center>
	...
	<tr style="background-color:lightgrey; color:black;">
		...
		<td>'.decryptIt($customer['cell_phone']).'</td>
