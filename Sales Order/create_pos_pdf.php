<?php
function create_pos3_pdf($dbc,$posid,$d_value,$comment, $gst_total, $pst_total,$company_software_name) {
	include ('../database_connection.php');
    $point_of_sell = mysqli_fetch_assoc(mysqli_query($dbc, "SELECT * FROM sales_order WHERE posid='$posid'"));
    $contactid = $point_of_sell['contactid'];
	$zenornot = $point_of_sell['cross_software'];
	if($zenornot == 'zen') {
		$zenordbc = $dbczen;
	} else {
		$zenordbc = $dbc;
    ...
    //Tax
    $point_of_sell_product = mysqli_fetch_assoc(mysqli_query($dbc, "SELECT SUM(gst) AS total_gst, SUM(pst) AS total_pst FROM sales_order_product WHERE posid='$posid'"));
    ...
    $invoice_footer = get_config($dbc, 'po_invoice_footer');
	...
	class MYPDF extends TCPDF {
		...
		public function Header() {
            ...
            $this->SetFont('helvetica', '', 9);
				...
				$footer_text = '<table border="0"><tr><td style="width:50%;padding:10px;"><br><br><br><br><br></td><td  style="width:50%;"><p style="text-align:right;"><h1><em>'.SALES_ORDER_NOUN.'</em></h1>Date: '.INVOICE_DATE.'<br>S.O. #: '.INVOICEID.'<br></p></td></tr></table>';
	...
	<table><tr><td style="width:20%">To:</td><td style="width:30%">'.$customer['name'].' '.$customer['first_name'].' '.$customer['last_name'].'<br>'.$customer['business_address'].'<br>'.$customer['city'].', '.$customer['state'].' '.$customer['zip_code'].'<br>'.$customer['cell_phone'].'<br>'.$customer['email_address'].'</td><td style="width:20%">Ship to:</td><td style="width:30%">'.$customer['name'].' '.$customer['first_name'].' '.$customer['last_name'].'<br>'.$customer['business_address'].'<br>'.$customer['city'].', '.$customer['state'].' '.$customer['zip_code'].'<br>'.$customer['cell_phone'].'<br>'.$customer['email_address'].'</td></tr></table><br><br>
    ...
    if($num_rows > 0 || $num_rows2 > 0) {
		$html .= '<h2>'.$titler.'</h2>
            ...
            <th>Part#</th><th>Product</th><th>Quantity</th><th>Price</th><th>Total</th></tr>';
		while($row = mysqli_fetch_array( $result )) {
			...
			if($inventoryid != '') {
				...
				$html .= '<tr>';
				$html .=  '<td>'.get_inventory($zenordbc, $inventoryid, 'part_no').'</td>';
				$html .=  '<td>'.get_inventory($zenordbc, $inventoryid, 'name').'</td>';
				$html .=  '<td>'.$quantity.'</td>';
				$html .=  '<td>$'.$price.'</td>';
    ...
    if($num_rows3 > 0) {
		...
		while($row = mysqli_fetch_array( $result )) {
			...
			if($inventoryid != '') {
				$amount = $price*$quantity;
				$html .= '<tr>';
				$html .=  '<td>'.get_products($zenordbc, $inventoryid, 'category').'</td>';
				$html .=  '<td>'.get_products($zenordbc, $inventoryid, 'heading').'</td>';
				$html .=  '<td>'.$quantity.'</td>';
				$html .=  '<td>$'.$price.'</td>';
				$html .=  '<td style="text-align:right; background-color:rgb(232,238,238);">$'.number_format($amount,2).'</td>';
				$html .= '</tr>';
    ...
    if($num_rows4 > 0) {
		...
		while($row = mysqli_fetch_array( $result )) {
			...
			if($inventoryid != '') {
				$amount = $price*$quantity;
				$html .= '<tr>';
				$html .=  '<td>'.get_services($zenordbc, $inventoryid, 'category').'</td>';
				$html .=  '<td>'.get_services($zenordbc, $inventoryid, 'heading').'</td>';
				$html .=  '<td>'.$quantity.'</td>';
				$html .=  '<td>$'.$price.'</td>';
				$html .=  '<td style="text-align:right; background-color:rgb(232,238,238);">$'.number_format($amount,2).'</td>';
				$html .= '</tr>';
    ...
    if($num_rows5 > 0) {
		...
		while($row = mysqli_fetch_array( $result )) {
			...
			if($inventoryid != '') {
				...
				$html .= '<tr>';
				$html .=  '<td>'.get_vpl($zenordbc, $inventoryid, 'part_no').'</td>';
				$html .=  '<td>'.get_vpl($zenordbc, $inventoryid, 'name').'</td>';
				$html .=  '<td>'.$quantity.'</td>';
				$html .=  '<td>$'.$price.'</td>';
				$html .=  '<td style="text-align:right; background-color:rgb(232,238,238);">$'.number_format($amount,2).'</td>';
				$html .= '</tr>';
...
function create_pos2_pdf($dbc,$posid,$d_value,$comment, $gst_total, $pst_total) {
	include ('../database_connection.php');
    $point_of_sell = mysqli_fetch_assoc(mysqli_query($dbc, "SELECT * FROM sales_order WHERE posid='$posid'"));
    $contactid = $point_of_sell['contactid'];
	$zenornot = $point_of_sell['cross_software'];
	...
	class MYPDF extends TCPDF {
		...
		public function Header() {
			...
			if($point_of_sell['delivery_type'] !== '' && $point_of_sell['delivery_type'] !== NULL) {
				$footer_text = '<p style="text-align:right;">Date : ' .INVOICE_DATE.'<br>S.O. # : '.INVOICEID.'<br>Ship Date : ' .SHIP_DATE.'<br>Sales Person : ' .SALESPERSON.'<br>Payment Type : ' .PAYMENT_TYPE.'<br>Shipping Method : '.$point_of_sell['delivery_type'].$tdduedate.'</p>';
			} else {
				$footer_text = '<p style="text-align:right;">Date : ' .INVOICE_DATE.'<br>S.O. # : '.INVOICEID.'<br>Ship Date : ' .SHIP_DATE.'<br>Sales Person : ' .SALESPERSON.'<br>Payment Type : ' .PAYMENT_TYPE.$tdduedate.'</p>';
			}
            $this->writeHTMLCell(0, 0, 0 , 10, $footer_text, 0, 0, false, "R", true);
		}
	...
	$html .= '<br><br><br><br><br><p style="text-align:left;">'.$customer['name'].' '.$customer['first_name'].' '.$customer['last_name'].'<br>'.$customer['business_address'].'<br>'.$customer['city'].', '.$customer['state'].' '.$customer['zip_code'].'<br>'.$customer['cell_phone'].'<br>'.$customer['email_address'].'<br>';
...
function create_pos1_pdf($dbc,$posid,$d_value,$comment, $gst_total, $pst_total) {
	include ('../database_connection.php');
    $point_of_sell = mysqli_fetch_assoc(mysqli_query($dbc, "SELECT * FROM sales_order WHERE posid='$posid'"));
    $contactid = $point_of_sell['contactid'];
	$zenornot = $point_of_sell['cross_software'];
	...
	$html = '<br><br><br /><br /><center><div style="margin-top:10px; text-align:center;"><h1>'.SALES_ORDER_NOUN.'  #'.$posid.'</h1></div></center>
