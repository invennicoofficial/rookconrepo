...
<div class="container triple-pad-bottom">
	...
	<div class="row hide_on_iframe">
		...
		<div class="tab-container mobile-100-container"><?php
			...
			<div class="pull-left tab">
				...
				if ( check_subtab_persmission($dbc, 'sales_order', ROLE, 'completed') === TRUE ) { ?>
					<a href="complete.php"><button type="button" class="btn brand-btn mobile-block mobile-100 active_tab">Completed <?= SALES_ORDER_TILE ?></button></a><?php
				} else { ?>
					<button type="button" class="btn disabled-btn mobile-block mobile-100 active_tab">Completed <?= SALES_ORDER_TILE ?></button><?php
				} ?>
			</div>
        ...
        <form name="invoice_table" method="post" action="complete.php" class="form-inline" role="form">
            ...
            <div class="single-pad-bottom">
				...
				<div class="col-lg-6 col-md-6 col-sm-6 col-xs-9">
					...
					<span class="popover-examples gap-left"><a data-toggle="tooltip" data-placement="top" title="Click to refresh the page and see all Completed <?= SALES_ORDER_TILE ?> within this tab."><img src="<?= WEBSITE_URL; ?>/img/info.png" width="20"></a></span>
            ...
            if (isset($_POST['search_invoice_submit'])) {
                $query_check_credentialss = "SELECT inv.*, c.* FROM sales_order inv,  contacts c WHERE inv.contactid = c.contactid AND inv.deleted = 0 AND (status='Completed')  ".$searchbyany." ".$searchbycustomer." ".$searchbytype." ".$starttimesql." ORDER BY inv.posid DESC LIMIT $offset, $rowsPerPage";
                $query = "SELECT count(c.contactid) FROM sales_order inv,  contacts c WHERE inv.contactid = c.contactid AND inv.deleted = 0 AND (status='Completed')  ".$searchbyany." ".$searchbycustomer." ".$searchbytype." ".$starttimesql." ORDER BY inv.posid DESC";
            } else {
                $query_check_credentialss = "SELECT * FROM sales_order WHERE deleted = 0 AND (status='Completed') ORDER BY posid DESC LIMIT 25";
            }
