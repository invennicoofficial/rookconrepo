...
?>
<?php
	$select_pdf_settings = mysqli_fetch_assoc(mysqli_query($dbc, "select count(pdfsettingid) as setting_count from inventory_pdf_setting where style = '$style_settings'"));
	if(isset($_POST['pdf_setting'])) {
		...
		if($select_pdf_settings['setting_count'] <= 0) {	
			...
			$select_pdf = mysqli_fetch_assoc(mysqli_query($dbc, "select pdf_logo from inventory_pdf_setting where style = '$style_settings'"));
			if($pdf_logo == '')
				$pdf_logo = $select_pdf['pdf_logo'];
			$update_pdf_settings = "UPDATE inventory_pdf_setting SET `style`='$style_settings',`file_name`='$file_name',`font_size`='$font_size',`font_type`='$font_type',`font`='$font',`pdf_logo`='$pdf_logo',`pdf_size`='$pdf_size',`page_ori`='$page_ori',`units`='$units',`left_margin`='$left_margin',`right_margin`='$right_margin',`top_margin`='$top_margin',`header_margin`='$header_margin',`bottom_margin`='$bottom_margin',`pdf_color`='$pdf_color' where style = '$style_settings'";
