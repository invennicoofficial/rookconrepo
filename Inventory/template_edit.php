...
foreach($ticket_field_list as $key => $ticket_field) {
	$field_list['ticket#*#'.$key] = $ticket_field[1];
}
asort($field_list); ?>
<script>
...
function save_field(src) {
	...
	$.ajax({
		url: 'inventory_ajax.php?action=save_template_field',
		method: 'POST',
		data: {
			table_name: table,
			heading_id: id,
...
function save_sort() {
	...
	$.ajax({
		url: 'inventory_ajax.php?action=set_sort_order',
		method: 'POST',
		data: {
			table_name: 'inventory_templates_headings',
			sort_ids: ids
		}
	});
}
...
<form class="form-horizontal">
	<div class="col-sm-12">
		...
		if($templateid > 0) {
			$template = mysqli_fetch_array(mysqli_query($dbc, "SELECT * FROM `inventory_templates` WHERE `id`='$templateid'"));
		}
		$headings = mysqli_query($dbc, "SELECT `id`, `heading_name`, `sort_order` FROM `inventory_templates_headings` WHERE `template_id`='$templateid' AND `deleted`=0 UNION SELECT '', '', 999999999999 ORDER BY `sort_order`"); ?>
		...
		<?php if($templateid == 'new' || $templateid > 0) { ?>
			<div class="form-group">
				...
				<div class="col-sm-8">
					<input type="text" name="template_name" value="<?= $template['template_name'] ?>" class="form-control" data-table="inventory_templates">
					<input type="hidden" name="id" value="<?= $template['id'] ?>" data-table="inventory_templates">
				</div>
			</div>
				...
				<div class="sort_table">
					<table class="table table-bordered">
						...
						<?php while($heading = mysqli_fetch_array($headings)) { ?>
							<tr>
								<td data-title="Field">
									<select name="heading_name" class="chosen-select-deselect form-control" data-id="<?= $heading['id'] ?>" data-table="inventory_templates_headings"><option></option>
								...
								<td data-title="Function">
									<img src="../img/remove.png" style="height: 1em;" onclick="remove_line(this);" data-table="inventory_templates_headings" data-id="<?= $heading['id'] ?>" name="deleted">
									<img src="../img/icons/ROOK-add-icon.png" style="height: 1em;" onclick="add_line(this);">
									<img src="../img/icons/drag_handle.png" style="height: 1em;" class="pull-right line-handle" data-table="inventory_templates_headings" data-id="<?= $heading['id'] ?>">
								</td>
							</tr>
						<?php } ?>
					</table>
