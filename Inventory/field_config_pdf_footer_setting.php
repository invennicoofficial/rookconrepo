...
?>
<?php
	$select_pdf_settings = mysqli_fetch_assoc(mysqli_query($dbc, "select count(pdfsettingid) as setting_count from inventory_pdf_setting where style = '$style_settings' and setting_type = 'footer'"));
	...
	if(isset($_POST['pdf_setting'])) {
		...
		if(!empty($_FILES["pdf_logo"]["name"])) {
			$pdf_logo = 'footer_'.$_FILES["pdf_logo"]["name"];
		}
		...
		if($select_pdf_settings['setting_count'] <= 0) {	
			$insert_pdf_settings = "INSERT INTO inventory_pdf_setting(`style`,`font_size`,`font_type`,`font`,`pdf_logo`,`pdf_color`,`setting_type`,`text`,`alignment`) VALUES('$style_settings','$font_size','$font_type','$font','$pdf_logo','$pdf_color','footer','$footer_text','$alignment')";
			$result_pdf_settings = mysqli_query($dbc, $insert_pdf_settings);
		}
		else {
			$select_pdf = mysqli_fetch_assoc(mysqli_query($dbc, "select pdf_logo from inventory_pdf_setting where style = '$style_settings' and setting_type = 'footer'"));
			if($pdf_logo == 'footer_')
				$pdf_logo = $select_pdf['pdf_logo'];
			$update_pdf_settings = "UPDATE inventory_pdf_setting SET `style`='$style_settings',`font_size`='$font_size',`font_type`='$font_type',`font`='$font',`pdf_logo`='$pdf_logo',`pdf_color`='$pdf_color',`text`='$footer_text',`alignment`='$alignment' where style = '$style_settings' and setting_type='footer'";
	...
	$alignment = '';
	if($select_pdf_settings['setting_count'] > 0) {
		$select_pdf_settings = mysqli_fetch_assoc(mysqli_query($dbc, "select * from inventory_pdf_setting where style = '$style_settings' and setting_type='footer'"));
		...
		$footer_text = $select_pdf_settings['text'];
		$pdf_logo = $select_pdf_settings['pdf_logo'];
		$alignment = $select_pdf_settings['alignment'];
	}
?>
<form action="" method="POST" enctype="multipart/form-data">
<h3 style="margin-left:10px"><?php echo "Footer Settings"; ?></h3>
<div class="clearfix"></div>
<div style="margin:10px;padding:25px;height:100%">
		<div class="pull-left" style="width:55%">
			<span class="pull-left"><label>Footer Text:</label></span>
			<span style="margin-left:2px;width:77%" class="pull-right"><input type="text" class="form-control" style="height:70px" value="<?php echo $footer_text; ?>" name="footer_text"/></span>
			<br><br><br><br>
		...
		<span class="pull-right">
			<a href="?type=pdfstyling&style_settings=<?php echo $style_settings; ?>&preview=<?php echo $style_settings; ?>"><button class="btn brand-btn hide-titles-mob" type="button" value="PDF Setting for Style <?php echo substr($style_settings, -1); ?>" >Preview For Style <?php echo substr($style_settings, -1); ?></button></a>
		</span>
