...
<div id='template_accordion' class='sidebar show-on-mob panel-group block-panels col-xs-12'>
	<?php $template_list = mysqli_query($dbc, "SELECT `id`, `template_name` FROM `inventory_templates` WHERE `deleted`=0");
...
<ul class='sidebar hide-titles-mob col-sm-3' style='padding-left: 15px;'>
	...
	<?php $template_list = mysqli_query($dbc, "SELECT `id`, `template_name` FROM `inventory_templates` WHERE `deleted`=0");
	while($template = mysqli_fetch_array($template_list)) { ?>
		<a href="?type=templates&template=<?= $template['id'] ?>"><li class="<?= $_GET['template'] == $template['id'] ? 'active blue' : '' ?>"><?= $template['template_name'] ?></li></a>
	<?php } ?>
	<a href="?type=templates&template=new"><li class="<?= $_GET['template'] == 'new' ? 'active blue' : '' ?>">Create New Template</li></a>
</ul>
