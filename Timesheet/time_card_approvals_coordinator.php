            ...
            if($search_staff_list != '') {
				foreach(array_filter(array_unique($search_staff_list), function($id) { return $id != 'ALL_STAFF'; }) as $search_staff) {
						...
						while(strtotime($date) <= strtotime($search_end_date)) {
							...
							if($row['date'] == $date) {
								...
								if($timesheet_approval_status_comments == 1) {
									if(!empty(trim($row['coord_approvals'],','))) {
										$approval_list = [];
										foreach(explode(',',$row['coord_approvals']) as $approval_coord) {
											if($approval_coord > 0) {
												$approval_list[] = get_contact($dbc, $approval_coord);
											}
										}
										$approval_status = 'Approved by '.implode(', ', $approval_list).'<br />';
									} else if(!empty($row['coordinator_name'])) {
										$approval_status = 'Approved by '.$row['coordinator_name'].'<br />';
									} else {
										$approval_status = 'Waiting for Approval<br />';
									}
								}
								$approval_initials = $row['coord_approvals'];
					...
					elseif($layout == 'position_dropdown' || $layout == 'ticket_task'):
						echo '<input type="hidden" name="supervisor" value="Coordinator">';
									...
									if($result[$i]['date'] == $date) {
										...
										if($timesheet_approval_status_comments == 1) {
											if(!empty(trim($row['coord_approvals'],','))) {
												...
												$comments = 'Approved by '.$row['coordinator_name'].'<br />'.$comments;
											} else {
									...
									<tr style="<?= $hl_colour ?>" class="<?= $show_separator==1 ? 'theme-color-border-bottom' : '' ?>">
										...
										<td data-title="Select to Approve">
											<?php if($timesheet_approval_initials == 1) { ?>
												...
												<?php foreach(explode(',', $row['coord_approvals']) as $approval_manager) {
													if($approval_manager > 0) {
														profile_id($dbc, $approval_manager);
													}
												}
