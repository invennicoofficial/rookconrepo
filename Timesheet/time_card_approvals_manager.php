...
<div class="container" id="timesheet_div">
    ...
    <div class="row timesheet_div">
        ...
        <?php echo get_tabs('Manager Approvals', 'Custom', array('db' => $dbc, 'field' => $value['config_field'])); ?>
        <br><br>
        <?php
            ...
            if($search_staff_list != '') {
				foreach(array_filter(array_unique($search_staff_list), function($id) { return $id != 'ALL_STAFF'; }) as $search_staff) {
					...
					elseif($layout == 'position_dropdown' || $layout == 'ticket_task'):
						echo '<input type="hidden" name="supervisor" value="Manager">';
						echo '<button type="submit" name="approve_position" value="approv_btn" class="btn brand-btn pull-right">Update and Approve Selected</button>';
							...
							<table class='table table-bordered'>
								...
								while(strtotime($date) <= strtotime($search_end_date)) {
									...
									if($result[$i]['date'] == $date) {
										...
										if($timesheet_approval_status_comments == 1) {
											if(!empty(trim($row['manager_approvals'],','))) {
												...
												$comments = 'Approved by '.implode(', ', $approval_list).'<br />'.$comments;
											} else if(!empty($row['manager_name'])) {
												$comments = 'Approved by '.$row['manager_name'].'<br />'.$comments;
											} else {
									...
									<tr style="<?= $hl_colour ?>" class="<?= $show_separator==1 ? 'theme-color-border-bottom' : '' ?>">
										...
										<td class="<?= $show_separator==1 ? 'theme-color-border-bottom' : '' ?>" data-title="Date"><?= (in_array('editable_dates',$value_config) ? '<input type="text" name="date_editable[]" value="'.$date.'" class="form-control datepicker">' : $date) ?></td>
										...
										<td data-title="Comments"><span><?= $comments ?></span><img class="inline-img add-row pull-right" src="../img/icons/ROOK-add-icon.png"><img class="inline-img rem-row pull-right" src="../img/remove.png"><img class="inline-img comment-row pull-right" src="../img/icons/ROOK-reply-icon.png"><input type="text" class="form-control" name="comment_box[]" value="<?= ($row['COMMENTS'] == $timesheet_comment_placeholder ? '' : $row['COMMENTS'])?>" style="display:none;"></td>
										<td data-title="Select to Approve">
											<?php if($timesheet_approval_initials == 1) { ?>
												<label <?= strpos(','.$row['manager_approvals'].',', ','.$_SESSION['contactid'].',') !== FALSE ? 'style="display:none;"' : '' ?>><input type="checkbox" name="approve_date_id[]" value="<?= $row['id'] ?>" /></label>
												<?php foreach(explode(',', $row['manager_approvals']) as $approval_manager) {
													if($approval_manager > 0) {
														profile_id($dbc, $approval_manager);
													}
												}
											} else { ?>
												<label><input type="checkbox" name="approve_date_id[]" value="<?= $row['id'] ?>" /></label>
											<?php } ?>
										</td>
									</tr>
								...
								<tr>
									...
									<?php if(in_array('start_time',$value_config)) { ?><td></td><?php } ?>
									<?php if(in_array('end_time',$value_config)) { ?><td></td><?php } ?>
